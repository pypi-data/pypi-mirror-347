...
from fold.metrics.clash import Clash
from fold.utils.distributed import traverse_and_aggregate
from fold.utils.data_process import make_them_on_same_device
...
def _compute_full_data_and_summary(
    ...
    token_pair_pde,  contact_probs = make_them_on_same_device(full_data["token_pair_pde"], full_data["contact_probs"])
    # summary_confidence["gpde"] = (
    #     full_data["token_pair_pde"] * full_data["contact_probs"]
    # ).sum(dim=[-1, -2]) / full_data["contact_probs"].sum(dim=[-1, -2])
    summary_confidence["gpde"] = (
        token_pair_pde * contact_probs
    ).sum(dim=[-1, -2]) / contact_probs.sum(dim=[-1, -2])
    ...
    pae_prob, token_has_frame, token_asym_id, token_is_ligand = make_them_on_same_device(pae_prob, token_has_frame, token_asym_id, token_is_ligand)
    ...
    atom_plddt, token_asym_id, atom_to_token_idx = make_them_on_same_device(full_data["atom_plddt"], token_asym_id, atom_to_token_idx)
    summary_confidence.update(
    ...
    _iptm, _ptm, _disorder, _has_clash = make_them_on_same_device(summary_confidence["iptm"], summary_confidence["ptm"], 
                                                summary_confidence["disorder"], summary_confidence["has_clash"])
...
def calculate_ptm(
    ...
    if token_mask is not None:
        ...
        pae_prob, token_mask = make_them_on_same_device(pae_prob, token_mask)
    ...
    token_token_ptm, has_frame = make_them_on_same_device(token_token_ptm, has_frame)
...
def calculate_iptm(
    ...
    token_token_ptm, is_diff_chain, has_frame = make_them_on_same_device(token_token_ptm, is_diff_chain, has_frame)
