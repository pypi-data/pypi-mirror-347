...
from fold.data.constants import (
    PRO_STD_RESIDUES,
...
from fold.openfold_local.data import parsers
from fold.openfold_local.data.msa_identifiers import (
    Identifiers,
    _extract_sequence_identifier,
    _parse_sequence_identifier,
)
from fold.openfold_local.data.msa_pairing import (
    CHAIN_FEATURES,
    MSA_FEATURES,
    MSA_GAP_IDX,
    MSA_PAD_VALUES,
...
from fold.openfold_local.np import residue_constants
...
def parse_prot_msa_data(
    ...
    """
    msa_data = {}
    assert len(seq_limits) == len(raw_msa_paths)
    for path, seq_limit in zip(raw_msa_paths, seq_limits):
        sequences, descriptions = parse_a3m(path, seq_limit)

        deletion_matrix = []
        for msa_sequence in sequences:
            msa_sequence = msa_sequence.replace("\x00", "")
            deletion_vec = []
            deletion_count = 0
        ...
        aligned_sequences = [s.translate(deletion_table).replace("\x00", "") for s in sequences]
        assert all([len(seq) == len(aligned_sequences[0]) for seq in aligned_sequences]), f"{[seq for seq in aligned_sequences]}"
        assert all([len(vec) == len(deletion_matrix[0]) for vec in deletion_matrix]), f"{[_ for _ in deletion_matrix]}"
