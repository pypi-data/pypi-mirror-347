...
from fold.data.constants import STD_RESIDUES, get_all_elems
from fold.data.tokenizer import Token, TokenArray
from fold.data.utils import get_ligand_polymer_bond_mask
from fold.utils.geometry import angle_3p
...
class Featurizer(object):
    ...
    def get_reference_features(self) -> dict[str, torch.Tensor]:
        ...
        for ref_space_uid in np.unique(self.cropped_atom_array.ref_space_uid):
            ...
            #     centralize=True,
            # ) # random_transform could change the ref_pos

            res_ref_pos = self.cropped_atom_array.ref_pos[
                    self.cropped_atom_array.ref_space_uid == ref_space_uid,
                ]
