...
from fold.data.constants import ELEMS, STD_RESIDUES
...
class AtomArrayTokenizer(object):
    ...
    def tokenize(self, atom_array=None) -> list[Token]:
        """
        ...
        """
        atom_array = self.atom_array if atom_array is None else atom_array
        tokens = []
        total_atom_num = 0
        for res in struc.residue_iter(atom_array):
            ...
            mol_type = first_atom.mol_type if hasattr(first_atom, "mol_type") else "protein"
            ...
            if res_token is not None and mol_type != "ligand":
                ...
                atom_names = [atom_array[i].atom_name for i in atom_indices]
                ...
                for atom in res:
                    ...
                    token.atom_names = [
                        atom_array[token.atom_indices[0]].atom_name
                    ]
                    tokens.append(token)
                    total_atom_num += 1

        assert total_atom_num == len(atom_array)
        return tokens
    ...
    def get_token_array(self, atom_array=None) -> TokenArray:
        """
        ...
        """
        tokens = self.tokenize(atom_array)
