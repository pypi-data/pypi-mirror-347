...
from fold.data.featurizer import Featurizer
from fold.data.json_parser import add_entity_atom_array, remove_leaving_atoms
from fold.data.parser import AddAtomArrayAnnot
from fold.data.tokenizer import AtomArrayTokenizer, TokenArray
from fold.data.utils import int_to_letters
...
class SampleDictToFeatures:
    def __init__(self, single_sample_dict=None):
        if single_sample_dict is not None:
    ...
    def get_entity_poly_type(self, atom_array=None) -> dict[str, str]:
        """
    ...
    def add_bonds_between_entities(self, atom_array: AtomArray) -> AtomArray:
        ...
        """
        if not hasattr(self, "input_dict") or "covalent_bonds" not in self.input_dict:
            return atom_array
    ...
    def add_atom_array_attributes(
        ...
        atom_array = AddAtomArrayAnnot.add_ref_info_and_res_perm(atom_array) # new_added
        return atom_array
    ...
    def get_atom_array(self, atom_array=None) -> AtomArray:
        """
        ...
        if atom_array is None:
            ...
            atom_array = self.rebuild_full_atom_array(atom_array)
        atom_array = self.add_bonds_between_entities(atom_array)
        atom_array = self.mse_to_met(atom_array)
        entity_poly_type = self.entity_poly_type if hasattr(self, "entity_poly_type") else None
        atom_array = self.add_atom_array_attributes(atom_array, entity_poly_type)
        return atom_array

    def get_feature_dict(self, atom_array=None) -> tuple[dict[str, torch.Tensor], AtomArray, TokenArray]:
        """
        ...
        Returns:
            A tuple containing:
                ...
                - A TokenArray object.
        """
        atom_array = self.get_atom_array(atom_array)
