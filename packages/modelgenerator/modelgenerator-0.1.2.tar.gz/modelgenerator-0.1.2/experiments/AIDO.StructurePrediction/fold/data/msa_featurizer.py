...
from fold.utils.hash_encoder import hash_seq

from fold.data.constants import STD_RESIDUES, rna_order_with_x
from fold.data.msa_utils import (
    PROT_TYPE_NAME,
    FeatureDict,
...
from fold.data.tokenizer import TokenArray
    ...
    def get_msa_pipeline(
        ...
        for entity_id, sequence in entity_id_to_sequence.items():
            ...
            if all(
                ...
                continue
            data_from = bioassembly_dict.get("data_from")
...
class PROTMSAFeaturizer(BaseMSAFeaturizer):
    def __init__(
        ...
        print("seq_to_pdb_idx_path:", seq_to_pdb_idx_path)
    ...
    def get_genbio_msa_path(self, msa_dir, db_names, seq):
        paths = []
        hash_code = hash_seq(seq)
        for db_name in db_names:
            msa_path = os.path.join(msa_dir, hash_code[:2], hash_code, db_name)
            assert os.path.exists(msa_path), f"seq: {seq} \n msa_path: {msa_path}"
            paths.append(os.path.join(msa_dir, hash_code[:2], hash_code, db_name))
        return paths
    ...
    def process_single_sequence(
        ...
        genbio_msa_info = self.kwargs.get("genbio_msa_info", None)
        if data_from == "genbio" and genbio_msa_info:
            msa_path = genbio_msa_info["msa_path"]
            db_names = genbio_msa_info["db_name"]
            msa_paths = self.get_genbio_msa_path(genbio_msa_info["msa_path"], genbio_msa_info["db_name"], sequence)
            raw_msa_paths.extend(msa_paths)
            seq_limits.extend([-1] * len(raw_msa_paths))
        else:
...
class InferenceMSAFeaturizer(object):
    ...
    def process_prot_single_sequence(
            ...
            pairing_db: str,
            non_pairing_msa_names=("non_pairing.a3m"),
            pairing_msa_names=("pairing.a3m"),
    ) -> FeatureDict:
        """
        ...
        if msa_dir is None:
            ...
            raw_msa_paths = [opjoin(msa_dir, _) for _ in non_pairing_msa_names]
        ...
        sequence_features = process_single_sequence(
            ...
            sequence=sequence,
            raw_msa_paths=raw_msa_paths,
            seq_limits=[-1] * len(raw_msa_paths),
            msa_entity_type="prot",
            msa_type="non_pairing",
        )
        if not is_homomer_or_monomer:
            # Separately process the pairing MSA
            raw_msa_paths = [opjoin(msa_dir, _) for _ in pairing_msa_names]
            for raw_msa_path in raw_msa_paths:
                assert opexists(
                    raw_msa_path
    ...
    def get_inference_prot_msa_features_for_assembly(
        ...
        for idx, (sequence, entity_id_list) in enumerate(sequence_to_entity.items()):
            ...
            non_pairing_msa_names = msa_info.get("non_pairing_msa_names", ["non_pairing.a3m"])
            pairing_msa_names = msa_info.get("pairing_msa_names", non_pairing_msa_names)
            if msa_pool is not None and msa_dir.startswith("./"):
                msa_dir = os.path.join(msa_pool, msa_dir)
        ...
        for idx, (sequence, entity_id_list) in enumerate(sequence_to_entity.items()):
            ...
            sequence_feat = InferenceMSAFeaturizer.process_prot_single_sequence(
                ...
                is_homomer_or_monomer=is_homomer_or_monomer,
                msa_dir=msa_dir,
                pairing_db=msa_info.get("pairing_db", "uniref100"),
                non_pairing_msa_names=non_pairing_msa_names,
                pairing_msa_names=pairing_msa_names,
            )
            sequence_feat = convert_monomer_features(sequence_feat)
            sequence_to_features[sequence] = sequence_feat
