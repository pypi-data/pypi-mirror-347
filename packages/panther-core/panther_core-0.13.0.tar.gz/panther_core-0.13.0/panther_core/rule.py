    ...
    def _get_alert_context(
        ...
        try:
            ...
            serialized_alert_context = json.dumps(
                alert_context, default=PantherEvent.json_encoder, allow_nan=False
            )
        except BaseException as err:  # pylint: disable=broad-except
            if use_default_on_exception:
                return json.dumps({ALERT_CONTEXT_ERROR_KEY: repr(err)}, allow_nan=False)
            raise
        ...
        if len(serialized_alert_context) > MAX_ALERT_CONTEXT_SIZE:
            ...
            return json.dumps({ALERT_CONTEXT_ERROR_KEY: alert_context_error}, allow_nan=False)
