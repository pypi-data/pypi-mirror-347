...
)
{
    ...
    float local_window_start = t[extrema[0]];
    float local_window_end = t[extrema[1]];
    ...
    for (int j=j_start; j<j_end; j++)
    {
        ...
        if (t0 > local_window_end) continue;
    ...
    while (true) {    
        ...
        if (t_noise > local_window_end) break;
    ...
    // 8- write results to the global signal array
    for (int i=extrema[0]; i<extrema[1]; i++) {
        int index = __mul24(pixel_id, n_window) + i;
