...
def compare_query_files(old_file, new_file):
    ...
    Returns:
        dict or False: 如果存在差异，返回差异结果字典，否则返回False
    """
...
def auto_update_testcases(diff_result, config_file, queries_file):
    ...
    for project in diff_result.get("added_projects", []):
        if project in projects_config:
            ...
            required = config.get("required", "false").lower() == "true"
            is_api = config.get("api", "false").lower() == "true"
    ...
    for project in diff_result.get("removed_projects", []):
        if project in projects_config:
            ...
            print(f"移除项目 {project} 在配置文件中未找到，跳过处理")
            ...
            if added_queries:
                ...
                for query in added_queries:
                    if project in queries_content and query in queries_content[project]:
                        ...
                        print(f"处理新增查询: {query}")
                        try:
                            # 根据查询名称查找对应的字段
                            field_info = None
                            for field_name, info in schema.root_fields.items():
                                if field_name == query:
                                    field_info = info
                                    break
                            ...
                            if field_info:
                                # 生成测试用例
                                if is_api:
                                    test_case = generator._generate_api_definition(query, field_info)
                                    file_suffix = "_api.yml"
                                else:
                                    test_case = generator._generate_query_test_case(query, field_info)
                                    file_suffix = "_test.yml"
                                
                                # 保存测试用例
                                file_path = os.path.join(output_dir, f"{operation_type}_{query}{file_suffix}")
                                with open(file_path, 'w', encoding='utf-8') as f:
                                    yaml.dump(test_case, f, default_flow_style=False, sort_keys=False, allow_unicode=True, width=9999)
                                
                                stats["added_queries"] += 1
                                print(f"新增查询 {query} 的测试用例文件: {file_path}")
                            else:
                                print(f"在Schema中未找到字段 {query}，无法生成测试用例")
                        except Exception as e:
                            print(f"生成新增查询 {query} 的测试用例失败: {e}")
            ...
            for query in changes.get("modified_queries", []):
                if project in queries_content and query in queries_content[project]:
                    ...
                    if files:
                        ...
                        print(f"未找到查询 {query} 对应的测试用例文件，将作为新增查询处理")
                        ...
                        try:
                            ...
                            if field_info:
                                ...
                                stats["added_queries"] += 1
                                print(f"修改查询 {query} 未找到原测试用例，创建新测试用例: {file_path}")
                            else:
                                print(f"在Schema中未找到修改查询字段 {query}，无法生成测试用例")
                        except Exception as e:
