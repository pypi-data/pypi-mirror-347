...
class HttpRunnerTestCaseGenerator:
    ...
    def _generate_single_case(self, output_dir: str, query_name: str, is_api_layer: bool = False):
        ...
        # 根据层级生成相应的测试用例
        if is_api_layer:
            test_case = self._generate_api_definition(query_name, field_info)
            file_suffix = "_api.yml"
        else:
            test_case = self._generate_query_test_case(query_name, field_info)
            file_suffix = "_test.yml"
        
        # 确定操作类型前缀
        operation_type = self._determine_operation_type(query_name)
        
        # 保存测试用例，添加操作类型前缀
        file_path = os.path.join(output_dir, f"{operation_type}_{query_name}{file_suffix}")
    ...
    def _generate_query_test_case(self, field_name: str, field_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        test_case = {
            'config': {
                ...
                'variables': {
                    'user': 'xxx_user' if self.base_url.startswith('http') else f"${{get_config({self.base_url},xxx_user)}}",
                    'pwd': 'xxx_pwd' if self.base_url.startswith('http') else f"${{get_config({self.base_url},xxx_pwd)}}",
                    'sessionId': 'xxx' if self.base_url.startswith('http') else f"${{get_login({self.base_url},$user,$pwd,xxx_session_key)}}"
                }
            },
            'teststeps': [
                {
