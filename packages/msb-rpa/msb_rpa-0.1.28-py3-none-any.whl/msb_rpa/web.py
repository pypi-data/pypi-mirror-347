...
def get_driver_chrome_autoprint(folder: str = r"C:\temp"):
    """
    Global webdriver, where files automatically is printed to specified folder
    ...
    if driver is None:
        ...
        settings = {
            ...
            "saveFile": {
                "defaultDirectory": f"{folder}"
            }
        }
        options.add_experimental_option('prefs', {
            'printing.print_preview_sticky_settings.appState': json.dumps(settings),
            'printing.default_destination_selection_rules.kind': 'local',
        })
        driver = webdriver.Chrome(options=options)
