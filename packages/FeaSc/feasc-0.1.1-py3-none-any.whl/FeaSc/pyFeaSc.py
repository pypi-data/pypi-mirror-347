...
# 多样本整合条件降维方法-1 (LEMUR)
def run_lemur(adata: anndata.AnnData,
              n_dim: int = 64,
              method: str = "mca",
              design: str = "~ label"):
	
	"""
	功能：LEMUR 多样本整合条件条件降维方法
...
if __name__ == '__main__':
	...
	# 设置参数
	config = ScampConfig(path_matrix, path_maker, path_gmt)
	config.method = 'mca'
	config.n_dim = 16
	config.design = "~ stim"
	config.batch = "stim"
	...
	# activity_scores.to_csv(f'../dataout/{filename}_{config.method}_{markername}_scores.csv')
	...
	activity_scores.to_csv(f'../dataout/{filename}_multi_{config.method}_{markername}_scores3.csv')
