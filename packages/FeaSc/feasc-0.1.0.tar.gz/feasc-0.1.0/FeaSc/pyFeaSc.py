...
from FeaSc.pymca import run_mca
from FeaSc.scamp import scamp
...
def run_reduction(
	...
	"""
	功能：
		在 AnnData 对象上运行降维方法，并添加降维结果到 AnnData 对象中。
	...
	if not isinstance(adata, anndata.AnnData):
		raise ValueError('请提供 AnnData 对象')
	if method not in ['mca', 'pca', 'nmf', 'all']:
		raise ValueError("method 必须是 'mca', 'pca', 'nmf' 或 'all' 之一")
	...
	# 获取数据矩阵
	if slot == 'count':
		data = adata[:, use_features].X
	else:
		data = adata.layers[slot][:, use_features]
...
# 多样本整合条件降维方法 (LEMUR)
def run_lemur(adata, n_dim=64, method="mca", design="~ label"):
	# try:
	# 	from FeaSc import pylemur  # 安装后
	# except ImportError:
	# 	import pylemur  # 本地调试
	from FeaSc import pylemur
	"""
	LEMUR 多样本单条件降维方法
	:param adata: AnnData 对象
	:param n_dim: 降维后的维度数
	:param method: 降维方法('mca', 'pca')
	:param design: 设计矩阵，默认为 "~ label"
	:return: AnnData 对象，添加了降维结果到 adata.obsm 和 adata.varm 中
	"""
	
	model = pylemur.tl.LEMUR(adata, design=design, n_embedding=n_dim, method=method)
	model.fit()
	model.align_with_harmony()
	
	embedding = model.embedding
	loading = model.reconstruct_harmony_loading().T
	
	adata.obsm[f"multi_{method}"] = embedding
	adata.varm[f"multi_{method}"] = loading
...
def run_scamp(adata,
	...
	topic_prop, beta = scamp(adata, n_topics=n_dim, method=method,
	                         n_comps=n_comps, n_comps_1=n_comps_1,
	                         batch=batch,
	                         flag=flag)
	adata.obsm.clear()
	adata.varm.clear()
...
# 多样本多条件降维方法
def run_multi_reduction(adata, n_dim=64, method="mca", design="~ label"):
	from FeaSc import pylemur
	"""
	基于 LEMUR 多样本多条件降维方法
	:param adata: AnnData 对象
	...
	for i, loading in enumerate(loadings):
		adata.varm[f"multi_{method}_{i + 1}"] = loading
...
def read_gmt(gmt_file: str) -> dict[str, list[str]]:
	...
	返回:
		字典：键（Key） 的类型是 str（字符串）。
			 值（Value） 的类型是 list[str]（字符串列表）。
	"""
...
def scoring(
	...
	# 计算加权得分
	score = weighting(wm, zweight.values, dweight.values, sd_weight)
	
	# 标准化
	if normalize == 1:
		score = (score - np.mean(score)) / np.std(score)
	elif normalize == 2:
		bscore = weighting(wm, bweight.values, dweight.values, sd_weight)
...
def run_regression(
	...
	# 检查基因集中有表达的基因数量
	if (gene_rate['geneset'] > 0).sum() < 5:
		raise ValueError("基因表达数据中找到的基因少于5个")
...
def calculate_activity(
	...
	# 检查降维方法是否存在
	if method not in adata.obsm:
		print(f"错误: 未找到指定的降维方法: {method}")
		return None
	
	# 运行回归分析
	reg_out = run_regression(adata, gene_rate=gene_rate, method=method)
...
def calculate_multi_activity(
	...
	# 运行回归分析
	reg_outs = run_multi_regression(adata, gene_rate=gene_rate, method=method)
...
if __name__ == '__main__':
	...
	# 背景基因集路径
	path_bg = "../data/gmt/kegg.gmt"
	
	# 表达矩阵路径
	path_gse = "../data/h5ad/GSE96583.h5ad"
	
	# 标志基因集路径
	path_b = "../data/maker/B-cell_marker.txt"
	
	# 设置参数
	config = {
			"path_matrix": path_gse,
			"path_maker":  path_b,
			"path_gmt":    path_bg,
			"method":      'mca',
			"n_dim":       16,
	...
	"""单样本分析"""
	# 单样本降维
	# run_reduction(adata, method=config['method'], n_dim=config['n_dim'])
	# # 计算活性得分
	...
	# activity_scores.to_csv(f'../output/{filename}_{config["method"]}_{markername}_scores.csv')
	
	"""多样本分析"""
	# 多样本多条件降维
	# adata = run_multi_reduction(adata, method=config['method'], n_dim=config['n_dim'], design=config['design'])
	...
	# 多样本整合条件降维 (lemur)
	# adata = run_lemur(adata, n_dim=config['n_dim'], method=config['method'], design=config['design'])
	...
	# 多样本整合条件降维 (scamp)
	adata = run_scamp(adata, n_dim=config['n_dim'], method=config['method'], flag=False)
	...
	filename = Path(config['path_matrix']).stem  # 'CHOL_GSE142784'
	markername = Path(config['path_maker']).stem  # 'B-cell_marker'
	activity_scores.to_csv(f'../dataout/{filename}_multi_{config["method"]}_{markername}_scores1.csv')
