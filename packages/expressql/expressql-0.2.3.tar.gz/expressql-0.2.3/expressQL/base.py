...
class SQLExpression:
    ...
    def __init__(
        ...
        if expression_type == "column" and not skip_validation:
            validate_name(expression_value, allow_dot=True, allow_digit=True)
...
class CurrentTimestamp(SQLExpression):
    def __init__(self, *args, **kwargs):
        self.expression_type = "column"
        self.expression_value = "CURRENT_TIMESTAMP"

    def sql_string(self, include_sign: bool = False, include_sign_only_if_negative: bool = True, invert = True, include_alias = None) -> str:
...
class Func(SQLExpression):
    ...
    def copy_with(self, func_name: str = None, args: List[SQLExpression] = None,
        ...
        return Func(
            ...
            *[arg.copy() for arg in args] if args else [arg.copy() for arg in self.args] if arguments is None else arguments,
            tag=tag or self.tag,
...
class InSubquery(SQLComparison):
    def __init__(self, subquery: str, placeholders: List[str] = None):
        if isinstance(subquery, SubQuery) or isinstance(subquery, SQLExpression) and subquery.expression_type == "query":
            subquery, placeholders = subquery.placeholder_pair()
        elif isinstance(subquery, (tuple, list)) and len(subquery) == 2:
            subquery, placeholders = subquery
