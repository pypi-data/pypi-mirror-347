...
data = np.load('/home/tihan/Arindam/tihan-sdv/tihan/sdv/visualizer/calibration_data.npz')
mtx = data['mtx']
dist = data['dist']
...
car_img = cv2.imread('/home/tihan/Arindam/tihan-sdv/tihan/sdv/visualizer/Resources/Images/maini_cart.png', cv2.IMREAD_UNCHANGED)
...
def draw_bezier_curve(img, p0, p1, p2, segments=30):
    for i in range(segments - 1):
        ...
        # thickness = max(1, int(line_thickness * (1 - (i / segments))))  # Fade thickness
        cv2.line(img, (x1, y1), (x2, y2), color, 4, lineType=cv2.LINE_AA)

def get_zone_color(index, total):
    ratio = index / total
    if ratio < 0.33:
        return (0, 255, 0)  # Soft green
    elif ratio < 0.66:
        return (0, 255, 255)  # Soft yellow
    else:
        return (0, 0, 255)  # Soft red
...
def draw_trajectory_lines(frame, steering_angle):
    ...
    for side in [-track_offset, track_offset]:
        p0 = (center_x + side, start_y)
        p2 = (center_x + side + offset_x, start_y - curve_length)
        p1 = (center_x + side + offset_x // 2, start_y - curve_length // 2)
