...
def factor_addindumv(factor=None, dbo=None, dts='20100101', dte='20241101', freq='m', offset='first'):
    ...
    factor = factor.join(
        factor.next_open.unstack().pct_change(fill_method=None).shift(-1).stack().to_frame('nextadjopenpct'))
    factor = factor.join(
        factor.adjclose.unstack().pct_change(fill_method=None).shift(-1).stack().to_frame('adjclosepct'))
