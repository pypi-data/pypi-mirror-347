...
def _sync_hook_implementation(
    ...
    try:
        ...
        messages_to_analyze, original_formatting_metadata = adapter.format_input_for_pillar(
            pillar, extracted
        )
        ...
        if extracted.stream:
            return adapter.handle_sync_stream(
                ...
                analyze_func=pillar.analyze_sync,
            )
        else:
        ...
        try:
            pillar.logger.warning(
                f"Falling back to original call for {log_function_name} due to hook error."
            )
    ...
    try:
        ...
        if extracted.stream:
            return adapter.handle_async_stream(
                ...
                stream_result=result,
                extracted_input=extracted,
                analyze_func=pillar.analyze_async,
            )
        else:
            # 7. Extract Output for Pillar
            output_messages = adapter.extract_output_for_pillar(pillar, result, extracted)
...
def create_generic_hook(pillar: "Pillar", adapter: FrameworkAdapter) -> Callable:
    ...
    if is_async_hook:
        ...
        @pillar.with_asession()
        async def async_hook_wrapper(
            wrapped: Callable, instance: Any, args: tuple, kwargs: dict
        ) -> Any:
            """Async hook wrapper that calls the core implementation."""
            return await _async_hook_implementation(
                pillar, adapter, wrapped, instance, args, kwargs
            )
