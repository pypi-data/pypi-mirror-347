...
class TaskRun(contextlib.AbstractContextManager):
    ...
    def __str__(self):
        args = ["..." if type(x) is list else repr(x) for x in self.args]
    ...
    def flow_progress(self):
        if flow_run := self.config.task.flow.run:
    ...
    def task_logging(self):
        ...
        duration = end - start
        if self.config.task.flow.verbose:
            self.config.task.flow.console.log(f"Task {self} finished in {duration:.2f}s")
