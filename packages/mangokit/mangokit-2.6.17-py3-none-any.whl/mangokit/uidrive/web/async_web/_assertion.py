...
from mangokit.decorator.method_callback import async_method_callback
...
class AsyncWebAssertion(metaclass=Meta):
    ...
    async def w_to_have_count(self, actual: Locator, expect: str):
        """元素是几个"""
        await exp(actual).to_have_count(int(expect)), f'实际={actual.count()}, 预期={expect}'
