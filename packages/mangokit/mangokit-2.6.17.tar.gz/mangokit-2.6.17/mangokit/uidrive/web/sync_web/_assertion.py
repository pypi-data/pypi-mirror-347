...
from mangokit.decorator.method_callback import sync_method_callback
...
class SyncWebAssertion(metaclass=Meta):
    ...
    def w_to_have_count(self, actual: Locator, expect: str):
        """元素是几个"""
        exp(actual).to_have_count(int(expect)), f'实际={actual.count()}, 预期={expect}'
