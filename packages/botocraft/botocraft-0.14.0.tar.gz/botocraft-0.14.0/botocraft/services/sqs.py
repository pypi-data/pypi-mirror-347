...
class QueueManager(QueueManagerMixin, Boto3ModelManager):
    ...
    def list(self, *, QueueNamePrefix: Optional[str] = None) -> List[str]:
        """
        Returns a list of your queues in the current region. The response includes a maximum of 1,000 results. If you
        specify a value for the optional ``QueueNamePrefix`` parameter, only queues with a name that begins with the
    ...
    def receive_messages(
        ...
        *,
        ...
        MessageSystemAttributeNames: Optional[
            List[
                Literal[
                    "All",
                    "SenderId",
    ...
    def batch_send_messages(
        ...
        """
        You can use ``SendMessageBatch`` to send up to 10 messages to the specified queue by assigning either identical
        or different values to each message (or by not assigning values at all). This is a batch version of
        ``SendMessage.`` For a FIFO queue, multiple messages within a single batch are enqueued in the order they are
        sent.
    ...
    def batch_delete_messages(
        ...
        of the action on each message is reported individually in the response.
    ...
    def send(
        ...
        *,
        ...
        MessageSystemAttributes: Optional[
            Dict[Literal["AWSTraceHeader"], "MessageSystemAttributeValue"]
        ] = None,
...
class Queue(TagsDictMixin, PrimaryBoto3Model):
    ...
    def pk(self) -> Optional[str]:
        """
        Return the primary key of the model.   This is the value of the :py:attr:`QueueUrl` attribute.
    ...
    def name(self) -> Optional[str]:
        """
        Return the name of the model.   This is the value of the :py:attr:`QueueName` attribute.
    ...
    def purge(self) -> "None":
        ...
        return (
            cast(QueueManager, self.objects)
            .using(self.session)
            .purge(
...
class MessageAttributeValue(Boto3Model):
    """
    The user-specified message attribute value. For string data types, the ``Value`` attribute has the same restrictions
    on the content as the message body. For more information, see  ``SendMessage.``
...
class Message(PrimaryBoto3Model):
    ...
    A new receipt handle is returned every time you receive a message. When deleting a message, you provide the last
    ...
    You can use this attribute to verify that Amazon SQS received the message correctly. Amazon SQS URL-decodes the
...
class ListQueuesResult(Boto3Model):
    ...
    QueueUrls: Optional[List[str]] = None
    """
...
class SendMessageBatchResult(Boto3Model):
    """
    For each message in the batch, the response contains a  ``SendMessageBatchResultEntry``  tag if the message succeeds
...
class DeleteMessageBatchResult(Boto3Model):
    ...
    deleted or a  ``BatchResultErrorEntry``  tag if the message can't be deleted.
    """
...
class ChangeMessageVisibilityBatchResult(Boto3Model):
    """
    For each message in the batch, the response contains a  ``ChangeMessageVisibilityBatchResultEntry``  tag if the
...
class SendMessageResult(Boto3Model):
    ...
    MD5OfMessageBody: Optional[str] = None
    """
    An MD5 digest of the non-URL-encoded message body string.
