...
def main():
    ...
    if a.extension in audio:
        ...
        ytdlp = (f"{venv}/bin/yt-dlp -f '{filt}' --merge-output-format {merge} "
    ...
    for line in out.splitlines():
        if "Destination:" in line: name = line.split("Destination: ")[1].strip(); break
        if "has already been downloaded" in line:
            start=line.find("] ")+2; end=line.find(" has"); name=line[start:end].strip(); break
