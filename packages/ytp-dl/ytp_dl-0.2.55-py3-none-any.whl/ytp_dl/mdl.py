...
def main():
    ...
    parser.add_argument("--output-dir", help="Directory to save the downloaded file", default="/root")
    ...
    if args.extension and args.extension in audio_extensions:
        # Audio download
        ytdlp_cmd = (
            f"{sys.executable} -m yt_dlp -x --audio-format {args.extension} "
            f"--embed-metadata "
        ...
        ytdlp_cmd = (
            f"{sys.executable} -m yt_dlp -f '{format_filter}' --merge-output-format {merge_extension} "
