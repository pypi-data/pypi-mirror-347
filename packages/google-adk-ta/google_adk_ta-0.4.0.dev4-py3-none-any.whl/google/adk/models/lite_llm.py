...
def _model_response_to_generate_content_response(
    ...
    if response.get("choices", None):
        message = response["choices"][0].get("message", None)
        response_usage = response.get("usage", {})  # dev2

    if not message:
        raise ValueError("No message in response")
    # return _message_to_generate_content_response(message)
    response_cost = None  # dev2
    if hasattr(response, "_hidden_params") and isinstance(
        response._hidden_params, dict
    ):  # dev2
        response_cost = response._hidden_params.get("response_cost", None)  # dev2

    return _message_to_generate_content_response(
        message, response_cost=response_cost, response_usage=response_usage
    )  # dev2
