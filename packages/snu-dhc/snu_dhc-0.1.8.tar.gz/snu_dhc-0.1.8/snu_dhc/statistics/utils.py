...
def _multi_sen(true, pred, cls):
    cm = confusion_matrix(true, pred, labels=np.unique(true))
    TP = cm[cls, cls]
    FN = cm[cls, :].sum() - TP
    return TP / (TP + FN + 1e-8)

def _multi_spe(true, pred, cls):
    cm = confusion_matrix(true, pred, labels=np.unique(true))
    TP = cm[cls, cls]
    FP = cm[:, cls].sum() - TP
    TN = cm.sum() - (TP + FP + cm[cls, :].sum() - TP)
    return TN / (TN + FP + 1e-8)

def _multi_ppv(true, pred, cls):
    cm = confusion_matrix(true, pred, labels=np.unique(true))
    TP = cm[cls, cls]
    FP = cm[:, cls].sum() - TP
    return TP / (TP + FP + 1e-8)

def _multi_npv(true, pred, cls):
    cm = confusion_matrix(true, pred, labels=np.unique(true))
    TP = cm[cls, cls]
    FN = cm[cls, :].sum() - TP
    TN = cm.sum() - (cm[cls, :].sum() + cm[:, cls].sum() - TP)
    return TN / (TN + FN + 1e-8)

def _multi_f1(true, pred, cls):
    cm = confusion_matrix(true, pred, labels=np.unique(true))
    TP = cm[cls, cls]
    FP = cm[:, cls].sum() - TP
    FN = cm[cls, :].sum() - TP
...
# Wrapper function
def multi_classification_metrics(true, pred, n_iter=1000, alpha=0.95):
    classes = np.unique(true)
    results = {}

    for cls in classes:
        metrics = {}
        metrics['sen'] = bootstrap_ci(true, pred, lambda t, p: _multi_sen(t, p, cls), n_iter, alpha)
        metrics['spe'] = bootstrap_ci(true, pred, lambda t, p: _multi_spe(t, p, cls), n_iter, alpha)
        metrics['ppv'] = bootstrap_ci(true, pred, lambda t, p: _multi_ppv(t, p, cls), n_iter, alpha)
        metrics['npv'] = bootstrap_ci(true, pred, lambda t, p: _multi_npv(t, p, cls), n_iter, alpha)
        metrics['f1']  = bootstrap_ci(true, pred, lambda t, p: _multi_f1(t, p, cls), n_iter, alpha)
