...
def cmd(commands:str,isEcho:bool=False):
	try:
        ...
        # 执行命令
		process = subprocess.Popen("call " + execfile, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, universal_newlines=True)
		# 输出结果
...
class SshClient:
	...
	def readResult(self,timeout:int=0):
		if timeout==0:
			while True:
				output=self.chn.recv(-1).decode('utf-8','ignore')
