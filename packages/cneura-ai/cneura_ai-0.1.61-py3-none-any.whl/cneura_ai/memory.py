...
class MemoryCollection:
    ...
    async def retrieve(self, query: str, top_k: int = 3, threshold: float = 0.7) -> List[str]:
        embedding = await asyncio.to_thread(self.safe_encode, query)

        results = self.collection.query(
            query_embeddings=[embedding.tolist()],
            n_results=top_k,
            include=["documents", "distances"]  # Only valid keys for include
        )

        # Chroma returns results["documents"], results["distances"], and results["ids"]
        documents = []
        keys = []

        for i, distance in enumerate(results["distances"][0]):
            similarity = 1.0 - distance  # Chroma uses cosine distance
            if similarity >= threshold:
                documents.append(results["documents"][0][i])
                keys.append(results["ids"][0][i])  # This is returned automatically

        logger.info(f"Retrieved documents (similarity >= {threshold}): {documents}")
