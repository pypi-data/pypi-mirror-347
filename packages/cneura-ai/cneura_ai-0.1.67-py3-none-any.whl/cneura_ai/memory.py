...
class MemoryCollection:
    ...
    threshold: float = 0.3
        ...
        for idx, stored_embedding in enumerate(all_embeddings):
            print("before",idx, stored_embedding)
            if stored_embedding is None:
                continue
            stored_embedding_np = np.array(stored_embedding).reshape(1, -1)
            similarity = cosine_similarity(query_embedding_np, stored_embedding_np)[0][0]

            logger.debug(f"[Manual Cosine Similarity] doc_id={all_ids[idx]} similarity={similarity:.4f}")

            if similarity >= threshold:
                documents.append(all_documents[idx])
                keys.append(all_ids[idx])

        if len(documents) > top_k:
            similarities = [
                cosine_similarity(query_embedding_np, np.array(emb).reshape(1, -1))[0][0]
                for emb in all_embeddings if emb is not None
            ]
            top_indices = sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)[:top_k]
            documents = [documents[i] for i in top_indices]
            keys = [keys[i] for i in top_indices]

        logger.info(f"Retrieved documents (manual cosine similarity >= {threshold}): {documents}")
