...
class MemoryCollection:
    def __init__(self, HOST, PORT, namespace: str, name: str, model: SentenceTransformer = SentenceTransformer('all-mpnet-base-v2')):
        try:
        ...
        if collection_name in existing_collections:
            ...
            self.collection = self.client.create_collection(collection_name, embedding_function=None,  metadata={"hnsw:space": "cosine"})
    ...
    async def store(self, key: str, content: str, metadata: Dict[str, Any] = None, similarity_threshold: float = 0.95):
        ...
        content = self.normalize(content)  # Normalize content before encoding
    ...
    async def retrieve(self, query: str, top_k: int = 3, threshold: float = 0.3, use_similarity: bool = True) -> List[str]:
        query = self.normalize(query)  # Normalize query before encoding
        ...
        for i, distance in enumerate(raw_distances):
            ...
            if use_similarity:
                logger.debug(f"[Similarity Filter] Distance: {distance:.4f}, Similarity: {similarity:.4f}")
                if similarity >= threshold:
                    documents.append(raw_documents[i])
                    keys.append(raw_ids[i])
            else:
                logger.debug(f"[Distance Filter] Distance: {distance:.4f}")
