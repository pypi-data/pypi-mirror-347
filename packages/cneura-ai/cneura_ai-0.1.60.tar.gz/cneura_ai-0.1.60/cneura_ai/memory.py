...
class MemoryCollection:
    ...
    async def retrieve(self, query: str, top_k: int = 3, threshold: float = 0.85) -> List[str]:
        ...
        # ChromaDB returns cosine distances (0 = identical, 1 = orthogonal), so similarity = 1 - distance
        documents = []
        keys = []
        ...
        for i, distance in enumerate(results["distances"][0]):
            ...
            if similarity >= threshold:
                ...
                keys.append(results["ids"][0][i])

        logger.info(f"Retrieved documents (above threshold {threshold}): {documents}")
        logger.info(f"Retrieved keys (above threshold {threshold}): {keys}")
