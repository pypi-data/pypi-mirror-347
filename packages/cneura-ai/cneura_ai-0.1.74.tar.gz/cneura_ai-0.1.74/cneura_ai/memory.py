...
class MemoryCollection:
    def __init__(self, HOST, PORT, namespace: str, name: str):
        try:
    ...
    async def store(self, key: str, content: str, metadata: Dict[str, Any] = None, similarity_threshold: float = 0.95):
        ...
        embedding = await self.model.aembed_query(content)
        ...
        existing_embeddings = all_data.get("embeddings") or []
    ...
    def safe_encode(self, query):
        if isinstance(query, list):
            ...
            query = [str(query)]
        return asyncio.run(self.model.aembed_query(query[0]))
