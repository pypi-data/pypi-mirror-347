"""
CLI command for running agent evaluations using the new V2 (ForkableResource) framework.
"""
import asyncio
try:
    import yaml
...
from reward_kit.agent_v2 import Orchestrator # Updated import path
...
def agent_eval_v2_command(args):
    """
    Run agent evaluation using the new Orchestrator and ForkableResource framework.
    """
    logger = logging.getLogger("agent_eval_v2") # Use a specific logger
    logger.info("Starting agent-eval-v2 command.")
    ...
    try:
        ...
        logger.info("agent-eval-v2 command finished successfully.")
        return 0
    except Exception as e:
        logger.error(f"Error during agent-eval-v2 execution: {e}")
        import traceback
