...
class MultilayeredNetwork(nn.Module):
    ...
    def activation_function(self, x: torch.Tensor) -> torch.Tensor:
        ...
        tanh(ReLU(self.tanh_steepness * x)), but replacable if needed.
...
class trainable_model_snb(MultilayeredNetwork):
    def __init__(
        self,
        all_weights: Union[torch.Tensor, sparse.spmatrix],
        sensory_indices: arrayable,
        ...
        all_types = set(idx_to_group.values())
        num_types = len(all_types)
        type2idx = {t: i for i, t in enumerate(all_types)}
        # default value tanh_steepness from MultilayeredNetwork
        self.slope = nn.Parameter(
            self.tanh_steepness * torch.ones(num_types),
        )
    ...
    def activation_function(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # slopes and biases are of shape (num_neurons, 1)
...
def train_model(
    ...
    Args:
        model (trainable_model_snb): The model to train.
    ...
    def get_activation_loss(actual_activations, targets):
        """Calculate the activation loss for the model."""
        ...
        for b in targets["batch"].unique():
            ...
            for _, row in batch_targets.iterrows():
                ...
                # get the max of neuron activation across timesteps
                actual_value = actual_activations[b, row["neuron_idx"], :].mean()
                batch_loss += (actual_value - target_value) ** 2
    ...
    # check column names of targets
    if not all(col in targets.columns for col in ["batch", "neuron_idx", "value"]):
        raise ValueError(
            ...
            wandb.log(
                {
                    ...
                    "steepness": model.tanh_steepness,
                    "paaram_reg_lambda": param_reg_lambda,
...
def saliency(
    ...
    """
    Computes saliency maps indicating the importance of each input element for specific
    neurons' activations. Note: by default, the input is divided by `divide_by`. This is
    because: if an input results in big activation of the neurons of interest, the
    saturating part of the tanh activation function is used, resulting in small
    gradients even when the input is important. So when calculating saliency, it's good
    to divide the input by a number, so that the activations of the neurons of interest
    is not too big.

    Args:
        model: A PyTorch model with activations attribute
        ...
        method: Saliency computation method. Options are:
            - "vanilla": Standard saliency map
            - "input_x_gradient": Input multiplied by gradient
        normalize: Whether to normalize saliency maps by their maximum value. Defaults
            to False.
    ...
    # if input_tensor is numpy, convert to tensor
    if isinstance(input_tensor, np.ndarray):
        input_tensor = torch.tensor(input_tensor, device=device)

    input_tensor = input_tensor / divide_by
    ...
    # Sum activations of neurons we care about (this is our "signal" to backpropagate)
    # Note: We're not calculating a "loss" to minimize - we're creating a scalar
    # that depends on the neurons we want to analyze
    activation_signal = 0
