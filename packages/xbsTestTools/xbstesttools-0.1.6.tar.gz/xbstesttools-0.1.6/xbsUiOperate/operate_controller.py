...
class XbsUiTest:
    ...
    def get_u2_xml(self):
        ...
        try:
            try:
                ...
                logging.debug(f"adb -s {self.dev} shell /data/local/tmp/atx-agent server --stop")
                subprocess.run(f"adb -s {self.dev} shell /data/local/tmp/atx-agent server --stop")  # 停止服务

                logging.debug(f"adb -s {self.dev} shell /data/local/tmp/atx-agent server -d")
                subprocess.run(f"adb -s {self.dev} shell /data/local/tmp/atx-agent server -d")  # 启动服务
                sleep(2)

                logging.debug(f"adb -s {self.dev} uninstall com.github.uiautomator.test")
                os.system(f"adb -s {self.dev} uninstall com.github.uiautomator.test")

                logging.debug(f"adb -s {self.dev} uninstall com.github.uiautomator")
                os.system(f"adb -s {self.dev} uninstall com.github.uiautomator")
                sleep(2)
                try:
                    d = u2.connect(self.dev)
                except:
                    logging.error(f"{self.dev} U2 初始化失败")
                    return False
            xml = d.dump_hierarchy()
            return xml
        except Exception as e:
            # logging.error(f"{self.dev} U2查找报错:{e}")
            return
    ...
    def get_shell_xml(self):
        ...
        通过adb获取XML
        """
        os.popen(f"adb -s {self.dev} shell uiautomator dump /sdcard/{self.dev}_ui.xml")
        sleep(3)
        os.popen(f"adb -s {self.dev} pull /sdcard/{self.dev}_ui.xml")
        sleep(3)
        # 解析XML文件
        dom = minidom.parse(f'{self.dev}_ui.xml')
...
class XbsUiOperate:
        ...
        if image_path:
            ...
            for i in range(timeout):
                ...
                if widget_info:
                    return True
                sleep(1)
        logging.warning(f"未找到元素 text:{text} index:{index} resourceId:{resourceId} class:{class_} image_path: {image_path}")
        return False
    ...
    def roll(self, roll_type="down_to_top", ratio="middle", edge=False):
        ...
        if not size:
            logging.error(f"{self.dev} 滑动屏幕失败")
            return False
    ...
    def app_start(self, package_name):
        ...
        try:
            ...
            logging.error(f"打开应用{package_name}失败: {e}")
            return False
        return True
    ...
    def install_app(self, apk_file):
        if not os.path.exists(apk_file):
            logging.error(f"{self.dev} 文件不存在： {apk_file}")
            return False
    ...
    def get_size(self):
        try:
            ...
            logging.error(f"{self.dev} 获取屏幕分辨率失败")
            return False
