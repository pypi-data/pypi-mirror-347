...
class XbsUiTest:
    ...
    def get_shell_xml(self):
        ...
        通过adb获取XML
        """
        os.system(f"adb -s {self.dev} shell uiautomator dump /sdcard/{self.dev}_ui.xml")
        os.system(f"adb -s {self.dev} pull /sdcard/{self.dev}_ui.xml")
...
class XbsUiOperate:
    ...
    def get_image_in_screenshot(self, small_image_path, threshold):
        ...
        coordinate[0] += height // 2  # 获取x轴中心
        coordinate[1] += width // 2  # 获取y轴中心
        return coordinate
    ...
    def app_start(self, package_name):
        ...
        try:
            command = f"adb -s {self.dev} shell monkey -p {package_name} -c android.intent.category.LAUNCHER 1"
    ...
    def screenrecord(self, file=None, time=None):
        """
        录制视频
        """
        try:
