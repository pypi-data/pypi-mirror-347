...
class DocumentResource(Resource):
    ...
    def create(
        ...
        data = {
            ...
            "collection": collection if isinstance(collection, str) else collection.id,
        }
        if batch_id:
            data["batch"] = batch_id
    ...
    @retry_with_backoff()
    def get(self, document: Union[str, Document]) -> Document:
        """Retrieves the document object. This doesn't retrieve the document itself, just the metadata associated with it"""
        document_id = document if isinstance(document, str) else document.id
    ...
    @retry_with_backoff()
    def delete(self, document: Union[str, Document]) -> dict:
        """Delete a document"""
        document_id = document if isinstance(document, str) else document.id
