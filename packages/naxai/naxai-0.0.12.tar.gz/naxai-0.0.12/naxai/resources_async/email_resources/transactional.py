...
class TransactionalResource:
    ...
    async def send(self, data: SendTransactionalEmailRequest):
        """
        ...
        Args:
            data (SendTransactionalEmailModel): A model containing all the information needed to send
                ...
                - template_id (Optional[str]): ID of a template to use instead of providing HTML/text
        ...
        Example:
            ...
            >>> response = await client.email.transactional.send(
        ...
        Note:
            ...
            - If using a template, the template_data should contain all variables required by the template
            ...
            - Tracking options (track_opens, track_clicks) require proper configuration of tracking domains
        ...
        """
        return SendTransactionalEmailResponse.model_validate_json(json.dumps(await self._client._request("POST", self.root_path + "/send", json=data.model_dump(by_alias=True, exclude_none=True), headers=self.headers)))
