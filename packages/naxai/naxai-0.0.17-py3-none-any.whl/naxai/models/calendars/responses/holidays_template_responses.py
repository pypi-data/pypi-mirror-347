...
class ListHolidaysTemplatesResponse(BaseModel):
    ...
    This class defines the structure for the API response when retrieving a list of holiday templates.
    It provides list-like behavior for accessing the templates, including length, indexing, and iteration.
    ...
    Note:
        - This class implements list-like behavior through __len__, __getitem__, and __iter__ methods
        - The model_validate_json method handles both array-style JSON and object-style JSON
        - When the API returns a plain array of templates, it's automatically wrapped in the root field
        - This model uses Pydantic's validation and serialization features
    """
