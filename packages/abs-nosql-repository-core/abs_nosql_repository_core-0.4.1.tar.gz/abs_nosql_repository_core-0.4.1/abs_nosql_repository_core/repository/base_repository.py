...
from ..schema import ListFilter, SortDirection, LogicalOperator, Operator, FindUniqueByFieldInput, NewListFilter, FieldRule, Aggregations, AggregationType
...
class BaseRepository:
    ...
    def _build_query_filter(self, find: ListFilter, collection_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        base_filter = self._build_filter_condition(find.filters.model_dump(), collection_name) if find.filters else None
    ...
    def _build_filter_condition(self, filter_dict: Dict[str, Any], collection_name: Optional[str] = None) -> Dict[str, Any]:
        """Recursively build MongoDB filter"""
        if not isinstance(filter_dict, dict):
            return {}

        if "operator" in filter_dict and "conditions" in filter_dict:
            conditions = [self._build_filter_condition(cond, collection_name) for cond in filter_dict["conditions"]]
        ...
        if all(k in filter_dict for k in ["field", "operator", "value"]):
            try:
                ...
                return apply_condition(model, op, filter_dict["field"], coerce_value(filter_dict["value"]))
            except ValueError:
                raise BadRequestError(f"Invalid comparison operator: {filter_dict['operator']}")

        return {}

    async def get_all(self, find: NewListFilter, collection_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if not find.aggregations:
            pipeline += [{"$skip": skip}, {"$limit": page_size}]

        data = await self.db[collection_name].aggregate(pipeline).to_list(length=None)
        count_result = await self.db[collection_name].aggregate(count_pipeline).to_list(length=1)
        if find.aggregations:
            count = len(data)
            total_pages = 1
        else:
    ...
    def _build_field_rules_stage(
        ...
        for rule in field_rules:
            ...
            condition_expr = self._build_recursive_condition(
                operator=conditions.operator.lower(),
    ...
    def _build_recursive_condition(
        self, operator: str, conditions: List[Dict[str, Any]], collection_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """Recursively construct MongoDB logical expressions for filter conditions"""
        logical_map = {"and": "$and", "or": "$or"}
        mongo_op = logical_map.get(operator.lower())
        ...
        for cond in conditions:
            if hasattr(cond, "field") and hasattr(cond, "value") and hasattr(cond, "operator"):
                expressions.append(
                    apply_condition(
                        model,
            ...
            elif hasattr(cond, "operator") and hasattr(cond, "conditions"):
                expressions.append(
                    self._build_recursive_condition(cond.operator, cond.conditions, collection_name)
                )

        return {mongo_op: expressions}
