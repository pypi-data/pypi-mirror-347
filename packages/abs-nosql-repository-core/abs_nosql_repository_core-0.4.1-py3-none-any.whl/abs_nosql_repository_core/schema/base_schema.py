...
class Aggregations(BaseModel):
    group_by: List[str]
    document_inclusion_mode: Optional[Literal["none","all","partial"]] = Field(default="none",description="Options: 'none', 'all', 'partial' . 'none' means no document fields will be included in the response, 'all' means all document fields will be included in the response, 'partial' means only the document fields that match the included_fields will be included in the response")
    included_fields: Optional[List[str]] = Field(default=None,description="Fields to include when document_inclusion_mode is 'partial'")
