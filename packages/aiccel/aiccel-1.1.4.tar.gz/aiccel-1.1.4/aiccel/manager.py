...
class AgentManager:
    ...
    def _clean_and_parse_json(self, response: str, trace_id: int, context: str) -> List[Dict[str, str]]:
        ...
        # Log raw response (truncated for safety)
        ...
        # Step 2: Try direct parsing with orjson
        try:
        ...
        # Step 3: Attempt to extract JSON substring
        json_pattern = r'\[\s*\{.*?\}\s*\]'
        ...
        # Step 4: Fallback to manual parsing (last resort)
        try:
            # Split response into potential sub-query entries
        ...
        # Step 5: Return empty list if all parsing fails
    ...
    async def collaborate_async(self, query: str, max_agents: int = 5, agent_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        async with self:  # Manage HTTP session
            ...
            agent_descriptions_text = self._build_agent_descriptions()
            query_split_prompt = (
                f"Instructions: Analyze the query and split it into distinct sub-queries, assigning each to the most appropriate agent based on their expertise and tools. "
                ...
                "Examples:\n"
                "Query: 'whats the weather in kollam now? and tell me about spur ai'\n"
                "```json\n"
                "[\n"
                "  {\"sub_query\": \"whats the weather in kollam now?\", \"agent\": \"weather_expert\"},\n"
            ...
            # Fallback: Simplify prompt and try again
            if not sub_queries:
                self.logger.warning("Primary query splitting failed, attempting simplified prompt")
                simplified_prompt = (
                    f"Instructions: Analyze the query and assign it to the most appropriate agent. "
            ...
            if agent_ids:
                selected_agents = [aid for aid in agent_ids if aid in self.agents and aid in selected_agents][:max_agents]
            ...
            async def run_agent_with_semaphore(agent_name: str, agent: Agent, queries: List[str]) -> Dict[str, Any]:
                async with self.semaphore:
                    ...
                    try:
                        # Combine queries for the same agent
            ...
            synthesis_prompt_parts = [
                "You combine agent outputs into a single, concise, and coherent response for the query. "
                ...
                "If an agent reports an error, state that the information could not be retrieved for that part of the query and provide any available data. "
                "Organize the response clearly, addressing each part of the original query. "
            ...
            for future in asyncio.as_completed(tasks):
                ...
                synthesis_prompt_parts.append(f"Agent {agent_name} (for queries: {', '.join(queries)}):\n{response}\n\n")
