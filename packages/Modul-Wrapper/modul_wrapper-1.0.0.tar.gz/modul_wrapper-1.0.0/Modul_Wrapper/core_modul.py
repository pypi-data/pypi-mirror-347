...
class Wrap:
	...
	def Path_Function(kimin, config, *args, **kwargs):
		...
		# class_name = config['object'] if not config['module'] == "" and not config['object'] == "" and not path == "" else config['module'] if not config['module'] == "" and config['object'] == "" and path == "" else config['object'] if not config['module'] == ""
		...
		if path == "":
			...
			# if not config['module'] == "" and len(config['module']) > 0:
				# ext = ".".join(config['module'])
				# path = f""
