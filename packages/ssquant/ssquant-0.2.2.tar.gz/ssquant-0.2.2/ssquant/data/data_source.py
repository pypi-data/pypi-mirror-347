...
class DataSource:
        ...
        for i, order in enumerate(self.pending_orders):
            ...
            if execution_time <= self.current_idx:
                ...
                if action == "开多":
                    ...
                    self.target_pos = self.current_pos - min(volume, max(0, self.current_pos))
                elif action == "开空":
                    self.target_pos = self.current_pos - volume
                elif action == "平空":
                    if volume is None:
    ...
    def buy(self, volume: int = 1, reason: str = "", log_callback=None, order_type='bar_close'):
        ...
        if order_type == 'bar_close':
            ...
            if 'ask1' in self.data.columns and self.current_idx < len(self.data):
                # 使用当前tick的卖一价格作为成交价
                price = self.data.iloc[self.current_idx]['ask1']
            else:
    ...
    def sell(self, volume: Optional[int] = None, reason: str = "", log_callback=None, order_type='bar_close'):
        ...
        if order_type == 'bar_close':
            ...
            if 'bid1' in self.data.columns and self.current_idx < len(self.data):
                # 使用当前tick的买一价格作为成交价
                price = self.data.iloc[self.current_idx]['bid1']
            else:
            ...
            if log_callback and debug_mode:
                log_callback(f"{self.symbol} {self.kline_period} 市价卖出: {volume}手 成交价:{price:.2f} 原因:{reason}")
            ...
            if log_callback and debug_mode:
                price_str = f"{price:.2f}" if price is not None else "待确定"
                log_callback(f"{self.symbol} {self.kline_period} 添加待执行订单: 平多 {volume}手 订单类型:{order_type} 预计价格:{price_str} 原因:{reason}")
    ...
    def buycover(self, volume: Optional[int] = None, reason: str = "", log_callback=None, order_type='bar_close'):
        ...
        if order_type == 'bar_close':
            ...
            if log_callback and debug_mode:
                log_callback(f"{self.symbol} {self.kline_period} 市价买平: {volume}手 成交价:{price:.2f} 原因:{reason}")
            ...
            if log_callback and debug_mode:
                price_str = f"{price:.2f}" if price is not None else "待确定"
                log_callback(f"{self.symbol} {self.kline_period} 添加待执行订单: 平空 {volume}手 订单类型:{order_type} 预计价格:{price_str} 原因:{reason}")
    ...
    def get_open(self) -> pd.Series:
        """获取开盘价序列"""
        return self.data['open'] if 'open' in self.data.columns else pd.Series()
        
    def get_high(self) -> pd.Series:
        """获取最高价序列"""
        return self.data['high'] if 'high' in self.data.columns else pd.Series()
        
    def get_low(self) -> pd.Series:
        """获取最低价序列"""
        return self.data['low'] if 'low' in self.data.columns else pd.Series()
...
class MultiDataSource:
    ...
    def add_data_source(self, symbol: str, kline_period: str, adjust_type: str = '1', data: pd.DataFrame = None) -> int:
        """
        添加数据源
    ...
    def align_data(self, align_index: bool = True, fill_method: str = 'ffill'):
        ...
        for ds in self.data_sources:
            if not ds.data.empty:
                ...
                # 填充缺失值
                if fill_method:
                    ds.data = ds.data.fillna(method=fill_method) 