...
def test_log_model(
    ...
    full_model_dir = os.path.join(models_resources_dir_path, "scikit_learn")
    ...
    mock_frog_ml.assert_called_once_with(
        repository=repository,
        model_name=model_name,
        model_path=ANY,
        model_type=ANY,
    ...
    # validate model type
    model_metadata = mock_frog_ml.call_args.kwargs["model_type"]
    assert model_metadata.framework == _SCIKIT_LEARN_MODEL_FLAVOR
    assert model_metadata.framework_version == sklearn.__version__
    assert model_metadata.serialization_format == "joblib"
    assert model_metadata.runtime == "python"
    assert model_metadata.runtime_version == platform.python_version()

    # validate dependencies
    dependencies_files_paths = mock_frog_ml.call_args.kwargs[
    ...
    archive_path = Path(mock_frog_ml.call_args.kwargs["code_archive_file_path"])
...
def test_get_model_info(get_entity_info_mock):
    ...
    version = "version"
    json_file_path = (
        "../../../tests/resources/models/scikit_learn/scikit_learn_model_info.json"
    )
...
def test_load_model(download_model_version_mock, get_entity_info_mock):
    ...
    full_model_dir_path = given_full_path_to_file(
        file_name="../../../tests/resources/models/scikit_learn"
    )
