...
class ConnectionDB:
    def __init__(
        ...
        dialect: Literal["pyodbc", "psycopg2"] = "pyodbc",
        db_type: Literal["sql_server", "mysql", "postgres", "sqlite", "supabase"] = "sqlite"
    ):
        ...
        if not self.connection_string and self._can_build_connection_string():
            ...
            self._build_supabase_connection()
    ...
    def __enter__(self):
        try:
            if self.db_type == "supabase":
                return self._build_supabase_connection()
