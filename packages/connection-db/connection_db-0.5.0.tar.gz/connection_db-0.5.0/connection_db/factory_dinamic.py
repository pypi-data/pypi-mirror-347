...
class FactoryDinamic:
    @staticmethod
    def create_entity_file(entity_name: str, columns: Dict[str, type] | pd.DataFrame, output_path: str = "./") -> str:
        if isinstance(columns, pd.DataFrame):
            columns = FactoryDinamic.df_to_sqlalchemy_types(columns)
    ...
    @staticmethod
    def df_to_sqlalchemy_types(pandas_dataframe) -> Dict[str, type]:
        type_mapping = {
            'int64': int,
            'int32': int,
            'float64': float,
        ...
        columns = {}
        for col, dtype in pandas_dataframe.dtypes.items():
            python_type = type_mapping.get(str(dtype), str)
            columns[col] = python_type
