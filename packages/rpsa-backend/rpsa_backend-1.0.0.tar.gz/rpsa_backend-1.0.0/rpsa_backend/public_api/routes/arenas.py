...
def arena_leaderboard(arena_id):
    ...
    Normalizes all scores by number of games so that strategies
    can be compared regardless of games_per_pair.
    """
    arena = get_accessible_arena_or_404(arena_id, g.current_user)

    # Sum of per‐game scores and count of games per strategy
    rows = (
        db.session.query(
        ...
        .order_by(func.sum(ResultModel.score) / func.count(ResultModel.id).desc())
        .all()
    )
