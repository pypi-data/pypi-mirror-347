...
def query_encaminha_erp(job_config_dict: dict, db_config: DatabaseConfig, pedido_oking_id=''):
    ...
    try:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,f'Erro ao consultar pedidos faturados no banco: {str(ex)}', 'error', 'encaminhar_entrega_job', 'ENCAMINHAR')
