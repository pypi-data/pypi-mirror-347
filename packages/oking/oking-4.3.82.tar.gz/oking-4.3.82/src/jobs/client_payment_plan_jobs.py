...
def job_client_payment_plan(job_config_dict: dict):
    with lock:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f'Plano Pagamento Cliente - Iniciado', 'exec', 'envia_plano_pagamento_cliente_job ',
        ...
        if db_config.sql is None:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                     ...
                     'warning', 'PLANO_PAGAMENTO_CLIENTE')
            return
        ...
        try:
            ...
            #             'error', 'envia_plano_pagamento_cliente_job ', 'PLANO_PAGAMENTO_CLIENTE')
            #    return
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                     f'Enviando os planos de pagamentos dos clientes via api okvendas', 'info',
                     'PLANO_PAGAMENTO_CLIENTE')
                ...
                try:
                    ...
                    if status_code == 207:
                        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                 ...
                                 f'{api_client_payment_plan.codigo_cliente} {message_response}',
                                 'warning', 'envia_plano_pagamento_cliente_job ', 'PLANO_PAGAMENTO_CLIENTE')
...
def query_list_client_payment_plan(job_config_dict: dict, db_config: DatabaseConfig) -> List[ClientPaymentPlan]:
    ...
    try:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f' Erro ao consultar os planos de pagamentos dos clientes no banco semaforo: {str(ex)}', 'error',
                 'PLANO_PAGAMENTO_CLIENTE')
