...
def final_query(db_config: DatabaseConfig):
    ...
    if db_config.sql.lower().__contains__("group by"):
        ...
        newsql = f"""{db_config.sql.lower().replace("from", ", 1 as existe from")} {clause} exists ({db_config.semaforo_sql})
                                  union 
                                 {db_config.sql.lower().replace("from", ", 0 as existe from")} {clause} not exists ({db_config.semaforo_sql.lower().replace("and s.data_alteracao", " -- and s.data_alteracao")})"""
