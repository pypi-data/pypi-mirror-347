...
def post_clients(body: List):
    try:
        ...
        if response.ok:
            ...
            return {'error': response.status_code, 'message': response.json()['message']}
    # json_response = response.json()
...
def get_order_queue(body: dict, stats) -> List[Queue]:
    ...
    try:
        ...
        response = requests.get(url, headers={'Accept': 'application/json', 'access-token': token},
        ...
        logger.error(f'Erro ao realizar GET na api oking hub {url}' + str(ex), exc_info=True)
        raise
...
def get_order(body: dict, pedido_oking_id) -> Order:
    ...
    try:
        ...
        response = requests.get(url_api, headers={'Accept': 'application/json', 'access-token': body.get('token_api')},
                                params={'token': body.get('token_oking'), 'pedido_oking_id': pedido_oking_id})
        if response.ok:
...
def post_order_erp_code(body) -> bool:
    ...
    try:
        if src.print_payloads:
            print(body)
        response = requests.post(url, json=body, headers={'Accept': 'application/json', 'access-token': token})
        if response.ok:
            return True
        else:
            logger.warning(f'Retorno sem sucesso {response.status_code} - {response.url}')
        ...
        logger.error(f'Erro ao realizar GET na api okinghub {url}' + str(ex), exc_info=True)
        return False
...
def put_protocol_orders(protocolos: List[str]) -> bool:
    ...
    try:
        ...
        logger.error(f'Erro ao protocolar pedidos na api oking hub {url}' + str(ex), exc_info=True)
        return False
...
def post_faturar(invoice: Invoice) -> None | str | GenericResponse:
    ...
    try:
        ...
        json_invoice = json_invoice.replace('"valor_nf": null', f'"valor_nf":{invoice.valor_nf}')
        if src.print_payloads:
            print(json_invoice)
...
def post_protocol_duplicated_order(duplicated_order):
    ...
    try:
        ...
        if src.print_payloads:
            print(json_protocolos)
        response = requests.post(url, json=json.loads(json_protocolos), headers={'Accept': 'application/json',
                                                                                'access-token': src.client_data.get(
                                                                                    'token_api')})
        if response.ok:
            return True
        ...
        logger.error(f'Erro ao protocolar pedidos duplicados na api okvendas {url}' + str(ex), exc_info=True)
        return False
