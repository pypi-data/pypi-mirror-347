    ...
    with lock:
        try:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Coleta cliente', 'exec', 'job_send_clients_colect', 'COLETA_DADOS_CLIENTE')
            ...
            if clients is not None and len(clients) > 0:
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                         f'Clientes para atualizar {len(clients)}', 'info', 'COLETA_DADOS_CLIENTE')
                ...
                for client in clients:
                    if api_clients.__len__() < 50:
                        ...
                        send_log(job_config_dict.get('job_name'), False, False,
                                 f'Enviando Pacote: {api_clients.__len__()}', 'info', 'COLETA_DADOS_CLIENTE', '')
                        response = api_okHUB.post_clients(api_clients)
                        api_clients = []
                        if 'error' in response:
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                     f'Erro {response["error"]}: {response["message"]}', 'error',
                                     'coleta_dados_cliente_job', 'COLETA_DADOS_CLIENTE')
                        else:
                ...
                if api_clients.__len__() > 0:
                    send_log(job_config_dict.get('job_name'), False, False, f'Enviando Pacote: {api_clients.__len__()}',
                             'info', 'COLETA_DADOS_CLIENTE', '')
                    response = api_okHUB.post_clients(api_clients)
                    if 'error' in response:
                        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                 f'Erro {response["error"]}: {response["message"]}', 'error', 'coleta_dados_cliente_job',
                                 'COLETA_DADOS_CLIENTE')
                    else:
                        send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
                                 'COLETA_DADOS_CLIENTE', '')
                ...
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                         f'Nao existem clientes a serem enviados no momento', 'warning', 'CLIENTE')
        except Exception as e:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro durante execucao do job: {str(e)}', 'error', 'coleta_dados_cliente_job', 'COLETA_DADOS_CLIENTE')
...
def query_colect_clients_erp(job_config_dict: dict, db_config: DatabaseConfig):
    ...
    try:
        ...
        if src.exibir_interface_grafica:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f' Erro ao consultar clientes no banco semaforo: {str(ex)}', 'error', 'COLETA_DADOS_CLIENTE')
    ...
    if response is not None:
        ...
        for item in response:
            ...
            if item.sucesso == 1 or item.sucesso == 'true':
                ...
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                         f'Erro ao atualizar Cliente para o sku: {identificador}, {item.Message}', 'warning',
                         'COLETA_DADOS_CLIENTE', identificador)
...
def job_send_sales_colect(job_config_dict: dict):
    ...
    with lock:
        try:
            ...
            logger.info(f'==== THREAD INICIADA -job: ' + job_config_dict.get('job_name'))
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Coleta venda', 'exec', 'coleta_dados_venda_job', 'COLETA_DADOS_VENDA')
            ...
            if vendas is not None and len(vendas) > 0:
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                         f'VENDAS para atualizar {len(vendas)}', 'info', 'COLETA_DADOS_VENDA')
                ...
                for venda in vendas:
                    if api_vendas.__len__() < 50:
                        ...
                        send_log(job_config_dict.get('job_name'), False, False,
                                 f'Enviando Pacote: {api_vendas.__len__()}', 'info', 'COLETA_DADOS_VENDA', '')
                        response = api_okHUB.post_vendas(api_vendas)
                        api_vendas = []
                        if 'error' in response:
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                     f'Erro {response["error"]}: {response["message"]}', 'error',
                                     'coleta_dados_venda_job', 'COLETA_DADOS_VENDA')
                        else:
                            send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
                                     'COLETA_DADOS_VENDA',
                                     '')
                ...
                if api_vendas.__len__() > 0:
                    send_log(job_config_dict.get('job_name'), False, False, f'Enviando Pacote: {api_vendas.__len__()}',
                             'info', 'COLETA_DADOS_VENDA', '')
                    response = api_okHUB.post_vendas(api_vendas)
                    if 'error' in response:
                        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                 f'Erro {response["error"]}: {response["message"]}', 'error',
                ...
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                         f'Nao existem vendas a serem enviadas no momento', 'warning', 'COLETA_DADOS_VENDA')
        except Exception as e:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro durante execucao do job: {str(e)}', 'error', 'coleta_dados_venda_job', 'COLETA_DADOS_VENDA')
...
def query_vendas_erp(job_config_dict: dict, db_config: DatabaseConfig):
    ...
    try:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f' Erro ao consultar vendas no banco semaforo: {str(ex)}', 'error',
                 'coleta_dados_venda_job', 'COLETA_DADOS_VENDA')
        if src.exibir_interface_grafica:
            raise
    ...
    if response is not None:
        ...
        for item in response:
            ...
            if item.sucesso == 1 or item.sucesso == 'true':
                ...
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                         f'Erro ao atualizar Venda para o kdm: {identificador}, {item.Message}', 'warning',
                         'COLETA_DADOS_VENDA', identificador)
...
def job_send_colect_physical_shopping(job_config_dict: dict):
    ...
    with lock:
        try:
            ...
            logger.info(f'==== THREAD INICIADA -job: ' + job_config_dict.get('job_name'))
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Coleta compras loja física', 'exec', 'coleta_compras_loja_fisica_job', 'COLETA_DADOS_COMPRA_FISICA')
            ...
            if compras is not None and len(compras) > 0:
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                         f'VENDAS para atualizar {len(compras)}', 'info', 'COLETA_DADOS_VENDA')

                for compra in compras:
                    send_log(job_config_dict.get('job_name'), False, False,
                             f'Enviando Pacote', 'info', 'COLETA_DADOS_COMPRA_FISICA', '')
                    response = api_OkVendas.post_colect_physical_shopping(compra)

                    if response is None:
                        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                                 f'Dados compra física enviados com sucesso',
                                 'info', 'coleta_compras_loja_fisica_job',
                        ...
                        if hasattr(response, 'status_code'):
                            if response.status_code == 404:
                                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                                        f'{str(response)}, verifique os dados: { jsonpickle.encode(compra, unpicklable=False)}',
                                        'warning', 'coleta_compras_loja_fisica_job',
                                        'COLETA_DADOS_COMPRA_FISICA')
                            else:
                                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                        f'Erro HTTP {response.status_code}: {str(response.content)}',
                                        'error', 'coleta_compras_loja_fisica_job',
                                        'COLETA_DADOS_COMPRA_FISICA')
                        else:
                            # Response de tipo desconhecido
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                    f'Falha ao enviar dados compra física para a api - tipo {type(response)}: {str(response)}',
                                    'error', 'coleta_compras_loja_fisica_job',
                                    'COLETA_DADOS_COMPRA_FISICA')
            else:
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                         f'Nao existem vendas a serem enviadas no momento', 'warning', 'COLETA_DADOS_COMPRA_FISICA')

        except Exception as e:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro durante execucao do job: {str(e)}', 'error', 'coleta_compras_loja_fisica_job',
                     'COLETA_DADOS_COMPRA_FISICA')
            raise e
...
def query_compras_erp(job_config_dict: dict, db_config: DatabaseConfig):
    ...
    try:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f' Erro ao consultar vendas no banco semaforo: {str(ex)}', 'error',
                 'coleta_compras_loja_fisica_job', 'COLETA_DADOS_COMPRA_FISICA')
        if src.exibir_interface_grafica:
            raise
    ...
    if response is not None:
        ...
        for item in response:
            ...
            if item.sucesso == 1 or item.sucesso == 'true':
                ...
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                         f'Erro ao atualizar Compra para o sku: {identificador}, {item.Message}', 'warning',
                         'COLETA_DADOS_COMPRA_FISICA', identificador)
