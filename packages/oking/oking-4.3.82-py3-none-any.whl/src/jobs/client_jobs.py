...
def job_send_transportadora_to_okvendas(job_config_dict: dict):
        """
        ...
        with lock:
            ...
            # LOG de Inicialização do Método - Para acompanhamento de execução
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Transportadora - Iniciado', 'exec', 'envia_transportadora_fob_job', 'TRANSPORTADORA_PARA_OKVENDAS')
            # Executa o Comando da Semaforo Antes do Query Principal
            if job_config_dict['executar_query_semaforo'] == 'S':
                logger.info(f'Executando a query semáforo')
                executa_comando_sql(db_config, job_config_dict)
            ...
            # Executa a query Principal
            logger.info(f'Executando a query principal')
            transportadoras = query_transportadora_to_okvendas(job_config_dict, db_config)

            if transportadoras is not None and len(transportadoras) or 0 > 0:
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                         f'Transportadoras serem inseridas: {len(transportadoras)}', 'info',
                         'envia_transportadora_fob_job', 'TRANSPORTADORA_PARA_OKVENDAS')
                api_transportadora = []
                    ...
                    try:
                        ...
                        if api_transportadora.__len__() < 50:
                            ...
                            send_log(job_config_dict.get('job_name'), False, False,
                                     ...
                                     'info', 'envia_transportadora_fob_job', 'TRANSPORTADORA_PARA_OKVENDAS')
                            response = api_okvendas. post_send_transportadora_parceiro(api_transportadora, job_config_dict)
                            api_transportadora = []
                            send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
                                     'envia_transportadora_fob_job',
                                     'TRANSPORTADORA_PARA_OKVENDAS')
                            validade_response_transportadora(response, db_config, job_config_dict)
                ...
                if api_transportadora.__len__() > 0:
                    send_log(job_config_dict.get('job_name'), False, False, f'Enviando Pacote: {api_transportadora.__len__()}',
                             'info',
                             'envia_transportadora_fob_job', 'TRANSPORTADORA_PARA_OKVENDAS')
                    response = api_okvendas.post_send_transportadora_parceiro(api_transportadora, job_config_dict)
                    send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
...
def query_transportadora_to_okvendas(job_config_dict, db_config):
    try:
        db = database.Connection(db_config)
        conn = db.get_conect()
        cursor = conn.cursor()
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f' Erro ao consultar transportadora para okvendas: {str(ex)}', 'error',
                 'TRANSPORTADORA_PARA_OKVENDAS')
        raise ex
...
def protocol_transportadora_to_okvendas(job_config_dict, db_config, transportadora):
    try:
        conexao = database.Connection(db_config)
        conn = conexao.get_conect()
        cursor = conn.cursor()
...
def validade_response_transportadora(response, db_config, job_config_dict):
