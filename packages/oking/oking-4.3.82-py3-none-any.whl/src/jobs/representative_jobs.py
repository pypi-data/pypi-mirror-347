...
def job_representative(job_config_dict: dict):
    with lock:
        """
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f'Enviar Representante- Iniciado', 'exec', 'envia_representante_job', 'REPRESENTANTE')
        if db_config.sql is None:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                     f'Comando sql para inserir os representantes no semaforo nao encontrado', 'warning',
                     'REPRESENTANTE')
            return
        if job_config_dict['executar_query_semaforo'] == 'S':
            executa_comando_sql(db_config, job_config_dict)
        try:
            db_representative = query_list_representative(job_config_dict, db_config)
            if not insert_update_semaphore_representative(job_config_dict, db_config, db_representative):
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                         f'Nao foi possivel inserir os representantes no banco semaforo', 'error', 'REPRESENTANTE')
                return
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                     f'Enviando os representantes via api okvendas', 'info', 'REPRESENTANTE')
            ...
            while limit <= total:
                ...
                for res in response:
                    ...
                    if res.status == 1:
                        if protocol_semaphore_representative(job_config_dict, db_config, identificador):
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                                     f'Representante {identificador} protocolado no banco semaforo', 'info',
                                     'REPRESENTANTE')
                        else:
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                                     f'Falha ao protocolar o representante {identificador}', 'warning', 'REPRESENTANTE')
...
def query_list_representative(job_config_dict: dict, db_config: DatabaseConfig) -> List[Representative]:
    """
    ...
    try:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f' Erro ao consultar representantes no banco semaforo: {str(ex)}', 'error', 'REPRESENTANTE')

    return []


def insert_update_semaphore_representative(job_config_dict: dict, db_config: DatabaseConfig,
...
def protocol_semaphore_representative(job_config_dict: dict, db_config: DatabaseConfig, identifier: str) -> bool:
    db = database.Connection(db_config)
    conn = db.get_conect()
    cursor = conn.cursor()
    try:
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'),
                 True, f' Erro ao protocolar representante no banco semaforo: {str(ex)}', 'error', 'REPRESENTANTE')
