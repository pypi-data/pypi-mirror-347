...
def post_send_transportadora_parceiro(body, job_config_dict):
    try:
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro ao realizar POST na api okvendas {src.client_data.get("url_api_principal")}/api/insert_transportadora_fob_parceiro '
                     + str(response.json()), 'error', 'envia_transportadora_fob_job', 'TRANSPORTADORA_PARA_OKVENDAS')
            return [SbyResponseError(**response.json())]

    except Exception as e:
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True
                 , f'Erro {str(e)}', 'error', 'envia_transportadora_fob_job', 'TRANSPORTADORA_PARA_OKVENDAS')
        return False
