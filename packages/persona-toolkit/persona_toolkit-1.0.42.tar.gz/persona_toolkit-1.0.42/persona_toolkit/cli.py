...
@app.command()
def copy_template_files(project_path: Path, traversable_dir=None, current_path: str = ""):
    """
    ...
    for item in template_files.iterdir():
        if item.is_dir():
            ...
            os.chdir(cwd)  # Change to the new directory
            copy_template_files(project_path, item, new_current_path)
        elif item.is_file():
