...
def get_consensus_edges(
    spatial, *omics, target_neighbors=8, max_iter=20
) -> torch.LongTensor:

    n_cells = spatial.shape[0]
    assert all(
        omic.shape[0] == n_cells for omic in omics
    ), "Omics data dimension mismatch"
    assert 0 < target_neighbors < n_cells, "Invalid target_neighbors"
    ...
    for _ in range(max_iter):
        ...
        edge_index_intersect = edge_lists_intersection(
            edge_index_spatial, edge_index_feat
        )
        avg_neighbors = len(edge_index_intersect) / n_cells
...
def _priori_cluster(
    ...
    for res in sorted(list(np.arange(0.01, 2.5, 0.01)), reverse=True):
        sc.tl.leiden(
            adata, resolution=res, flavor="igraph", n_iterations=2, directed=False
        )
