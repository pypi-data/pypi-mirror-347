...
class ControlledFPSVideoCapture:
    FRAME_SELECTION_LEGACY = "legacy"
    FRAME_SELECTION_FFMPHISDP = "ffmphisdp"

    def __new__(cls, *args, **kwargs):
        kwargs_copy = kwargs.copy()
        frame_selection_version = kwargs_copy.pop(
            "frame_selection_version", ControlledFPSVideoCapture.FRAME_SELECTION_LEGACY
        )
        if frame_selection_version == ControlledFPSVideoCapture.FRAME_SELECTION_LEGACY:
            reader = ControlledFPSVideoCapture_skcvideo(*args, **kwargs_copy)
            reader.frame_selection_version = frame_selection_version
            return reader
        elif frame_selection_version == ControlledFPSVideoCapture.FRAME_SELECTION_FFMPHISDP:

            def set_convert(self, prop_id, value):
                """Convert form global frame index to selection frame index"""
                if prop_id != cv2.CAP_PROP_POS_FRAMES:
                    raise NotImplementedError
                idx_global = self.frame_selection_index_mapping[value]
                print(f"Automatically converting index from {value} to {idx_global} (local to global)")
                self.set_frame_idx(idx_global)

            def get_convert(self, prop_id):
                """convert from global frame index to selection frame index"""
                if prop_id == cv2.CAP_PROP_POS_FRAMES:
                    return self.frame_selection_index_mapping.index(self.current_frame_index)
                elif prop_id == cv2.CAP_PROP_FRAME_COUNT:
                    return len(self.frame_selection_index_mapping)
                else:
                    raise NotImplementedError

            ControlledFPSVideoCapture_ffmphisdp.set = set_convert
            ControlledFPSVideoCapture_ffmphisdp.get = get_convert
            ControlledFPSVideoCapture_ffmphisdp.output_frame = property(
                lambda self: get_convert(self, cv2.CAP_PROP_POS_FRAMES)
            )
            ControlledFPSVideoCapture_ffmphisdp.input_frame = property(lambda self: self.current_frame_index)
            reader = ControlledFPSVideoCapture_ffmphisdp(*args, **kwargs_copy)
            reader.frame_selection_version = frame_selection_version
            reader.frame_selection_index_mapping = [
                idx_global for idx_global, selected in enumerate(reader.frame_selection) if selected
            ]
            return reader
        else:
            raise ValueError("Invalid frame_selection_version")


class ControlledFPSVideoCapture_skcvideo:
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        message = (
            f"Initializing ControlledFPSVideoCapture with input_fps={self.input_fps} and output_fps={self.output_fps}"
        )
        logger.info(message)
    ...
    def read(self):
        ...
        if self.output_frame < 2:
            ...
            frames_to_get = int(
                math.ceil((self.output_frame + 1) * float(self.input_fps) / self.output_fps)
