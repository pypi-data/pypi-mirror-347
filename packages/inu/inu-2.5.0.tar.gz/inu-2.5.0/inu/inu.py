...
class waypoints:
    ...
    def __init__(self,
        ...
        bounds : callable or None, default None
            External function which will take the full array of (x,y) position
            values and a position index and return the corresponding bounds to
            plot about the path at that index.
        """
        ...
        self.plot_bounds = None if self.bounds is None else \
            self.ax.fill([], [], color=self.color, edgecolor=None,
            alpha=0.2)[0]

        # Initialize the progress bar.
        self.plot_progress, = self.ax.plot([], [],
            color="orange", linewidth=10, transform=self.ax.transAxes)
    ...
    def __repr__(self):
        ...
        for n in range(self.Np):
            out += f"    ({self.points[1, n]}, {self.points[0, n]})"
            if n < self.Np - 1:
                out += "\n"
        return out
    ...
    def join(self,
        ...
        x = np.zeros(self.Nb*self.Nt - self.Nb + 1)
        y = np.zeros(self.Nb*self.Nt - self.Nb + 1)
        na = 0
        nb = self.Nt
        for j in range(self.Nb):
            if j == 0:
    ...
    def show_bounds(self):
        ...
        dist = np.zeros(K)
        for k in range(K):
            dist[k] = self.bounds(self.path[:2], k)

        # Compute offset paths.
        x_plus,  y_plus  = x + dist*nx, y + dist*ny
        x_minus, y_minus = x - dist*nx, y - dist*ny

        # Update the fill path.
        x_poly = np.concatenate([x_plus, x_minus[::-1]])
        y_poly = np.concatenate([y_plus, y_minus[::-1]])
        self.plot_bounds.set_xy(np.array([x_poly, y_poly]).T)
    ...
    def hide_bounds(self):
        ...
        # Clear the x,y data.
        self.plot_bounds.set_xy(np.array([]).reshape(0, 2))
