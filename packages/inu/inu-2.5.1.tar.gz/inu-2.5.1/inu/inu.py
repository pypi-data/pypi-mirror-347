...
def path_box(
    ...
    p_t = waypoints(points, seg_len, radius, plot=False, ned=False).path
    if ned:
        p_t = ned_enu(p_t)
...
def path_clover(
    ...
    # Convert waypoints to a path.
    p_t = waypoints(points, seg_len, 0.01, plot=False, ned=False).path
    if ned:
        p_t = ned_enu(p_t)
...
class waypoints:
    ...
    def show_bounds(self):
        ...
        if self.ned:
            self.plot_bounds.set_xy(np.array([y_bounds, x_bounds]).T)
        else:
            self.plot_bounds.set_xy(np.array([x_bounds, y_bounds]).T)
