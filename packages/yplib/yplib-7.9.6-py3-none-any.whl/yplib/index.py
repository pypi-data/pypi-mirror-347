...
def can_use_json(data):
    # 排除不能被 json 序列化的类型
    if isinstance(data, (bytes, complex)):
        return False
    try:
        json.dumps(data)
        return True
...
def to_print_file_thread(*args, file_path=None, file_name=None, file_name_prefix=None, file_name_suffix=None, mode='a', interval=None):
    """
    ...
    """
    __THREAD_LOCAL_INDEX_DATA.to_print_file_thread = file_path = file_path if file_path is not None else getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_thread', None)
    interval = __THREAD_LOCAL_INDEX_DATA.to_print_file_thread_interval = interval or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_thread_interval', None)
    file_name = __THREAD_LOCAL_INDEX_DATA.to_print_file_thread_file_name = file_name or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_thread_file_name', None)
    file_name_suffix = __THREAD_LOCAL_INDEX_DATA.to_print_file_thread_file_name_suffix = file_name_suffix or getattr(__THREAD_LOCAL_INDEX_DATA,
                                                                                                                     'to_print_file_thread_file_name_suffix', None)
    file_name_prefix = __THREAD_LOCAL_INDEX_DATA.to_print_file_thread_file_name_prefix = file_name_prefix or getattr(__THREAD_LOCAL_INDEX_DATA,
                                                                                                                     'to_print_file_thread_file_name_prefix', None)
...
def to_print_date_file(*args, file_path=None, file_name=None, file_name_prefix=None, file_name_suffix=None, mode='a', interval=None):
    """
    将 文本输出到 date_file 中
    例如
    """
    __THREAD_LOCAL_INDEX_DATA.to_print_date_file = file_path = file_path if file_path is not None else getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_date_file', None)
    interval = __THREAD_LOCAL_INDEX_DATA.to_print_date_file_interval = interval or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_date_file_interval', None)
    file_name = __THREAD_LOCAL_INDEX_DATA.to_print_date_file_file_name = file_name or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_date_file_file_name', None)
    file_name_prefix = __THREAD_LOCAL_INDEX_DATA.to_print_date_file_file_name_prefix = file_name_prefix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_date_file_file_name_prefix',
                                                                                                                   None)
    file_name_suffix = __THREAD_LOCAL_INDEX_DATA.to_print_date_file_file_name_suffix = file_name_suffix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_date_file_file_name_suffix',
                                                                                                                   None)
    to_txt(data_list=[to_print(*args, interval=interval)],
           file_name=('' if file_name_prefix is None else file_name_prefix)
                     + (datetime.today().strftime('%Y-%m-%d') if file_name is None else file_name)
...
def to_print_file(*args, file_path=None, file_name=None, file_name_prefix=None, file_name_suffix=None, mode='a', interval=None):
    """
    将 print 数据, 写入到 print_file 文件
    文件按照 日期自动创建
    例如: print_file/2020-01-01.txt
    """
    __THREAD_LOCAL_INDEX_DATA.to_print_file = file_path = file_path if file_path is not None else getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file', None)
    interval = __THREAD_LOCAL_INDEX_DATA.to_print_file_interval = interval or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_interval', None)
    file_name = __THREAD_LOCAL_INDEX_DATA.to_print_file_file_name = file_name or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_file_name', None)
    file_name_prefix = __THREAD_LOCAL_INDEX_DATA.to_print_file_file_name_prefix = file_name_prefix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_file_name_prefix', None)
    file_name_suffix = __THREAD_LOCAL_INDEX_DATA.to_print_file_file_name_suffix = file_name_suffix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_file_file_name_suffix', None)
    to_txt(data_list=[to_print(*args, interval=interval)],
           file_name=('' if file_name_prefix is None else file_name_prefix)
                     + (datetime.today().strftime('%Y-%m-%d') if file_name is None else file_name)
                     + ('' if file_name_suffix is None else file_name_suffix),
...
def to_print_txt(*args, file_path=None, file_name=None, file_name_prefix=None, file_name_suffix=None, mode='a', interval=None):
    """
    将 print 数据, 写入到 print_txt 文件
    文件按照 日期自动创建
    例如: print_txt/2020-01-01.txt
    """
    __THREAD_LOCAL_INDEX_DATA.to_print_txt = file_path = file_path if file_path is not None else getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_txt', None)
    interval = __THREAD_LOCAL_INDEX_DATA.to_print_txt_interval = interval or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_txt_interval', None)
    file_name = __THREAD_LOCAL_INDEX_DATA.to_print_txt_file_name = file_name or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_txt_file_name', None)
    file_name_prefix = __THREAD_LOCAL_INDEX_DATA.to_print_txt_file_name_prefix = file_name_prefix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_txt_file_name_prefix', None)
    file_name_suffix = __THREAD_LOCAL_INDEX_DATA.to_print_txt_file_name_suffix = file_name_suffix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_print_txt_file_name_suffix', None)
    to_txt(data_list=[to_print(*args, interval=interval)],
           file_name=('' if file_name_prefix is None else file_name_prefix)
                     + (datetime.today().strftime('%Y-%m-%d') if file_name is None else file_name)
                     + ('' if file_name_suffix is None else file_name_suffix),
...
def to_log_file(*args, file_path=None, file_name=None, file_name_prefix=None, file_name_suffix=None, time_prefix=True, mode='a', interval=None):
    """
    将 log 数据, 写入到 log_file 文件
    文件按照 日期自动创建
    例如: log_file/2020-01-01.log
    """
    __THREAD_LOCAL_INDEX_DATA.to_log_file = file_path = file_path if file_path is not None else getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_file', None)
    interval = __THREAD_LOCAL_INDEX_DATA.to_log_file_interval = interval or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_file_interval', None)
    file_name = __THREAD_LOCAL_INDEX_DATA.to_log_file_file_name = file_name or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_file_file_name', None)
    file_name_prefix = __THREAD_LOCAL_INDEX_DATA.to_log_file_file_name_prefix = file_name_prefix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_file_file_name_prefix', None)
    file_name_suffix = __THREAD_LOCAL_INDEX_DATA.to_log_file_file_name_suffix = file_name_suffix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_file_file_name_suffix', None)
...
def to_log_txt(*args, file_path=None, file_name=None, file_name_prefix=None, file_name_suffix=None, time_prefix=True, mode='a', interval=None):
    """
    将 log 数据, 写入到 log_txt 文件夹中
    文件按照 日期自动创建
    例如: log_txt/2020-01-01.txt
    """
    __THREAD_LOCAL_INDEX_DATA.to_log_txt = file_path = file_path if file_path is not None else getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_txt', None)
    interval = __THREAD_LOCAL_INDEX_DATA.to_log_txt_interval = interval or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_txt_interval', None)
    file_name = __THREAD_LOCAL_INDEX_DATA.to_log_txt_file_name = file_name or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_txt_file_name', None)
    file_name_prefix = __THREAD_LOCAL_INDEX_DATA.to_log_txt_file_name_prefix = file_name_prefix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_txt_file_name_prefix', None)
    file_name_suffix = __THREAD_LOCAL_INDEX_DATA.to_log_txt_file_name_suffix = file_name_suffix or getattr(__THREAD_LOCAL_INDEX_DATA, 'to_log_txt_file_name_suffix', None)
    to_txt(data_list=[to_log(*args, time_prefix=time_prefix, interval=interval)],
           file_name=('' if file_name_prefix is None else file_name_prefix)
                     + (datetime.today().strftime('%Y-%m-%d') if file_name is None else file_name)
                     + ('' if file_name_suffix is None else file_name_suffix),
