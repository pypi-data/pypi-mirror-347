...
def get_space(s: Union[int, str] = None,
              use_start_index: int = 0,
              html_data: str = None,
              key: str = None) -> str:
    """
...
# 使用 table 的基本图表
def table_list_html(table_list: list,
                    header_obj: list,
                    title: str) -> str:
    uid = random_letter(is_upper=True)
    s = '''
...
# 使用 text 的基本图表
def text_list_html(text_list: list,
                   title: str) -> str:
    uid = random_letter(is_upper=True)
    s = '''
        <!DOCTYPE html>
