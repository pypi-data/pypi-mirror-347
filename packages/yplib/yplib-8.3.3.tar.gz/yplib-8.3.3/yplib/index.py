...
def rreplace(s: str, old: str, new: str, count=1) -> str:
...
def to_java_more(*args: Any) -> Optional[Tuple[Any, ...]]:
    # 使用列表推导式进行过滤和映射
...
def to_underline_one(s: str) -> Optional[str]:
    """
    将驼峰命名转成下划线命名
    例如: userId -> user_id
    """
...
def to_underline_more(*args: Any) -> Optional[Tuple[Any, ...]]:
    # 使用列表推导式进行过滤和映射
...
# 是否能用 json
def can_use_json(param: Any) -> bool:
    # 排除不能被 json 序列化的类型
...
def to_datetime(s: Any = None,
                r_str: bool = False) -> Union[datetime, str]:
    """
...
def to_datetime_str(s: Any = None) -> Union[datetime, str]:
...
def to_str(param: Any = None) -> str:
    return json.dumps(param, ensure_ascii=False) if can_use_json(param) else str(param)


def match_str(pattern: str, str_a: str = '') -> Optional[str]:
    """
    匹配字符串
...
def sort_by_json_key(data_obj: Any,
                     sep: str = '&',
                     join: str = '=',
                     join_list: str = ',') -> str:
    """
...
def set_config_data(file_name: str = 'config',
                    param: Optional[Dict[str, Any]] = None) -> None:
    set_data_in_user_home(file_name, {} if param is None else param)


# 从 config 中获得 配置数据
def get_config_data(file_name: str = 'config') -> Optional[Dict[str, Any]]:
...
def get_data_from_user_home(file_name: str = 'config') -> Optional[Dict[str, Any]]:
...
def get_data_from_path_detail(file_name: str = 'config',
                              file_path: str = None,
                              path_name: str = __CONFIG_PATH) -> Optional[Dict[str, Any]]:
...
def get_latest_file(
    ...
    return html_list[0] if len(html_list) > 0 else None
...
def get_and_save_data_to_thread(file_path: str = None,
                                ...
                                line_with_space_count: int = None,
                                mode: str = 'a',
                                file_name_use_path_with_date: bool = False,
...
def to_print_file(*args,
    ...
    to_txt(data_list=[to_print(*args, line_with_space_count=line_with_space_count, interval=interval)],
...
def to_log_file(*args,
    ...
    to_txt(data_list=[to_log(*args, time_prefix=time_prefix, line_with_space_count=line_with_space_count, interval=interval)],
           file_name=('' if file_name_prefix is None else file_name_prefix)
                     + (datetime.today().strftime('%Y-%m-%d') if file_name is None else file_name)
                     + ('' if file_name_suffix is None else file_name_suffix),
...
def to_list(file_name: str = 'a.txt',
            ...
            sep_all: str = None,
            ignore_start_with: Union[list, set, str, None] = None,
            ignore_end_with: Union[list, set, str, None] = None,
            ...
            count: int = None,
            sheet_index: int = 1,
            column_index: Union[list, int, str, None] = None,
            column_date: Union[list, int, str, None] = None,
            column_datetime: Union[list, int, str, None] = None) -> list:
    """
    当读取 txt 之类的文件的时候
...
def to_list_list(data_list: Optional[list] = None,
                 count: int = 10) -> list:
    """
...
# 将多个文件 读取成 list
def to_list_from_txt_list(file_list: Optional[list] = None) -> list:
...
def to_data_from_file(file_name: str = 'a.txt',
                      ...
                      column_index: Union[list, int, str, None] = None,
                      column_date: Union[list, int, str, None] = None,
                      column_datetime: Union[list, int, str, None] = None,
                      r_json: bool = False,
                      str_join: str = '',
                      r_str: bool = False) -> Union[str, Dict[str, Any]]:
    """
    在 to_list 方法上再嵌套一层,
...
# 将文件导出成excel格式的
def to_excel(data_list: Union[set, list, tuple, None],
             file_name: str = None,
...
def to_csv(data_list: Union[set, list, tuple, Dict, None],
           file_name: str = None,
