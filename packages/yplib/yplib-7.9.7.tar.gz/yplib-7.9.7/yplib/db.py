...
def get_connect_from_config(db_config=None, database=None, user=None, password=None, charset=None, port=None, host=None):
    db_config = __THREAD_LOCAL_DB_DATA.get_connect_from_config_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'get_connect_from_config_db_configs', 'db')
...
def exec_sql(sql='', db_conn=None, db_config=None, commit=True, is_log=False, database=None):
    db_config = __THREAD_LOCAL_DB_DATA.exec_sql_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'exec_sql_db_config', 'db')
    """
    执行 SQL 语句，并提交（默认提交）。
...
def get_doris_conn(db_config=None):
    db_config = __THREAD_LOCAL_DB_DATA.get_doris_conn_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'get_doris_conn_db_config', 'doris')
    return get_connect_from_config(db_config)


# 执行 sql 语句, 并且提交, 默认值提交的了
def exec_doris_sql(sql='', db_config=None, database=None):
    db_config = __THREAD_LOCAL_DB_DATA.exec_doris_sql_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'exec_doris_sql_db_config', 'doris')
    database = __THREAD_LOCAL_DB_DATA.exec_doris_sql_database = database or getattr(__THREAD_LOCAL_DB_DATA, 'exec_doris_sql_database', 'mx_risk')
    exec_sql(sql, db_config=db_config, database=database)


def get_data_from_doris(sql='', db_config=None):
    db_config = __THREAD_LOCAL_DB_DATA.get_data_from_doris_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'get_data_from_doris_db_config', 'doris')
...
def get_data_line_one_from_doris(sql='', db_config=None):
    db_config = __THREAD_LOCAL_DB_DATA.get_data_line_one_from_doris_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'get_data_line_one_from_doris_db_config', 'doris')
...
# 执行 sql 语句, 不提交
def exec_sql_un_commit(sql='', db_conn=None, db_config=None, database=None):
    db_config = __THREAD_LOCAL_DB_DATA.exec_sql_un_commit_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'exec_sql_un_commit_db_config', 'db')
    exec_sql(sql=sql, db_conn=db_conn, db_config=db_config, commit=False, database=database)


# 执行 sql 获得 数据
def get_data_from_sql(sql='', db_conn=None, db_config=None, is_log=False, database=None):
    db_config = __THREAD_LOCAL_DB_DATA.get_data_from_sql_db_config = db_config or getattr(__THREAD_LOCAL_DB_DATA, 'get_data_from_sql_db_config', 'db')
    if not sql:
        is_log and to_log_file("db_conn is None or sql is None or sql == '', so return")
        return
    db_conn = db_conn or get_connect_from_config(db_config, database=database)
