...
def do_post(url=None,
    ...
    # 记录到日志文件中
    if get__thread_local_index_data()._use_fun_flag:
        log_fun = get__thread_local_index_data()._use_fun
        for log_one in list(filter(lambda x: x[1] is not None, [
            ['url', url],
            ['data', data],
            ['session', session],
            ['headers', headers],
