...
# 是否能用 json
def can_use_json(data: Any) -> bool:
    # 排除不能被 json 序列化的类型
...
def random_uuid(length: Optional[int] = 32) -> str:
    r = uuid.uuid4().hex
    while len(r) < length:
        r += uuid.uuid4().hex
    return r[0:length]
...
def random_str(length: Optional[int] = 64,
               start_str: Optional[int] = 1,
               end_str: Optional[int] = 32) -> str:
    """
    获得随机数
    length    ：随机数长度
