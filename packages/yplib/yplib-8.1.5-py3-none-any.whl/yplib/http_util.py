...
def do_get(url=None,
    ...
    '''
    this_key = 'do_get_headers'
    if headers_save:
        # 保存 headers
        __THREAD_LOCAL_HTTP_UTIL_DATA[this_key] = headers
    headers = headers if headers is not None else __THREAD_LOCAL_HTTP_UTIL_DATA.get(this_key, None)
...
def do_http_log(url=None,
    ...
    for key, [value, flag] in log_fields.items():
        if value is not None:
            if flag:
                log_fun(f'{key}:')
                log_fun(json.dumps(value, indent=4, ensure_ascii=False, sort_keys=True))
            else:
                log_fun(f'{key}:', value)
    # 输出响应正文
    log_fun('response_text:')
    log_fun(json.dumps(response_text, indent=4, ensure_ascii=False, sort_keys=True) if r_json else response_text)
