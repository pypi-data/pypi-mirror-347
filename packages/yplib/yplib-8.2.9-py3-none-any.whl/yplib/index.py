...
# 创建一个线程本地存储对象
__THREAD_LOCAL_INDEX_DATA = threading.local().data = {}
...
def to_print(*args, time_prefix=False, line_with_space_count=None, interval=None):
    """
    记录日志, 如果是对象会转化为 json
    数据直接 print, 不记录到文件
    例如: aaa
    ...
    if interval is None or get_timestamp() - __THREAD_LOCAL_INDEX_DATA.get('to_print_time', 0) >= interval:
...
def to_log(*args, time_prefix=None, line_with_space_count=None, interval=None):
    """
    记录日志, 如果是对象会转化为 json
    前面加了时间
...
def get_and_save_data_to_thread(file_path=None,
    ...
    __THREAD_LOCAL_INDEX_DATA['_use_fun_flag'] = True if mode != 'a' else __THREAD_LOCAL_INDEX_DATA.get('_use_fun_flag', False)
    fun_name_str = fun_name.__name__
    # 存储函数引用
    __THREAD_LOCAL_INDEX_DATA['_use_fun'] = fun_name
    r_list = []
    # 输出非空字段
    for key, value in {
        'file_path': file_path,
        ...
        value_temp = value if value is not None else __THREAD_LOCAL_INDEX_DATA.get(this_key, None)
        __THREAD_LOCAL_INDEX_DATA[this_key] = value_temp
