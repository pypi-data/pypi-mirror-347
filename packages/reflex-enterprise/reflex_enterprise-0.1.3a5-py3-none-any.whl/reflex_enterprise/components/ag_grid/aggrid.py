...
from reflex.event import EventSpec, passthrough_event_spec
...
class AgGridAPI(rx.Base):
    ...
    @property
    def _api(self) -> Var[str]:
        return Var(f"refs['{self.ref}']?.current?.api")

    def __getattr__(self, name: str) -> Callable[..., EventSpec]:
        """Get the API method with the given name."""
        ...
        def _call_api(*args, **kwargs) -> EventSpec:
            ...
            """
            func = rx.vars.FunctionStringVar(f"{self._api}.{to_camel_case(name)}")
    ...
    def _format_props(cls, props: dict[str, Any]) -> dict[str, Any]:
        if (column_defs := props.get("column_defs")) is not None:
            if not isinstance(column_defs, Var):
                props["column_defs"] = [
                    (
                        ColumnGroupDef(**col_def)
                        if "children" in col_def
                        else ColumnDef(**col_def)
                    )
                ...
                props["column_defs"] = rx.vars.function.FunctionStringVar(
                    "formatColumnDefs"
                ).call(column_defs)
        ...
        if (
            ...
            default_col_def = ColumnDef(**default_col_def).dict()
        ...
        if (column_types := props.get("column_types")) is not None and not isinstance(
            ...
            for col_type in column_types:
                col_def = ColumnDef(**column_types[col_type]).dict()
...
class LoadingCellOptions(ComponentEnterprise):
    ...
    loading_cell_renderer_params: Var[Mapping[str, Any]]
...
class MasterDetailOptions(ComponentEnterprise):
    ...
    detail_cell_renderer_params: Var[Mapping[str, Any] | Callable]
...
class RowFullWidthOptions(ComponentEnterprise):
    ...
    full_width_cell_renderer_params: Var[Mapping[str, Any]]
...
class TreeDataOptions(ComponentEnterprise):
    ...
    def get_modules(cls, props: dict[str, Any]) -> dict[str, set[str]]:
        ...
        return (
            {"ag-grid-enterprise": {"TreeDataModule"}}
            if any(key in props for key in cls.get_props())
            else {}
        )
...
class GridEvents(ComponentEnterprise):
    ...
    on_grid_ready: rx.EventHandler[passthrough_event_spec]
...
class AgGrid(GridOptions, GridEvents, ComponentEnterprise):
    ...
    def add_custom_code(self) -> list[str]:
        ...
        register_code = (
            Var("ModuleRegistry.registerModules")
            .to(FunctionVar)
            .call(tuple(Var(mod) for mod in _modules_to_register))
        )

        codes.append(str(register_code))
...
function formatColumnDefs(list) {{
  ...
  const formatted_columns = list.map(obj => {{
    if (obj !== null && typeof obj === "object") {{
      return Object.fromEntries(
        Object.entries(obj).map(([key, value]) => {{
          ...
          if ({list(ColumnDef.get_js_expressions(camel_case=True))}.includes(camelKey)) {{
            if (typeof value === 'string') {{
              if (/\(?[\w,\s]*\)?\s*=>\s*(\{{[\s\S]*\}}|[\s\S]*)/.test(value)) {{
                // If the string is already an arrow function just eval it
                return [camelKey, eval(value)]
              }}
              // If value is a string, convert it to a function
              return [camelKey, new Function('params', `return ${{value}};`)];
            }} else if (typeof value === 'object' && value !== null) {{
              if ('function' in value) {{
                // If value is an object containing "function", extract and convert it
                return [camelKey, new Function('params', `return ${{value.function}};`)];
              }} else if ("_js_expr" in value) {{
                // If value is an object containing "_js_expr", extract and convert it
