...
class Agent:
    ...
    def from_dict(cls, data: dict) -> 'Agent':
        ...
        filtered_data = {k: v for k, v in data.items() if k not in ['orders', 'invoices', 'status', 'createdAt', 'updatedAt']}
        ...
        if 'ProductAttribute' in filtered_data:
            filtered_data['agentAttributes'] = filtered_data.pop('ProductAttribute')
            # Also map productId to agentId in each attribute and convert prices
            if filtered_data['agentAttributes']:
                for attr in filtered_data['agentAttributes']:
...
class CreateAgentRequest:
    ...
    def to_product_dict(self) -> dict:
        ...
        data = {
            ...
            'productCode': self.agentCode or f"AGENT_{self.name.upper().replace(' ', '_')}"
        }
        return data
