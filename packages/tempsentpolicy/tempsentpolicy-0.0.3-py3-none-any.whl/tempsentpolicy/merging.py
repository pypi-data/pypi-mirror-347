...
def dare(
    ...
    Args:
        ...
        num_labels (int): Number of labels for classification.
        label2id (dict): Label->ID mapping for classification.
        p (float): Percentage of entries to drop.
        lambda_scale (float): Scaling factor λ for the final merged vector (default=1.0).
    """
...
def fisher_merging(
    ...
    Args:
        ...
        save_path (str): Path to save merged model.
        base_model_name (str): Name or path of the original base model.
        dataloaders (list): True if models in specified models_folder are sorted in sequential order, and correspond to the order in `dataloaders`. 
        num_labels (int): Number of labels for classification.
        label2id (dict): Label->ID mapping for classification.
        sorted_folders (bool): True if models in specified models_folder are sorted in sequential order
        lambda_terms (dict): Scaling factor, defaulted to None
...
def regmean_merging(
    ...
    Args:
        ...
        save_path (str): Path to save merged model.
        base_model_name (str): Name or path of the original base model.
        dataloaders (list): list of dataloaders containing data that corresponds to each model in the same order as models_folder
        num_labels (int): Number of labels for classification.
        label2id (dict): Label->ID mapping for classification.
        sorted_folders (bool): True if models in specified models_folder are sorted in sequential order, and correspond to the order in `dataloaders`. 
        float16 (bool): uses the more efficient float16 if True
        use_autocast (bool): wraps forward pass in bfloat16 autocast if True
        cpu_matmul (bool): do matmul for hooking on CPU to save GPU memory even further.
        alpha (dict): Scaling factor, defaulted to 1
        device (str): device to run on. 
    """
    if label2id is None:
        label2id = {"Negative": 0, "Neutral": 1, "Positive": 2}
