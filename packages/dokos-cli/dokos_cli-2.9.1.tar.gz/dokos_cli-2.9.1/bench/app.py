...
class AppMeta:
	...
	def folder_name(self):
		...
		return reversed_app_map.get(self.app_name, self.app_name)

	@property
	def path_to_app_dir(self):
		return os.path.join(self.bench.cwd, "apps", self.folder_name)
...
class App(AppMeta):
	...
	def remove(self, no_backup: bool = False):
		active_app_path = os.path.join("apps", self.folder_name)
...
def version_offset(offset: int):
	def _offset(version: str) -> str:
		major, minor, patch = version.split(".")
		return f"{int(major) + offset}.{minor}.{patch}"

	return _offset


DOKOS_ECOSYSTEM_VERSION_MOCK = {
	# Frappe 15 = Dodock 4, offset by 11
	"frappe": version_offset(11),
	"erpnext": version_offset(11),
}
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		if dep in DOKOS_ECOSYSTEM_VERSION_MOCK:
			mock_version = DOKOS_ECOSYSTEM_VERSION_MOCK[dep](dep_version)
			if sv.Version(mock_version) in sv.SimpleSpec(req_version):
				return  # ok
