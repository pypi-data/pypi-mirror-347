import sys
import torch
from RxnIM.reaction import Reaction
import json
...
model = Reaction(ckpt_path, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

def get_reaction(image_path: str) -> list:
    raw_predictions = model.predict_image_file(
        image_path,
        molscribe=True,
        ocr=True
    )
    preds = raw_predictions if isinstance(raw_predictions, list) else [raw_predictions]

    all_results = []
    for pred in preds:
        structured = {}
        for section in ('reactants', 'conditions', 'products'):
            items = pred.get(section, [])
            structured[section] = []
            for item in items:
                ...
                if section in ('reactants', 'products'):
                    ...
                    if item.get('smiles'):
                        entry['smiles'] = item['smiles'],
                        entry['bbox'] = item['bbox'],
                    if item.get('text'):
                        entry['text'] = item['text']
                structured[section].append(entry)
