            ...
            async def event_stream() -> AsyncIterator[str]:
                ...
                async for chunk in upstream_iter:
                    ...
                    if isinstance(tool_data, list):
                        ...
                        break # detenmos para ejecutar herramientas

                    if not tool_detected:
                        yield f"data: {json.dumps(payload)}\n\n"
                    
                    # Acumulamos texto para memoria
                    if content:
                        buf.append(content)
                        tokens += len(content)
                ...
                if tool_detected:
                    ...
                    # Copia segura del historial hasta antes del assistant actual, si lo hay
                    previous_msgs = msgs[:]
                    if previous_msgs and previous_msgs[-1].get("role") == "assistant":
                        previous_msgs.pop()

                    # Asegurarse que el último mensaje sea del usuario antes de tool_calls
                    if previous_msgs and previous_msgs[-1]["role"] != "user":
                        self._log("⚠️ Unexpected message order before tool_calls, may cause invalid OpenAI message chain.")

                    # Insertar tool_calls como continuación del user message
                    tool_call_msg = {
                        "role": "assistant",
                        "content": None,
                    ...
                    followup_msgs = previous_msgs + [tool_call_msg] + tool_results
