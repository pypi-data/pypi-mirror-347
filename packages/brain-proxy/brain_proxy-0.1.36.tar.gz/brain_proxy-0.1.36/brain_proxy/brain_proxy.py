...
class BrainProxy:
    ...
    def _prune_msgs_for_tool_followup(self, msgs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Remueve cualquier bloque de tools anteriores para preparar una nueva secuencia válida."""
        pruned = []
        for m in msgs:
            if m["role"] == "tool":
                break  # corta antes del bloque de tools anteriores
            pruned.append(m)
        return pruned
            ...
            async def event_stream() -> AsyncIterator[str]:
                ...
                async for chunk in upstream_iter:
                    ...
                    if isinstance(tool_data, list):
                        tool_detected = True
                        for partial_call in tool_data:
                            call_id = partial_call.get("id")
                            if not call_id:
                                continue
                            existing = tool_call_parts.get(call_id, {})
                            tool_call_parts[call_id] = merge_dicts(existing, partial_call)
                ...
                if tool_detected:
                    # Emitir los chunks recolectados
                    ...
                    for tool_call in tool_calls:
                        ...
                        if not args_str or not args_str.strip():
                            self._log(f"⚠️ Tool {name} called with empty arguments.")
                            args = {}
                        else:
                            try:
                                self._log(f"Tool call arguments for {name}: {repr(args_str)}")
                                args = json.loads(args_str)
                            except json.JSONDecodeError as e:
                                self._log(f"❌ Failed to parse arguments for tool {name}: {e}")
                                args = {}
                    ...
                    followup_msgs = self._prune_msgs_for_tool_followup(msgs) + [
                        tool_call_msg,
                        *tool_results,
                    ]
                    ...
                    self._log("🔁 Streaming follow-up response after tool calls...")
                    async for chunk in followup_iter:
                        self._log(f"⬇️ follow-up chunk: {chunk}")
                        payload = json.loads(chunk.model_dump_json())
