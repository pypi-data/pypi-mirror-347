...
class BrainProxy:
    ...
    def __init__(
        ...
        *,
        ...
        max_workers: int = 10,
        local_tools_handler: Optional[Callable[[str, List[Dict[str, Any]]], Any]] = None,  # tenant, tool_calls
    ):
    ...
    async def _dispatch(self, msgs, model: str, *, stream: bool, tools: Optional[List[Dict[str, Any]]] = None, tenant: Optional[str] = None):
        ...
        # For client-side tools, handle tool execution if handler is defined
        if tenant and (tenant in self._tenant_tools or self.local_tools_handler):
            if stream:
                ...
                async def format_stream():
                    ...
                    async for chunk in response:
                        ...
                        # Skip empty deltas
                        if not chunk_dict['choices'][0]['delta']:
                            continue
                            
                        # Add finish_reason if present
                        if hasattr(chunk.choices[0], 'finish_reason') and chunk.choices[0].finish_reason:
                            chunk_dict['choices'][0]['finish_reason'] = chunk.choices[0].finish_reason
                            
                            # If we have a local tools handler and tool calls, execute them
                            if self.local_tools_handler and tool_calls_buffer:
                                try:
                                    tool_results = await _maybe(self.local_tools_handler, tenant, tool_calls_buffer)
                                    if tool_results:
                                        # Make a new completion with the tool results
                                        msgs.append({"role": "assistant", "content": None, "tool_calls": tool_calls_buffer})
                                        msgs.append({"role": "tool", "content": json.dumps(tool_results), "tool_call_id": tool_calls_buffer[0]['id']})
                                        return await self._dispatch(msgs, model, stream=stream, tools=tools, tenant=tenant)
                                except Exception as e:
                                    self._log(f"Error executing local tools: {e}")
                                    
                        yield chunk_dict
                return format_stream()
            else:
                self._log(f"Returning non-streaming client tool response for {tenant}")
                response_dict = {
                    'choices': [{
                        'message': {
                ...
                # If we have a local tools handler and tool calls, execute them
                if self.local_tools_handler and hasattr(response.choices[0].message, 'tool_calls'):
                    try:
                        tool_calls = [{
                            'function': {
                                'name': tc.function.name,
                        ...
                        tool_results = await _maybe(self.local_tools_handler, tenant, tool_calls)
                        if tool_results:
                            # Make a new completion with the tool results
                            msgs.append({"role": "assistant", "content": None, "tool_calls": tool_calls})
                            msgs.append({"role": "tool", "content": json.dumps(tool_results), "tool_call_id": tool_calls[0]['id']})
                            return await self._dispatch(msgs, model, stream=stream, tools=tools, tenant=tenant)
                    except Exception as e:
                        self._log(f"Error executing local tools: {e}")
        ...
        async def chat(request: Request, tenant: str):
            ...
            async def event_stream() -> AsyncIterator[str]:
                ...
                async for chunk in upstream_iter:
                    ...
                    # Handle tool calls, content, or finish_reason
                    if content or delta.get('tool_calls') or choice.get('finish_reason'):
