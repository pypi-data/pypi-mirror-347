            ...
            async def event_stream() -> AsyncIterator[str]:
                ...
                def merge_dicts(a: dict, b: dict) -> dict:
                    ...
                    for key, value in b.items():
                        if isinstance(value, dict):
                            result[key] = merge_dicts(a.get(key, {}), value)
                        else:
                            result[key] = value
                    return result

                def merge_tool_call(existing: dict, incoming: dict) -> dict:
                    result = existing.copy()
                    for key, value in incoming.items():
                        if key == "function":
                            result_function = result.get("function", {}).copy()
                            for sub_key, sub_val in value.items():
                                if sub_val is not None:
                                    result_function[sub_key] = sub_val
                ...
                async for chunk in upstream_iter:
                    ...
                    if tool_data:
                        ...
                        for i, partial_call in enumerate(tool_data):
                            call_id = partial_call.get("id") or f"call_{i}"
                            # 🔒 Asegurar que siempre haya un id válido tipo str
                            partial_call["id"] = call_id
                            if call_id not in tool_call_parts:
                                tool_call_parts[call_id] = partial_call
                            else:
                                tool_call_parts[call_id] = merge_tool_call(tool_call_parts[call_id], partial_call)
                                if "name" not in tool_call_parts[call_id].get("function", {}):
                                   print(f"❗ name perdido después del merge en call_id={call_id}: {tool_call_parts[call_id]}")

                    if tool_calls_detected:
                        # ⚠️ No emitir nada todavía, estamos en modo tool call
                ...
                available_tools = {t["function"]["name"]: t["function"] for t in final_tools}
                local_tools = {t["function"]["name"]: t["function"] for t in req.tools or []}
                tool_results = []
