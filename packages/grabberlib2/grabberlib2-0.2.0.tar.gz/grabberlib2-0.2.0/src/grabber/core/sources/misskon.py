    ...
    **kwargs,
    ...
    for source_url in tqdm_sources_iterable:
        ...
        for index, url in enumerate(urls):
            ...
            page_title = soup.find("title").get_text(strip=True).split(" – MissKon.com")[0].split("– Page")[0].strip()

            if index == 0 or not page_title:
                page_title = (
                    soup.find("title").get_text(strip=True).split(" – MissKon.com")[0].split("– Page")[0].strip()
                )
