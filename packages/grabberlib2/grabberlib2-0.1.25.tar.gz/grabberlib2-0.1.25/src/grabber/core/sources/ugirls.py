...
async def get_images_from_pagination(url: str, headers: dict | None = None) -> list[str]:
    pages_count_query = "nav[aria-label='Pagination Navigation'] > div > span > a:not([aria-label='Next &raquo;'])"
    ...
    **kwargs,
    ...
    for source_url in tqdm_sources_iterable:
        ...
        for url in urls:
            ...
            page_title = soup.find("title").get_text(separator=" ", strip=True).split("|")[0].split("Page")[0].rstrip()
