...
class BLPY:
    def __init__(self, vestion):
        self.ver = vestion
        if self.ver != "1.4":
            raise ValueError("Save version not supported")
        self.world = [[{None:None} for _ in range(256)] * 256]
        #region blockIds
        self.BlockIDs = {
                            "1": "AND Gate",
                            "b": "NAND Gate",
        ...
        self.IDBlocks = {v: k for k, v in self.BlockIDs.items()}
        #endregion
    
        
    def DecodeBlockItem(self, raw:str) -> dict[str:str|list|tuple]:
        return {
            "id": self.BlockIDs[raw[0]],
            "positon": tuple(base64Decode(raw[1:5])),
            "rotation": raw[5],
            "colour": tuple(base64Decode(raw[6:]))
        }

    def DecodeBlockSave(self, raw:str) -> list[dict[str:str|list|tuple]]:
        return [self.DecodeBlockItem(i) for i in raw.split(";")]

    def EncodeBlockItem(self, block:dict[str:str|list|tuple]) -> str:
        return f"{self.IDBlocks[block['id']]}{base64Encode(bytes(block['position']))}{block['rotation']}{base64Encode(bytes(block['colour']))}"
    
    def EncodeBlockSave(self, blocks:list[dict[str:str|list|tuple]]) -> str:
        return (";".join([self.EncodeBlockItem(i) for i in blocks]))[1:]
'''
1.4 
first number == relese (0 is in test 1 is current 2 is the new)
second number == number of changes to that relese
'''
