...
def setup_logging_for_emr():
    ...
    try:
        ...
        if not cluster_id:
            print("Nao foi possível identificar o cluster.")
            return None

        cluster_name = get_cluster_name(emr_client, cluster_id)
        if not cluster_name:
            print("Cluster nao possui a tag 'Name'.")
            return None

        step_name = os.getenv("STEP_NAME")
        if not step_name:
            print("Nao foi possivel obter o nome da step usando getenv. Usando status RUNNING")
