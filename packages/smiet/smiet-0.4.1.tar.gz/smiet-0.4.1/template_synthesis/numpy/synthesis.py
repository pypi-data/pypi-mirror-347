...
from .utilities import units, angle_between
...
class SliceSynthesis:
    ...
    def __init__(self, slice_grammage, ce_linear=True):
        self.logger = logging.getLogger("template_synthesis.numpy.synthesis.SliceSynthesis")
    ...
    def get_normalisation_factors(self, shower: Shower):
        ...
        if n_slice < 0.001 * shower.nmax:
            self.logger.info("Slice contains too few particles, not using this one...")
            return 0, 0
...
class TemplateSynthesis:
    ...
    Then one can pass a ``SlicedShower`` as an origin to the `TemplateSynthesis.make_template`
    method, which construct all the necessary arrays to perform the synthesis.
    To perform the synthesis, you then call the `TemplateSynthesis.map_template` method with a
    ...
    def __init__(self, freq_ar: list = None) -> None:
        self.logger = logging.getLogger("template_synthesis.numpy.synthesis.TemplateSynthesis")
    ...
    def load_template(self, file_path):
        ...
        # TODO: this only allows non-curved, and non-GDAS atnospheres for now
    ...
    def get_origin_shower(self):
        ...
        field are stored in the `TemplateSynthesis.template_information` dictionary. This information
    ...
    def slice_create_interpolators(self, my_slice_ind, shower: Shower, frequencies, use_ce_linear=None):
        ...
        `SliceSynthesis.create_interpolators` method.
    ...
    def slice_get_spectra(self, my_slice_ind, delta_xmax, frequencies):
        ...
        The `delta_xmax` and `frequencies` parameters are passed on to the `SliceSynthesis.get_spectra`
        function.
    ...
    def slice_get_normalisation_factors(self, my_slice_ind, shower: Shower):
        ...
        The ``shower`` parameter is used as argument to the :func:`SliceSynthesis.get_normalisation_factors`.
