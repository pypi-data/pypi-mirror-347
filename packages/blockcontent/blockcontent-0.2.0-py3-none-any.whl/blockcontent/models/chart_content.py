...
class ChartContent(BaseModel):
    ...
    ChartContent
    """ # noqa: E501
    chart_type: StrictStr = Field(description="type of chart")
    labels: List[StrictStr]
    datasets: List[ChartContentDatasetsInner]
    __properties: ClassVar[List[str]] = ["chart_type", "labels", "datasets"]

    @field_validator('chart_type')
    def chart_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['bar', 'line', 'pie', 'scatter']):
            raise ValueError("must be one of enum values ('bar', 'line', 'pie', 'scatter')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "datasets": [ChartContentDatasetsInner.from_dict(_item) for _item in obj["datasets"]] if obj.get("datasets") is not None else None
        })
        return _obj
