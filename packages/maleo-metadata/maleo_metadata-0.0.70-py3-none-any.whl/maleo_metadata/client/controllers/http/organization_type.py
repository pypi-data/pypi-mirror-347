...
class MaleoMetadataOrganizationTypeHTTPController(MaleoClientHTTPController):
    async def get_organization_types(self, parameters:MaleoMetadataOrganizationTypeClientParametersTransfers.GetMultiple) -> BaseClientHTTPControllerResults:
        ...
        async with self._manager.get_client() as client:
            ...
            params = MaleoMetadataOrganizationTypeClientParametersTransfers.GetMultipleQuery.model_validate(parameters.model_dump()).model_dump(exclude={"sort_columns", "date_filters"}, exclude_none=True)
