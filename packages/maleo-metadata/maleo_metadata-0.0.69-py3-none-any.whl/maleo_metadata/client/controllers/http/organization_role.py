...
class MaleoMetadataOrganizationRoleHTTPController(MaleoClientHTTPController):
    async def get_organization_roles(self, parameters:MaleoMetadataOrganizationRoleClientParametersTransfers.GetMultiple) -> BaseClientHTTPControllerResults:
        ...
        async with self._manager.get_client() as client:
            ...
            params = MaleoMetadataOrganizationRoleClientParametersTransfers.GetMultipleQuery.model_validate(parameters.model_dump()).model_dump(exclude={"sort_columns", "date_filters"}, exclude_none=True)

            #* Create auth
            token = await self._service_manager.generate_token()
