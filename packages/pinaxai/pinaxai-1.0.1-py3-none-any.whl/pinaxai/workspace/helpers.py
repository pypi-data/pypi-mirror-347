...
def get_workspace_dir_from_env() -> Optional[Path]:
    ...
    from pinaxai.constants import WORKSPACE_DIR_ENV_VAR
...
def get_workspace_dir_path(ws_root_path: Path) -> Path:
    ...
    Pinaxai workspace dir can be found at:
        1. subdirectory: workspace
        2. In a folder defined by the pyproject.toml file
    """
    from pinaxai.utils.pyproject import read_pyproject_pinaxai
    ...
    if ws_pyproject_toml.exists() and ws_pyproject_toml.is_file():
        pinaxai_conf = read_pyproject_pinaxai(ws_pyproject_toml)
        if pinaxai_conf is not None:
            pinaxai_conf_workspace_dir_str = pinaxai_conf.get("workspace", None)
            pinaxai_conf_workspace_dir_path = ws_root_path.joinpath(pinaxai_conf_workspace_dir_str)
            logger.debug(f"Searching {pinaxai_conf_workspace_dir_path}")
            if pinaxai_conf_workspace_dir_path.exists() and pinaxai_conf_workspace_dir_path.is_dir():
                return pinaxai_conf_workspace_dir_path
