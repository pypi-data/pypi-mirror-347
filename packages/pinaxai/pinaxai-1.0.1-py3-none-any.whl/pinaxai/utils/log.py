...
class PinaxaiLogger(logging.Logger):
...
def build_logger(logger_name: str, source_type: Optional[str] = None) -> Any:
    ...
    logging.setLoggerClass(PinaxaiLogger)
    ...
    rich_handler = ColoredRichHandler(
        ...
        show_path=True if getenv("PINAXAI_API_RUNTIME") == "dev" else False,
...
agent_logger: PinaxaiLogger = build_logger(LOGGER_NAME, source_type="agent")
team_logger: PinaxaiLogger = build_logger(TEAM_LOGGER_NAME, source_type="team")

# Set the default logger to the agent logger
logger: PinaxaiLogger = agent_logger
