...
from pinaxai.api.schemas.user import TeamSchema, UserSchema
from pinaxai.api.schemas.workspace import WorkspaceSchema
...
class PinaxaiCliConfig:
    """The PinaxaiCliConfig class manages user data for the pinaxai cli"""
    ...
    def __init__(
        ...
        # Active ws dir - used as the default for `pa` commands
    ...
    def clear_user_cache(self) -> None:
        ...
        pinaxai_cli_settings.ai_conversations_path.unlink(missing_ok=True)
        logger.info("Workspaces cleared, please setup again using `pinaxai ws setup`")
    ...
    def _add_or_update_ws_config(
        ...
        if existing_ws_config is None:
            logger.error(f"Could not find workspace at: {ws_root_str}, please run `pinaxai ws setup`")
            return None
    ...
    def add_new_ws_to_config(
        ...
        """Adds a newly created workspace to the PinaxaiCliConfig"""
    ...
    def delete_ws(self, ws_root_path: Path) -> None:
        """Handles Deleting a workspace from the PinaxaiCliConfig and api"""
        ...
        print_info("Note: this does not delete any data locally or from pinax.tech, please delete them manually\n")
    ...
    def save_config(self):
        ...
        write_json_file(file_path=pinaxai_cli_settings.config_file_path, data=config_data)

    @classmethod
    def from_saved_config(cls) -> Optional["PinaxaiCliConfig"]:
        try:
            config_data = read_json_file(file_path=pinaxai_cli_settings.config_file_path)
            ...
            logger.warning(e)
            logger.warning("Please setup the workspace using `pinaxai ws setup`")
            return None

    ######################################################
    ## Print PinaxaiCliConfig
    ######################################################
    ...
    def print_to_cli(self, show_all: bool = False):
        ...
        if self.active_ws_dir:
            ...
            print_info(
                "Please create a workspace using `pinaxai ws create` or setup an existing workspace using `pinaxai ws setup`"
            )
