"""Pinaxai Workspace Cli

This is the entrypoint for the `pinaxai ws` application.
"""
...
from pinaxai.utils.log import logger, set_log_level_to_debug

ws_cli = typer.Typer(
    name="ws",
    short_help="Manage workspaces",
    help="""\b
Use `pinaxai ws [COMMAND]` to create, setup, start or stop your workspace.
Run `pinaxai ws [COMMAND] --help` for more info.
""",
...
def create(
    ...
    \b
    Examples:
    > pinaxai ws create -t ai-app                -> Create an `ai-app` in the current directory
    > pinaxai ws create -t ai-app -n my-ai-app   -> Create an `ai-app` named `my-ai-app` in the current directory
    """
    if print_debug_log:
        set_log_level_to_debug()

    from pinaxai.workspace.operator import create_workspace
...
def setup(
    ...
    \b
    Examples:
    > `pinaxai ws setup`           -> Setup the current directory as a workspace
    > `pinaxai ws setup ai-app`    -> Setup the `ai-app` folder as a workspace
    """
    if print_debug_log:
        set_log_level_to_debug()

    from pinaxai.workspace.operator import setup_workspace
...
def up(
    ...
    \b
    Examples:
    > `pinaxai ws up`            -> Deploy all resources
    > `pinaxai ws up dev`        -> Deploy all dev resources
    > `pinaxai ws up prd`        -> Deploy all prd resources
    > `pinaxai ws up prd:aws`    -> Deploy all prd aws resources
    > `pinaxai ws up prd:::s3`   -> Deploy prd resources matching name s3
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from pinaxai.utils.resource_filter import parse_resource_filter
    from pinaxai.workspace.config import WorkspaceConfig
    from pinaxai.workspace.helpers import get_workspace_dir_path
    from pinaxai.workspace.operator import setup_workspace, start_workspace
    ...
    ws_at_current_path: Optional[WorkspaceConfig] = pinaxai_config.get_ws_config_by_path(current_path)
    if ws_at_current_path is not None:
        logger.debug(f"Found workspace at: {ws_at_current_path.ws_root_path}")
        if str(ws_at_current_path.ws_root_path) != pinaxai_config.active_ws_dir:
            logger.debug(f"Updating active workspace to {ws_at_current_path.ws_root_path}")
            pinaxai_config.set_active_ws_dir(ws_at_current_path.ws_root_path)
    ...
    if ws_to_start is None:
        active_ws_config: Optional[WorkspaceConfig] = pinaxai_config.get_active_ws_config()
    ...
    if ws_to_start is None:
        log_active_workspace_not_available()
        avl_ws = pinaxai_config.available_ws
        if avl_ws:
...
def down(
    ...
    \b
    Examples:
    > `pinaxai ws down`            -> Delete all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from pinaxai.workspace.config import WorkspaceConfig
    from pinaxai.workspace.helpers import get_workspace_dir_path
    from pinaxai.workspace.operator import setup_workspace, stop_workspace
...
def patch(
    ...
    Examples:
    \b
    > `pinaxai ws patch`           -> Patch all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from pinaxai.workspace.config import WorkspaceConfig
    from pinaxai.workspace.helpers import get_workspace_dir_path
    from pinaxai.workspace.operator import setup_workspace, update_workspace
...
def restart(
    ...
    """\b
    Restarts the active workspace. i.e. runs `pinaxai ws down` and then `pinaxai ws up`.
...
def config(
    ...
    \b
    Examples:
    $ `pinaxai ws config`         -> Print the active workspace config
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from pinaxai.cli.config import PinaxaiCliConfig
    from pinaxai.cli.operator import initialize_pinaxai
    from pinaxai.utils.load_env import load_env
    from pinaxai.workspace.config import WorkspaceConfig
...
def delete(
    ...
    all_workspaces: bool = typer.Option(
        ...
        "-a",
        "--all",
        help="Delete all workspaces from Pinaxai",
    ),
    print_debug_log: bool = typer.Option(
        False,
        "-d",
    ...
    """\b
    Deletes the workspace record from pinaxai.
    NOTE: Does not delete any physical files.

    \b
    Examples:
    $ `pinaxai ws delete`         -> Delete the active workspace from Pinaxai
    $ `pinaxai ws delete -a`      -> Delete all workspaces from Pinaxai
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from pinaxai.cli.config import PinaxaiCliConfig
    from pinaxai.cli.operator import initialize_pinaxai
    from pinaxai.workspace.operator import delete_workspace
    ...
    if ws_name is not None:
        ws_config = pinaxai_config.get_ws_config_by_dir_name(ws_name)
        ...
        if all_workspaces:
            ws_to_delete = [ws.ws_root_path for ws in pinaxai_config.available_ws if ws.ws_root_path is not None]
        else:
            # By default, we assume this command is run for the active workspace
            if pinaxai_config.active_ws_dir is not None:
                ws_to_delete.append(Path(pinaxai_config.active_ws_dir))

    delete_workspace(pinaxai_config, ws_to_delete)
