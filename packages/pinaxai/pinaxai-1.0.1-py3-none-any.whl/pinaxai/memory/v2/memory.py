...
from pinaxai.media import AudioArtifact, ImageArtifact, VideoArtifact
from pinaxai.memory.v2.db.base import MemoryDb
...
from pinaxai.utils.log import log_debug, log_warning, logger, set_log_level_to_debug, set_log_level_to_info
...
class Memory:
    ...
    def get_model(self) -> Model:
        if self.model is None:
            try:
                ...
                logger.exception(e)
                logger.error(
                    "Pinaxai uses `openai` as the default model provider. Please provide a `model` or install `openai`."
                )
                exit(1)
    ...
    def set_log_level(self):
        if self.debug_mode or getenv("PINAXAI_DEBUG", "false").lower() == "true":
