...
from pinaxai.api.schemas.user import EmailPasswordAuthSchema, UserSchema
...
def user_ping() -> bool:
    if not pinaxai_cli_settings.api_enabled:
        return False
...
def authenticate_and_get_user(auth_token: str, existing_user: Optional[UserSchema] = None) -> Optional[UserSchema]:
    ...
    auth_header = {pinaxai_cli_settings.auth_token_header: auth_token}
    anon_user = None
...
def sign_in_user(sign_in_data: EmailPasswordAuthSchema) -> Optional[UserSchema]:
    ...
    with api.Client() as api_client:
        try:
            ...
            pinaxai_auth_token = r.headers.get(pinaxai_cli_settings.auth_token_header)
            if pinaxai_auth_token is None:
            ...
            if current_user is not None:
                save_auth_token(pinaxai_auth_token)
                return current_user
        except Exception as e:
...
def user_is_authenticated() -> bool:
    ...
    if pinaxai_config is None:
        return False
    user: Optional[UserSchema] = pinaxai_config.user
    if user is None:
        return False
