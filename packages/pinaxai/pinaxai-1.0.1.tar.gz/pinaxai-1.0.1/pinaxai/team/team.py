...
from pinaxai.exceptions import ModelProviderError, RunCancelledException
from pinaxai.knowledge.agent import AgentKnowledge
from pinaxai.media import Audio, AudioArtifact, AudioResponse, File, Image, ImageArtifact, Video, VideoArtifact
from pinaxai.memory.agent import AgentMemory
from pinaxai.memory.team import TeamMemory, TeamRun
from pinaxai.memory.v2.memory import Memory, SessionSummary
...
from pinaxai.utils.string import is_valid_uuid, parse_response_model_str, url_safe_string
...
class Team:
    ...
    # monitoring=True logs Team information to pinax.tech for monitoring
    ...
    def _set_monitoring(self) -> None:
        ...
        monitor_env = getenv("PINAXAI_MONITOR")
        if monitor_env is not None:
            self.monitoring = monitor_env.lower() == "true"

        telemetry_env = getenv("PINAXAI_TELEMETRY")
    ...
    def _set_default_model(self) -> None:
        ...
        if self.model is None:
            try:
                ...
                log_exception(e)
                log_error(
                    "Pinaxai agents use `openai` as the default model provider. "
            ...
            if isinstance(self.memory, TeamMemory):
                try:
                    ...
                    if "memories" in session.memory:
                        from pinaxai.memory.memory import Memory as UserMemoryV1
                ...
                if "team_context" in session.memory:
                    from pinaxai.memory.v2.memory import TeamContext
                ...
                if "memories" in session.memory:
                    if self.memory.memories is not None:
                        ...
                        from pinaxai.memory.v2.memory import UserMemory as UserMemoryV2
                ...
                if "summaries" in session.memory:
                    if self.memory.summaries is not None:
                        ...
                        from pinaxai.memory.v2.memory import SessionSummary as SessionSummaryV2
    ...
    def _add_reasoning_step_to_extra_data(self, run_response: TeamRunResponse, reasoning_step: ReasoningStep) -> None:
        if run_response.extra_data is None:
            from pinaxai.run.response import RunResponseExtraData
    ...
    def _log_team_session(self, session_id: str, user_id: Optional[str] = None):
        ...
        from pinaxai.api.team import TeamSessionCreate, upsert_team_session
