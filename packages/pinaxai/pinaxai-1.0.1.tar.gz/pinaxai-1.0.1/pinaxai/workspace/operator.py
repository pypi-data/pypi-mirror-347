...
from pinaxai.api.schemas.user import TeamIdentifier, TeamSchema
from pinaxai.api.schemas.workspace import (
    WorkspaceCreate,
    WorkspaceEvent,
    WorkspaceSchema,
    WorkspaceUpdate,
)
from pinaxai.api.workspace import log_workspace_event
from pinaxai.cli.config import PinaxaiCliConfig
from pinaxai.cli.console import (
    console,
...
from pinaxai.utils.common import str_to_int
from pinaxai.utils.log import logger
from pinaxai.workspace.config import WorkspaceConfig
from pinaxai.workspace.enums import WorkspaceStarterTemplate
...
TEMPLATE_TO_REPO_MAP: Dict[WorkspaceStarterTemplate, str] = {
    WorkspaceStarterTemplate.agent_app: "https://github.com/Pinax-Labs/agent-app-aws",
    WorkspaceStarterTemplate.agent_api: "https://github.com/Pinax-Labs/agent-api-aws",
}
...
def create_workspace(
    ...
    from pinaxai.utils.filesystem import rmdir_recursive
    from pinaxai.utils.git import GitCloneProgress
    from pinaxai.workspace.helpers import get_workspace_dir_path

    current_dir: Path = Path(".").resolve()

    # Initialize Pinaxai before creating a workspace
    ...
    if not pinaxai_config:
        ...
        if not pinaxai_config:
            log_config_not_available_msg()
            return None
    pinaxai_config = cast(PinaxaiCliConfig, pinaxai_config)
    ...
    pinaxai_config.add_new_ws_to_config(ws_root_path=ws_root_path)
...
def setup_workspace(ws_root_path: Path) -> Optional[WorkspaceConfig]:
    """Setup an Pinaxai workspace at `ws_root_path` and return the WorkspaceConfig

    1. Pre-requisites
    1.1 Check ws_root_path exists and is a directory
    1.2 Create PinaxaiCliConfig if needed
    ...
    2.1 Check if a ws_schema exists for this workspace, meaning this workspace has a record in pinaxai-api
    ...
    from pinaxai.cli.operator import initialize_pinaxai
    from pinaxai.utils.git import get_remote_origin_for_dir
    from pinaxai.workspace.helpers import get_workspace_dir_path
    ...
    ws_config: Optional[WorkspaceConfig] = pinaxai_config.get_ws_config_by_path(ws_root_path)
    if ws_config is None:
        ...
        ws_config = pinaxai_config.create_or_update_ws_config(ws_root_path=ws_root_path, set_as_active=True)
        if ws_config is None:
    ...
    if pinaxai_config.user is None:
    ...
    if pinaxai_config.user is not None:
        # 2.2 Create WorkspaceSchema if it doesn't exist
        if ws_schema is None or ws_schema.id_workspace is None:
            from pinaxai.api.workspace import create_workspace_for_user, get_teams_for_user
            ...
            logger.debug(f"Getting teams for user: {pinaxai_config.user.email}")
            ...
            with Live(transient=True) as live_log:
                ...
                live_log.update(status)
                teams = get_teams_for_user(pinaxai_config.user)
                status.stop()
            ...
            ws_config = pinaxai_config.create_or_update_ws_config(
        ...
        if git_remote_origin_url is not None and ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from pinaxai.api.workspace import update_workspace_for_team, update_workspace_for_user
    ...
    if ws_config is not None:
        ...
        print_info("\tpinaxai ws up")
        print_info("2. Stop workspace:")
        print_info("\tpinaxai ws down")

        if ws_config.ws_schema is not None and pinaxai_config.user is not None:
            log_workspace_event(
                user=pinaxai_config.user,
...
def start_workspace(
    ...
    """Start an Pinaxai Workspace. This is called from `pinaxai ws up`"""
    if ws_config is None:
    ...
    if (
        pinaxai_config.user is not None
...
def stop_workspace(
    ...
    """Stop an Pinaxai Workspace. This is called from `pinaxai ws down`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def update_workspace(
    ...
    """Update an Pinaxai Workspace. This is called from `pinaxai ws patch`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def delete_workspace(pinaxai_config: PinaxaiCliConfig, ws_to_delete: Optional[List[Path]]) -> None:
    if ws_to_delete is None or len(ws_to_delete) == 0:
        print_heading("No workspaces to delete")
        return

    for ws_root in ws_to_delete:
        pinaxai_config.delete_ws(ws_root_path=ws_root)
...
def set_workspace_as_active(ws_dir_name: Optional[str]) -> None:
    ...
    ######################################################
    ######################################################
    # 1.1 Check PinaxaiCliConfig is valid
    ######################################################
    pinaxai_config: Optional[PinaxaiCliConfig] = PinaxaiCliConfig.from_saved_config()
    if not pinaxai_config:
        pinaxai_config = initialize_pinaxai()
    ...
    if ws_dir_name is None:
        ...
        ws_config: Optional[WorkspaceConfig] = pinaxai_config.get_ws_config_by_dir_name(ws_dir_name)
        if ws_config is None:
    ...
    active_ws_config: Optional[WorkspaceConfig] = pinaxai_config.get_ws_config_by_path(ws_root_path)
    if active_ws_config is None:
        ...
        print_info("If this workspace has not been setup, please run `pinaxai ws setup` from the workspace directory")
        return
    ...
    pinaxai_config.set_active_ws_dir(active_ws_config.ws_root_path)
