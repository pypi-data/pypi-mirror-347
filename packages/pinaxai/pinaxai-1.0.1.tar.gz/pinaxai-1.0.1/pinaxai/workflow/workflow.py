...
from pinaxai.memory.workflow import WorkflowMemory, WorkflowRun
from pinaxai.run.response import RunEvent, RunResponse  # noqa: F401
from pinaxai.storage.base import Storage
from pinaxai.storage.session.workflow import WorkflowSession
from pinaxai.utils.common import nested_model_dump
...
class Workflow:
    ...
    # monitoring=True logs Workflow information to pinax.tech for monitoring
    monitoring: bool = field(default_factory=lambda: getenv("PINAXAI_MONITOR", "false").lower() == "true")
    # telemetry=True logs minimal telemetry for analytics
    # This helps us improve the Workflow and provide better support
    telemetry: bool = field(default_factory=lambda: getenv("PINAXAI_TELEMETRY", "true").lower() == "true")
    ...
    def set_monitoring(self) -> None:
        if self.monitoring or getenv("PINAXAI_MONITOR", "false").lower() == "true":
            self.monitoring = True
        else:
            self.monitoring = False

        if self.telemetry or getenv("PINAXAI_TELEMETRY", "true").lower() == "true":
            self.telemetry = True
        else:
