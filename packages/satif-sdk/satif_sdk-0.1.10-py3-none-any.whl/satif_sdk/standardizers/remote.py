...
class RemoteStandardizer(AsyncStandardizer):
    ...
    async def _download_run_result(self, run_id: str, output_path: Path) -> None:
        result_url_relative = f"{self._runs_path_prefix.lstrip('/')}/{run_id}/result"
        log.info(
            f"Downloading result for run {run_id} from {self._client.base_url}{result_url_relative}..."
        )
        try:
            ...
            try:
                ...
                pass  # Ignore
            log.error(
                f"Failed to download result: {e.response.status_code}{error_detail}",
                exc_info=True,
            )
            raise RuntimeError(
                f"Failed to download result file: {e.response.status_code}{error_detail}"
            ) from e
        except OSError as e:
            log.error(
                f"Failed to write result file to {output_path}: {e}", exc_info=True
            )
            raise OSError(  # Re-raise as OSError for consistent error type
    ...
    async def standardize(
        ...
        *,
        ...
        5. Downloading the resulting SDIF file upon successful completion.
        6. Saving the downloaded SDIF file.
        ...
        try:
            ...
            await self._download_run_result(run_id, output_path_obj)
            ...
            raise  # Re-raise known and handled exceptions
        finally:
