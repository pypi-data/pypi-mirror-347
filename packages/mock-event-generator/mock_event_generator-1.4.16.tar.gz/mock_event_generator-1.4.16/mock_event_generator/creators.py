...
class GEventCreator:
    ...
    async def create(
        ...
        # Add MOCK label
        try:
            self.target.write_label(event_id, 'MOCK')
        except HTTPError as exc:
            logger.error(exc.response.text)
            raise
        except NameError:
            logger.info(
                f'Error in Writting label MOCK for ({group},{search}) event {event_id}.'
            )
    ...
    async def _upload_additional_data(self, event_id: str, upload: Upload) -> None:
        ...
        # check uploads sanity (reduce max delay to 60s)
        if upload.delay > 60:
            logger.info(
                f'--> Delay for "{upload.filename}"'
                f' is {upload.delay}. Set delay to 60s'
            )
            await trio.sleep(60)
        else:
