...
class MouseDragEvent(object):
    ...
    def __repr__(self):
        if self.currentItem is None:
            ...
            lp = self.lastPos()
            p = self.pos()
        return "<MouseDragEvent (%g,%g)->(%g,%g) buttons=%d start=%s finish=%s>" % (lp.x(), lp.y(), p.x(), p.y(), str(self.buttons()), str(self.isStart()), str(self.isFinish()))
    ...
    def __repr__(self):
        try:
            if self.currentItem is None:
                ...
                p = self.pos()
            return "<MouseClickEvent (%g,%g) button=%d>" % (p.x(), p.y(), str(self.button()))
        except:
            return "<MouseClickEvent button=%d>" % (str(self.button()))
...
class HoverEvent(object):
    ...
    def __repr__(self):
        ...
        if self.currentItem is None:
            ...
            lp = self.lastPos()
            p = self.pos()
        return "<HoverEvent (%g,%g)->(%g,%g) buttons=%d enter=%s exit=%s>" % (lp.x(), lp.y(), p.x(), p.y(), str(self.buttons()), str(self.isEnter()), str(self.isExit()))
