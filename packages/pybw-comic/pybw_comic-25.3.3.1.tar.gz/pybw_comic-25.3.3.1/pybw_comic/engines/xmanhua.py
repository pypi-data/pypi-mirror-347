...
def get_dic_xpath():
    dic_xpath = {
                 'catelogue-booktopic': '/html/body/div[3]/div/div/p[3]/span[3]/span', 
                 'catelogue-bookstate': '/html/body/div[3]/div/div/p[3]/span[2]/span', 
                 'catelogue-bookname': '/html/body/div[3]/div/div/p[1]',
                 'catelogue-chapters': '//*[@id="chapterlistload"]/a',
...
class Catelogue():
    '''
    '''
    def __init__(self, url='https://www.xmanhua.com/75xm/'):
        self.url = url
    ...
    def _click_webpage(self):
        to_click = {
                    'enter_webpage': '/html/body/div[10]', 
                    'rank_order': '/html/body/div[5]/div[1]/a', 
                    'unfold_chapters': '/html/body/div[5]/a'
                    }
        # to_click.pop('enter_webpage')
        for k, v in to_click.items():
            try:
                self._click_by_xpath(v)
            except:
                pass
        return
    
    def _click_by_xpath(self, pat_xpath):
        try:
            time.sleep(1)
            _find = driver.find_elements(By.XPATH, pat_xpath)
            if _find:
                _find[0].click()
                time.sleep(1)
        except:
...
class OneChapter():
    '''
    '''
    def __init__(self, url='https://www.xmanhua.com/m227010/'):
        self.url = url
    ...
    def get_pages(self):
        ...
        pages['text'] = [CleanText(i).clean_text for i in pages['text']]
        
        pages['index_text'] = ['page_{}-{}'.format(str(i+1).zfill(6), text) for i, text in enumerate(pages['text'])]
        
        pages['url_part'] = [i.get_property(dic_xpath['onechapter-pages-Property_url_part']) for i in finds]
        
        if dic_xpath['onechapter-pages-Property_url_full']:
            pages['url'] = [i.get_property(dic_xpath['onechapter-pages-Property_url_full']) for i in finds]
        else:
    ...
    def cal_images(self):
        ...
        for page in self.pages:
            time.sleep(1)
            img_url_i = OneChapter(page[1]).image_url
            zip_images.append([page[0], img_url_i])
        
        zip_images_urls = [i[1] for i in zip_images]
        if len(zip_images_urls) != len(set(zip_images_urls)):
            raise Exception('images of onechapter are repeated')
...
class Book():
    ...
    '''
    def __init__(self, 
                 url='https://www.xmanhua.com/75xm/', 
                 dire='', 
                 headless=False):
        self.url = url
        self._headless = headless
        
        self.website_label = 'xmanhua'
...
if __name__ == '__main__':
    ...
    urls = [
            'https://www.xmanhua.com/75xm/', 
            'https://www.xmanhua.com/18704xm/',
           ]
