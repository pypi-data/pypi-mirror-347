...
def cleanup_cache():
    ...
    if total_removed > 0:
        logging.info(f"Cleaned up {len(search_keys_to_remove)} expired search caches and {len(project_keys_to_remove)} project asset caches")
            ...
            if create_params and isinstance(create_params, dict):
                ...
                if analysis and isinstance(analysis, dict):
                    ...
                    if people and len(people) > 0:
                        people_names = [person.get("name", "Unknown") for person in people[:5]]
        ...
        # Handle video_edit assets
            ...
            if clips and len(clips) > 0:
                ...
                for i, clip in enumerate(clips[:3]):
                    ...
                    formatted.append(f"    Clip {i+1}: {clip_id} of type {asset_type} from {start} to {end}")
                if len(clips) > 3:
    ...
    if name == "get-project-assets" and arguments:
        ...
        if asset_cache_id and asset_cache_id in _project_assets_cache:
            ...
            if current_page_items:
                formatted_assets = [format_asset_info(asset) for asset in current_page_items]
        ...
        try:
            ...
            logging.info(f"After filtering by types {asset_types}: {len(project_assets)} assets remaining")
            ...
            _project_assets_cache[new_cache_id] = {
                ...
                "project_info": {"id": project_id, "name": project.name, "description": project.description},
                "timestamp": time.time()
            }
            ...
            # Format assets
            if first_page_items:
                formatted_assets = [format_asset_info(asset) for asset in first_page_items]
                response_text.extend(formatted_assets)
            else:
