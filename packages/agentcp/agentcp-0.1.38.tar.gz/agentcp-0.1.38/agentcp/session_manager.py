...
from agentcp.message import AgentCmdBlock
...
class Session:
    ...
    def send_msg(self, msg: list, receiver: str, ref_msg_id : str = "",message_id:str =""):
        if len(msg) == 0:
        ...
        last_element = msg[-1]
        cmd_msg = None
        if isinstance(last_element, AgentCmdBlock):
            cmd_msg = last_element
            msg.pop()
...
class SessionManager:
    ...
    def send_msg(self, session_id: str, msg: list, receiver: str, ref_msg_id: str = "",message_id: str = ""):
        with self.lock:
            if session_id not in self.sessions:
                log_error(f"session {session_id} does not exist.")
                return False
