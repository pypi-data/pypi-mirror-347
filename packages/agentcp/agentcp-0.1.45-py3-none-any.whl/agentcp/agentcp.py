...
class AgentID(abc.ABC): 
    ...
    def send_add_friend_cmd_message(self,session_id: str,to_aid,add_aid:str,description:str=""):
        agent_cmd_block:AgentInstructionBlock = AgentInstructionBlock(cmd="add_friend",params={"aid":add_aid,"description":description})
        self.send_cmd_message(session_id,to_aid,agent_cmd_block)
        
    # 发送自定义指令消息
    def send_cmd_message(self,session_id: str,to_aid: str,agent_cmd_block:AgentInstructionBlock=None,ref_msg_id: str=""):
        # 处理对象转换为字典
        if session_id == "" or session_id is None:
            return
        self.send_message(session_id,[to_aid],None,agent_cmd_block=agent_cmd_block)
        
    def send_message(self, sessionId: str,to_aid_list: list,  message: Union[AssistantMessageBlock,list[Union[AssistantMessageBlock]], dict,str],ref_msg_id: str="",message_id:str="",agent_cmd_block:AgentInstructionBlock=None):
        # 处理对象转换为字典
