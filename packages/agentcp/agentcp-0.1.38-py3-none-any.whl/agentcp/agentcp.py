...
from agentcp.message import AssistantMessageBlock,AgentCmdBlock
...
class AgentID(abc.ABC): 
    ...
    def add_message_handler(
        self, handler: typing.Callable[[dict], typing.Awaitable[None]],router:str="",session_id:str=""
    ):
        """消息监听器装饰器"""
    ...
    def get_conversation_list(self,main_aid,page,page_size):
        return self.db_manager.get_conversation_list(self.id,main_aid,page,page_size)
    ...
    def insert_message(self,role,aid,session_id, parent_message_id, to_aids, message: Union[AssistantMessageBlock, list[AssistantMessageBlock], dict,str],message_id:str=""):
        # 处理对象转换为字典
    ...
    # 发送系统指令消息-添加好友
    def send_add_friend_cmd_message(self,session_id: str,to_aid,add_aid:str,description:str=""):
        agent_cmd_block:AgentCmdBlock = AgentCmdBlock(cmd="add_friend",params={"aid":add_aid,"description":description})
        self.send_cmd_message(session_id,to_aid,agent_cmd_block)
        
    # 发送自定义指令消息
    def send_cmd_message(self,session_id: str,to_aid: str,agent_cmd_block:AgentCmdBlock=None,ref_msg_id: str=""):
        # 处理对象转换为字典
        if session_id == "" or session_id is None:
            return
        self.send_message(session_id,[to_aid],None,agent_cmd_block=agent_cmd_block)
        
    def send_message(self, sessionId: str,to_aid_list: list,  message: Union[AssistantMessageBlock, AgentCmdBlock,list[Union[AssistantMessageBlock,AgentCmdBlock]], dict,str],ref_msg_id: str="",message_id:str="",agent_cmd_block:AgentCmdBlock=None):
        # 处理对象转换为字典
        ...
        if agent_cmd_block is not None:
            message_data.append(agent_cmd_block)
        ...
        self.session_manager.send_msg(sessionId ,message_data, ";".join(to_aid_list), ref_msg_id,message_id)
