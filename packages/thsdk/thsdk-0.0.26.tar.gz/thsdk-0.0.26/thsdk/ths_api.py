...
def _time2int(t: datetime) -> int:
    """将datetime对象转换为整数时间戳"""
    if not t.tzinfo:
        t = pytz.timezone("Asia/Shanghai").localize(t)
...
class HQ:
    ...
    def _initialize_library(self) -> None:
        ...
        try:
            ...
            self._lib.HQ(json.dumps(self.ops).encode('utf-8'))
        except OSError as e:
    ...
    def query_data(self, req: str, query_type: str = "zhu"):
        ...
        Args:
            req: 查询请求字符串
            query_type: 查询类型，默认为"zhu"，"zhu":主行情服务,
                                            "fu":副行情服务,
                                            "zx":资讯行情服务,
                                            "bk":板块行情服务,
...
class ResponseData:
    ...
    def __repr__(self) -> str:
        return (f"ResponseData(type={self.type}, data={self.data}, "
                f"dic_extra={self.dic_extra}, extra={self.extra})")
...
class THS:
    ...
    def login_required(func):
        ...
        def wrapper(self, *args, **kwargs):
            if not self._login:
                ...
                return Response('{"err_code": 401, "err_message": "Not logged in", "data": {}}')
    ...
    def connect(self) -> Response:
        ...
        if not response:
            raise ValueError("No data returned from connect.")
        rs = Response(response)
        if rs.is_success():
            self._login = True
            logger.info("Connected to THS server successfully")
        else:
            logger.error(f"Failed to connect: {rs.err_message}")
        return rs
    ...
    def disconnect(self) -> Response:
        ...
        rs = Response(response)
        logger.info("Disconnected from THS server")
        return rs
    ...
    def about(self) -> str:
        ...
        Returns:
            版本信息字符串
        """
        return self._hq.about().decode('utf-8')

    def query_data(self, req: str, query_type: str = "zhu") -> Response:
        """内部方法，统一处理数据查询
        ...
        if not response:
            raise ValueError(f"No data found for request: {req}")
    ...
    @login_required
    def history_minute_time_data(self, code: str, date: str, fields: Optional[List[str]] = None) -> Response:
        """获取分钟级历史数据

        Args:
            code: 证券代码，格式为10位，以'USHA'或'USZA'开头
            date: 日期，格式为YYYYMMDD
            fields: 可选，指定返回的字段列表
        ...
        """
        if len(code) != 10 or not (code.upper().startswith(('USHA', 'USZA'))):
    ...
    @login_required
    def security_bars(self, code: str, start: int, end: int, adjust: str, interval: int) -> Response:
        """获取K线数据
        ...
        """
        code = code.upper()
        if len(code) != 10 or not code.startswith(('USHA', 'USZA')):
            raise InvalidCodeError("Code must be 10 characters long and start with 'USHA' or 'USZA'")
    ...
    @login_required
    def get_transaction_data(self, code: str, start: int, end: int) -> Response:
        """获取3秒tick成交数据
    ...
    @login_required
    def get_super_transaction_data(self, code: str, start: int, end: int) -> Response:
        """获取3秒超级盘口数据（带委托档位）
    ...
    @login_required
    def get_l2_transaction_data(self, code: str, start: int, end: int) -> Response:
        """获取L2成交数据
    ...
    @login_required
    def download(self, code: str, start: Optional[Any] = None, end: Optional[Any] = None,
            ...
            if start is None or end is None:
                ...
                for var, name in [(start, "start"), (end, "end")]:
                    if isinstance(var, str):
                        var = var.replace("-", "")
                        if len(var) == 8:
                            dt = datetime.datetime.strptime(var, "%Y%m%d")
                            dt = dt.replace(hour=9 if name == "start" else 15,
                                            minute=15 if name == "start" else 30)
                            locals()[name] = _time2int(bj_tz.localize(dt))
                    elif isinstance(var, int) and len(str(var)) == 8:
                        dt = datetime.datetime.strptime(str(var), "%Y%m%d")
                        dt = dt.replace(hour=9 if name == "start" else 15,
                                        minute=15 if name == "start" else 30)
                        locals()[name] = _time2int(bj_tz.localize(dt))
                    elif isinstance(var, datetime.datetime):
                        locals()[name] = _time2int(var)
                if end <= start:
                    raise ValueError("End must be greater than start")
    ...
    def wencai_base(self, condition: str) -> pd.DataFrame:
        ...
        """
        reply = self.query_data(condition, "wencai_base")
        return pd.DataFrame(reply.resp.data)
    ...
    def wencai_nlp(self, condition: str, domain: Optional[str] = "") -> pd.DataFrame:
        ...
        reply = self.query_data(condition, query_type)
        return pd.DataFrame(reply.resp.data)
