...
class TestFindGitRepositories:
    ...
    def test_find_git_repositories_with_gitr_file(self, mock_walk, mock_path):
        """Test find_git_repositories uses .gitr file when present."""
        # Setup mock Path
        ...
        mock_gitr_file = MagicMock()
        mock_gitr_file.exists.return_value = True
        mock_base_path.__truediv__.return_value = mock_gitr_file

        mock_path.return_value = mock_base_path

        # Mock reading .gitr file
        mock_file = MagicMock()
        mock_file.__enter__.return_value = ["repo1\n", "repo2\n"]
        mock_gitr_file.open.return_value = mock_file

        # Mock repo paths from .gitr
        ...
        mock_base_path.__truediv__.side_effect = [mock_gitr_file, mock_repo1, mock_repo2]
