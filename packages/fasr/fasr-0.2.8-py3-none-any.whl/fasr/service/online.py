...
class RealtimeASRService(BaseModel):
    ...
    def setup(self):
        ...
        logger.info(
            f"ASR Model: {self.asr_model_name}, VAD Model: {self.vad_model_name}"
        )
        logger.info(
            f"VAD Config: chunk_size_ms: {self.vad_chunk_size_ms}, end_silence_ms: {self.vad_end_silence_ms}, threshold: {self.vad_threshold}"
        )
    ...
    async def vad_task(self, ws: WebSocket, span_queue: Queue):
        ...
        while True:
            ...
            if bytes_data is None:
                logger.warning("Received data is None, skipping")
                continue
