...
class Prepare:
    ...
    def __init__(self, cache_dir: str | Path = FASR_DEFAULT_CACHE_DIR):
    ...
    def offline(self):
        ...
        for model in models:
            snapshot_download(model_id=model, cache_dir=self.cache_dir)
...
class Benchmark:
        ...
        if interactive:
            ...
            while True:
                ...
                for chunk in chunks:
                    ...
                    if print_infer_time:
                        start = time.perf_counter()
                    for span in vad.detect_chunk(
                        state=state, is_last=False, waveform=chunk.waveform
                    ):
                        if not print_infer_time:
    ...
    def stream_asr(
        ...
        if compile:
            ...
            for chunk in chunks:
                chunk: AudioChunk
                for token in asr.transcribe_chunk(
                    waveform=chunk.waveform, is_last=chunk.is_last, state=state
                ):
                    pass
        if interactive:
            import pyaudio
    ...
    def pipeline(
        ...
        import time
        from loguru import logger
        from fasr import AudioPipeline, AudioList
