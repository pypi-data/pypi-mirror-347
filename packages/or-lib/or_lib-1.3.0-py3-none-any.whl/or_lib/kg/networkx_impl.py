...
class NetworkXStorage(BaseGraphStorage):
    ...
    async def get_nodes_bulk(self, entity_ids: list[str]) -> dict[str, dict]:
        """
        Retrieve multiple nodes from the in-memory graph.
    
        Args:
            entity_ids: List of node IDs to retrieve.
    
        Returns:
            A dictionary where keys are node IDs and values are their attributes.
        """
        graph = await self._get_graph()
        nodes_bulk = {}
        for node_id in entity_ids:
            if graph.has_node(node_id):
                nodes_bulk[node_id] = dict(graph.nodes[node_id])
            else:
                nodes_bulk[node_id] = None  # Node not found
        return nodes_bulk
            ...
            if graph.has_edge(source, target):
                ...
                for key, default_value in required_keys.items():
                    if key not in edge_data:
                        ...
                        logger.warning(
                            f"Edge ({source}, {target}) missing '{key}', defaulting to {default_value}"
                        )
                edge_map[(source, target)] = edge_data
            else:
                edge_map[(source, target)] = None  # Edge not found
    ...
    async def upsert_nodes_batch(self, nodes: list[dict[str, str]]) -> None:
        """
        Batch upsert nodes into the graph.
    ...
    async def upsert_edges_batch(self, edges: list[dict]) -> None:
        ...
        Each edge must include 'src_id', 'tgt_id', and 'edge_data'.
        """
        graph = await self._get_graph()

        for edge in edges:
            if "src_id" not in edge or "tgt_id" not in edge or "edge_data" not in edge:
                raise ValueError("Each edge must include 'src_id', 'tgt_id', and 'edge_data'")

            graph.add_edge(edge["src_id"], edge["tgt_id"], **edge["edge_data"])