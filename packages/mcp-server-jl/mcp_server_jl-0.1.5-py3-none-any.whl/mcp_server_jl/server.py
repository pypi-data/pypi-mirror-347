...
import sys
import anyio
# Use standard TimeoutError from concurrent.futures
...
async def convert_to_markdown(uri: str) -> str:
    ...
    try:
        # Kiểm tra và xác thực URI file
        if uri.startswith('file:'):
            parsed_uri = urlparse(uri)
            path = unquote(parsed_uri.path)
            
            # Trên Windows, xóa dấu / đầu tiên từ đường dẫn
            if os.name == 'nt' and path.startswith('/'):
                path = path[1:]
            
            # Kiểm tra file có tồn tại không
            if not os.path.exists(path):
                return f"Lỗi: File không tồn tại tại đường dẫn {path}"
            
            print(f"Đang xử lý file: {path}")
        
        # Thực hiện chuyển đổi với timeout
        try:
            async with anyio.fail_after(30):  # Timeout sau 60 giây
                # Chạy trong một luồng riêng biệt để không chặn luồng chính
                return await anyio.to_thread.run_sync(
                    lambda: MarkItDown().convert_uri(uri).markdown
                )
        except TimeoutError:
            return "Lỗi: Quá thời gian xử lý file (timeout sau 30 giây)"
