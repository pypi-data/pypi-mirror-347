...
class _MaterPreProcess:
    ...
    def infinite_geometric_series(self, A: np.ndarray):
        ...
        This version uses mpmath for high-precision arithmetic and avoids
        explicit Python for loops by using list conversions.
        """
        # Set desired precision (50 decimal places in this example)
        mp.dps = 50
        n = A.shape[0]

        # Convert A to an mpmath matrix using A.tolist() and the conversion function mp.mpf
        A_mp = matrix(A.tolist(), mp.mpf)

        # Create the identity matrix in NumPy and convert it similarly
        I_mp = matrix(np.eye(n).tolist(), mp.mpf)

        # Compute the inverse of (I - A) using matrix power -1
        inv_matrix_mp = (I_mp - A_mp) ** (-1)

        # Convert the result back to a NumPy array without explicit loops
        inv_matrix_np = np.array(inv_matrix_mp.tolist(), dtype=float)
        return inv_matrix_np
    ...
    def _assembly_computation(self):
        ...
        df2 = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [list(df.index.get_level_values("location").unique()), labels, labels], names=list(df.index.names)
            ),
            columns=df.columns,
        )
