...
parser.add_argument("--output-folder", type=str, required=True, help="Name of the folder where the outputs are stored.")
...
def plot_df(df: pd.DataFrame, data_name: str, plot: bool, all_data: bool, plot_type: str):
    if plot & all_data:
        ...
        if plot_type == "pie":
            ...
            date_time = get_closest_date(pd.to_datetime(time), df.columns)
            # plot
            fig, ax = plt.subplots()
            # Filter
            ...
            # Transform back to date
            date_time_steps = [
                get_closest_date(pd.to_datetime(time_steps[0]), df.columns),
                get_closest_date(pd.to_datetime(time_steps[1]), df.columns),
            ]
            # Filter
