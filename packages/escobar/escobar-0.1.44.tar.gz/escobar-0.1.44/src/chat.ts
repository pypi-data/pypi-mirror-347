...
import { initPythonBridge, callPython, registerFunction, get_ws, voittal_call_log, PYTHON_CALL_EVENTS } from './voitta/pythonBridge_browser'
 ...
 * 
 * @returns The username if found, or "VoittaDefaultUser" if not in a JupyterHub environment
 */
function getJupyterHubUsername(): string {
  ...
  if (hubUserMatch && hubUserMatch[1]) {
    console.log('Found JupyterHub username from URL:', hubUserMatch[1]);
    return hubUserMatch[1];
  }
  ...
  try {
    ...
    if (baseMatch && baseMatch[1]) {
      console.log('Found JupyterHub username from baseURI:', baseMatch[1]);
      return baseMatch[1];
    }
  } catch (error) {
...
export class ChatWidget extends Widget {
  ...
  async init() {
    ...
    const username = getJupyterHubUsername();

    // Update the username in settings
    this.settings.username = username;
    console.log(`Using username: ${this.settings.username}`);
