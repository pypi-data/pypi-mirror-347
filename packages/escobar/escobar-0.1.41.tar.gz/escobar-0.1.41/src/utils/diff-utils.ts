 ...
 * Synchronized with the reference implementation in manual_diff_development
 */
...
/**
 * Normalizes content to handle whitespace consistently
...
export function applyDiffToContent(
    ...
    // Normalize the original content for consistent whitespace handling
    let normalizedOriginalContent = originalContent;
    if (opts.normalize) {
        normalizedOriginalContent = normalizeContent(originalContent);
        if (!silent) {
            console.log('Applied normalizeContent to original content');
        }
    }
    ...
    if (opts.normalize) {
        processedDiff = normalizeContent(processedDiff);
        if (!silent) {
            console.log('Applied normalizeContent to diff');
        }
    }
    ...
    if (!processedDiff.includes('@@ -') || processedDiff.includes('@@ ... @@')) {
        ...
        return applySimpleDiff(normalizedOriginalContent, processedDiff, silent, opts.normalize);
    }
    ...
    for (let i = chunks.length - 1; i >= 0; i--) {
        ...
        for (let j = changeGroups.length - 1; j >= 0; j--) {
            ...
            if (group.type === 'del') {
                // Remove all lines at once (matches reference implementation)
                modifiedLines.splice(lineIndex + position, group.changes.length);
                if (!silent) {
                    console.log(`  Removed ${group.changes.length} lines at position ${lineIndex + position}, modifiedLines.length=${modifiedLines.length}`);
                }
            } else if (group.type === 'add') {
                // Add lines
    ...
    if (opts.normalize) {
        ...
        if (!silent) {
            console.log('Applied normalizeContent to result');
        }
    }
 ...
 * @returns The content after applying the diff
 */
export function applySimpleDiff(originalContent: string, diff: string, silent: boolean = true, normalize: boolean = true): string {
    // Split the original content and diff into lines
    ...
    if (removedLines.length > 0 && addedLines.length > 0) {
        ...
        if (allLinesRemoved || removedLines.length >= originalLines.length * 0.8) {
            ...
            // Add lines at a more appropriate position
            // Try to find a good insertion point based on context
            if (addedLines.length > 0) {
                // First, try to find a position based on surrounding context
                let insertPosition = 0;

                // Look for any context in the diff that might help position the new lines
                const contextLines = diffLines.filter(line => line.startsWith(' '));
                if (contextLines.length > 0) {
                    // Use the first context line as a reference point
                    const contextLine = contextLines[0].substring(1);
                    const contextIndex = resultLines.findIndex(line => line.trim() === contextLine.trim());
                    if (contextIndex !== -1) {
                        // Insert after the context line
                        insertPosition = contextIndex + 1;
                        if (!silent) {
                            console.log(`Found context line "${contextLine.trim()}" at position ${contextIndex}`);
                        }
                    }
                }

                // Insert the added lines at the determined position
                resultLines.splice(insertPosition, 0, ...addedLines);
                if (!silent) {
                    console.log(`Added ${addedLines.length} lines at position ${insertPosition}`);
                }
            }
        }
