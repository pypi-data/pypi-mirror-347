...
def main():
    ...
    if not os.path.exists(ros2_env_file):
        ament_prefix_path = os.environ.get("AMENT_PREFIX_PATH", "")
        python_path = os.environ.get("PYTHONPATH", "")
        if not ament_prefix_path or not python_path:
            print(
                "===> Your ROS2 workspaces (in AMENT_PREFIX_PATH) or PYTHONPATH have not been found. Please ensure you manually source them before running `vyomcloudbridge setup`. <==="
            )
            sys.exit(0)
    else:
        load_env_from_file(ros2_env_file)


    ament_prefix_path = os.environ.get("AMENT_PREFIX_PATH", "")
    python_path = os.environ.get("PYTHONPATH", "")
    print("In main reading, ament_prefix_path-",ament_prefix_path)
    print("In main reading, python_path-",python_path)
