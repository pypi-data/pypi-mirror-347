...
class ServiceManager:
    ...
    def _service_runner(self, service_instance, alive_wait_time):
        try:
        ...
        **kwargs,
        ...
        Returns:
            Tuple[bool, Optional[str], Optional[str]]: 
                Success status, instance ID, and instance name
        """
        try:
            ...
            while instance_name in existing_names:
                logger.warning(
                    f"Instance name {instance_name} already exists, generating a new one"
                )
            ...
            # Configure the process to be daemon so it doesn't block program exit
            proc = multiprocessing.Process(
                target=self._service_runner,
                args=(service_instance, self.alive_wait_time),
                kwargs=kwargs,
                daemon=True  # Make it a daemon process so it doesn't block program exit
            )
            
            # Start the process
            proc.start()
            
            # Track the service in our registry
            ...
            # Save the state of running services
            self.save_running_services()
            
            logger.info(
                f"Started service {service_name} ({instance_name}) with PID {proc.pid}"
            )
    ...
    def _stop_by_instance_id(self, instance_id: str) -> bool:
        try:
            info = self.services.get(instance_id)
            if not info:
                logger.warning(f"No such instance: {instance_id}")
                return False
            ...
            if proc and proc.is_alive():
                ...
                info["status"] = "stopped"
                self.save_running_services()
                logger.info(f"Stopped service instance {instance_id} (PID {proc.pid})")
                return True
            else:
                logger.warning(
                    f"Process already terminated or not found for {instance_id}"
                )
                return False
    ...
    def check_library_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        status["home_directory"] = {
            "status": os.path.exists(vyom_home_dir),
            "message": f"Home directory at {vyom_home_dir}",
        }
        ...
        status["service_file"] = {
            "status": os.path.exists(service_file),
            "message": f"Service file at {service_file}",
        }
    ...
    def display_cleanup_results(self, results: Dict[str, bool]) -> None:
        ...
        sections = {
            ...
            "System Service": [service_file, "service_stopped"],
        }
