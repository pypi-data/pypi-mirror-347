    ...
    def create_listener_function(self, msg_type, f_topic):
        ...
        def f(msg):
            ...
            if msg_type == "sensor_msgs.msg.Image":
                ...
                # print(
                #     f"Image data type: {type(current_value)}, filename: {filename}, data_type: {data_type}"
                # )
            else:
                ...
                if hasattr(msg, "header") and hasattr(msg.header, "stamp"):
                    ...
                    if timestamp_sec == 0 and timestamp_nsec == 0:
                        ...
                        timestamp_nsec = current_time_ns % 1_000_000_000
                        self.logger.info(
                            f"Timestamp is zero, using current time {timestamp_sec} seconds, {timestamp_nsec} nanoseconds"
                        )
                    # self.logger.info(
                    #     f"Timestamp1: {timestamp_sec} seconds, {timestamp_nsec} nanoseconds"
                    # )
                else:
                    ...
                    timestamp_nsec = current_time_ns % 1_000_000_000
                    # self.logger.info(
                    #     f"Timestamp not available, using placeholder {timestamp_sec}"
                    # )
            ...
            # print(f"data_type {data_type}, msg_type {msg_type}, f_topic {f_topic}")
...
class RosPublisher(ServiceAbstract):
    ...
    def __del__(self):
        ...
        try:
            self.logger.info(
                "Destructor called by garbage collector to ensure resources cleanup"
            )
            self.stop()
        except Exception as e:
            pass
