...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../vyomcloudbridge/utils")))
...
def generate_random_data(msg_type):
    ...
    if msg_type == "Access":
        data["encrypted"] = ''.join(random.choices(string.ascii_letters + string.digits, k=16))

    elif msg_type == "Accessinfo":
        data["end_time"] = random.randint(1600000000, 1700000000)  # Random timestamp
        data["current_date"] = random.randint(1600000000, 1700000000)  # Random timestamp
        data["user_id"] = random.randint(1000, 9999)

    elif msg_type == "Ack":
        data["msgid"] = ''.join(random.choices(string.ascii_letters + string.digits, k=8))
        data["chunk_id"] = random.randint(1, 100)

    elif msg_type == "Auth":
        data["auth_key"] = ''.join(random.choices(string.ascii_letters + string.digits, k=32))
...
def get_input_data():
    ...
    use_random = input("Do you want to use random values for testing? [Y/n]: ").strip().lower() == 'y'

    # Loop through each message type and prompt the user for data or generate random data
    for msg_type in message_types:
        print(f"\nEnter data for message type: {msg_type}" if not use_random else f"\nGenerating random data for message type: {msg_type}")
        ...
        if use_random:
            ...
            if msg_type == "Access":
                ...
                try:
                    msg_data["end_time"] = int(input("End time (timestamp in seconds): "))
                    msg_data["current_date"] = int(input("Current date (timestamp in seconds): "))
                    msg_data["user_id"] = int(input("User ID: "))
                except ValueError:
                    print("Invalid input for timestamps or user ID. Please enter integers.")
