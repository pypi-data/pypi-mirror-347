...
def setup_ros2_workspaces():
    ...
    ) and not os.path.exists(ros2_env_file):
        missing_vars = []
        if not python_path:
    ...
    try:
        if (
            ...
            with open(ros2_env_file, "w") as f:
                f.write("#!/bin/bash\n")
                if python_path:
            ...
            subprocess.run(["chmod", "+x", ros2_env_file], check=True)

            print(f"✅ ROS2 environment variables saved to: {ros2_env_file}")
        else:  # the file saved already before
            print(f"✅ Using Exiting ROS2 env variables saved at: {ros2_env_file}")
        return True
...
def setup_service():
    ...
    for attempt in range(MAX_PATH_ATTEMPTS):
        bashrc_path = input(
            "Enter the complete path of your bashrc file for environment variables: "
        ).strip()
        # Ensure it starts with '/'   eg: /home/shitiz/.bashrc
        if not bashrc_path.startswith("/"):
            bashrc_path = "/" + bashrc_path
        # Check if file exists
        if os.path.isfile(bashrc_path):
            print(f"Valid .bashrc path provided: {bashrc_path}")
            break
        else:
            print(f"Invalid path: {bashrc_path}. Please try again.")
    else:
...
ExecStart=/bin/bash -c 'source {bashrc_path} && {start_script_file}'
Restart=on-failure
RestartSec=10
...
"""
            ...
            print("sudo systemctl stop vyomcloudbridge.service")
...
def setup(interactive=True):
    ...
    # Final status
    if success:
        subprocess.run(["systemctl", "start", "vyomcloudbridge.service"], check=True)
        print("vyomcloudbridge background service started.")
        print(
