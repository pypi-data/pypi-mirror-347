...
class _GXSynchronousMediaBase:
    ...
    def appendData(self, data, index, count):
        ...
        with self.__sync:
            ...
            if self.__receivedSize + count > len(self.__receivedBuffer):
                ...
                self.__receivedBuffer[0 : self.__receivedSize] = tmp[
                    0 : self.__receivedSize
                ]

            self.__receivedBuffer[
                self.__receivedSize : self.__receivedSize + count - index
    ...
    def __findData(self, args):
        ...
        while foundPosition == -1:
            ...
            if waitTime != -1:
                waitTime = args.waitTime - (
                    (datetime.datetime.now().time().microsecond - startTime) / 1000
                )
            ...
            with self.__sync:
                isReceived = not (
                    ...
                    or self.__receivedSize < nMinSize
                )
            ...
            with self.__sync:
                ...
                if nSize == 0:
                    ...
                    if (
                        self.__lastPosition != 0
                        and self.__lastPosition < self.__receivedSize
                    ):
                    ...
                    if isinstance(args.eop, (list)):
                        raise ValueError("Only single byte terminator allowed.")
                    if len(terminator) != 1 and self.__receivedSize - index < len(
                        terminator
                    ):
                        index = self.__receivedSize - len(terminator)
                    foundPosition = self.indexOf(
                        self.__receivedBuffer, terminator, index, self.__receivedSize
                    )
    ...
    def receive(self, args):
        ...
        if args.eop is None and args.count == 0:
            raise ValueError("Either Count or Eop must be set.")
        ...
        if foundPosition != -1:
            with self.__sync:
                ...
                if foundPosition != 0:
                    ...
                    self.__receivedSize = max(self.__receivedSize, 0)
                    if self.__receivedSize != 0:
                        self.__receivedBuffer[
                            0 : self.__receivedSize
                        ] = self.__receivedBuffer[
                            foundPosition : foundPosition + self.__receivedSize
                        ]
            ...
            if args.reply is None:
                ...
                if newArray:
                    ...
                    # Copy new values.
                    arr[len(oldArray) : len(oldArray) + len(newArray)] = newArray[
                        0 : len(newArray)
                    ]
                    args.reply = arr
