...
parser = argparse.ArgumentParser(prog='gemiTask', description='AI-powered CLI Task Manager using Google\'s Gemini')
subparsers = parser.add_subparsers(dest='command')

# Add task
add_parser = subparsers.add_parser('add', help='Add a new task')
add_parser.add_argument('description', type=str, help='Task description')
add_parser.add_argument('-p', '--priority', type=int, choices=range(1, 6), help='Task priority (1-5, where 1 is highest)')
add_parser.add_argument('-d', '--deadline', type=str, help='Task deadline (YYYY-MM-DD)')
...
# Set API key
api_parser = subparsers.add_parser('set-api-key', help='Set your Gemini API key')
api_parser.add_argument('--key', type=str, help='Your Gemini API key (optional, will prompt if not provided)')
...
def main():
    ...
    if args.command == 'set-api-key':
        api_key = args.key if args.key else getpass("Enter your Gemini API key: ").strip()
        ...
        if args.priority:
            task_details['priority'] = args.priority
        if args.deadline:
            task_details['deadline'] = args.deadline
