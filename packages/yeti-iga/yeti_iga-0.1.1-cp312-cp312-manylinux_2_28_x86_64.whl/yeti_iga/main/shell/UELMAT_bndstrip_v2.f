...
!     For material matrix
      Double precision :: E, nu, h, matH, coef, e_AE, Pmtx
      dimension matH(3,3), e_AE(2,2), Pmtx(3,3)
...
c     --------------
      ...
      NDOFEL  = NNODE*MCRD      ! nb dof per element
...
c     Material behaviour
      h = PROPS(2)
      E = MATERIAL_PROPERTIES(1) * 10.d0**6.d0
      nu= zero
      coef = E/(one-nu**two)
      matH = zero
c
...
c     Computing material matrix
         ...
         Pmtx(1,3) = e_AE(1,1)*e_AE(1,2)*0.5D0
         Pmtx(2,1) = e_AE(2,1)*e_AE(2,1)
         Pmtx(2,2) = e_AE(2,2)*e_AE(2,2)
         Pmtx(2,3) = e_AE(2,1)*e_AE(2,2)*0.5D0
         Pmtx(3,1) = e_AE(1,1)*e_AE(2,1)*two
         Pmtx(3,2) = e_AE(1,2)*e_AE(2,2)*two
         Pmtx(3,3) = e_AE(1,1)*e_AE(2,2)
         
         matH(:,:) = zero
         matH(1,1) = coef*h**3.0D0/12.0D0
         call MulATBA(matH, Pmtx, matH, Pmtx, 3, 3)
         
         
c     Computing stiffness matrix
         bendingStiff = zero
         call usfbnd_shell(NNODE,MCRD,NDOFEL,matH,AI,dAI1dxi,
...
!            do k1 = 1,k2
!               i = (k1-1)*MCRD + 3
!               amatrx(i,j) = amatrx(i,j) + bendingStiff(i,j)*dvol
!            enddo
!         enddo
         
         do k2 = 1,NDOFEL
            do k1 = 1,k2 !MCRD*NNODE
...
c     Symmetry
      ...
      IF (JELEM == 1) then
         call Text(AMATRX, NDOFEL,NDOFEL)
      Endif
