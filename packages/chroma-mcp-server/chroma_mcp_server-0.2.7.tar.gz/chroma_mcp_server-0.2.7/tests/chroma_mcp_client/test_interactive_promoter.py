...
def test_run_interactive_promotion_promote_action_success(
    ...
    # The calls are: action, description, pattern, code_ref_selection, tags, confidence, include_context
    assert mock_input.call_count == 7
    input_calls = [
        call("\nAction (p=promote, i=ignore, s=skip, v=view context, q=quit): "),
        call(f"Description (default: use auto-generated): "),
        ...
        call(f"Tags (comma-separated, default: ): "),
        call(f"Confidence (0.0 to 1.0, default: 0.8): "),
        call("Include rich context from source chat? (Y/n): "),
    ]
...
def test_run_interactive_promotion_promote_action_failure(
    ...
    # User inputs: p -> description -> pattern -> code_ref ('n') -> tags -> confidence -> include_context
    mock_input.side_effect = ["p", "Test Desc", "Test Pattern", "n", "Test Tags", "0.5", "y"]
    ...
    # Check input calls
    assert mock_input.call_count == 7  # p, desc, pattern, code_ref, tags, confidence, include_context
    input_calls = [
        call("\nAction (p=promote, i=ignore, s=skip, v=view context, q=quit): "),
        call(f"Description (default: use auto-generated): "),
        call("Pattern (e.g., code snippet, regex, textual key insight): "),
        call(f"Code Reference (select 1-0, type manually, or 'n' for N/A): "),  # No results, length is 0
        call(f"Tags (comma-separated, default: ): "),
        call(f"Confidence (0.0 to 1.0, default: 0.8): "),
        call("Include rich context from source chat? (Y/n): "),
    ]
