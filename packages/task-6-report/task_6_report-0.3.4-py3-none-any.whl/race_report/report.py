...
class RaceData:
    ...
    def __init__(self, abbr_path: Path, start_path: Path, end_path: Path, folder: Path) -> None:
        """
        ...
        :param folder: Folder where the log files are located.
        """
        self.abbr_path = abbr_path
        self.start_path = start_path
        self.end_path = end_path
        self.folder = folder
        self.records: list[Record] = []
        self.valid_records: list[Record] = []  # Список валідних записів
        self.invalid_records: list[Record] = []  # Список невалідних записів
    ...
    def read_abbreviations(self) -> tuple[dict[str, Record], list[Record]]:
        ...
        print(f"\n==> Всього: {len(valid_records)} валідних, {len(invalid_records)} невалідних записів")
    ...
    def build_report(self, order: str) -> dict[str, list[Record]]:
        logger.debug("== Створення звіту ==")

        # Читання даних з логів
        start_times, start_invalid = self.read_times("start.log", is_start=True)
        end_times, end_invalid = self.read_times("end.log", is_start=False)

        logger.debug(f"Зчитано дані з start.log: {len(start_times)} записів")
        logger.debug(f"Зчитано дані з end.log: {len(end_times)} записів")

        # Об'єднання невалідних записів
        invalid_records = start_invalid + end_invalid
        logger.debug(f"Кількість невалідних записів: {len(invalid_records)}")

        # Фільтрація валідних записів
        valid_records = [r for r in self.records if not r.errors]
        logger.debug(f"Кількість валідних записів: {len(valid_records)}")

        # Сортування валідних записів
        valid_records.sort(key=lambda r: r.start_time, reverse=(order != "asc"))
        logger.debug(f"Записи відсортовано за {order} порядком")
