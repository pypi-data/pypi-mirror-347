...
class RaceData:
    ...
    def build_report(self, order: str = "asc") -> dict:
        ...
        logger.debug(f"Починаємо побудову звіту. Порядок: {order}")

        # Беремо тільки валідні записи
        valid_records = [r for r in self.records if not r.errors]

        # Сортуємо за часом
        reverse = (order == "desc")
        valid_records.sort(key=lambda r: r.lap_time if r.lap_time is not None else datetime.timedelta(days=365*100), reverse=reverse)

        # Невалідні беремо з self.records
        invalid_records = [r for r in self.records if r.errors]

        # Зберігаємо результат у self для подальшого використання
        self.valid_records = valid_records
        self.invalid_records = invalid_records

        logger.debug("Звіт сформовано")
