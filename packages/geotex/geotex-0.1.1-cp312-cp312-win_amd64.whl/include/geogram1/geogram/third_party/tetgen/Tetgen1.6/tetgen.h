...
#include <set> // [Bruno]
#include <geogram/basic/common.h> // [Bruno]
...
//   an array if it is not a nullptr.  Note that it assumes that the memory is   //
//   allocated by the C++ "new" operator. Otherwise, the user is responsible  //
//   to free them and all pointers must be nullptr.                              //
//                                                                            //
//============================================================================//
...
public:
  ...
  void initialize()
  {
    ...
    refine_elem_list = (int *) nullptr;
    refine_elem_vol_list = (REAL *) nullptr;
  ...
  void deinitialize() { clean_memory(); } // [Bruno] Backward compatibility
  ...
  void clean_memory()
  {
    ...
    if (refine_elem_list != (int *) nullptr) {
      delete [] refine_elem_list;
      if (refine_elem_vol_list != (REAL *) nullptr) {
...
//============================================================================//
  ...
  public:
    ...
    void init() {
      ...
      tt.tet = nullptr; tt.ver = 0;
      ss.sh = nullptr; ss.shver = 0;
      forg = fdest = fapex = foppo = noppo = nullptr;
      nextitem = nullptr;
    }
  };
...
//============================================================================//
  ...
  enum locateresult {UNKNOWN, OUTSIDE, INTETRAHEDRON, ONFACE, ONEDGE, ONVERTEX,
                     ...
                     INSTAR, BADELEMENT, nullptrCAVITY, SHARPCORNER, FENSEDIN,
...
//============================================================================//
  ...
  void initializetetgenmesh()
  {
    ...
    split_segments_pool = split_subfaces_pool = nullptr;
    unsplit_badtets = unsplit_subfaces = unsplit_segments = nullptr;
    check_tets_list = nullptr;
    badqual_tets_pool = nullptr;

    stack_enc_segments = stack_enc_subfaces = nullptr;
  
    flippool = nullptr;
    flipstack = unflip_queue_front = unflip_queue_tail = nullptr;
    later_unflip_queue = unflipqueue = nullptr;

    cavetetlist = cavebdrylist = caveoldtetlist = nullptr;
    cave_oldtet_list = nullptr;
    ...
    skipped_segment_list = skipped_facet_list = nullptr;
    ...
    idx_segment_facet_list = nullptr;
    segment_facet_list = nullptr;
    idx_ridge_vertex_facet_list = nullptr;
    ridge_vertex_facet_list = nullptr;

    segmentendpointslist_length = 0;
    segmentendpointslist = nullptr;
    segment_info_list = nullptr;
    idx_segment_ridge_vertex_list = nullptr;
    segment_ridge_vertex_list = nullptr;

    subdomains = 0;
    subdomain_markers = nullptr;
  ...
  void freememory()
  {
    ...
    if (segment_info_list != nullptr) {
      delete [] segment_info_list;
    }

    if (subdomain_markers != nullptr) {
...
// for returing the generated tetrahedral mesh. If it is a nullptr pointer, the  //
// output mesh is saved to file(s). If 'bgmin' != nullptr, it contains a back-   //
...
inline bool tetgenmesh::isdeadsh(face& s) {
  return ((s.sh == nullptr) || (s.sh[3] == nullptr));
}
