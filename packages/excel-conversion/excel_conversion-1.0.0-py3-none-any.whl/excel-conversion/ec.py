...
def excel2json(xlsx_book,sheet_name,id_key,id_value,data_tag='{}',id_num_f=None):
    """
    :function 通过唯一关键字的值将excel造数串联成json或xml
    :param xlsx_book: xlsx/xls对象,工作薄
    :param sheet_name: sheet名称,待转json的起始页面名称
    :param id_key: Json唯一标识字段名称
    :param id_value: Json唯一标识,串联具体json的关键字值
    :param data_tag: 数据标识，对象{}，数组[]
    :return root_array/root_obj: 返回本层的json数据
    """
            ...
            for c in range(id_key_col + 1, cols):
                ...
                # 判断是否为数组[]或对象{}，若是，则下钻
                    ...
                    if i_value in sheet_names: 
                        ...
                        # 开始递归下钻
                        i_value=excel2json(xlsx_book,i_value,id_key,id_value,i_v[0]+i_v[-1],id_num)
                    else:
                        i_value=i_v
                # 正常取值的情况
                else:
...
def main():
    ...
    logger = logging.getLogger('excel2json')
    # 获取文件
    tmp_file=sys.argv[1]
    # 获取绝对路径
    ...
    for case in cases_set:
        ...
        json_data[case] = excel2json(xlsx_book,cases_set[case],id_key,case)
