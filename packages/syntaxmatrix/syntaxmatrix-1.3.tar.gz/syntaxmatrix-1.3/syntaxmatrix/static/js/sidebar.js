...
function openRenameModal(sessionId, currentTitle) {
    ...
    modal.innerHTML = `
      <div style="background: #fff; padding: 20px; border-radius: 8px; min-width: 300px;">
        ...
        <input type='text' id='new-session-name' value='` + currentTitle + `' style='width: 100%; padding: 5px;'/>
        <div style="margin-top: 10px; text-align: right;">
          <button onclick="submitRenameModal('` + sessionId + `')">Rename</button>
          <button onclick="document.getElementById('rename-modal').remove()">Cancel</button>
        </div>
      </div>
    `;
...
function openDeleteModal(sessionId) {
    ...
    modal.innerHTML = `
      <div style="background: #fff; padding: 20px; border-radius: 8px; min-width: 300px;">
        ...
        <div style="margin-top: 10px; text-align: right;">
          <button onclick="submitDeleteModal('` + sessionId + `')">Yes</button>
          <button onclick="document.getElementById('delete-modal').remove()">No</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
