...
class LiberalAlphaClient:
    ...
    def subscribe_data(self, record_id=None, max_reconnect=5, on_message: callable = None):
        ...
        if not self.private_key:
            logger.warning("No private key provided. Encrypted messages will not be decrypted.")
    ...
    def my_historical_entries(self, record_id: int, page: int = 1, page_size: int = 10):
        ...
        encrypted_entries_exist = any(entry.get('is_encrypted') for entry in entries)

        # Warn the user if entries are encrypted but no private key is provided
        if encrypted_entries_exist and not self.private_key:
            logger.warning(
                "Some entries are encrypted, but no private key was provided. "
                "Decryption is not possible. Please initialize the client with a private_key."
            )
