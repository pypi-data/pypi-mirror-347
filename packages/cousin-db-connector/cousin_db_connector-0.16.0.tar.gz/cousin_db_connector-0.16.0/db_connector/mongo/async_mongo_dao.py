...
class AsyncMongoDAO(BaseDAO):
    ...
    async def update(self, item_id: str, update_data):
    ...
    async def insert_many(self, data_list):
        """Insert multiple documents in MongoDB with tenant_id filter if necessary."""
        try:
            data_list = [self._apply_tenant_filter(self._to_mongo(data)) for data in data_list]
            result = await self._db[self._collection_name].insert_many(data_list)
