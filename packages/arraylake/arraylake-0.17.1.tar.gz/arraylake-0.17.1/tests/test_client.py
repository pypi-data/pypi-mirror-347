...
from arraylake.types import DBID, AuthProviderConfig, BucketResponse, RepoKind, UserInfo
...
async def test_modify_repo(token, isolated_org_with_bucket, repo_kind_to_test: RepoKind) -> None:
    ...
    await aclient.modify_repo(
        repo_name, description=new_description, add_metadata=add_metadata, remove_metadata=remove_metadata, update_metadata=update_metadata
    )
