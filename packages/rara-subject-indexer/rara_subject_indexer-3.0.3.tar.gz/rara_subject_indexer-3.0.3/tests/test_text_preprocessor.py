...
def test_lemmatizing():
    ...
    expected_output = "kui Arno isa koolimaja jõudma , olema tund juba alanud ."
    true_output_et = text_processor.lemmatize(text, lang_code="et")
    assert expected_output == true_output_et
    
    # If lang code != "et", the original text should be returned
    # as only Estonian lemmatization is currently supported
    true_output_en = text_processor.lemmatize(text, lang_code="en")
    assert text == true_output_en
