...
def load_custom(
        ...
        for id, row in enumerate(reader):
            ...
            if query_id in queries:
                answer = queries[query_id].get("answers", [])
                answer.append(
                    {
                        "corpus_id": corpus_id,
                        "score": score,
                ...
                new_query["answers"] = answer
                queries[query_id] = new_query

        # remove queries without answers
        queries = {k: v for k, v in queries.items() if "answers" in v and len(v["answers"]) > 0}
        ...
        if filter_corpus:
            filtered_ids = set([ a["corpus_id"] for q in queries_as_array for a in q["answers"]])
