...
def visualize_semantic_F1(corpus, answers, output_file='visualization.html', write_to_file=False):
    ...
    for a in answers:
        ...
        qrels[query_id] = [doc["corpus_id"] for doc in a["query"]["answers"]]
    ...
    for idx, query_id in enumerate(qrels):
        ...
        data.append({
            'query': query_id,
            'f1_score': f1,
            'semantic_score': 1-semantic_score,
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f"Query ID: {d['query']}<br>F1: {d['f1_score']:.2f}<br>Semantic Score: {d['semantic_score']:.2f}<br>{d['stats']}" for d in data],
        hoverinfo='text'
    ))

    fig.update_layout(
        title="Semantic Difficulty of queries, number of documents retrieved as color",
        xaxis_title="F1 Score (Right is better)",
        yaxis_title="Semantic Difficulty Score (Difficult queries are on the top)",
        xaxis=dict(range=[0, 1]),
        yaxis=dict(range=[0, 1]),
        shapes=[
            dict(type='line', x0=0.5, x1=0.5, y0=0, y1=1, line=dict(dash='dot', color='grey')),
            dict(type='line', x0=0, x1=1, y0=0.5, y1=0.5, line=dict(dash='dot', color='grey'))
        ]
    )
