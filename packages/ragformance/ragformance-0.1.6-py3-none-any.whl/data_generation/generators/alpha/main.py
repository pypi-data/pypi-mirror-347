...
from ragformance.data_generation.generators.alpha.prompts import QUESTION_GENERATION_SYSTEM_PROMPT, QUESTION_GENERATION_USER_PROMPT, SUMMARIZATION_USER_PROMPT
from ragformance.data_generation.generators.alpha.parsing_engine import parse_qa_pairs_from_response, _extract_tag_content
import pandas as pd
import requests
import os
import re
...
def call_backend_agent(system,message, API_KEY="YOUR_KEY", API_URL="http://your.url/v1/chat/completions", API_MODEL="YOUR_MODEL"):
        headers = {
...
def summarize(document, API_KEY="YOUR_KEY", API_URL="http://your.url/v1/chat/completions", API_MODEL="YOUR_MODEL"):

    message=SUMMARIZATION_USER_PROMPT.format(
        document=document
    )
...
def generate_questions(chunks, file, summary, output_path="questions", API_KEY="YOUR_KEY", API_URL="http://your.url/v1/chat/completions", API_MODEL="YOUR_MODEL"):
    ...
    for id, chunk in enumerate(chunks):
        ...
        answer = call_backend_agent(QUESTION_GENERATION_SYSTEM_PROMPT, message, API_KEY, API_URL, API_MODEL)
        ...
        if isinstance(parsed_answer, list):
            for p_answer in parsed_answer:
                ...
                if p_answer is None or "answer" not in p_answer or len(p_answer["answer"])==0:
                    print(f"No question generated for chunk {id} in file {file}")
                    continue
                answer = p_answer["answer"]
                if p_answer is None or "question" not in p_answer or len(p_answer["question"])==0:
                    print(f"No question generated for chunk {id} in file {file}")
                    continue
            ...
            if parsed_answer is not None and "question" in parsed_answer and "answer" in parsed_answer:
...
def run(folder_path: str, output_path: str, temporary_folder:str = "converted_data",API_KEY="YOUR_KEY", API_URL="http://your.url/v1/chat/completions", API_MODEL="YOUR_MODEL") -> None:
    ...
    for file in os.listdir(temporary_folder):
        if not file.endswith(".md"):
            continue
        with open(os.path.join(temporary_folder, file), "r") as f:
            ...
            generate_questions(chunks, file, summary, output_path, API_KEY, API_URL, API_MODEL)
