...
def normalize_tool_name(raw_name: str, max_length: Optional[int] = None) -> str:
    ...
    try:
        ...
        if tool_name_prefix:
            # Get separator from environment variable or use default (empty string)
            separator = os.getenv("TOOL_NAME_PREFIX_SEPARATOR", "")
            tool_name = f"{tool_name_prefix}{separator}{tool_name}"
