...
# Get tool name prefix and separator from environment variables
TOOL_NAME_PREFIX = os.getenv("TOOL_NAME_PREFIX", "")
TOOL_NAME_PREFIX_SEPARATOR = os.getenv("TOOL_NAME_PREFIX_SEPARATOR", "")

# Function to apply prefix to tool names
def apply_prefix_to_tool_name(name: str) -> str:
    """Apply prefix to tool name if TOOL_NAME_PREFIX is set."""
    if TOOL_NAME_PREFIX:
        return f"{TOOL_NAME_PREFIX}{TOOL_NAME_PREFIX_SEPARATOR}{name}"
    return name
...
@mcp.tool(name=apply_prefix_to_tool_name("list_functions"))
def list_functions(*, env_key: str = "OPENAPI_SPEC_URL") -> str:
    ...
    # Add internal tools with prefix if set
    list_resources_name = apply_prefix_to_tool_name("list_resources")
    functions[list_resources_name] = {
    ...
    read_resource_name = apply_prefix_to_tool_name("read_resource")
    functions[read_resource_name] = {
    ...
    list_prompts_name = apply_prefix_to_tool_name("list_prompts")
    functions[list_prompts_name] = {
        "name": list_prompts_name,
    ...
    get_prompt_name = apply_prefix_to_tool_name("get_prompt")
    functions[get_prompt_name] = {
...
@mcp.tool(name=apply_prefix_to_tool_name("call_function"))
def call_function(*, function_name: str, parameters: Optional[Dict] = None, env_key: str = "OPENAPI_SPEC_URL") -> str:
    ...
    # Strip prefix from function_name for internal function checks
    original_function_name = function_name
    if TOOL_NAME_PREFIX and function_name.startswith(f"{TOOL_NAME_PREFIX}{TOOL_NAME_PREFIX_SEPARATOR}"):
        prefix_length = len(TOOL_NAME_PREFIX) + len(TOOL_NAME_PREFIX_SEPARATOR)
        function_name_without_prefix = function_name[prefix_length:]
        logger.debug(f"Stripped prefix from function name: {original_function_name} -> {function_name_without_prefix}")
    else:
        function_name_without_prefix = function_name

    # Check for internal functions using the name without prefix
    if function_name_without_prefix == "list_resources":
        return json.dumps([{"name": "spec_file", "uri": "file:///openapi_spec.json", "description": "The raw OpenAPI specification JSON"}])
    if function_name_without_prefix == "read_resource":
    ...
    # For OpenAPI function lookup, use the original function name
    ...
    for path, path_item in paths.items():
        ...
        if function_def:
            break
    if not function_def:
        if function_name_without_prefix == "get_file_report":
            simulated_response = {
                "response_code": 1,
