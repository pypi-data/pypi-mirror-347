    ...
    async def serve(self, custom_handlers: Optional[Dict[str, Callable]] = None) -> None:
        ...
        async def call_tool(
            ...
            try:
                ...
                # Critical fix: Handle case where result is itself a coroutine
                # This happens if the tool function returned another coroutine without awaiting it
                if inspect.iscoroutine(result):
                    self.logger.warning(f"Tool '{name}' returned a coroutine - awaiting it")
                    result = await result
