...
class GridSim(Grid):
    ...
    def print_init_power_flow(self, dae: DaeSim) -> None:
        ...
        power_flow_bus_table = pd.DataFrame(
            {
                "Bus": self.buses,
                "V Magnitude (p.u.)": (vinit_mag).tolist(),
                "V Phase (deg)": (vinit_phase * 180 / np.pi).tolist(),
                "P Generation (kW)": Pinit.tolist(),
                "Q Generation (kVAr)": Qinit.tolist(),
                "P shunt loss (kW)": Ploss_shunt.tolist(),
                "Q shunt loss (kVAr)": Qloss_shunt.tolist(),
            }
        )
