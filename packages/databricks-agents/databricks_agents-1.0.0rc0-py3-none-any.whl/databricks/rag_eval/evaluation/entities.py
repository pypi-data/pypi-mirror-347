...
class OverallAssessment:
    ...
    def to_mlflow_assessment(
        ...
        overall_assessment_rating = self.rating
        overall_assessment_metadata = {
            _ROOT_CAUSE_ASSESSMENT: self.root_cause_assessment,
            _ROOT_CAUSE_RATIONALE: self.root_cause_rationale,
            _SUGGESTED_ACTION: self.suggested_action,
            _IS_OVERALL_ASSESSMENT_KEY: str(True),
        }
        assessment_args = {}
        if trace_id is not None:
            assessment_args["trace_id"] = trace_id
        if span_id is not None:
            assessment_args["span_id"] = span_id
...
class PerRequestAssessmentResult(AssessmentResult):
    ...
    def to_mlflow_assessment(
        self, trace_id: Optional[str] = None, root_span_id: Optional[str] = None
    ):
        """
        ...
        return mlflow_entities.Assessment(
            ...
            # These two fields are created service-side.
            create_time_ms=0,
            last_update_time_ms=0,
            **value_kwarg,
        )
    ...
    def make_legacy_metric(metric_name, metric_value):
        ...
        return MetricResult(
            metric_value=mlflow_entities.Assessment(
                ...
                # These two fields are created service-side, but don't matter because they aren't used
                create_time_ms=0,
                last_update_time_ms=0,
            ),
            legacy_metric=True,
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        for metric in self.metric_results:
            ...
            converted_metric_results.append(
                metric.to_mlflow_assessment(trace_id, root_span_id)
            )
        ...
        for assessment in self.assessment_results:
            if isinstance(assessment, PerRequestAssessmentResult):
                converted_assessment_results.append(
                    assessment.to_mlflow_assessment(trace_id, root_span_id)
                )
        ...
        overall_assessment = (
            [
                self.overall_assessment.to_mlflow_assessment(
