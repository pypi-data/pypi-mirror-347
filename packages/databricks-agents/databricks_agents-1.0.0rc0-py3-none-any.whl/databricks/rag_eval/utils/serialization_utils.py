...
def serialize_trace(trace: Optional[mlflow_entities.Trace]) -> Optional[str]:
    ...
    try:
        # TODO(ML-53619): Remove this patch logic to avoid parsing issues with serialized expectation values
        trace_dict = json.loads(trace.to_json())
        if trace_dict["info"].get("assessments") is not None:
            for assessment in trace_dict["info"]["assessments"]:
                if (
                    assessment.get("expectation") is not None
                    and assessment["expectation"].get("serialized_value") is not None
                ):
                    assessment["expectation"]["value"] = json.loads(
