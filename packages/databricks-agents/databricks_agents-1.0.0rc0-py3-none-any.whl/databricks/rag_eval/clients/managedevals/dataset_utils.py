...
def eval_item_to_rest_dataset_record(
    ...
    if eval_item.expected_facts:
        expectations[schemas.EXPECTED_FACTS_COL] = rest_entities.Expectation(
            value=eval_item.expected_facts
        )

    if eval_item.ground_truth_answer:
        expectations[schemas.EXPECTED_RESPONSE_COL] = rest_entities.Expectation(
            value=eval_item.ground_truth_answer
        )

    if eval_item.guidelines:
        expectations[schemas.GUIDELINES_COL] = rest_entities.Expectation(
    ...
    if isinstance(eval_item.custom_expected, dict):
        expectations.update(
            {
                k: rest_entities.Expectation(value=v)
                for k, v in eval_item.custom_expected.items()
            }
        )
    elif eval_item.custom_expected is not None:
        expectations[schemas.CUSTOM_EXPECTED_COL] = rest_entities.Expectation(
