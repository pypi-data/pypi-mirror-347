...
from .exceptions import MeetingInitializationError, InvalidMeetingConfigError
...
class VideoSDK:
    ...
    def init_meeting(**kwargs: Union[MeetingConfig, Any]) -> Meeting:
        ...
        if mode not in ["CONFERENCE", "VIEWER"]:
            raise InvalidMeetingConfigError(
                f'"mode" can only be either "CONFERENCE" or "VIEWER"'
            )

        if meta_data is not None and not isinstance(meta_data, dict):
            raise InvalidMeetingConfigError(
                '"meta_data" can only be a dictionary.')
