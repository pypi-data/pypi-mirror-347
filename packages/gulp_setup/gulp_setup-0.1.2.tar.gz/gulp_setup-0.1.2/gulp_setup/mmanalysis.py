...
def find_regions(connected_components, atoms_indices_at_breaking_point, all_regions):
    '''
    Function to find the regions in the MOF.
    parameters
    ----------
    connected_components: List of connected components in the MOF.
    atoms_indices_at_breaking_point: List of indices of atoms at the breaking point.
    all_regions: List of all regions in the MOF.
    '''
    flattened_list = [item for pair in atoms_indices_at_breaking_point.items() for item in pair]
    point_of_ext = {}
    all_atom_mapping = {}
...
def find_sbu_regions(ase_atom):
    ...
    return: A
    '''
    connected_components, atoms_indices_at_breaking_point, _, all_regions = mof_deconstructor.secondary_building_units(ase_atom)
    point_of_ext, all_atom_mapping = find_regions(connected_components, atoms_indices_at_breaking_point,all_regions)
    return point_of_ext, all_atom_mapping
...
def get_bond_matrix(sbu):
    """Guesses bond order matrix using covalent radii + networkx for ring detection."""
    bonds = numpy.zeros((len(sbu), len(sbu)))
    symbols = numpy.array(sbu.get_chemical_symbols())
    numbers = numpy.array(sbu.get_atomic_numbers())
    positions = numpy.array(sbu.get_positions())
...
def write_gin_with_region(path,
    ...
    with open(path, "w") as fileobj:
        ...
        for s, (x, y, z), atom_index in zip(symb_types, atoms.get_positions(), range(len(symbols))):
