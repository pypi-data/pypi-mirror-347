...
class EMS(object):
			...
			with open(conf_name, "r") as config_file:
				...
				for category in config:
					...
					if response.status_code == 200 and category == "mas":
						requests.put(f"https://{self.__base_url}/{category}/flight_paths", json=config[category]["flight_paths"], auth=self.__auth, verify=False)
	...
	def start(self):
		if self.__state in ["Off", "Stopped"]:
			response = requests.post(f"https://{self.__base_url}/starts", json={}, auth=self.__auth, verify=False)
	...
	def stop(self):
		if self.__state not in ["Off", "Stopped"]:
			response = requests.post(f"https://{self.__base_url}/stops", json={}, auth=self.__auth, verify=False)
			return response.status_code == 201
		else:
			return False

	def restart(self):
		response = requests.post(f"https://{self.__base_url}/restarts", json={}, auth=self.__auth, verify=False)
		return response.status_code == 201

	def accept_calibration(self):
		response = requests.post(f"https://{self.__base_url}/calibration/confirmations", json={}, auth=self.__auth, verify=False)
		return response.status_code == 200
	...
	def retrieve_layout(self):
		response = requests.get(f"https://{self.__base_url}/mgs/layout", auth=self.__auth, verify=False)
		layout = {}
