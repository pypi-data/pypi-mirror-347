...
@click.option("--name", default="", show_default=True)
@click.option("--user", default="", show_default=True)
@click.option("--token", default="", show_default=True)
def minio_create(environment, refresh_access_token=False, name="", user="", token=""):
    """Create minio bucket if not exists."""

    config = get_environment_config(environment)
    token = token or os.getenv("EXONET_API_TOKEN", "")
    name = name or config.exonet_project_name
    user = user or MINIO_USER
    ...
    # User
    storage_user = get_resource_by_name(client, "object_storage_users", user)
    if not storage_user:
        storage_user = ApiResource("object_storage_users")
        storage_user.attribute("name", user)
    ...
    # Bucket
    storage_bucket = get_resource_by_name(client, "object_storage_buckets", minio_name)
    if not storage_bucket:
        storage_bucket = ApiResource("object_storage_buckets")
        storage_bucket.attribute("name", minio_name)
        storage_bucket.attribute("versioning", False)
    ...
    # Access token
    access_token = get_resource_by_name(
        client, "object_storage_access_keys", minio_name
    )
    ...
    if not access_token:
        ...
        access_token.attribute("name", minio_name)
    ...
    click.echo(f"Bucket name: {storage_bucket.attribute('object_storage_name')}")
    ...
    # Temp prefilled migration commands, for faster migration
    click.echo("")
    click.echo(click.style("--- Migration commands ---\n", bold=True))
    click.echo(click.style("Create alias: ", bold=True), nl=False)
    click.echo(
        f"mc alias set {minio_name} https://fd-cdn.nl {access_token.attribute('access_key')}"  # noqa: E501
    )
    click.echo(click.style("Mirror data (Minio -> Minio): ", bold=True), nl=False)
    click.echo(
        f"mc mirror fourdigits/{minio_name} {minio_name}/{storage_bucket.attribute('object_storage_name')}"  # noqa: E501
    )
    click.echo(click.style("Mirror data (Volume -> Minio): ", bold=True))
    click.echo(
        f"  mc mirror data/static/ {minio_name}/{storage_bucket.attribute('object_storage_name')}/static/"  # noqa: E501
    )
    click.echo(
        f"  mc mirror data/media/ {minio_name}/{storage_bucket.attribute('object_storage_name')}/media/"  # noqa: E501
    )
    click.echo(
        f"  mc mirror data/private/ {minio_name}/{storage_bucket.attribute('object_storage_name')}/private/"  # noqa: E501
    )
    click.echo()
    click.echo(click.style(".env.local: ", bold=True))
    click.echo(
        f"AWS_STORAGE_BUCKET_NAME={storage_bucket.attribute('object_storage_name')}"
    )
    click.echo("AWS_S3_ENDPOINT_URL=https://fd-cdn.nl/")
    click.echo(f"AWS_ACCESS_KEY_ID={access_token.attribute('access_key')}")
    click.echo(click.style("Set secret: ", bold=True), nl=False)
    click.echo("nano secrets/aws_secret_access_key")
    click.echo()
    click.echo(click.style("Check data: ", bold=True), nl=False)
    click.echo(f"mc du {minio_name}/{storage_bucket.attribute('object_storage_name')}")
    click.echo(click.style("Delete old data (Minio): ", bold=True), nl=False)
    click.echo(f"mc rm --recursive --force fourdigits/{minio_name}")
