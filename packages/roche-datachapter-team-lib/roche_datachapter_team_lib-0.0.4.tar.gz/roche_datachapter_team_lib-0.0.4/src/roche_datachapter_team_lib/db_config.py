...
ENV_VAR_NAMES = ["SQLSERVER_SERVER", "SQLSERVER_USER", "SQLSERVER_PWD",
                 ...
                 "SNOWFLAKE_USER", "SNOWFLAKE_PASSWORD", "REDSHIFT_HOST", "REDSHIFT_USER",
                 "REDSHIFT_PASSWORD", "REDSHIFT_DB", "REDSHIFT_PORT"]
...
class DbConfig():
    ...
    @classmethod
    def __execute_select_query_on_redshift__(cls, query: str = '') -> pandas.DataFrame:
        df = pandas.DataFrame()
        try:
            str_conn = f"dbname='{REDSHIFT_DB}' user='{REDSHIFT_USER}' password='{REDSHIFT_PASSWORD}' host='{REDSHIFT_HOST}' port='{REDSHIFT_PORT}'"  # pylint:disable=undefined-variable
            with psycopg.connect(str_conn, options='-c client_encoding=UTF8') as conn:
                with conn.cursor() as cursor:
                    cursor.execute(query)
                    result = cursor.fetchall()
    ...
    def execute_custom_select_query(cls, query: str, p_bind: str, result_set_as: ResultType = ResultType.PANDAS_DATA_FRAME, snowflake_ctx: dict | None = None) -> pandas.DataFrame | List[Dict[str, Any]]:
        ...
        elif p_bind.lower() == 'redshift':
            df = cls.__execute_select_query_on_redshift__(query)
