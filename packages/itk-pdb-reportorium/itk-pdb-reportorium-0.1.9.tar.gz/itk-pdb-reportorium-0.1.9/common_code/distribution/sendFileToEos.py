...
from common_code.config import load_report_type_map  # Import the new function
...
#---------- Loading the json file from metadata directory dynamically using importlib.resources
data = load_report_type_map()  # Load data using the new function
...
# After loading data from reportTypeMap.json
# Extract the data for specific WBS_code
wbs_mappings = {}
for item in data:
    for wbs_code, wbs_data in item.items():
        if wbs_code not in ["general", "ext"]:
            # construct the  reportTypeMap_WBS_code
            report_type_key = f"reportTypeMap_WBS{wbs_code.replace('.', '')}"
            wbs_mappings[wbs_code] = { "reportTypeMap": wbs_data.get(report_type_key, {}), "subtypes": wbs_data.get("subtypes", []) }
