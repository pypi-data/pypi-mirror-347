...
sub preprocess_for_databricks
{
	...
	{
		my $proc_var = $var_def->{NAME};$MR->log_error('shevida');
		if (!$IS_PYTHON_SCRIPT and !$IS_SCRIPT)
		{
			my $new_proc_var = '${var.'.$proc_var.'}';
			$cont_string =~ s/$proc_var(?!\s*\:\=)/$new_proc_var/gis;
			$cont_string =~ s/$proc_var\s*\:\=/set var.$proc_var = /gis;$MR->log_error('shevida');
		}
	}
	...
	if ($Globals::ENV{CONFIG}->{tab_count_for_curly_bracket})
	{
...
sub databricks_default_handler
{
	...
	my $tab_tag = $Globals::ENV{CONFIG}->{tab_tag};
	...
	else
	{
		foreach my $arg_def (@{ $Globals::ENV{PRESCAN}->{PROC_ARGS} })
		{
			my $proc_param = $arg_def->{NAME};
			my $new_proc_param = '${'.$proc_param.'}';
			foreach my $item (@{$Globals::ENV{CONFIG}->{to_string_types}})
			{
...
# may or may not have args to populate the template with here.
{
	...
	{  
		$nb_main = $MR->read_file_content($PYTHON_TEMPLATE);
		$MR->log_error($nb_main);
    }
    else
	{
		$nb_main = $MR->read_file_content($Globals::ENV{CONFIG}->{invoked_sql_template_file});
	}
	...
	foreach my $arg_def (@{ $Globals::ENV{PRESCAN}->{PROC_ARGS} })
	{
		...
		if ($arg_def->{ARG_TYPE} eq 'IN' || $arg_def->{ARG_TYPE} eq 'INOUT')
		{
			...
			if($Globals::ENV{CONFIG}->{generate_widget_to_variable_assignments})
			{
				# Arg "get"
	...
	{   
		...
		{
			my $spark_with_head = $Globals::ENV{CONFIG}->{commands}->{SPARK_WITH_HEAD};
...
sub databricks_function_arg_defs
{
	...
	my $nb_main = $MR->read_file_content($Globals::ENV{CONFIG}->{$FUNCTION_TEMPLATE});
...
sub databricks_create_macro
{
	...
	my $template = $Globals::ENV{CONFIG}->{macro_template};
...
sub select_into_variable
{
	...
	if ($vars[0] ne '*')
	{
		...
		$str = $MR->trim($str);
		my $select_into_query = $Globals::ENV{CONFIG}->{commands}->{SELECT_INTO_QUERY};
		...
		{
			$v= $MR->trim($v);
			my $select_into_set = $Globals::ENV{CONFIG}->{commands}->{SELECT_INTO_SET};
	...
	{
		$ret_str = $Globals::ENV{CONFIG}->{commands}->{SWAP};
...
sub check_if_script_contains_python_elements
{
	...
	if($Globals::ENV{CONFIG}->{always_python})
	{
		return 1;
	}
	...
	if($is_full_script)
	{
		foreach my $regex (@{$Globals::ENV{CONFIG}->{set_global_python_keys}})
		{
			if ($script =~ /$regex/gis)
			{
				$is_exception = 1;
	...
	foreach my $regex (@{$Globals::ENV{CONFIG}->{python_keys_regex}})
	{
		if ($script =~ /$regex/gis)
		{
            return 1;
...
sub get_begin_tag
{
	...
	if ($BEGIN_LEVEL > 1)
	{		
		$begin_tag = $Globals::ENV{CONFIG}->{begin_tag};
...
sub unpivot_to_stack
{
	...
	if ($cont =~ /\bFROM\s+(\w+)\s+UNPIVOT\b(.*?)\(\s*(\w+)\s+\bFOR\s+(\w+)\s+.*?\((.*?)\)/gis)
	{
		...
		my $final_expression = $Globals::ENV{CONFIG}->{commands}->{UNPIVOT_TO_STACK};
		if ($include_exclude_nulls ne 'INCLUDE NULLS')
		{
			$final_expression = $Globals::ENV{CONFIG}->{commands}->{UNPIVOT_TO_STACK_INCLUDE_NULLS_WHERE};
		}
...
#		$content =~ s/\<tab_count\:5\>(\n*)\t*\s*/$1\t\t\t\t\t/gis;
	...
	else
	{
		...
		$content =~ s/\breturn\b\s*(\(.*?\))/dbutils.notebook.exit$1/gis;
