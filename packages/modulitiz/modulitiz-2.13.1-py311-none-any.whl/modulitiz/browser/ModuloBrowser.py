...
class ModuloBrowser(AbstractModuloBrowser):
	...
	def __init__(self,cartellaGeckodriver:str):
		...
		if ModuloSystem.isWindows():
			...
			if ModuloSystem.isSystem64bit() is True:
		...
		self.percorsoCartellaProfilo=None
	...
	# =================================================================================================================
	def createDriver(self,nomefileLog:str|None,profiloBrowser:selenium.webdriver.FirefoxProfile,hideWindow:bool,privateMode:bool):
		if nomefileLog is None:
			nomefileLog=ModuloSystem.getFolderNull()
		options=self.createOptions(hideWindow,privateMode)
		options.profile=profiloBrowser
		# creo il driver
		self.driver=selenium.webdriver.Firefox(options,Service(self.fullpathGeckodriver,log_output=nomefileLog))
		# get cartella effettiva profilo
		self.percorsoCartellaProfilo=profiloBrowser._profile_dir
	
	@staticmethod
	def createOptions(hideWindow:bool,privateMode:bool)->selenium.webdriver.FirefoxOptions:
		options=selenium.webdriver.FirefoxOptions()
		options.headless=hideWindow
		if privateMode is True:
			options.add_argument("-private-window")
		return options
	
	def createProfile(self,cartellaProfilo:str|None,permessoImmagini:int|None,noAudio:bool,grantPermissions:bool,openNewTab:bool,
		...
		if cartellaProfilo is None:
			profilo=selenium.webdriver.FirefoxProfile()
		else:
			profilo=selenium.webdriver.FirefoxProfile(cartellaProfilo)
		# parametric options
		if permessoImmagini is not None:
			profilo.set_preference("permissions.default.image",permessoImmagini)
		if noAudio is True:
			profilo.set_preference("media.volume_scale", "0.0")
		if grantPermissions is True:
			profilo.set_preference('media.navigator.permission.disabled',True)
		if openNewTab is True:
			profilo.set_preference("browser.link.open_newwindow",3)
			profilo.set_preference("browser.link.open_newwindow.restriction",0)
			profilo.set_preference("browser.link.open_newwindow.override.external",3)
		if cartellaDownload is not None:
			profilo.set_preference("browser.preferences.instantApply",True)
			profilo.set_preference("browser.helperApps.neverAsk.saveToDisk",BrowserConstants.NEVER_ASK_SAVE_FILES)
			profilo.set_preference("browser.helperApps.alwaysAsk.force",False)
			profilo.set_preference("browser.helperApps.alwaysAsk.focusWhenStarting",False)
			profilo.set_preference('browser.download.manager.showWhenStarting',False)
			profilo.set_preference('browser.download.manager.useWindow',False)
			profilo.set_preference("browser.download.panel.shown",False)
			profilo.set_preference('browser.download.folderList',2)
			profilo.set_preference('browser.download.dir',cartellaDownload)
		if socksProxy is not None:
			socksProxyIp,socksProxyPorta=socksProxy
			if socksProxyIp is None:
				socksProxyIp="127.0.0.1"
			profilo.set_preference('network.proxy.type',BrowserProxyTypeEnum.MANUAL)
			profilo.set_preference('network.proxy.socks',socksProxyIp)
			profilo.set_preference('network.proxy.socks_port',socksProxyPorta)
			profilo.set_preference('network.proxy.socks_version',5)
			profilo.set_preference('network.proxy.socks_remote_dns',True)
		if protezioneAntiTracciamento is not None:
			profilo.set_preference('privacy.trackingprotection.enabled',protezioneAntiTracciamento)
			profilo.set_preference('privacy.trackingprotection.socialtracking.enabled',protezioneAntiTracciamento)
		if userAgent is not None:
			profilo.set_preference("general.useragent.override",userAgent)
		return profilo
	...
	def closeAll(self):
		...
		# delete profile dir
		if os.path.exists(self.percorsoCartellaProfilo):
			shutil.rmtree(self.percorsoCartellaProfilo)
		# close driver
		self.driver.quit()
		self.driver=None
