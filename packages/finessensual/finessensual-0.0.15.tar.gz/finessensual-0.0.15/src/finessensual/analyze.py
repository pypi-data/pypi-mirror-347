...
def analyze( datadir: str,
    ...
    if not os.path.isfile( projects_filename ):
        ...
        try:
            ...
            fatalerror( "could not parse projects.\n{e}" )
    ...
    if not os.path.isfile( persons_filename ):
        ...
        try:
            ...
            fatalerror( "could not parse persons.\n{e}" )
        except KeyError as e:
            fatalerror( "internal error.\n{e}" )
    ...
    if not os.path.isfile( planning_filename ):
        ...
        try:
            ...
            fatalerror( "could not parse planning.\n{e}" )
        except KeyError as e:
            fatalerror( "internal error.\n{e}" )
    ...
    for ub in glob.glob( os.path.join( datadir, 'UB-*.xml' ) ):
        print( f"  - {ub}" )
        ubdir, ubtarget = toxlsx( ub )
        files_toclean.append( ubtarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{ubdir}" "{ub}"',
                                 shell = True,
                                 stdout = errlog
                                )
        if not os.path.isfile( ubtarget ):
            fatalerror( "the conversion of {ub} to {ubtarget} failed." )
    ...
    for bb in glob.glob( os.path.join( datadir, 'Budgetboekingen-*.xls' ) ):
        print( f"  - {bb}" )
        bbdir, bbtarget = toxlsx( bb )
        files_toclean.append( bbtarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{bbdir}" "{bb}"',
                                 shell = True,
                                 stdout = errlog
                                )
        if not os.path.isfile( bbtarget ):
            fatalerror( "the conversion of {bb} to {bbtraget} failed." )
    ...
    for vt in glob.glob( os.path.join( datadir, 'VBTransacties-*.xls' ) ):
        print( f"  - {vt}" )
        vtdir, vttarget = toxlsx( vt )
        files_toclean.append( vttarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{vtdir}" "{vt}"',
                                 shell = True,
                                 stdout = errlog
                                )
        if not os.path.isfile( vttarget ):
            fatalerror( "the conversion of {vt} to {vttarget} failed." )
    ...
    for ff in glob.glob( os.path.join( datadir, 'Transacties-FF*.xls' ) ):
        print( f"  - {ff}" )
        ffdir, fftarget = toxlsx( ff )
        files_toclean.append( fftarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{ffdir}" "{ff}"',
                                 shell = True,
                                 stdout = errlog
                                )
    ...
    for lt in glob.glob( os.path.join( datadir, 'Lonen-transacties-*.xml' ) ):
        print( f"  - {lt}" )
        ltdir, lttarget = toxlsx( lt )
        files_toclean.append( lttarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{ltdir}" "{lt}"',
                                 shell = True,
                                 stdout = errlog
                                )

        if not os.path.isfile( bbtarget ):
            fatalerror( "the conversion of {lt} to {lttarget} failed." )
    ...
    for uu in glob.glob( os.path.join( datadir, 'UU-*.xml' ) ):
        print( f"  - {uu}" )
        uudir, uutarget = toxlsx( uu )
        files_toclean.append( uutarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{uudir}" "{uu}"',
                                 shell = True,
                                 stdout = errlog
                                )

        if not os.path.isfile( bbtarget ):
            fatalerror( "the conversion of {uu} to {uutarget} failed." )
    ...
    for uv in glob.glob( os.path.join( datadir, 'UV-*.xml' ) ):
        print( f"  - {uv}" )
        uvdir, uvtarget = toxlsx( uv )
        files_toclean.append( uvtarget )
        retval = subprocess.run( f'{xlsxconvert} --convert-to xlsx --outdir "{uvdir}" "{uv}"',
                                 shell = True,
                                 stdout = errlog
                                )

        if not os.path.isfile( bbtarget ):
            fatalerror( "the conversion of {uv} to {uvtarget} failed." )
...
def toxlsx( path: str ) -> str:
    ...
    if path == retval:
        fatalerror( f"Dangerous internal error with risk for data loss - aborting!\n" +
