...
class BaseAgenticPipeline:
    ...
    def __init__(
        ...
        for step in self.name_order:
            worker = (
                self.generators[step]
                if step in self.generators
                else self.validators[step]
            )
            ...
            PipelineStep(
                SaveGenerator(
                    ...
                    save_keys=(
                        save_keys
                        if save_keys
                        else list(self.order[-1].worker.output.keys())
                    ),
                )
            )
        )
    ...
    def _process_and_save_data(
        ...
        if is_final_results:
            pd.DataFrame(data_io).to_json(
                f"{data_io_path}.jsonl", orient="records", lines=True
            )
        else:
    ...
    def run_pipeline(
        ...
        try:
            ...
            for step_ in self.order[start_from:]:
                step_results = []
                with tqdm(
                    total=len(step_.queue), desc=f"{step_.worker.name} Execution"
                ) as pipeline_tracker:
                    while step_.queue:
                        ...
                        try:
                            ...
                            if step_.next:
                                if isinstance(output, list):
                                    ...
                                    step_.next.queue.extend(exp_objs)
                                    ...
                                    if hasattr(output, "response") and output.response:
                                        ...
                                        step_.next.queue.append(exp_obj)
    ...
    def pipeline_step_logic(self, exp_obj: ExperimentObject):
        ...
        for step in self.order:
            self.logger.debug(
                f"Validating step '{step.worker.name}' input requirements"
            )
            if not set(step.worker.input.keys()).issubset(
                set(prior_step_output.keys())
            ):
                self.logger.error(
    ...
    def pipline_spotcheck(self, exp_obj: ExperimentObject, debug: bool = False):
        ...
        with tqdm(
            ...
            desc=f"Running Pipeline Spotcheck",
            disable=self.quiet,
        ) as pipeline_tracker:
            for idx, step in enumerate(self.order):
                self.logger.debug(
                    f"Executing step {idx+1}/{len(self.order)}: {step.worker.name}"
                )
                pipeline_tracker.set_description(
                    f"Running Pipeline Spotcheck - {idx+1}/{len(self.order)} - {step.worker.name}"
                )
                try:
                    ...
                    if isinstance(output, list):
                        ...
                        if len(output) == 0:
                            self.logger.warning(
                                f"Step {step.worker.name} returned an empty response, most likely due to an error from step inference (possible causes: prompt too long, invalid input format). Run in debug mode to see more details."
                            )
                            raise Exception(
                                f"Step {step.worker.name} returned an empty response, most likely due to an error from step inference. Check if your prompt length exceeds the model's token limit or if the input format is invalid. Run in debug mode to see more details."
                            )
                        exp_obj = output[0]
                        self.logger.debug(
                            f"Step {step.worker.name} returned list output, using first element"
                        )
                        ...
                        if not step_output:
                            self.logger.warning(
                                f"Step {step.worker.name} returned an empty response, most likely due to an error from step inference"
                            )
                            raise Exception(
                                f"Step {step.worker.name} returned an empty response, most likely due to an error from step inference"
                            )
                    ...
                    self.logger.error(
                        f"Step {step.worker.name} failed with error: {str(e)}"
                    )
                    raise e
                pipeline_tracker.update(1)
    ...
    def __call__(
        self,
        prompt_obj: Union[
            PromptObject, ExperimentObject, List[PromptObject], List[ExperimentObject]
        ],
        debug: bool = False,
    ):
        ...
        if isinstance(prompt_obj, PromptObject):
            ...
            exp_object = ExperimentObject(
                experiment_step=self.order[0], data=prompt_obj.data
            )
            ...
            for obj in prompt_obj:
                if isinstance(obj, PromptObject):
                    obj = ExperimentObject(experiment_step=self.order[0], data=obj.data)
                exp_objs.append(obj)
        ...
        if self.return_step_outputs:
            return {"final_results": results[0], "step_outputs": results[1]}
        return results
