...
class SaveGenerator(BaseGenerator):
    ...
    def __call__(self, exp_obj: ExperimentObject, debug=False):
        ...
        if self.input:
            data = {key_: remove_non_ascii(exp_obj.data.get(key_, None)) for key_ in self.input}
        else:
            data = {key_: remove_non_ascii(exp_obj.data.get(key_, None)) for key_ in exp_obj.data}
        try:
