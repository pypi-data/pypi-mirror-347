...
class BaseSQLGenerator(BaseGenerator):
    ...
    def to_json(self):
        ...
        return {
            ...
            "db_type": self.db_type if hasattr(self, "db_type") else None,
            "db_params": self.db_params if hasattr(self, "db_params") else None,
            "schema": self.schema if hasattr(self, "schema") else None,
        }

    @classmethod
    def from_json(cls, debugger_config: Dict[str, Any]):
        """Load a generator from a JSON file.

        Args:
            debugger_config (Dict[str, Any]): The debugger configuration.

        Returns:
            BaseGenerator: The loaded generator.
        """
        # If output_type is a string representation of a dict, evaluate it
        if isinstance(debugger_config.get("output_type"), str) and debugger_config[
            "output_type"
        ].startswith("{"):
            try:
                debugger_config["output_type"] = eval(debugger_config["output_type"])
            except:
                pass
        # If output_type is a dictionary with schema information, convert it to a dict
        elif (
            isinstance(debugger_config.get("output_type"), dict)
            and "properties" in debugger_config["output_type"]
        ):
            debugger_config["output_type"] = {
                k: v.get("type", "string")
                for k, v in debugger_config["output_type"]["properties"].items()
            }
        return cls(**debugger_config)
