...
def create(
    ...
    typer.echo(f"{name}/")
    typer.echo("├── run.sh            <-  entry point using 'uv' to manage Python and pip packages")
    typer.echo("├── requirements.txt  <-  python requirements")
    typer.echo("├── app.py            <-  your fastapi and syftbox application")
    typer.echo("└── main.py           <-  uvicorn hotreloading script")
    typer.echo("--------------------------------------------------")
    typer.echo("You can optionally control the port by setting SYFTBOX_ASSIGNED_PORT=8080")
