...
def data_loader_to_frame(dataloader, task):
    if 'dose' in task:
        ...
        for nctid_lst, label_lst, smiles_lst, mesh_lst in dataloader:
            all_data['nctid'].extend(nctid_lst)
            all_data['label'].extend(label_lst)
            all_data['smiles'].extend(smiles_lst)
            all_data['mesh'].extend(mesh_lst)
    else:
        all_data = {
            'nctid': [],
            'label': [],
        ...
        for nctid_lst, label_lst, icdcode_lst, smiles_lst, criteria_lst, tabular_lst, text_lst, mesh_lst in dataloader: 
            ...
            all_data['label'].extend(label_lst)
            all_data['smiles'].extend(smiles_lst)
            all_data['icdcode'].extend(icdcode_lst)
            all_data['criteria'].extend(criteria_lst)
            all_data['tabular'].extend(tabular_lst)
            all_data['text'].extend(text_lst)
            all_data['mesh'].extend(mesh_lst)
...
def dose(phase):
    ...
    def dose_collate_fn(x):
        ...
        # label_vec = default_collate([list(i[1]) for i in x])
        smiles_lst = [smiles_txt_to_lst(i[2]) for i in x]
    ...
    train_nctid_lst = X_train.index.tolist()
    train_label_lst = y_train.values.tolist()
    # remove the first column (dataset name)
