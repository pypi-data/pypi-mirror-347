...
def check_and_upgrade():
    EXTRACT_PATH = os.path.join(os.environ["appdata"], "xlOil", "latest_release")
    ...
    for subdir, dirs, files in os.walk(EXTRACT_PATH):
        if not os.path.basename(subdir).startswith(
            "zhixinpy-"
        ):  # 检查目录名是否以zhixin开头
            continue
        xlo.log.info(f"找到升级文件{subdir}")
        zhixinpy_dir_path = os.path.join(subdir, "zhixinpy")
        zhixinui_path = os.path.join(subdir, "zhixinui.py")
        version_path = os.path.join(subdir, "VERSION.txt")
        requirement_path = os.path.join(subdir, "requirements.txt")

        # 检查文件和目录是否存在
        if (
            os.path.exists(zhixinui_path)
            and os.path.exists(version_path)
            and os.path.exists(zhixinpy_dir_path)
        ):
            # 复制zhixinui.py到p目录下，如果存在则覆盖
            shutil.copy2(zhixinui_path, parent_dir)

            # 复制VERSION.txt到p目录下，如果存在则覆盖
            shutil.copy2(version_path, parent_dir)

            # 复制requirements.txt到p目录下，如果存在则覆盖
            shutil.copy2(requirement_path, parent_dir)

            # 复制zhixinpy目录到p目录下，如果存在则覆盖
            # 首先删除p目录下可能存在的同名目录
            if os.path.exists(os.path.join(parent_dir, "zhixinpy")):
                shutil.rmtree(os.path.join(parent_dir, "zhixinpy"))
            shutil.copytree(
                zhixinpy_dir_path, os.path.join(parent_dir, "zhixinpy"), dirs_exist_ok=True
            )
            xlo.log.info(f"完成升级文件更新{subdir}")
        else:
            xlo.log.info(f"更新文件出现异常？{subdir}")
    shutil.rmtree(EXTRACT_PATH)
    xlo.log.info("已完成升级文件的检查及更新并删除升级文件")
...
def onlineUpgrade(ctrl):
    # 先尝试去更新包
    python_executable = os.path.join(
        os.environ["appdata"], "xlOil", "WPy64-31290", "python", "python.exe"
    )
    requirements_path = os.path.join(os.environ["appdata"], "xlOil", "requirements.txt")
    ...
    from zhixinpy.func_upgrade import check_and_download

    print_status("正在下载升级文件...")
    ret = check_and_download()
    MsgBox(f"{ret}")
...
def button_image(ctrl):
    match ctrl.tag:
        case "zt_Qrcode":
            icon_name = 'proicons--qr-code.png'
        case _:
            icon_name = 'proicons--qr-code.png'
    if not os.path.exists(os.path.join(os.environ["appdata"], "xlOil", "resource", icon_name)):
        im = Image.new("RGB", (1, 1), (0, 0, 0))
    else:
        im = Image.open(os.path.join(os.environ["appdata"], "xlOil", "resource", icon_name))
    return im
			...
			<tab id="tab1" label="❤️知心❤️" insertAfterMso="TabHome">
				...
				<group id="chartGroup" label="知心函数">
                	...
                	<menu id="toolMenu" size="large" label="工具" imageMso="ControlToolboxOutlook" itemSize="normal">
						...
						<button id="tool18" label="屏幕投射" imageMso="Camera" onAction="display_formula" tag="zt_screencast" screentip="实现屏幕画面实时捕捉和流媒体传输"/>
					...
					<menu id="chartMenu" size="large" label="图表" imageMso="ChartTypeBarInsertGallery">
						<menu id="chart1" label="基础统计" imageMso="Chart3DColumnChart" itemSize="normal">
							<button id="chart1-1" label="折线图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Line" screentip="展示连续数据的变化趋势（2D）"/>
							<button id="chart1-2" label="三维折线图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Line3d" screentip="折线图的3D版本，适合空间维度趋势展示"/>
							...
							<button id="chart1-4" label="柱状图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Bar" screentip="类别数据对比常用图表，支持横/竖展示"/>
							...
							<button id="chart1-6" label="饼图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Pie" screentip="展示占比结构"/>
						...
						<menu id="chart2" label="散点与空间分布" imageMso="Chart3DColumnChart" itemSize="normal">
							<button id="chart2-1" label="散点图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_Scatter" screentip="二维空间下的点分布图"/>
						...
						<menu id="chart3" label="层级与流向关系" imageMso="Chart3DColumnChart" itemSize="normal">
                            ...
                            <button id="chart3-5" label="地图" imageMso="MapContactAddress" onAction="display_formula" tag="zc_Map" screentip="在地图中显示数据"/>
                            <button id="chart3-6" label="坐标图" imageMso="CreateMap" onAction="display_formula" tag="zc_Geo" screentip="在地图中显示坐标"/>
                            <button id="chart3-7" label="坐标路线图" imageMso="CreateMap" onAction="display_formula" tag="zc_GeoLine" screentip="在地图中显示两地之间的传送路径"/>
                            <button id="chart3-8" label="3D曲面图" imageMso="CreateMap" onAction="display_formula" tag="zc_surface3d" screentip="3D曲面图"/>
                            <button id="chart3-9" label="饼状柱状图" imageMso="CreateMap" onAction="display_formula" tag="zc_pie_bar" screentip="饼图与柱状图相互转换"/>
                            ...
                            <button id="chart3-12" label="排名图" imageMso="CreateMap" onAction="display_formula" tag="zc_rank" screentip="从数据中提取前N排名图"/>
						</menu>
						<menu id="chart4" label="文本与可视化" imageMso="Chart3DColumnChart" itemSize="normal">
							<button id="chart4-1" label="词云图" imageMso="ChartTypeBarInsertGallery" onAction="display_formula" tag="zc_WordCloud" screentip="根据词频生成大小不一的关键词云"/>
						</menu>
					</menu>
				...
				<group id="env" autoScale="false" centerVertically="false" label="知心配置">
					...
					<menu id="helpmenu" size="large" label="帮助" imageMso="Help">
						...
						<button id="env-4" label="在线更新" onAction="onlineUpgrade" imageMso="MailMergeUpdateLabels" screentip="在线更新" supertip="点击可在线更新插件。若想离线更新，可联系zhixin_excel@163.com获取全量离线包" />
...
def zs_FillWithKNN(arr1: xlo.Array(), arr2: xlo.Array(), n_neighbors=3):
    ...
    """
    assert len(np.array(arr1.flatten(), dtype=float)) == len(
        np.array(arr2.flatten(), dtype=float)
    ), "arr1和arr2长度必须一样"
    assert n_neighbors > 0, "n_neighbors必须是正整数"
    from zhixinpy import func_series
...
def zs_SeasonalDecompose(data: xlo.Array(), ds, y, period=12):
    ...
    """
    assert data.shape[0] > 1 and data.shape[1] > 1, "选取的区域必须是表格"
    assert any(isinstance(item, str) for item in data[0]), "选取的区域请包含表格头"
    assert ds in data[0], "指定的日期字段名不存在"
    assert y in data[0], "指定的值字段名不存在"
    assert period > 0, "period必须是正整数"
    from zhixinpy import func_series
...
def _set_image_size(image, mode, width, height):
    ...
    c = xlo.Caller()
    cell = xlo.Range(c.address(a1style=True))
...
def zt_Qrcode(
    ...
    """
    assert mode in (1, 2, 3, 4), "mode只能在1 2 3 4之中选取"
    from zhixinpy import func_tools

    if mode == 4:
        assert height > 0, "height必须是正整数"
        assert width > 0, "width必须是正整数"
    image = None
    try:
...
def zt_Image(url, mode=3, height=None, width=None):
    ...
    assert func_text.isURL(url), "url必须是合法URL"
    assert mode in (1, 2, 3, 4), "mode只能在1 2 3 4之中选取"
    if mode == 4:
        assert height > 0, "height必须是正整数"
        assert width > 0, "width必须是正整数"
...
async def zt_Mail(
    ...
    assert func_text.isEmail(MAIL_SENDER), f"邮箱地址不合法 {MAIL_SENDER}"
    for recipient in recipients:
        assert func_text.isEmail(recipient), f"邮箱地址不合法 {recipient}"
    for filename in filenames:
        assert os.path.isfile(filename), f"文件不存在 {filename}"
...
async def zt_LLM(
    ...
    assert len(llm_model_name) > 1, "请确保模型名配置正确"
...
async def zt_LLMGen(
    ...
    assert len(examples) > 0, "示例数据不能为空"
...
@xlo.func(
    ...
    args={
        "target_url": "录屏目标URL",
        "record_time": "[可选 默认4] 需要录制的秒数",
        "audio_device": "[可选 默认自动取到第一个可用的设备] 音频设备名称",
    },
)
async def zt_screencast(
    target_url: str,
    record_time: int = 4,
    title: str = "知心录制",
    audio_device: str = "",
    ...
    """解析 JSON 并按 JSONPath 提取数据，返回 Excel 友好的表格
    示例:
        =zt_screencast("http://example.com")
    步骤:
        1 关闭所有chrome程序
        2 找到chrome的路径，在cmd中加启动参数，例如 C:\Program Files (x86)\Google\Chrome\Application\chrome.exe --remote-debugging-port=9222 --headless
        3 访问chrome://inspect -> 确保Discover network targets勾选上 -> 点Configure -> 增加localhost:9222 -> 确保Enable port forwarding勾选上
    """
    from zhixinpy import func_tools

    audio_device_list = func_tools._list_audio_devices()
    assert (
        len(audio_device_list) > 0
    ), "未找到声卡设备，请在控制面板-声音-录制-立体声混音-启用"
    if audio_device:
        pass
    else:
        audio_device = audio_device_list[0]  # 默认取第一个
    async for status in func_tools.capture_screencast(
...
def zc_Sankey_income(nodes: xlo.Array(), connections: xlo.Array(), title=""):
    ...
    简介
        通过节点定义、节点链接定义，画桑基图
    """
    assert nodes.shape[1] >= 2, "必须要包含2列。节点列，类型列"
    assert connections.shape[1] >= 3, "必须包含3列，源节点名，目的节点名，值"
    from zhixinpy import func_chart
    ...
    for src in connections["source"].values:
        assert src in nodes["nodeName"].values, f"源节点{src}不在参数1定义的节点列表里"
    for dest in connections["destination"].values:
        assert (
            dest in nodes["nodeName"].values
        ), f"目的节点{dest}不在参数1定义的节点列表里"
    ...
    for i, row in nodes.iterrows():
        ...
        if (
            ...
            # 中间节点
            assert (
                row["nodeName"] in middle_node_data
            ), f"中间节点{row['nodeName']}没有数据"
...
def zc_Scatter(
    ...
    **kwargs,
    ...
    简介
        画散点图，每一行数据代表一个观测值
    """
    assert data_frame.shape[1] >= 2, "必须带有表格名"
...
def zq_Realtime(symbol, field="lastPrice"):
    ...
    """
    assert symbol is not None and len(str(symbol)) > 0, "股票标的代码不能为空"
