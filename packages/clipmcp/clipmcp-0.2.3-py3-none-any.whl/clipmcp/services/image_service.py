...
# 导入路径修正
from clipmcp.utils.auth import AuthManager
from clipmcp.utils.config import (
...
def generate_image(prompt: str, output_dir: str) -> dict:
    """生成图像并保存到指定目录"""
...
class ImageService:
    ...
    def __init__(self, access_key=ACCESS_KEY, secret_key=SECRET_KEY, api_base_url=API_BASE_URL, download_dir=DEFAULT_DOWNLOAD_DIR):
        """初始化图像服务"""
        self.access_key = access_key
        self.secret_key = secret_key
        self.api_base_url = api_base_url
        self.default_download_dir = download_dir if download_dir else DEFAULT_DOWNLOAD_DIR
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, json=request_data, params=auth_params) as response:
                        ...
                        print(f"状态响应: {json.dumps(response_data, indent=2, ensure_ascii=False)}")
                        
                        if response_data.get("code") == 0 and "data" in response_data:
                            # 检查响应数据中是否包含图像 - 这是确定任务完成的最可靠方法
                            if (response_data['data'].get('images') and 
                                any(image for image in response_data['data']['images'] if image is not None)):
                                print("任务完成，获取到图像数据。")
                            ...
                            # 0: 排队中, 1: 生成中, 2: 已完成, 3: 失败
                                ...
                                print("图像生成已完成，等待获取图像数据...")
                            elif status == 3:
                                print(f"生成失败: {response_data['data'].get('msg', '未知错误')}")
                                return None
                            else:
                                print(f"未知状态: {status}")
                        else:
                            print(f"查询状态失败: {response_data.get('msg', '未知错误')}")
            except Exception as e:
                print(f"查询状态出错: {e}")
            
            print(f"任务尚未完成，等待 {polling_interval} 秒...")
            # 等待下一次轮询
