...
try:
    from clipmcp.services.image_service import FastMCP as ImageServiceClient # Rename to avoid conflict
...
def _load_config_from_file_if_needed():
    ...
    config_path = os.path.join(os.path.expanduser("~"), ".clipmcp", "config.json")
    ...
    if not (ACCESS_KEY and SECRET_KEY) and os.path.exists(example_config_path):
         ...
         try:
             with open(example_config_path, "r", encoding="utf-8") as f:
                 config = json.load(f)
                 mcp_config = config.get("mcpServers", {}).get("clipmcp", {})
