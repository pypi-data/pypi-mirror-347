    ...
    if country_code == 'ES':
        ...
        try:
            ...
            if sys.version_info >= (3, 9):
                try:
                    with pkg_resources.files("orama_utils").joinpath("holiday_db", file_name).open("rb") as f:
            ...
            # For Python 3.7-3.8
            try:
                with pkg_resources.path("orama_utils.holiday_db", file_name) as path:
                    holidays_df = pd.read_csv(path)
                    return _process_holidays_df(holidays_df)
            except Exception as e:
                print(f"Method 2 failed: {e}")
            
            # Try using resource_string approach
            try:
                csv_data = pkg_resources.read_binary("orama_utils.holiday_db", file_name)
                holidays_df = pd.read_csv(io.BytesIO(csv_data))
                return _process_holidays_df(holidays_df)
            except Exception as e:
                print(f"Method 3 failed: {e}")
                
            # Try direct path to orama_utils package
            try:
                import orama_utils
