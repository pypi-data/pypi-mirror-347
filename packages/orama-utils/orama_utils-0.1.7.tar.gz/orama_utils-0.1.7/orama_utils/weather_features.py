...
def add_weather_features(
    ...
    # Create weather_db directory with absolute path to avoid permission issues
    if not os.path.isabs(weather_db_path):
        weather_db_path = os.path.join(os.getcwd(), weather_db_path)
    ...
    for location in locations:
        ...
        if needs_update:
            ...
            # Also, replace spaces with + which works better for some APIs
            encoded_location = location.replace(' ', '+').replace(',', '%2C')
            
            # Use the full API URL with all necessary parameters (restored from your original code)
            url = (
            ...
            try:
                ...
                # Raise an error when weather data fetching fails
                raise Exception(f"Failed to fetch weather data for {location}: {str(e)}")
