...
class Molecule:
    def __init__(self, charge: int, mult: int, xyz: str, name: str) -> None:
        self.charge = charge
        self.mult = mult
        self.xyz = xyz
        self.name = name
    ...
    def _parse_charge_mult_from_comment(
        ...
        """Read charge/mult from comment in either format:
        - Key-value pairs like 'charge=1 mult=1'
        - JSON dict like '{"charge": 1, "mult": 1}'

        JSON format is preferred as it allows for easier extension in the future.
        """
        ...
        try:
            comment_dict = json.loads(comment.strip())
            charge = comment_dict.get("charge")
            mult = comment_dict.get("mult")
            debug(f"Parsed JSON comment: charge={charge}, mult={mult}")
            return charge, mult
