...
def slurm(config_file: Path, no_submit: bool, _debug: bool) -> None:
    ...
    try:
        ...
        slurm_config = SlurmConfig(
            ...
            orcastrator_command=f"uvx orcastrator run {'--debug' if _debug else ''} {config_file.resolve()}",
...
@cli.command()
@click.argument(
    "output_file", type=click.Path(dir_okay=False, path_type=Path), required=False
)
@click.option(
    "--force", "-f", is_flag=True, help="Overwrite the output file if it already exists"
)
def init(output_file: Optional[Path] = None, force: bool = False) -> None:
    ...
    if output_file.exists() and not force:
        error(f"File {output_file} already exists. Use --force to overwrite.")
        sys.exit(1)

    template = f"""# Orcastrator Configuration Template (v{__version__})
# Generated with: orcastrator init

[main]
# Directory where calculation results will be saved
output_dir = "output"
# Number of CPU cores to use for each calculation
cpus = 4
...
directory = "molecules"
# Default charge for molecules (overridden by values in XYZ files if present)
default_charge = 0
# Default multiplicity for molecules (overridden by values in XYZ files if present)
default_mult = 1
...
[[step]]
name = "opt"
keywords = ["B3LYP", "def2-SVP", "D3BJ", "Opt", "TightSCF"]
# Optional: Additional ORCA blocks
blocks = ["%scf maxiter 150 end"]

[[step]]
name = "freq"
keywords = ["B3LYP", "def2-SVP", "D3BJ", "Freq", "TightSCF"]
# Optional: Use a different multiplicity for this step
# mult = 3

[[step]]
name = "sp"
keywords = ["B3LYP", "def2-TZVP", "D3BJ", "TightSCF"]
blocks = ["%scf maxiter 300 end"]
"""

    try:
        output_file.write_text(template)
        info(f"Template configuration written to {output_file}")
        info("Edit this file to customize your calculation settings, then run:")
        info(f"  orcastrator run {output_file}")
    except Exception as e:
