...
class BoughArgumentCase(EasySet):

	"""
		GrowingPeel, BlindPeek, LeafGrowth, Germination -> Rejuvenation
		Single sprout
		Single bough (passed as argument in init)
		This test is duplication for easy1, except bough providing
	"""
	...
	def tearDownClass(cls):
		...
		if	cls.clean_up:
			if	os.path.isfile(cls.EASY_HANDLER_13): os.remove(cls.EASY_HANDLER_13)

	@classmethod
	def setUpClass(cls):

		super().setUpClass()
		cls.make_loggy_file(cls, cls.EASY_HANDLER_13)

	def setUp(self):
		class Sakura(Tree):
			class loggy(LibraryContrib):

				handler		= self.EASY_HANDLER_13
				init_level	= 10
	...
	def test_first_flourish(self):
		...
		with self.assertLogs("first_flourish", 10) as case_loggy:
			...
			self.test_case = self.test_case(bough=self.EASY_SET_BOUGH)
			self.test_case.sync()
