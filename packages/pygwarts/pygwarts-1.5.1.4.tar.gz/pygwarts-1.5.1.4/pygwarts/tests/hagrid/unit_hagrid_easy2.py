...
class ReorderedRegularCase(EasySet):

	"""
		GrowingPeel, BlindPeek, LeafGrowth, Rejuvenation -> Germination (testing the order doesn't mater
		cause any planting object creates parent folder by default - 0.6.4.4+)
		Single sprout
		Single bough
	"""
	...
	def tearDownClass(cls):
		...
		if	cls.clean_up:
			if	os.path.isfile(cls.EASY_HANDLER_2): os.remove(cls.EASY_HANDLER_2)

	@classmethod
	def setUpClass(cls):

		super().setUpClass()
		cls.make_loggy_file(cls, cls.EASY_HANDLER_2)
