...
class WriteBatch:
    """
    WriteBatch class for batched writes.

    Provides a way to perform multiple writes as a single atomic operation.
    """
    ...
    def _ensure_tables_exist(self):
        ...
        for collection in collections:
            try:
                ...
                logger.debug(
                    f"バッチ操作のためのテーブル確認: {collection} は存在します"
                )
            except Exception as e:
                logger.error(
                    f"テーブル {collection} の確認中にエラーが発生しました: {str(e)}"
                )
                raise DatabaseError(
    ...
    def _execute_operation_with_retry(self, op, max_retries=3):
        ...
        while retries < max_retries:
            try:
                if op[0] == "set":
                    ...
                    logger.debug(f"バッチ操作実行: SET - ドキュメントID: {doc_ref.id}")
                    return doc_ref.set(data, merge=merge)
                elif op[0] == "update":
                    _, doc_ref, data = op
                    logger.debug(
                        f"バッチ操作実行: UPDATE - ドキュメントID: {doc_ref.id}"
                    )
                    return doc_ref.update(data)
                elif op[0] == "delete":
                    _, doc_ref = op
                    logger.debug(
                        f"バッチ操作実行: DELETE - ドキュメントID: {doc_ref.id}"
                    )
                    return doc_ref.delete()
                else:
                    logger.warning(f"不明なバッチ操作タイプ: {op[0]}")
                ...
                if "no such table" in str(e) and retries < max_retries - 1:
                    ...
                    logger.info(
                        f"テーブル {collection} が存在しないため作成して再試行します (試行 {retries+1}/{max_retries})"
                    )
                    try:
                        self._store._ensure_table_exists(collection)
                        retries += 1
                        ...
                        logger.error(
                            f"テーブル {collection} の作成中にエラーが発生しました: {str(create_error)}"
                        )
                        raise DatabaseError(
                ...
                if op[0] == "update" and retries < max_retries - 1:
                    ...
                    _, doc_ref, data = op
                    logger.info(
                        f"ドキュメント {doc_ref.id} が存在しないため、updateの代わりにsetを試行します"
                    )
                    try:
                        return doc_ref.set(data, merge=True)
                    except Exception as set_error:
                        logger.error(
                            f"setへのフォールバック中にエラーが発生しました: {str(set_error)}"
                        )
                        raise
                else:
                    logger.warning(f"ドキュメントが見つかりません: {str(e)}")
                    raise
            except Exception as e:
                logger.error(
                    f"バッチ操作 {op[0]} の実行中に例外が発生しました: {str(e)}"
                )
                raise
        ...
        if last_error:
            logger.error(
                f"最大再試行回数 ({max_retries}) を超えました。最後のエラー: {str(last_error)}"
            )
            raise last_error
        else:
            logger.error(f"最大再試行回数 ({max_retries}) を超えました。不明なエラー")
            raise RuntimeError(
                f"Maximum retries ({max_retries}) exceeded with unknown error"
            )
    ...
    def commit(self):
        ...
        if not self._operations:
            logger.warning("空のバッチが実行されました。操作はありません。")
            return []

        logger.debug(f"バッチ処理を開始します: {len(self._operations)}件の操作")
            ...
            try:
                for op in self._operations:
                    try:
                        ...
                        logger.error(
                            f"バッチ操作中にエラーが発生しました: {str(op_error)}"
                        )
                ...
                # トランザクションをコミット
                conn.commit()
                logger.debug(f"バッチ処理が正常に完了しました: {len(results)}件の操作")
                return results
            except Exception as e:
                # エラーが発生した場合はロールバック
                conn.rollback()
                logger.error(
                    f"バッチ処理中にエラーが発生したためロールバックしました: {str(e)}"
                )
