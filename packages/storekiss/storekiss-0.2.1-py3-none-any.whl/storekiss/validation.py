...
class DateTimeField(FieldValidator):
    ...
    def _validate(self, value: Any) -> datetime.datetime:
        if isinstance(value, str):
            ...
            raise ValidationError(
                f"Expected datetime or ISO format string, got {type(value).__name__}"
            )
    ...
    def _validate(self, value: Any) -> List[Any]:
        ...
        for i, item in enumerate(value):
            try:
                ...
                raise ValidationError(f"Error at index {i}: {str(e)}")
    ...
    def _validate(self, value: Any) -> bytes:
        if isinstance(value, str):
            try:
                ...
                raise ValidationError(f"Invalid base64 encoded string: {value}")
        elif isinstance(value, bytes):
            return value
        else:
            raise ValidationError(f"Expected bytes or base64 string, got {type(value).__name__}")
    ...
    def _validate(self, value: Any) -> GeoPoint:
        ...
        if hasattr(value, "__class__") and value.__class__.__name__ == self.geopoint_class_name:
            try:
                ...
                raise ValidationError(f"GeoPoint must be iterable with latitude and longitude")
                
        if isinstance(value, dict) and "latitude" in value and "longitude" in value:
            try:
                return GeoPoint(value["latitude"], value["longitude"])
            except (TypeError, ValueError) as e:
                raise ValidationError(f"Invalid GeoPoint dict: {e}")
                
        if isinstance(value, (list, tuple)) and len(value) == 2:
            try:
                return GeoPoint(value[0], value[1])
            except (TypeError, ValueError) as e:
                raise ValidationError(f"Invalid GeoPoint coordinates: {e}")
                
        raise ValidationError(f"Expected GeoPoint, got {type(value).__name__}")
    ...
    def validate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.allow_extra_fields:
            ...
            if extra_fields:
                import logging
                logging.warning(
                    f"Unexpected fields in document: {', '.join(extra_fields)}"
                )
