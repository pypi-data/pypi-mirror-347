...
def analyze_returns_by_period(returns):
    ...
    dow_returns = pd.DataFrame({
        ...
        'day_of_week': daily_returns.index.day_name()
    })
    dow_analysis = dow_returns.groupby('day_of_week')['returns'].agg(
    ...
    moy_analysis = moy_returns.groupby('month')['returns'].agg(
        ['mean', 'std', 'count', lambda x: (x > 0).mean()]
    ).rename(columns={'<lambda_0>': 'positive_pct'})
