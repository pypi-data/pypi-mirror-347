...
class ContactPoint(CompanyAssetModel):
    source_checker_method : SourceCheckerType = Field(default=None)
    source_id: Optional[StrObjectId] = Field(default=None)
    template_name : Optional[str] = Field(default=None)
    source: Source = Field(exclude=True, default=None) #This is only used inside the API as a shortcut, so it's not stored in the DB
