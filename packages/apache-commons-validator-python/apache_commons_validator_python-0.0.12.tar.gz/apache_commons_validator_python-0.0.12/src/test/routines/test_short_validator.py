...
class TestShortValidator(TestAbstractNumberValidator):
    ...
    def set_up(self):
        ...
        self._min_minus_one = ShortValidator.SHORT_MIN - 1
    ...
    def test_integer_validator_methods(self):
        ...
        assert ShortValidator.get_instance().validate(default_val) == expected
        assert ShortValidator.get_instance().validate(value=locale_val, locale=locale) == expected
        assert ShortValidator.get_instance().validate(value=pattern_val, pattern=pattern) == expected
        # assert ShortValidator.get_instance().validate(value=german_pattern_val, pattern=pattern, locale=locale) == expected

        assert ShortValidator.get_instance().is_valid(default_val) is True
        assert ShortValidator.get_instance().is_valid(value=locale_val, locale=locale) is True
        assert ShortValidator.get_instance().is_valid(value=pattern_val, pattern=pattern) is True
        # assert ShortValidator.get_instance().is_valid(value=german_pattern_val, pattern=pattern, locale=locale) is 
        
        assert ShortValidator.get_instance().validate(xxxx) is None
        assert ShortValidator.get_instance().validate(value=xxxx, locale=locale) is None
        assert ShortValidator.get_instance().validate(value=xxxx, pattern=pattern) is None
        assert ShortValidator.get_instance().validate(value=xxxx, pattern=pattern, locale=locale) is None

        assert ShortValidator.get_instance().is_valid(xxxx) is False
        assert ShortValidator.get_instance().is_valid(value=xxxx, locale=locale) is False
        assert ShortValidator.get_instance().is_valid(value=xxxx, pattern=pattern) is False
        assert ShortValidator.get_instance().is_valid(value=xxxx, pattern=pattern, locale=locale) is False
