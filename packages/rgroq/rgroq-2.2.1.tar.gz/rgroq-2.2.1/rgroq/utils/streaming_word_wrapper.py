...
class StreamingWordWrapper:
    ...
    def streamOutputs(self, streaming_event, completion, openai=False):
        ...
        def finishOutputs(wrapWords, chat_response, terminal_width=terminal_width):
            ...
            tokens_per_second = self.tokens["completion"] / elapsed_time if elapsed_time > 0 else 0

            print("\n" + "─" * terminal_width)
            print(f"Model: {self.model_name} | Temperature: {self.temperature:.2f}")
            print(f"Tokens - Prompt: {self.tokens['prompt']} | Completion: {self.tokens['completion']} | Total: {self.tokens['total']}")
            print(f"Speed: {tokens_per_second:.1f} tokens/sec | Time: {elapsed_time:.2f}s")
            # auto pager feature
        ...
        for event in completion:
            if not streaming_event.is_set() and not self.streaming_finished:
                # Extract Groq metadata from first chunk
                if first_chunk and isinstance(event, dict):
                    ...
                    if "choices" in event and len(event["choices"]) > 0:
                        ...
                        if "temperature" in choice:
                            self.temperature = choice["temperature"]
                ...
                if openai:
                    ...
                    if isinstance(event, dict):
                        ...
                        if 'temperature' in event:
                            self.stats['temperature'] = event['temperature']
                    ...
                    # Update completion tokens for non-first chunks
                    if not first_chunk and "usage" in event:
                        self.tokens["completion"] = event["usage"].get("completion_tokens", self.tokens["completion"])
                        self.tokens["total"] = event["usage"].get("total_tokens", self.tokens["total"])
                else:
                    # vertex ai
                    answer = event.text
                # transform
