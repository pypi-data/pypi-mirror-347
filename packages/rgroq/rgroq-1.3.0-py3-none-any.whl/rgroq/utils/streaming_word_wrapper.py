...
class StreamingWordWrapper:
    ...
    def streamOutputs(self, streaming_event, completion, openai=False):
        ...
        def finishOutputs(wrapWords, chat_response, terminal_width=terminal_width):
            ...
            print("\n" + "─" * terminal_width)
            print(f"Tokens: {self.token_count} | Speed: {tokens_per_second:.1f} tokens/sec", end='')
            if 'temperature' in self.stats:
                print(f" | Temperature: {self.stats['temperature']}", end='')
            print()
            # auto pager feature
        ...
        for event in completion:
            if not streaming_event.is_set() and not self.streaming_finished:
                ...
                if openai:
                    ...
                    if 'stats' in event:
                        if 'tokens' in event['stats']:
                            self.token_count = event['stats']['tokens']
                        if 'temperature' in event['stats']:
                            self.stats['temperature'] = event['stats']['temperature']
                else:
                    # vertex ai
                    answer = event.text
                # transform
