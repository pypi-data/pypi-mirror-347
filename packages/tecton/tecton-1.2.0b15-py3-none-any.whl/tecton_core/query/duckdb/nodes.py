...
def _compute_delta_partitioning(self: "DeltaOfflineStoreScanNode") -> Partitioning:
    ...
    * partition 99: user_id [Y - Z]
    ...
    For efficiency we use only one time-based (physical) partition from Delta table for sample.
    However, if that wouldn't be sufficient (sample won't be representative)
    we should sample multiple time-based (physical) partitions and merge the results.
    """
    ...
    # use only one physical partition
    time_filter = pendulum.Period(self.partition_time_filter.end, self.partition_time_filter.end)
    ...
    # only supporting single column join key for now
    boundaries = pa_ds.take(sample_indexes, columns=[fdw.join_keys[0]])
    boundaries = sorted(boundaries.to_pydict()[fdw.join_keys[0]])
