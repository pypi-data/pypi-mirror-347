...
class Papr(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        - `api_key` from `PAPR_API_KEY`
        - `bearer_token` from `PAPR_BEARER_TOKEN`
        """
        if api_key is None:
            api_key = os.environ.get("PAPR_API_KEY")
        if api_key is None:
            raise PaprError(
                "The api_key client option must be set either by passing api_key to the client or by setting the PAPR_API_KEY environment variable"
            )
        self.api_key = api_key

        if bearer_token is None:
            bearer_token = os.environ.get("PAPR_BEARER_TOKEN")
        if bearer_token is None:
            raise PaprError(
                "The bearer_token client option must be set either by passing bearer_token to the client or by setting the PAPR_BEARER_TOKEN environment variable"
            )
