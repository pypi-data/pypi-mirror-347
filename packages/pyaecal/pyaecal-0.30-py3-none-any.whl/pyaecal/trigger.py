...
class Trigger(Function):
    ...
    def evaluate(self, data):
        ...
        if self.trigger_pre:
            if self.trigger_pre not in data.columns:
                ...
                pre_condition = (
                    pl.col(f"{self.trigger_pre}_mod") > 0 if self.pre_up else pl.col(f"{self.trigger_pre}_mod") < 0
                )
        ...
        if self.trigger_post:
            if self.trigger_post not in data.columns:
                ...
                post_condition = (
                    pl.col(f"{self.trigger_post}_mod") > 0 if self.post_up else pl.col(f"{self.trigger_post}_mod") < 0
                )
    ...
    def __change(self, name, data):
        ...
        data = data.with_columns(
            ...
            (pl.col(name) - pl.col(name).shift(1)).alias(f"{name}_mod")
        )
