...
class Temp:
    ...
    def WIP_FOLDER():
        ...
        if not WIP_FOLDER:
            ...
            WIP_FOLDER = DevPathHandler.get_project_abspath() + 'yta_wip/'
...
class BaseTemp:
    ...
    def log_filename(
        ...
        if not hasattr(self, '_log_filename'):
            self._log_filename = f'{self.temp_folder_abspath}/yta_temp_register.log'
    ...
    def create_file(
        ...
        mode = (
            ...
            if (
                content is None or
                # TODO: Create ParameterValidator.validate_mandatory_bytes
                PythonValidator.is_instance(content, bytes)
            ) else
