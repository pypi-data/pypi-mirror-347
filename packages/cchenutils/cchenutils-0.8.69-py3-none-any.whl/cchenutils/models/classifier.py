...
class Classifier:
    ...
    def load_models(self):
        ...
        for fold in range(self.configs['num_folds']):
            model = (AutoModelForSequenceClassification
                     .from_pretrained(self.model_name, subfolder=str(fold))
                     .to(self.device))
            model.eval()
            models.append(model)
        return models
    ...
    def predict(self, text, batch_size=32):
        ...
        for i in trange(num_batches, desc=self.model_name.split('/')[-1]):
    ...
    def get_config(model_name):
        match model_name:
            case 'phantomkidding/bertweet-large-bragging':
...
class Bragging(Classifier):
    """
    @inproceedings{jin2022automatic,
