...
const getSidebarPlaylist = async () => {
    /* Get all playlist for sidebar one at a time 
       Using counter as index guidance.  
       If counter == len(allModules), then noMoreModule is set to True and break out.
    */
  
    document.querySelector('#insertPlaylist').innerHTML = '';
    let counter = 0;
    let retry = 0;
    let retryMax = 3;

    while (true) {
        const data = await commons.postData("/api/v1/playlist",  
                      {remoteController: sessionStorage.getItem("remoteController"), counter: counter})

        if (data.playlist) {
            document.querySelector('#insertPlaylist').innerHTML += data.playlist;
            counter++;
...
window.getSidebarPlaylist = getSidebarPlaylist;
