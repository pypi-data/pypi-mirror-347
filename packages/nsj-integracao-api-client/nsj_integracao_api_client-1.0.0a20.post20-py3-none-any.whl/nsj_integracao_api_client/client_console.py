...
class ClientConsole:
    ...
    def get_integrador(self, injector, env : integrador.Environment,  forca_tenant: int = None) -> integrador.IntegradorService:

        integrador.out_func = self.mensagem
        self._integrador_service = integrador.IntegradorService(injector, self, env, forca_tenant)
    ...
    def executar_integracao(self, entidades: List[str], env: integrador.Environment, forca_tenant: int = None):
        with InjectorFactory() as injector:
            if entidades:
    ...
    def executar_carga_inicial(self, entidades: list, env: integrador.Environment, forca_tenant: int = None):
    ...
    def _imprime_texto_saida(self, mensagem: str):
    ...
    def modo_janela(self):
        ...
        try:
            with InjectorFactory() as injector:
                ...
                self.add_listener(self._imprime_texto_saida)
    ...
    def modo_interativo(self):
        ...
        while True:
            try:
                ...
                self.mensagem(f"\033[91mErro: {e}\033[0m")
            except Exception as e:
                self.mensagem(f"\033[91mErro: {e}\033[0m")
