...
class IntegradorService():
    ...
    @medir_tempo("Carga inicial", out_func, color=94 if _is_console else 0)
    def executar_carga_inicial(self, entidades: list):
        ...
        try:
            ...
            for entidade in entidades_carga_inicial:
                ...
                _filtros_integracao = self._filtros_integracao(entidade)
                filters = _filtros_particionamento | _filtros_integracao
                search_query = None
                ...
                _dao.atualiza_ultima_integracao(entidade, _entidades_filtros_integracao[entidade])
    ...
    @medir_tempo("Integração", out_func, color=94 if _is_console else 0)
    def executar_integracao(self):
        ...
        try:
            ...
            entidades_pendentes = {entidade: entidades_pendentes[entidade] for entidade in _entidades_integracao if entidade in entidades_pendentes.keys()}
    ...
    @medir_tempo("Verificação de integridade", out_func, color=94 if _is_console else 0)
    def executar_verificacao_integridade(
        ...
        try:
            ...
            for entidade in reversed(entidades_verificacao):
                ...
                # if not entidade in ['ns.df_docfis']:
                #     continue
            ...
            if _diferencas:
                self._log(self._color("Ocorreram diferenças na checagem da integridade, verifique a saída.\r\n", 93, _is_console))
