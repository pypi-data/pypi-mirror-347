...
def visualize_preprocessed_data(
    ...
    try:
        ...
        gdf_map.plot(ax=ax, markersize=point_size, alpha=point_alpha, color=point_color)
        ...
        if show_background_map:
            try:
                ...
                print(
                    "Please ensure you have contextily installed: pip install contextily"
                )
        ...
        if fig_format.lower() in ["pdf", "svg"]:
            plt.savefig(spatial_viz_path, bbox_inches="tight", format=fig_format)
        else:
            plt.savefig(
    ...
    try:
        if time_col in gdf.columns and pd.api.types.is_datetime64_any_dtype(gdf[time_col]):
            ...
            # Convert to pandas datetime explicitly to ensure compatibility
            if not isinstance(gdf[time_col].iloc[0], pd.Timestamp):
                gdf[time_col] = pd.to_datetime(gdf[time_col])
                
            # Use safer date conversion
            try:
                gdf["date"] = gdf[time_col].dt.date
            except:
                # Alternative approach if .dt.date fails
                gdf["date"] = pd.to_datetime(gdf[time_col]).dt.floor('D')
                
            time_series = gdf.groupby("date").size()
            
            # Ensure time_series index is proper datetime for plotting
            ...
            if fig_format.lower() not in ["pdf", "svg"]:
                plt.savefig(time_viz_path, bbox_inches="tight", format=fig_format)
            else:
                plt.savefig(
