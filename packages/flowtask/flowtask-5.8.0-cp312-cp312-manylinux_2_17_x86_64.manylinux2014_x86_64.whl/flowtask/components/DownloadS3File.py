...
class DownloadS3File(Boto3Client, FlowComponent, PandasDataframe):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        self.bucket_name: Optional[str] = kwargs.get('bucket_name', None)
        ...
        self._config_section = kwargs.pop('config_section', 'default')  # This will use AWS_CREDENTIALS['my_aws_config']
    ...
    async def start(self, **kwargs):
        ...
        if not self.credentials:
            self.credentials = AWS_CREDENTIALS.get(self._config_section, None)
        if not self.credentials:
            raise ValueError(
                f'Credentials not found for section {self._config_section}.'
            )
        if self.previous:
