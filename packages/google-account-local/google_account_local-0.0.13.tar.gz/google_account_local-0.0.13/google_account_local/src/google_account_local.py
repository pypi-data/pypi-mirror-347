...
class GoogleAccountLocal(
    ...
    def __init__(self, is_test_data: bool = False) -> None:
        ...
        self.logger = Logger.create_logger(object=GoogleAccountLocalConstants.LoggerSetupConstants.GOOGLE_ACCOUNT_LOCAL_CODE_LOGGER_OBJECT)
    ...
    def authenticate(self, email: str):
        ...
        if not self.creds or not self.creds.valid:
            ...
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                # the old where: where="username=%s AND is_refresh_token_valid=TRUE AND system_id=%s",
                if auth_details:
                ...
                elif refresh_token and self.__check_if_expired(expiry):
    ...
    def __authorize(self, oauth_state: str = None) -> int:
        ...
        token__user_external_id = self.user_externals_local.token__user_externals.insert_or_update_user_external_access_token(
            user_external_id=user_external_id,
            username=self.user_email,
        ...
        if not auth_code:
            ...
            self.logger.error(
                ...
                f"Please check if you have chosen the correct Google account {self.user_email} in the browser opened."
            )
    ...
    def __check_if_expired(self, expiry: str) -> bool:
        """
        ...
        # Convert the expiry string to an offset-aware UTC datetime
        expiry_timestamp = datetime.strptime(expiry, "%Y-%m-%d %H:%M:%S.%f").replace(tzinfo=timezone.utc)
