...
def feves_auto(cube, angle_list, cube_ref=None, ncomp=1, algo=pca_annular, 
               nit_max=30, regul=True, fwhm=4, buff=1, thr=0.5, thr_per_ann=False, 
               n_frac=6, asizes=None, n_segments=None, thru_corr=False, 
    ...
    algo: function, opt, {vip_hci.pca.pca_annular, vip_hci.nmf.nmf_annular}
        Either PCA or NMF in concentric annuli, used iteratively.
    nit_max: int, opt
        Max. number of iterations at each fractionation level. After each 
        iteration, a cross-correlation criterion is calculated between the 
        image obtained with the best circumstellar signals (CS) estimate 
        subtracted from the cube and the image obtained with CS left in the 
        cube. If the CS estimate is good, cross-correlation should be very low 
        (the first image should mostly contain random noise). If the criterion
        is met, the algorithm moves to the next fractionation level, otherwise 
        it runs up to `nit_max` iterations before moving to the next 
        fractionation level. Recommended value > 20.
    buff: float, opt
        Radial buffer expressed in fwhm, used to smooth final image. The feves 
        algorithm will be repeated int(buff*fwhm), with radial increments of 
        one pixel for the inner mask size. The final results are then the 
    ...
    regul: bool, opt
        ...
        kernel of fwhm/2 - this can help to recover faint signals, avoid 
        getting sharp edges, and acts as spatial regularization.
    ...
    if nproc>1:
        res = pool_map(nproc, _do_one_buff, iterable(range(buffer)), cube, 
                       angle_list, ref_cube, algo, nit_max, thr, thr_per_ann, 
                       radius_int, fwhm, asizes, n_segments, n_neigh,
                       thru_corr, psfn, n_br, interp_order, strategy, delta_rot, 
                       ncomp, svd_mode, init_svd, min_frames_lib, 
                       max_frames_lib, tol, scaling, imlib, interpolation, 
        ...
        for bb in range(buffer):
            res = _do_one_buff(bb, cube, angle_list, ref_cube, algo, nit_max, thr, 
                               thr_per_ann, radius_int, fwhm, asizes, 
                               n_segments, n_neigh, thru_corr, psfn,
                               n_br, interp_order, strategy, delta_rot, ncomp, 
                               svd_mode, init_svd, min_frames_lib,
...
def _do_one_buff(bb, cube, angle_list, ref_cube, algo, n_it, thr, thr_per_ann, 
    ...
    while it_f < n_it_frac:
        cond3, cond4 = False, False # initialize these values
        for pp, npc in enumerate(ncomp[ai][(it_f%n_it_frac)//n_it]):
           ...
           # write_fits("TMP_derot_msig_image.fits",sig_image)
            ...
            cc_map = correlate2d(frame, frame_nd, mode='same')
            cc_peak = np.amax(cc_map)
            arg_max = np.unravel_index(np.argmax(cc_map), cc_map.shape)
            mask_cc = mask_circle(cc_map, 2*fwhm, np.nan, cy=arg_max[0], 
                                  cx=arg_max[1])
            ...
            #tmp=frame.copy()
            #frame[np.where(frame>0)] = frame[np.where(frame>0)]/thru_2d[np.where(frame>0)]            
            # JUST APPLY IT TO FRAME_ND
            frame_nd[np.where(frame_nd>0)] = frame_nd[np.where(frame_nd>0)]/thru_2d[np.where(frame_nd>0)]
