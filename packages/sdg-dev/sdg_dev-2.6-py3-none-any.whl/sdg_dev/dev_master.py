        ...
        try:
            ...
            while c.is_alive():
                ...
                if type(ack) is DevBadCode:
                    if safe:
                        self.log.info(f"Safe '{self._name}' Answer on cmd{c.name} is {c.ack}")
                        return None
                    raise DevException(f"DevException '{self._name}' Answer on cmd{c.name} is", badcode=c.ack)
                if ack is not None:
                    return ack
            if safe:
                self.log.info(f"Safe '{self._name}' No answer to cmd{c.name}!")
                return None
            raise DevException(f"DevException '{self._name}' No answer to cmd{c.name}! [{dump(rx)}]")
        except IOError as e:
    ...
    def check_tx(self):
        ...
        if self.live and time() > self.time:
            ...
            if self.live:
                ...
                repit = '' if self.live > self.REMIX else f"?repit{self.REMIX - self.live + 1}"
        ...
        # адрес совпал
            ...
            if rx and rx[0] == self.msg[0] | 0x80:
                ...
                # неправильный размер сообщения
                    ...
                    self.log.warning(f"<ack{self.name}!size wait({waitsize})!={rxlen}")
