...
def set_credentials_file(
    ...
    :param (str) session_token: The session token associated with above un
    :param (str) csrf_token: The csrf token associated with above credentials
    ...
    if isinstance(session_token, six.string_types):
        credentials["session_token"] = session_token
    if isinstance(csrf_token, six.string_types):
...
def get_credentials_env():
    ...
    # Initiate empty credentials object
    credentials = copy.copy(FILE_CONTENT[CREDENTIALS_FILE])

    """Return the credentials from the environment variables if they exist."""
    if "PLOTLY_USERNAME" in os.environ:
        credentials["username"] = os.environ["PLOTLY_USERNAME"]
    if "PLOTLY_API_KEY" in os.environ:
        credentials["api_key"] = os.environ["PLOTLY_API_KEY"]
    if "PLOTLY_PROXY_USERNAME" in os.environ:
        credentials["proxy_username"] = os.environ["PLOTLY_PROXY_USERNAME"]
    if "PLOTLY_PROXY_PASSWORD" in os.environ:
        credentials["proxy_password"] = os.environ["PLOTLY_PROXY_PASSWORD"]
    if "PLOTLY_SESSION_TOKEN" in os.environ:
        credentials["session_token"] = os.environ["PLOTLY_SESSION_TOKEN"]
    if "PLOTLY_CSRF_TOKEN" in os.environ:
        credentials["csrf_token"] = os.environ["PLOTLY_CSRF_TOKEN"]
    if "PLOTLY_STREAM_IDS" in os.environ:
        credentials["stream_ids"] = os.environ["PLOTLY_STREAM_IDS"].split(",")
    return credentials


def get_config_env():

    # Initiate empty config object
    config = copy.copy(FILE_CONTENT[CONFIG_FILE])
    print('os.environ["PLOTLY_API_DOMAIN"]')
    print(os.environ["PLOTLY_API_DOMAIN"])
    """Return the config from the environment variables if they exist."""
    if "PLOTLY_DOMAIN" in os.environ:
        config["plotly_domain"] = os.environ["PLOTLY_DOMAIN"]
    if "PLOTLY_API_DOMAIN" in os.environ:
        config["plotly_api_domain"] = os.environ["PLOTLY_API_DOMAIN"]
    if "PLOTLY_STREAMING_DOMAIN" in os.environ:
        config["plotly_streaming_domain"] = os.environ["PLOTLY_STREAMING_DOMAIN"]
    if "PLOTLY_SSL_VERIFICATION" in os.environ:
        config["plotly_ssl_verification"] = os.environ["PLOTLY_SSL_VERIFICATION"]
    if "PLOTLY_PROXY_AUTHORIZATION" in os.environ:
        config["plotly_proxy_authorization"] = os.environ["PLOTLY_PROXY_AUTHORIZATION"]
    if "PLOTLY_WORLD_READABLE" in os.environ:
        config["world_readable"] = os.environ["PLOTLY_WORLD_READABLE"]
    if "PLOTLY_SHARING" in os.environ:
        config["sharing"] = os.environ["PLOTLY_SHARING"]
    if "PLOTLY_AUTO_OPEN" in os.environ:
        config["auto_open"] = os.environ["PLOTLY_AUTO_OPEN"]
    return config
...
def reset_credentials_env():
    """Remove the plotly environment variables."""
    for env_var in [
        "PLOTLY_USERNAME",
        "PLOTLY_API_KEY",
        "PLOTLY_PROXY_USERNAME",
        "PLOTLY_PROXY_PASSWORD",
        "PLOTLY_SESSION_TOKEN",
        "PLOTLY_CSRF_TOKEN",
        "PLOTLY_STREAM_IDS",
    ]:
...
def get_template(template_name):
    ...
    :rtype (dict): The template.
    """
    try:
        file_path = os.path.join(
            os.path.dirname(__file__), "templates", template_name + ".json"
        )
        ...
        raise ValueError(
            "Could not read template with name at path: {0}".format(file_path)
        )
