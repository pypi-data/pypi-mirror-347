...
def createLineChart(df, chartDefinition, colors):
    ...
    if len(chartDefinition['metrics']) == 1:                
        ...
        if 'options' in chartDefinition:
            if 'line-width' in chartDefinition['options']:
                fig.update_traces(line_width=int(chartDefinition['options']['line-width']))
        ...
        #form our line width
        try:
            lineWidth = int(chartDefinition['options']['line-width'])
        except:
            lineWidth = 1
    ...
    if 'options' in chartDefinition:
        ...
        if 'horizontal-grid-lines' in chartDefinition['options']:
            ...
            if chartDefinition['options']['horizontal-grid-lines']['show'] == 'false':
                fig.update_yaxes(showgrid=False)
            else:
                fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor=chartDefinition['options']['horizontal-grid-lines']['color'])

        if 'vertical-grid-lines' in chartDefinition['options']:
            #decide how to show them
            if chartDefinition['options']['vertical-grid-lines']['show'] == 'false':
                fig.update_xaxes(showgrid=False)
            else:
                fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor=chartDefinition['options']['vertical-grid-lines']['color'])
    ...
    if 'options' in chartDefinition:
        ...
        if 'legend-position-y' in chartDefinition['options']:
            legend_position_y = chartDefinition['options']['legend-position-y']            
