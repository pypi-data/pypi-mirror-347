            ...
            match args[1]:
                ...
                case '-k': # use the kept resultset from previous succesfull run
                     ...
                     if len(resultset)==0:
                            ...
                            reread=False
                case _:
                     print("""getTrades v1.6
Read trades and calculate overall profit/loss
usage: python getTrades.py [option]\n
where [option] can be:
  -d [#]  delete a config section (asks for config number if # ommitted)
...
When no option is provided, a standard run will be executed, 
- when more than one config excist, a config number is asked to run the program with.
...
pre-requisites: https://pypi.org/project/pipx/
