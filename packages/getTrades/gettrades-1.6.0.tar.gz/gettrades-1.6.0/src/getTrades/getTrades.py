...
if debug: print('import hashlib')
import hashlib

if debug: print('import hmac')
import hmac

if debug: print('import json')
import json

if debug: print('import pandas')
import pandas as pd # pip install pandas

if debug: print('import websockets')
import websockets # pip install websockets

if debug: print('import StringIO')
from io import StringIO

if debug: print('import threading')
import threading

# if debug: print('import logging')
# import logging

if debug: print('import datetime')
from datetime import datetime

if debug: print('import websockets')
from websockets.sync.client import connect

# if debug: print('import pprint')
# from pprint import pprint

if debug: print('import base64')
import base64

if debug: print('import time')
import time

if debug: print('import asyncio')
import asyncio

if debug: print('import sys')
import sys

if debug: print('import tabulate') 
from tabulate2 import tabulate # pip install tabulate2

if debug: print('import keyring') 
import keyring # pip install keyring

# if debug: print('import configparser')
# import configparser

# if debug: print('import jinja2')
# import jinja2 # pip install Jinja2

if debug: print('import os')
import os

if debug: print('import configparser-crypt')
from configparser_crypt import ConfigParserCrypt # pip install configparser-crypt

# if debug: print('import yubico')
# import yubico   # pip install python-yubico
# 
# from mfa import MFA # pip install mfa-authenticator
# if debug: print('import mfa-authenticator')
# 
# import qrcode # pip install qrcode
# if debug: print('import qrcode')
...
async def send_message(messages, responses,debug=False):
    ...
    try:
        #\033[<N>A
        if not debug:x=x+1 if x<3 else -1;print(twister[x],'\033[1A')
        if debug: print('-entry---',threading.current_thread().name,'-----------------------------------------------------')
        async with websockets.connect(uri) as websocket:
            ...
            for cmd in messages:
                ...
                if cmd == messages[-1]:
                    if debug: print('! last')
                    last=True
                if debug: print('> sending   : ', threading.current_thread().name, cmd)
                #print(f'> {cmd}')
                ...
                while receiving:
                    ...
                    if debug: print('< receiving : ', threading.current_thread().name, message )
                    ...
                    if debug: print(json.loads(message)['response'] or 'x')
    ...
    #print(responses)
    if not debug:x=x+1 if x<3 else -1;print(twister[x],'\033[1A')
    if debug: print('-exit----',threading.current_thread().name,'-----------------------------------------------------')
    return responses


######## 
def create_signature(timestamp: int):
    if debug: print('> create_signature() |','\r')
...
def authenticate(requestId):
    if debug: print('> def authenticate() |','\r')
    access_window = 10000
...
########
def tx2df(responses, col_types):
    if debug: print('> tx2def()')
    if debug: print('responses: ',type(responses))
    if debug: print('col_types: ',type(col_types))
...
def group_tokens(df):
    ...
    r=['type','sentAmount','receivedCurrency','receivedAmount']
    mtch = (df_a['receivedCurrency']==coin)
    print(df_a.loc[mtch,r])
    print(df_a.loc[mtch,r].agg('sum'))


    r=['executedAt','type','sentCurrency','sentAmount','receivedCurrency','receivedAmount','price']
    mtch = (df['receivedCurrency']==coin)

    #print(df.loc[mtch,r].sort_values('executedAt'))    
    dfb.index.name='symbol'
    ...
    try:
        ...
        if args is not None:
            match args[1]:
                case '-d': # delete
                    if len(args)==3:
                        selected_Config_Nr=int(args[2])
                        delete_config_section(selected_Config_Nr);exit(0)
                    else:
                        delete_config_section();exit(0)
                case '-a': # add
                     write_new_config_section();exit(0)
                case '-l': # list all
                     list_all_config_sections();exit(0)
                case '-u': # use specific section nr
                    if len(args)==3:
                        selected_Config_Nr=int(args[2])
                    else:
                        print('ERROR: -u needs a config section number')
                        exit(0)
                case '-k': # use the kept resultset from previous succesfull run
                     ...
                     if len(resultset)==0:
                            ...
                            reread=False
                case _:
                    print('getTrades 1.6 Read trades and calculate overall profit/loss')
                    print('usage: python getTrades.1.6.py3 [option]')
                    print('where [option] can be:')
                    print('     -d [#]  delete a config section optional by adding a number')
                    print('     -a      add a config section')
                    print('     -l      list all config section')
                    print('     -u #    use the number of config section')
                    print('     -k      (advanced) use saved resultset, don\'t query BV again')
                    print('')
                    print('When no option is provided, a standard run will be executed')
        ...
        if reread:        
            ...
            if debug: print('#################### 337')
            if debug: print('length of task queue: ',len(tasks))
        ...
        if reread:
            ...
            for t in tasks:
                t.join()
                if debug: print(f'< task {t.name} done')
            if debug: print('#################### 386')
        tasks = []
        ...
        if len(y) == 1: 
            ...
            #df_wallet_balance.loc['EUR']=[0,0,0]
            #df_wallet_balance.drop(['EUR'])
            if debug: print('########## print(df_wallet_balance)')
            if debug: print(df_wallet_balance)
            if debug: print()
        else:
            raise Exception('unexpected return length (400)')
        ...
        if len(y) >= 0: df_trades = tx2df(y, col_types) # tx history

        # save_dataset('df_trades.hdf',df_trades)

        ################ add now-ticker-price
        if debug: print('############### add now-ticker-price (450)')
        
        symbols = df_wallet_balance.loc[df_wallet_balance['Balance-total'] > 0].index.to_list() #list of symbols
        # nds 9may25 symbols.append('EUR')
        if debug: print('########## symbols in wallet')
        ...
        if reread: # reread if no previous resultset file
            ...
            cmd_seq[cmdID] = []
            # if debug: print('###############')
            # if debug: print(type(resultset), resultset)
            # if debug: print('###############')
            requestId = 7
            for i in symbols:
        ...
        if debug: print('############### 475 ticker price > df_wallet x EURO')
        x=7 # getTickerPrice
        y = [json.loads(r) for r in resultset if json.loads(r)['requestId'] == x]
        ...
        for i in y:
            ...
            if debug: print({i["response"]["symbol"]:float(i["response"]["price"])})
            itms.append({i["response"]["symbol"]:float(i["response"]["price"])})

        if len(itms) >= 0: df_price = tx2df([itmsbase],col_types) # unrealized
...
def read_config(selected_Config_Nr=None):
    ...
    #print('pk:',keyring.get_password("system", "trades"))
    conf_file.aes_key = base64.b64decode(keyring.get_password("system", "trades"))
    ...
    if selected_Config_Nr is None:
        ...
        if len(conf_file_sections)==1:
            print('NOTICE: only 1 config section, ignoring -u option')
            selected_Config_Nr=0
...
def write_new_config():
    ...
    keyring.set_password("system", "trades", base64EncodedStr)
...
if __name__ == "__main__":
    ...
    uri = "wss://ws.bitvavo.com/v2/"  # Change to your WebSocket server URI
    # get all data from BV into dataframes
    if debug: print(type(sys.argv),sys.argv)
    if len(sys.argv) > 1:
        args=sys.argv
    else:
        args=None
    df_trades, df_wallet = asyncio.run(main(uri, args))

    if debug: print('vvvv na main() vvvvvvvvvvv')
    if debug: print(df_trades.columns)
    r=['type', 'receivedCurrency',
                 'receivedAmount','priceCurrency', 'priceAmount',
                 'sentCurrency', 'sentAmount', 'feesCurrency', 'feesAmount']
    if debug: print(df_trades[r].head(8))

    df = group_tokens(df_trades)
    #print(df[r])
    s1=['receivedCurrency','sentCurrency']
    #s1=['type']
    s2=['receivedCurrency','priceCurrency','sentCurrency','feesCurrency']
    u1=['receivedAmount','sentAmount']

    r1=[ 'type',
        'receivedCurrency','receivedAmount',
        'priceCurrency','priceAmount',
        'sentCurrency', 'sentAmount',
        'feesCurrency', 'feesAmount']
    ...
    s1=['receivedCurrency','sentCurrency']
    #s1=['type']
    s2=['receivedCurrency','sentCurrency','feesCurrency']
    u1=['receivedAmount','sentAmount']
    
    v1=['sentAmount','receivedAmount']
    v2=['receivedAmount']
    #t1=['receivedAmount','sentAmount']
    i1=['sentCurrency','receivedCurrency']
    i2=['receivedCurrency']
    #s3=['type','sentCurrency','sentAmount','receivedCurrency','receivedAmount']
    c1=['type']

    #print(df[r2].head(30))
    #print(pd.pivot_table(df[r], values=t1, index=s2,columns='type', aggfunc="sum", dropna=False).dropna(thresh=2))
    #df2=pd.pivot_table(df[r2], values=v1, index=i1, aggfunc="sum", dropna=False).dropna(thresh=2)
    
    #df3=pd.pivot_table(df[r2], values=v2, index=i2, columns=c1, aggfunc="sum", dropna=False).dropna(thresh=2)
    ...
    #df4=df.loc['ADA'][r5]
    #df5.loc['_TOTALS']=df5[['receivedAmount','receivedAmount+fees','sentAmount','sentAmount+fees','feesAmount']].agg('sum')
    if debug: print(df5.columns)
    r5=['receivedAmount+fees', 'sentAmount+fees', 'feesAmount']
    if debug: print('df5.len: ',len(df5))

    # we have all realized tokens and realized euro's here
    #exit(-1)



    # add EUR_wallet value (real euro's in wallet) to EUR(realized)
    #print('############### 703')
    #print(df)
    #print(df.columns)

    '''
    u=['index', 'transactionId', 'executedAt', 'type', 'debitCurrency',
       'debitAmount', 'address', 'priceCurrency', 'priceAmount',
       'creditAmount', 'feesCurrency', 'feesAmount', 'EUR(C-realized)',
       'EUR(D-realized)', 'TOK(C-realized)', 'TOK(D-realized)']

    # get wallet euro's vs trade euro's separted
    wallet_df = df_wallet.index.isin(['EUR_wallet'])
    w=df_wallet[wallet_df]['EUR(unrealized)'].agg('sum')

    e=['type','EUR(C-realized)', 'EUR(D-realized)','creditAmount']
    df.loc['EUR_wallet'] = \
      [len(df)+1, 0, time.time(), 'wallet', w,
       1, w, '', 3,
       w, '', 4, 0,
       w, 0,0]

    df_byType   = df[r].groupby(['type']).agg(agginstruction).reset_index().sort_values(['type']).set_index(['type'])
    df_bySymbol = df[r].groupby(['symbol']).agg(agginstruction).reset_index().sort_values(['symbol']).set_index(['symbol'])

    pd.options.display.float_format = '{:.2f}'.format

    ############ RENAME ##############
    values={'creditAmount':'EUR(realized)'}

    df_byType =  df_byType.rename(columns = values)
    df_bySymbol = df_bySymbol.rename(columns = values)
    
    ############ ADD UNREALIZED ##############
    # by type
    if debug: print('vvvv ADD UNREALIZED (df_wallet) vvvvvvvvvv')
    if debug: print(df_wallet)

    df_byType_ur   = df_wallet.agg('sum')['EUR(unrealized)']
    
    if debug: print('############### 702')
    bad_df = df_wallet.index.isin(['EUR_wallet'])
    df_wallet=df_wallet[~bad_df]

    print('############### 714')
    print(df_byType)

    df_byType['EUR(unrealized)'] =[0,0,0,0,0,df_wallet['EUR(unrealized)'].agg('sum'),0] # add column 'unrealized' - EUR_wallet
    

    t=['EUR(C-realized)','EUR(D-realized)','EUR(realized)','EUR(unrealized)']
    u=['EUR(realized)','EUR(unrealized)']
    if debug: print('############### 709')
    if debug: print(df_byType)
    df_byType.loc['_TOTALS']=df_byType[t].agg('sum') # add row totals for t
    df_byType['_TOTALS']=[0,0,0,0,0,df_byType.loc['wallet'][u].agg('sum'),0,df_byType.loc['_TOTALS'][u].agg('sum')] #add column grand_total 

    '''

    ############ ADD UNREALIZED ##############
    r5=['receivedAmount+fees', 'sentAmount+fees']
    r6=['Balance-total', 'EUR(unrealized)', 'price']
    df_bySymbol = pd.merge(df5, df_wallet, how='outer', left_index=True, right_on=['symbol'])


    r7=['symbol', 'receivedAmount', 'receivedAmount+fees', 'sentAmount','sentAmount+fees', 'feesAmount', 'price', 'Balance-available','Balance-inOrder', 'Balance-total', 'EUR(unrealized)']
    r7=['receivedAmount+fees', 'sentAmount+fees', 'EUR(unrealized)']
    
    df_bySymbol=df_bySymbol.set_index('symbol')
    #df_bySymbol.loc['_TOTALS']=df_bySymbol[['receivedAmount+fees','sentAmount+fees','EUR(unrealized)']].agg('sum')

    if debug: print(df_bySymbol.columns)
    ...
    r8=['Token Balance', 'EUR(realized)','EUR(unrealized)','EUR(total)']
    pd.options.display.float_format = '{:.2f}'.format
    #print(df_bySymbol[r8])
    #exit(1)
    if debug: print('#################### ')
    ...
    df_bySymbol['|'] = "|"
    #df_bySymbol_ur['BE-price']  = df_bySymbol_ur['EUR(realized)'].div(df_bySymbol_ur['Balance-total']).agg('abs') #BE price
    ...
    df_bySymbol=df_bySymbol.sort_values(by=['BE%','EUR(unrealized)','EUR(realized)'])
    

    #t=['EUR(realized)','EUR(unrealized)']
    df_bySymbol.loc['_TOTALS']=df_bySymbol[t+['EUR(total)']].agg('sum') # add row totals for t
    
    if debug: print(df_bySymbol[r9])
    ...
    #df_byType_txt = df_byType.map(txt_color_negative_red)
    ...
    #print(tabulate(df_byType_txt,headers=colnames,floatfmt=numfmt, tablefmt="fancy_grid"))
    #colnames=('EUR(C-realized)','EUR(D-realized)','EUR(realized)','EUR(unrealized)','_TOTALS')
    colnames=['EUR(realized)', 'EUR(unrealized)', '_TOTALS','price', 'Balance-total', 'BE-price', 'BE%']
