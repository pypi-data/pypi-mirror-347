...
@mcp.tool()
async def generate_taobao_title(image_urls: List[str], temperature: float = 0.7, ctx: Context = None) -> TitleResponse:
    """根据商品图片生成优化的淘宝商品标题。
    ...
    Args:
        image_urls: 商品图片URL列表，至少提供一张图片
        temperature: 控制生成结果的随机性（0.0-1.0），默认为0.7
    ...
    """
    if ctx:
        await ctx.info(f"开始处理淘宝标题生成请求，共 {len(image_urls)} 张图片")
    
    # 将图片URL转换为base64编码
    base64_images = []
    for i, url in enumerate(image_urls):
        if ctx:
            await ctx.report_progress(i, len(image_urls), f"正在处理图片 {i+1}/{len(image_urls)}")
    ...
    if not base64_images:
        error_msg = "无法处理提供的图片URL，请检查URL是否有效"
        if ctx:
            await ctx.error("没有成功转换任何图片")
        raise ValueError(error_msg)
    
    if ctx:
        await ctx.report_progress(len(image_urls), len(image_urls), "图片处理完成，正在生成标题")
