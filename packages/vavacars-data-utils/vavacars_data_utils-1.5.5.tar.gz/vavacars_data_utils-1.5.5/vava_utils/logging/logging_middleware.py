...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    def _process(self, data: bytes) -> str:
        """
        Decode JSON, apply summaries, truncate by size, fallback to one-line.
        """
        ...
        try:
            ...
            if len(dumped) > self.max_log_size:
                return f"[json] payload too large ({len(dumped)} chars)"
            return dumped
        except Exception:
    ...
    def _log_info(
        ...
        req = self._process(req_bytes)
        if media_type in ALLOWED_MEDIA_TYPES:
            res = self._process(res_bytes)
        else:
            res = f"[{media_type}] - Not logged"
