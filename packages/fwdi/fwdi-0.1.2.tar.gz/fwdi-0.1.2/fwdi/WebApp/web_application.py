...
class WebApplication():
    ...
    def add_web_page(self, inst: gr.Blocks, path:str="/panel", is_auth:bool=False):
        from ..Presentation.DefaultControllers.http_auth import HttpAuthFWDI

        self.__log__(f"{__name__}:{path}:{inst}")
        if not is_auth:
            self.__app = gr.mount_gradio_app(self.__app, inst, path=path)
        else:
            self.__app = gr.mount_gradio_app(self.__app, inst, path=path, auth=HttpAuthFWDI.login)
