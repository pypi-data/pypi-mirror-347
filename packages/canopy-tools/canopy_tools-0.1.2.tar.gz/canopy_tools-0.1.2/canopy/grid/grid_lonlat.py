...
class GridLonLat(Grid):
    ...
    def __init__(self,
        ...
        super().__init__(axis0='lon', axis1='lat', gridop0=lon_gridop, gridop1=lat_gridop)
    ...
    def __repr__(self):
        if self.is_reduced('lon') or self.is_reduced('both'):
            lon_status = f"reduced (gridop = {self.axis_gridop['lon']})"
        else:
            lon_status = f"{self.lon_min} to {self.xmax['lon']} (step: {self.dx['lon']})"
        if self.is_reduced('lat') or self.is_reduced('both'):
            lat_status = f"reduced (gridop = {self.axis_gridop['lat']})"
        else:
            lat_status = f"{self.xmin['lat']} to {self.xmax['lat']} (step: {self.dx['lat']})"

        status = '\n'.join(["Longitude: ", lon_status, "Latitude: ", lat_status])
