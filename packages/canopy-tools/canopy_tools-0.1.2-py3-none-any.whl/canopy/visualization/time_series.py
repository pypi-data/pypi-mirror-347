...
def plot_time_series(fig, ax, time_series, layers, reverse_hue_style, ts_labels, colors, move_legend, legend_style, max_labels_per_col, rolling_size, no_style, **kwargs):
    """
    ...
    if len(time_series) > 1:
        ...
        combined_ts = pd.concat([df.assign(DataFrame=label) for df, label in zip(time_series, ts_labels)])
...
def make_time_series(fields: cp.Field | List[cp.Field], output_file: Optional[str] = None, layers: Optional[List[str] | str] = None, redop: Optional[str] = 'av',
                     make_diff: Optional[bool] = False, yaxis_label: Optional[str] = None, ts_labels: Optional[List[str]] = None, 
                     ...
                     x_fig: Optional[float] = 10, y_fig: Optional[float] = 10, **kwargs):
    """
    ...
    **kwargs
    ...
    if len(time_series) > 1:
        if ts_labels is None:
            raise ValueError("ts_labels must be defined for multiple time-series.")
        elif len(ts_labels) != len(time_series):
            raise ValueError("ts_labels should be of the same size as the number of time series.")
        
    # Check for different conditions
    if len(time_series) > 1:
        if legend_style != 'default' and no_style == False:
    ...
    # Plot
    if stacked is False:
        fig, ax = plot_time_series(fig, ax, time_series, layers, reverse_hue_style, ts_labels, colors, move_legend, legend_style, max_labels_per_col, rolling_size, no_style, **kwargs)
    else:
