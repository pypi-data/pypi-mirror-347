...
class LASSIE:
        ...
        **kwargs,
        ...
        def dispatcher(group: pd.DataFrame) -> pd.Series:
            ...
            if summarize_func is None:
                raise ValueError(
                    f"No summarization function defined for activity '{activity}'"
                )
        ...
        try:
            summary_df = (
                self.grouped.apply(dispatcher).reset_index()
            )
            
            summary_df = summary_df.rename(columns={"level_3": "metric"}, inplace=True)
            summary_df = summary_df.rename(columns={0: "value"}, inplace=True)
            
            self.grouped_summary = summary_df
            log_info(
    ...
    def inspect(self, plot_type: str = "distribution"):
        ...
        if plot_type == "distribution":
            ...
            log_info("[I] Distribution plot generated.", {"session_timestamp": self.timestamp})
