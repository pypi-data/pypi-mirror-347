...
def list_zip_contents(zip_path: str) -> Dict[str, int]:
    ...
    with zipfile.ZipFile(zip_path, "r") as zip_ref:
        return {
            name: zip_ref.getinfo(name).file_size
...
def parse_json_to_dfs(df: pd.DataFrame, activity_name_col: str = "activity_name") -> Dict[str, pd.DataFrame]:
    """
...
def unnest_trial_level_data(
    ...
    column_order = column_order or [
        "participant_id", "session_id", "group", "wave",
        "activity_id", "study_id", "document_uuid"
    ]
    all_trials = [
        trial
        for _, row in df.iterrows()
        for trial in row["content"].get("trials", [])
    ]
...
def summarize_common_metadata(x: pd.DataFrame, trials_expected: int) -> Dict[str, Any]:
    ...
    return {
        "activity_begin_iso8601_timestamp": x["activity_begin_iso8601_timestamp"].iloc[0],
        "n_trials": n_trials,
...
# pyreadr.write_rdata("my_data.RData", df, df_name="my_dataframe")

# # Write the DataFrame to an Rds file
# pyreadr.write_rds("my_data.Rds", df)
        else:
            raise ValueError(f"Unsupported file format: {format}")

        print(f"[EXPORT] DataFrame exported to: {file_name_with_extension}")
...
def summarize_by_group_key(
    scored_df_dict: Dict[str, pd.DataFrame],
    summary_func_map: Dict[str, Callable],
    **kwargs,
) -> Dict[str, pd.DataFrame]:
    """
    Applies group-specific summary functions to each scored DataFrame.

    Args:
        scored_df_dict (Dict[str, pd.DataFrame]): Scored dataframes by group key.
        summary_func_map (Dict[str, Callable]): Mapping from group key to summary function.
        kwargs: Optional arguments passed to each summary function.

    Returns:
        Dict[str, pd.DataFrame]: Summary dataframes by group key.
    """
    summary_result_dict = {}

    for group_key, df in scored_df_dict.items():
        summary_func = summary_func_map.get(group_key)

        if summary_func is None:
            print(f"[WARN] No summary function defined for '{group_key}', skipping.")
            continue

        summary_df = summary_func(df, **kwargs)
        summary_result_dict[group_key] = summary_df

    return summary_result_dict


def stack_datasets(dict_of_dfs, key_name='source'):
    """
    Stacks a dictionary of pandas DataFrames into one, adding the key as a column.

    Parameters:
    - dict_of_dfs (dict): Dictionary of {key: DataFrame}
    - key_name (str): Name of column to hold the dict key (e.g., dataset source)

    Returns:
    - pd.DataFrame: Stacked DataFrame
    """
    return pd.concat(
        [df.assign(**{key_name: key}) for key, df in dict_of_dfs.items()],
        ignore_index=True
    )

def get_grouped_df(df, activity_name_col):
    return {
        name: group.reset_index(drop=True)
        for name, group in df.groupby(activity_name_col)
    }