...
class LASSIE:
    ...
    def load(self, source_name: str, source_path: str = None, source_map: dict = None, activity_name_col="activity_name"):
        self.source_name = source_name

        if source_name == "multicsv":
            if activity_name_col is None:
                log_error("[L] Missing activity_name_col for multicsv load", {})
                raise ValueError("`activity_name_col` must be provided for source_name='multicsv'")
            if source_map is None:
                log_error("[L] Missing source_map for multicsv load", {})
                raise ValueError("`source_map` must be provided for source_name='multicsv'")
            ...
            if source_path is None:
                log_error("[L] Missing source_path for standard load", {"source_name": source_name})
                raise ValueError("`source_path` must be provided for source_name other than 'multicsv'")
    ...
    def score(self, scoring_func_map: Dict[str, List[Tuple[str, Callable]]] = settings.DEFAULT_FUNC_MAP_SCORING):
    ...
    def summarize(
        ...
        **kwargs,
        ...
        Parameters:
            grouping (List[str]): Group-by columns (must include activity_col).
            summary_func_map (Dict[str, Callable]): Map of task name -> summarization function.
            activity_col (str): Column used to determine which function to apply.
        ...
        def dispatcher(group: pd.DataFrame) -> pd.Series:
            if activity_col not in group.columns:
                raise KeyError(f"'{activity_col}' not found in group columns")

            activity = group[activity_col].iloc[0]
            summarize_func = summary_func_map.get(activity)

            if summarize_func is None:
                raise ValueError(f"No summarization function defined for activity '{activity}'")

            return summarize_func(group, **kwargs)

        try:
            summary_df = (
                self.flat_scored.groupby(grouping)
                .apply(dispatcher)
                .reset_index()
            )
            self.grouped_summary = _append_metadata(
                summary_df,
                _generate_metadata(self.flat_scored, summary_df)
            )
            log_info("[S] Summarization by activity completed.", {"session_timestamp": self.timestamp})
    ...
    def export(
        ...
        if self.grouped_summary is not None:
            for fmt in formats:
                export_dataframe(
                    self.grouped_summary,
                    export_dir / f"{file_basename}_summary",
                    format=fmt,
                )
    ...
    def whats_inside(self, verbose: bool = True) -> Dict[str, Union[str, int, bool, None]]:
        state = {
            ...
            "flat_scored_shape": self.flat_scored.shape if self.flat_scored is not None else None,
            "grouped_scored": self.grouped_scored is not None,
            "n_grouped_scored": len(self.grouped_scored) if self.grouped_scored else None,
            "grouped_summary": self.grouped_summary is not None,
            "grouped_summary_shape": self.grouped_summary.shape if self.grouped_summary is not None else None,
