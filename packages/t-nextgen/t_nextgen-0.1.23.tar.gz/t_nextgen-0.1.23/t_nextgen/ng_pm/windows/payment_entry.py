...
class PaymentEntryWindow(NextGenWindow):
    ...
    def insert_control_number_in_resub(self, control_number: str) -> None:
        ...
        """
        self.desktop_app.child_window(title="_txtDataEntry_116", control_type="Edit").set_text(control_number)

    def click_show_all_lines_check_box(self) -> None:
        """This method clicks on the show all lines check box."""
        check_box = self.desktop_app.child_window(title="chkShowAll", control_type="CheckBox")
        if check_box.get_toggle_state() == 0:
            try:
                check_box.toggle()
            except _ctypes.COMError:  # type: ignore[attr-defined]
                pass
    ...
    def check_bad_debt(self) -> bool:
        ...
        bad_debt_text = self.desktop_app.child_window(
        ...
        if bad_debt_text.exists():
            ...
            balance_window = self.desktop_app.child_window(title="BalanceControl", control_type="Window").wait(
                "visible", 10
            )
