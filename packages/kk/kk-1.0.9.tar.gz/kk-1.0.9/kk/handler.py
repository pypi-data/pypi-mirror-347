...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    def listdir(self, path):
        entries = self.app.scan_dir(path)
    ...
    def preview_audio(self, path):
        ...
        urls = [self.request.path] + [f'{Path(self.request.path).parent}/{urllib.parse.quote(x.name)}' for x in files]
        audios = [{
            'theme': '#fff',
...
<script>
    ...
    function prevImage() {
        ...
        html = tmpl.generate(url=urls[index], index=index, urls=json.dumps(urls))
        self.finish(html)
            ...
            if path.is_file():
                ...
                if self.opt.auth and not self.request.path.startswith('/disk/public'):
                    await self.db.visit.update_one(
                        {'path': name},
                        {'$set': {'path': name, 'uid': int(name.split('/')[0]), 'size': path.stat().st_size, }, '$inc': {'count': 1}},
                        upsert=True)
    ...
    async def post(self, name):
        ...
        if self.args.action == 'folder':
            ...
            if self.args.dirname.startswith('/'):
                dirpath = '/'.join(self.request.path.split('/')[2:(3 if self.opt.auth else 2)])
            else:
