...
class VoltusClient:
    ...
    def __init__(self, api_base_url: str, token: str, verify_requests: bool = True):
        ...
        response = requests.get(
            ...
            url=f"https://{self.url}/v1/datasets/examples/list",  # This endpoint does not require auth
            headers={
    ...
    def _handle_response(self, response: requests.Response, model_class=None, is_list_of_models=False):
        ...
        try:
            ...
            if json_data is None: # Should not happen for 2xx if server sends JSON, but as a fallback
                raise ValueError(f"Successful response but no JSON body: {response.text[:500]}")
            ...
            if model_class:
                if is_list_of_models:
                    if isinstance(json_data, list):
                        ...
                        raise ValueError(f"Expected a list for {model_class.__name__}, but got {type(json_data)}")
                else:
            ...
            if json_data and isinstance(json_data, dict) and "detail" in json_data:
                detail = json_data["detail"] # Use detail from server's JSON error response if available
            elif response_text:
        ...
        except json.JSONDecodeError: # Should be caught by the initial try for json_data
        ...
        except Exception as err: # Catch-all for other unexpected errors during processing
            raise Exception(f"An unexpected error occurred while handling response: {err}") from err

    def healthcheck(self) -> bool:
        try:
            response = requests.get(
                verify=self.verify_requests,
                url=f"http://{self.url}/v1/current_authenticated_user",
                headers={
                    "Authorization": f"Bearer {self.token}",
                    "accept": "application/json",
                },
            )
            print("Healthcheck response:", response.status_code, response.text)
    ...
    def train_model(
        ...
        response = requests.post(
            f"https://{self.url}/v1/machinelearning/train",
            headers={
                "Authorization": f"Bearer {self.token}",
                "accept": "application/json",
            },
        ...
        try:
            ...
            if isinstance(json_data, dict):
                ...
                return ClientApiResponse(message=msg, task_id=tid) # Pass string directly
...
if __name__ == "__main__":
    ...
    print(
        client.list_feature_functions(
            detailed=True, function_type="analytical_function"
        )
    )
