...
def animate_text(text, colorful=True, small=False):
    ...
    # Start with sparkles animation for both modes
    print_sparkles()
    
    # Multiple animations for colorful mode
    if colorful:
        animations = [
            lambda: (clear_screen(), print("\n" * (rows//4)), print_fancy_text(text)),
            lambda: (clear_screen(), 
                    print("\n" * (rows//4)), 
                    print_border(columns),
                    print("\n"), 
                    print_centered(f"✨ {gradient_text(' '.join(text))} ✨"), 
                    print("\n"),
                    print_border(columns)),
            lambda: (clear_screen(), 
                    print("\n" * (rows//4)), 
                    print_centered(f"★彡 {rainbow_color(' '.join(text), bold=True)} 彡★")),
        ]
        ...
        # Simple animations for non-colorful mode
        for _ in range(2):
            clear_screen()
            print("\n" * (rows//3))
            print("=" * columns)
...
def main():
    ...
    try:
        ...
        # Check for special cases (boaz or yuval)
        name_lower = name.lower()
        if name_lower == "boaz" or name_lower == "yuval":
