"""
`jobman` is a job management package is designed to submit and monitor jobs on remote machines. It is built on the top of the [dpdispatcher](https://docs.deepmodeling.com/projects/dpdispatcher) package.
...
Setting remote machines follow the [remote machine schema](https://thangckt.github.io/thutil_doc/schema_doc/config_remote_machine/).

1. Distribute jobs to single remote machines
...
- Use function [submit_job_chunk()](\#thutil.jobman.submit_job_chunk) to submit jobs to remote machines.

```python
from thutil.jobman import submit_job_chunk, Task
...
2. Distribute jobs to multiple remote machines
...
```python
from thutil.jobman import alff_submit_job_multi_remotes
from thutil.config import load_config
import asyncio
...
from thutil import THUTIL_ROOT
from thutil.config import validate_config
from thutil.pkg import create_logger
...
def submit_job_chunk(
    ...
    Args:
        mdict (dict): a dictionary contain settings of the remote machine. The parameters described in the [remote machine schema](https://thangckt.github.io/thutil_doc/schema_doc/config_remote_machine/). This dictionary defines the login information, resources, execution command, etc. on the remote machine.
...
async def alff_submit_job_multi_remotes(
    ...
    Args:
        multi_mdict (dict): the big_dict contains multiple `mdicts`. Each `mdict` contains parameters of one remote machine, which parameters as in the [remote machine schema](https://thangckt.github.io/thutil_doc/schema_doc/config_remote_machine/).
...
def _init_default_logger(logfile: str = _DEFAULT_LOG_FILE):
    ...
    time_str = time.strftime("%y%b%d_%H%M%S")  # "%Y%b%d_%H%M%S" "%Y%m%d_%H%M%S"
...
def validate_machine_config(machine_file: str):
    ...
    SCHEMA_ARG_MACHINE_FILE = f"{THUTIL_ROOT}/schema/schema_arg_remote_machine.yml"
