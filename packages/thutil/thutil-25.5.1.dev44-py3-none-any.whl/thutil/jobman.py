"""
`jobman` is a job manager package is designed to submit and monitor jobs on remote machines. It is built on the top of the [dpdispatcher](https://docs.deepmodeling.com/projects/dpdispatcher) package.
...
For general purpose, just need to use either [submit_job_chunk()](#submit_job_chunk) or [async_submit_job_chunk()](#async_submit_job_chunk) to submit jobs to remote machines. Setting remote machines follow the [remote machine schema](https://thangckt.github.io/thutil_doc/schema_doc/config_remote_machine/).
"""
...
SCHEMA_ARG_MACHINE = f"{THUTIL_ROOT}/schema/schema_arg_remote_machine.yml"
...
def validate_machine_config(machine_file: str):
    ...
    schema = yaml.safe_load(open(SCHEMA_ARG_MACHINE))
