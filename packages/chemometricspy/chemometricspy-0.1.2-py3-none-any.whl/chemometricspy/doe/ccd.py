...
class CentralCompositeDesign:
    ...
    def randomize(self, matrix):
        """
        Randomize the run order of the design matrix.

        Parameters:
        -matrix (dataframe): Real matrix with real levels

        Returns: 
        - pd.DataFRame: Randomized design matrix

        """
        randomized_matrix = matrix.sample(frac=1)
