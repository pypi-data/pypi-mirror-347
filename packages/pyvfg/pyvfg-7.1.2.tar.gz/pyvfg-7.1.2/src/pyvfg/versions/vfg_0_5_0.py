...
DUMMY_CONTROL_STATE_NAME = "__DUMMY_CONTROL_STATE__"
...
class BayesianNetwork(VFG):
    ...
    @staticmethod
    def from_file(file_path: str) -> BayesianNetwork:
        return BayesianNetwork.model_validate(json.load(open(file_path)))
...
class MarkovRandomField(VFG):
    ...
    @staticmethod
    def from_file(file_path: str) -> MarkovRandomField:
        return MarkovRandomField.model_validate(json.load(open(file_path)))
...
class POMDP(VFG):
    ...
    @staticmethod
    def from_file(file_path: str) -> POMDP:
        return POMDP.model_validate(json.load(open(file_path)))

    @property
    def transitions(self):
        return [f for f in self.factors if f.role == FactorRole.Transition]

    @property
    def likelihoods(self):
        return [f for f in self.factors if f.role == FactorRole.Likelihood]

    @property
    def preferences(self):
        return [f for f in self.factors if f.role == FactorRole.Preference]

    @property
    def initial_state_priors(self):
        return [f for f in self.factors if f.role == FactorRole.InitialStatePrior]

    @property
    def beliefs(self):
        return [f for f in self.factors if f.role == FactorRole.Belief]
    ...
    def validate(self, raise_exceptions: bool = True):
        ...
        has_transition = set()
        is_lateral_transition_source = set()
        has_preference_dist = set()
        control_vars = set()
        unlabeled_transitions = set()
        unlabeled_factors = set()

        # Gather info on existing factors
        for factor_idx, factor in enumerate(self.factors):
            if factor.role == FactorRole.Transition:
                target = factor.variables[0]
                conditioning = factor.variables[1:-1]
                control_var = factor.variables[-1]
                has_transition.add(target)
                is_lateral_transition_source.update(
                    [x for x in conditioning if x != target]
                )
                ...
                if (
                    len(factor.variables) > 1
                    and factor.variables[0] == factor.variables[1]
                ):
                    unlabeled_transitions.add(factor_idx)
                else:
        ...
        # Deal with conflicting variable roles
        # TODO: Technically this should likely be done at the PymdpPomdp level, but we have
        # the requisite information here already
        for var in state_vars & control_vars:
            errors.extend(
                ConflictingVariableRoles(
                    var,
                    [VariableRole.Latent, VariableRole.ControlState],
                )
            )

        # TODO: Key this to a level in a potentially hierarchical model
        for var in obs_vars & state_vars:
            errors.extend(
                ConflictingVariableRoles(
                    var,
                    [VariableRole.Observation, VariableRole.Latent],
                )
            )

        for var in obs_vars & control_vars:
            errors.extend(
                ConflictingVariableRoles(
                    var,
                    [VariableRole.Observation, VariableRole.ControlState],
                )
            )
        ...
        # Deal with state variables that don't influence anything
        # NOTE: There's no obvious way to auto-add missing factors in this case,
        # but we can check for missing labels

        inert_state = state_vars - (is_lateral_transition_source | has_likelihood_state)
        if inert_state:

            diffs = []
            for var in inert_state:

                # We can check whether any unlabeled transitions fit the bill
                unlabeled_transition_idx = None
                for factor_idx in unlabeled_transitions:
                    factor = self.factors[factor_idx]
                    if var in factor.variables[1:-1]:
                        diffs.append(
                            {
                                "op": "replace",
                                "path": f"/factors/{factor_idx}/role",
                        ...
                        unknown.remove(factor.variables[-1])
                        control_vars.add(factor.variables[-1])
                        is_lateral_transition_source.add(var)
                        transition_factor_found = True

                if unlabeled_transition_idx is not None:
                    unlabeled_transitions.remove(unlabeled_transition_idx)
                ...
                for factor_idx in unlabeled_factors:
                    ...
                    if (
                        ...
                        and factor.variables[-1] not in control_vars
                        and maybe_obs not in obs_vars
                    ):
        ...
        if no_transition:
            diffs = []
            transition_factor_found = False
            for var in no_transition:

                # We can check whether any unlabeled factors look like suitable transitions
                for factor_idx in unlabeled_transitions:
                    factor = self.factors[factor_idx]
                    if factor.variables[0] == var:
                        diffs.append(
                            {
                                "op": "replace",
                                "path": f"/factors/{factor_idx}/role",
                        ...
                        unlabeled_transitions.remove(factor_idx)
                        unknown.remove(factor.variables[-1])
                        control_vars.add(factor.variables[-1])
                        transition_factor_found = True
                        break

                if not transition_factor_found:
