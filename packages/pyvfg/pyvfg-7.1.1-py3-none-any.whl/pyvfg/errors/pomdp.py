...
class InertStateVariable(ValidationError):
    def __init__(self, which_vars: list[str], patch: Optional[JsonPatch] = None):
        super(InertStateVariable, self).__init__(
            "State variable(s) '%s' have no outgoing connections (and thus no effect on outcomes)."
            % ", ".join(which_vars),
            {"variables": which_vars},
            patch,
        )
...
class ObsVarMissingLikelihood(ValidationError):
    def __init__(self, which_vars: list[str], patch: Optional[JsonPatch] = None):
        super(ObsVarMissingLikelihood, self).__init__(
            "Observation variable(s) '%s' lack likelihood factors."
            % ", ".join(which_vars),
            {"variables": which_vars},
            patch,
        )
...
class VariableRoleIndeterminate(ValidationError):
    def __init__(self, which_vars: list[str], patch: Optional[JsonPatch] = None):
        super(VariableRoleIndeterminate, self).__init__(
            "Role for variable(s) '%s' cannot be inferred from VFG information."
            % which_vars,
            {"variables": which_vars},
            patch,
        )
