...
def get_dmhalo(model,**params):
    ...
    if model=='dehnen_cusp':
        ...
        if type(params['m_triangle']) is ap.units.quantity.Quantity:    
            phi0=(-4.*np.pi*g_dim*rho_scale*r_scale**2*(fncore(0.,params['r_core'],params['n_core'])*(2.-1./(1.+params['c_triangle']))+params['n_core']*1./params['c_triangle']/params['r_core']*fncore(0.,params['r_core'],ncorem1)*(1.-fncore(0.,params['r_core'],two))*(-np.log(1.+params['c_triangle'])-float(npmath.polylog(2.,-params['c_triangle']))))).to(u.km*u.km/u.s/u.s)
        else:
            phi0=-4.*np.pi*g*rho_scale*r_scale**2*(fncore(0.,params['r_core'],params['n_core'])*(2.-1./(1.+params['c_triangle']))+params['n_core']*1./params['c_triangle']/params['r_core']*fncore(0.,params['r_core'],ncorem1)*(1.-fncore(0.,params['r_core'],two))*(-np.log(1.+params['c_triangle'])-float(npmath.polylog(2.,-params['c_triangle']))))
