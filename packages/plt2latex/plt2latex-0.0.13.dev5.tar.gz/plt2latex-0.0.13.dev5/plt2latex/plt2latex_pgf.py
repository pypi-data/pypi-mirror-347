...
_DEFAULT_DOCUMENTCLASS = r"\documentclass[20pt]{extarticle}"
...
def _get_preamble():
    ...
    return "\n".join([
        ...
        # Add new command to measure approximate font size in apropriate way.
        r"""
            \makeatletter
            \newcommand{\measurefontsize}[2]{%
              \begingroup
                #2
                \newdimen\mydim
                \mydim=\fontdimen6\font
                \typeout{SIZE: #1: \the\mydim}%
              \endgroup
            }
            \makeatother
        """,
...
def _tex_escape(text):
    ...
    LaTeX documents.
    """
    # Remove any existing \mathdefault{...} wrapper (just in case)
    text = re.sub(r'\\mathdefault\s*\{([^{}]*)\}', r'\1', text)
...
def _metadata_to_str(key, value):
    r"""Convert metadata key/value to a form that hyperref accepts."""
...
def _crop_pdf(path: pathlib.Path, width_in: float, height_in: float):
    r"""
    Crops the input PDF file to the specified width and height.

    Args:
        path (pathlib.Path): The path to the input PDF file.
        width_in (float): The desired width of the cropped PDF in inches.
        height_in (float): The desired height of the cropped PDF in inches.

    Returns:
        pathlib.Path: The path to the cropped PDF file, or the original path if no cropping is necessary.

    Notes:
        This function checks if the current size of the PDF page is already close to the desired size.
        If the size is within a small tolerance, no cropping is performed. Otherwise, the PDF is cropped
        to the specified dimensions with a small fudge factor to avoid clipping due to floating-point rounding.
    """
    try:
        doc = fitz.open(path)
        page = doc[0]
        ...
        target_rect = fitz.Rect(0, 0, width_in * mpl_in_to_pt, height_in * mpl_in_to_pt)

        tol = 0.5
        if (
            abs(current_rect.width - target_rect.width) < tol and
            abs(current_rect.height - target_rect.height) < tol
        ):
            return path

        # Cropping with a small fudge factor
        fudge = 0.01
        crop_rect = fitz.Rect(
            0, 0,
            width_in * mpl_in_to_pt + fudge,
            height_in * mpl_in_to_pt + fudge
        )
        ...
        _log.warning(f"PDF cropping failed: {e}")
        return path


def _get_closest_fontsize(pt: float) -> str:
    """
    Finds the closest predefined LaTeX font size for a given point size.

    The function retrieves font sizes from `p2l.rcParams` and returns the
    corresponding LaTeX command for the closest available font size.

    Parameters:
    pt (float): The font size in points to be matched.

    Returns:
    str: The LaTeX font size command corresponding to the closest available size.
    """
    font_sizes = {
        r'\tiny': p2l.rcParams['fontsizes.tiny'],
        r'\scriptsize': p2l.rcParams['fontsizes.scriptsize'],
        r'\footnotesize': p2l.rcParams['fontsizes.footnotesize'],
        r'\small': p2l.rcParams['fontsizes.small'],
        r'\normalsize': p2l.rcParams['fontsizes.normalsize'],
        r'\large': p2l.rcParams['fontsizes.large'],
        r'\Large': p2l.rcParams['fontsizes.Large'],
        r'\LARGE': p2l.rcParams['fontsizes.LARGE'],
        r'\huge': p2l.rcParams['fontsizes.huge'],
        r'\Huge': p2l.rcParams['fontsizes.Huge'],
    }

    sorted_sizes = sorted(font_sizes.items(), key=lambda x: x[1])
    closest_command, closest_size = min(sorted_sizes, key=lambda x: abs(x[1] - pt))
    min_command, min_size = sorted_sizes[0]
    max_command, max_size = sorted_sizes[-1]
...
def get_latex_font_sizes() -> dict:
    return LatexManager._get_cached_or_new()._get_font_sizes()
...
class LatexManager:
    ...
    def _setup_latex_process(self, *, expect_reply=True):
        _log.debug("Setting up LaTeX process")
    ...
    def get_width_height_descent(self, text, prop):
        ...
        """
        return self._get_box_metrics(self._escape_and_apply_props(text, prop))
    ...
    def _get_font_sizes(self):
        """
        Retrieve actual font sizes (in pt) for LaTeX commands like \small, \Large, etc.
        Each size is requested individually. If a size is not returned, -1.0 is used as fallback.

        Returns:
            dict[str, float]: Mapping from font size name (e.g. 'tiny') to point size.
        """
        if LatexManager._FONT_SIZES is not None:
            return LatexManager._FONT_SIZES
        ...
        for size in sizes:
            cmd = r'\measurefontsize{%s}{\%s}' % (size, size)
            self._stdin_writeln(cmd)
            line = self._expect_prompt().strip()
            while True:
                match = re.match(r".*?SIZE:\s*(\\?[A-Za-z]+\*?)\s*:\s*([\d.]+)", line)
                if match:
                    name, pt = match.groups()
                    font_sizes[name.strip("\\")] = float(pt)
                    break
                elif line == "":
                    continue  # skip empty lines
                # else: ignore unrelated lines

        # Fill missing sizes with fallback value -1.0
        # for size in sizes:
        #     font_sizes.setdefault(size, 1.0)

        # Sort and store
        LatexManager._FONT_SIZES = dict(sorted(font_sizes.items(), key=lambda item: item[1]))
        return LatexManager._FONT_SIZES


    def _escape_and_apply_props(self, s, prop):
        """
        Generate a TeX string that renders string *s* with font properties *prop*,
        using either LaTeX size commands (\\normalsize, etc.) or precise \\fontsize{},
        depending on the configured font_size_method.
        """

        commands = []

        families = {"serif": r"", "sans": r"\sffamily", "sans-serif": r"\sffamily", "monospace": r"\ttfamily"}
        ...
        if family in families:
            ...
            commands.append(
                r"\ifdefined\pdftexversion\else\setmainfont{%s}\fi" % family)
        else:
            _log.warning("Ignoring unknown font: %s", family)

        size = prop.get_size_in_points()
        method = p2l.rcParams.get("font_size_method", "default")

        if method == "precise":
            commands.append(r"\fontsize{%f}{%f}" % (size, size * 1.2))
            commands.append(r"\selectfont")
        else:
            commands.append(_get_closest_fontsize(size))
...
class RendererPgf(RendererBase):
    ...
    def draw_markers(self, gc, marker_path, marker_trans, path, trans,
        ...
        maxcoord = 16383 / latex_pt_to_in * self.dpi  # Max dimensions in LaTeX.
    ...
    def draw_text(self, gc, x, y, s, prop, angle, ismath=False, mtext=None):
        ...
        s = LatexManager._get_cached_or_new()._escape_and_apply_props(s, prop)
        # s = r'\fbox{%s}' % s
...
class FigureCanvasPlt2latexPgf(FigureCanvasBase):
    ...
    def _print_pgf_to_fh(self, fh, *, bbox_inches_restore=None):
        ...
        header_text = """%%TEST!!!!!!!!!!!!!!TEST Creator: Matplotlib, PGF backend 
%%
...
"""
    ...
    def print_pgf(self, fname_or_fh, **kwargs):
        ...
        """
        for key in ["facecolor", "edgecolor", "bbox_inches_restore", "orientation"]:
            kwargs.pop(key, None)
    ...
    def print_pdf(self, fname_or_fh, *, metadata=None, **kwargs):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            cropped = _crop_pdf(tmppath / "figure.pdf", w, h)
            with cropped.open("rb") as orig, cbook.open_file_cm(fname_or_fh, "wb") as dest:
    ...
    def _repr_mimebundle_(self, include=None, exclude=None):
        """Jupyter автоматически вызывает этот метод, чтобы отобразить temp.png."""
        print("✅ CustomFigureCanvasPgf is used!")  # Лог для проверки

        temp_image_path = os.path.join(os.path.dirname(__file__), "temp.png")

        if not os.path.exists(temp_image_path):
            print("⚠️ Ошибка: Файл temp.png не найден!")
            return {}
...
class _BackendPgf(_Backend):
    FigureCanvas = FigureCanvasPlt2latexPgf
...
class PdfPages:
    ...
    def _write_header(self, width_inches, height_inches):
        ...
        latex_header = "\n".join([
            _DEFAULT_DOCUMENTCLASS,
            r"\usepackage[pdfinfo={%s}]{hyperref}" % pdfinfo,
            r"\paperwidth=%fin" % width_inches,
            r"\paperheight=%fin" % height_inches,
            r"\hoffset=-1in",
            r"\voffset=-1in",
            r"\oddsidemargin=0in",
            r"\evensidemargin=0in",
            ...
            r"\headheight=0pt",
            r"\headsep=0pt",
            r"\textwidth=%fin" % width_inches,
            r"\textheight=%fin" % height_inches,
            r"\usepackage{pgf}",
            _get_preamble(),
