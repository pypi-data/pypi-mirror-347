...
def generate_tags_metadata(router: FangoRouter) -> list[dict]:
    ...
    """
    return [
        {"name": viewset._basename, "description": viewset.__doc__ or _(viewset.queryset.model._meta.verbose_name)}
        for viewset in router.viewsets
    ]


def fmt_date(lookup: str, region: str = "ru") -> Func:
    """
    Format date
    ...
    return Func(F(lookup), Value(format), function="TO_CHAR", output_field=CharField())


def get_object_id(request: Request) -> int | UUID | None:
    """
    Get object_id from url path parameters.

    """
    if object_id := request.path_params.get("object_id"):
        if object_id.isdigit():
            return int(object_id)
        try:
            return UUID(object_id)
        except ValueError:
            raise HTTPException(status_code=400, detail=f"Invalid object_id parameter: {object_id}")


def replace_url_schema(url: str | None) -> str | None:
    """
    Replace url schema for current environment.
...
def get_orm_object_or_404(django_model: ModelBase, pk: int) -> models.Model:
    try:
        return django_model.objects.get(pk=pk)
    except ObjectDoesNotExist:
        raise HTTPException(status_code=404, detail="Item not found")


async def aget_orm_object_or_404(django_model: models.Model, pk: int) -> models.Model:
    try:
        return await django_model.objects.aget(pk=pk)
    except ObjectDoesNotExist:
        raise HTTPException(status_code=404, detail="Item not found")
