...
# {
    ...
    @classmethod
    def determine_expected_kwargs(cls):
        sig = inspect.signature(cls.__init__)
        expected_kwargs = [
            param_name
            for param_name, param in sig.parameters.items()
            if param_name != "self" and param.kind != inspect.Parameter.VAR_KEYWORD
        ]
...
class MixedOrderedLogit(OrderedLogitLong, MixedLogit):
    ...
    def __init__(self, **kwargs):
        super(OrderedLogitLong,self).__init__()
    ...
    def setup(self, **kwargs):
        ...
        ordered_kwargs = OrderedLogitLong.determine_expected_kwargs()
        # Split kwargs for MixedLogit
        mixed_kwargs = MixedLogit.determine_expected_kwargs()

        # Call parent setups with filtered kwargs
        OrderedLogitLong.setup(self, **ordered_kwargs)
        MixedLogit.setup(self, **mixed_kwargs)
