...
@click.option('--profile', '-p', help='Profile for login, if not provided use default profile')
@click.option('--override-config-path', '-c', help='Path to config.json, default config stored in ~/.config.json')
@click.option('--existing-id', '-id', help='An existing component id to update')
@click.option('--component-path', help='A path to poetry project that include the component source code')
@click.pass_context
def upload(ctx, profile=None, override_config_path=None, existing_id=None, component_path=None):
    commands.upload_component_from_package(ctx, profile, override_config_path, existing_id, component_path)
