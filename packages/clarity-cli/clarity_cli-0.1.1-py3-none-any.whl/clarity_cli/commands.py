...
class CliCommands():
    ...
    def write_profile_to_config(self, ctx, profile_name=None, client_id=None, client_secret=None, token_endpoint=None,
        ...
        if profile_name and client_id and client_secret and token_endpoint and scope and domain:
            ...
            table.add_data([("Profile name", profile_name), ("Client id", client_id), ("Client secret", client_secret), ("Clarity domain", domain),
                            ("Default Project id", project), ("Default workspace id", workspace), ("Default agent id", agent_id), ("Is default", str(default))])
            table.print_table()
    ...
    def upload_component_from_package(self, ctx, profile=None, override_config_path=None, existing_id=None, component_path=None):
        self.out.setup_context(ctx)
        config, profile_name = self._get_current_profile_and_config(profile, override_config_path)
        web_token = is_logged_in(profile_name)
        ...
        parse_and_upload_poetry_package(package_dir=component_path, domain=config.domain,
                                        workspace_id=config.workspace, step_id=existing_id, profile_name=profile_name, auth_token=web_token)
