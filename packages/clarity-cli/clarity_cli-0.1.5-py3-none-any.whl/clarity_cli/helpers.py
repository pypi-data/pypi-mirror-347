...
from clarity_cli.defs import (CONFIG_DIR, TOKEN_FILE, CONFIG_FILE, ProfileConfig, EXECUTION_VIEW_PATH, TEST_LIST,
                              DEVICES_LIST, SUPPORTED_PYTHON_VERSION, STEP_UPLOAD, STEP_CREATE, SPECIFIC_STEP, RunningEnv)
...
def create_step(step_to_upload: dict, domain: str, headers: dict) -> str:
    ...
    if response.status_code == 400:
        out.vprint(response.text)
        out.warning("Step already exists, please bump version to create a new step")
        return None
...
def zip_and_upload_step(step_to_upload: dict, package_dir: str, url_to_upload: str, pyc: bool) -> str:
    with tempfile.TemporaryDirectory() as temp_dir:
        zip_path = os.path.join(temp_dir, f"{step_to_upload['step_name']}-{step_to_upload['step_version']}.zip")
        # Create the zip file
        ...
        if response.status_code not in (200, 201):
            out.vprint(f"Got status code {response.status_code}: {response.text}")
            raise StopCommand("Upload new zip file failed")


def get_upload_url(step_id: str, domain: str, headers: dict, is_override: bool = False) -> str:
...
def validate_python_version(python_version: str) -> str:
    ...
    if not Version(SUPPORTED_PYTHON_VERSION) in spec:
        raise StopCommand(f"Cyclarity currently doesn't support this python version {python_version}, please use: {SUPPORTED_PYTHON_VERSION}")
...
def validate_step_metadata(step_to_upload: dict) -> dict:
    ...
    if not step_to_upload.get("step_name"):
        step_to_upload["step_name"] = input.ask_for_text_input("Please enter the step name")
    if not step_to_upload.get("step_version"):
        step_to_upload["step_version"] = input.ask_for_text_input("Please enter the step version")
    if not step_to_upload.get("description"):
        step_to_upload["description"] = input.ask_for_text_input("Please enter the step description", default="", optional=True)
    if not step_to_upload.get("tags"):
        step_to_upload["tags"] = input.ask_for_text_input("Do you want to add tags to the step? (comma separated, e.g. tag1,tag2,tag3)", default="", optional=True)
        final_tags = [tag.strip().upper() for tag in step_to_upload['tags'].split(',') if tag.strip()]
        out.vprint(f"Final tags: {final_tags}")
        step_to_upload["tags"] = final_tags
    return step_to_upload


def add_entrypoint_and_running_env(step_to_upload: dict, clarity_section: dict, env_component_id: str, step_id: str, default_entrypoint: str, default_running_env: str) -> dict:
    entrypoint = default_entrypoint or clarity_section.get('entrypoint')
    if not entrypoint:
        entrypoint = input.ask_for_text_input("Please enter the runnable class entrypoint separated by dots(e.g. src.main.my_class)", default="")
    else:
        approved = input.ask_for_confirmation(f"found entrypoint in pyproject.toml, using: {entrypoint}")
        if not approved:
            entrypoint = input.ask_for_text_input("Please enter the runnable class entrypoint separated by dots(e.g. src.main.my_class)", default="")
    step_to_upload["execution_config"]["entrypoint"] = entrypoint
    if not step_id:
        out.vprint("No step_id provided, using component_id from pyproject.toml")
        step_id = clarity_section.get(env_component_id)
    running_env = default_running_env or clarity_section.get('running_env', "")
    if running_env not in RunningEnv.__members__.keys():
        out.warning(f"Invalid running environment {running_env}, please use one of: {RunningEnv.__members__.keys()}")
        running_env_input = input.ask_for_input_from_list("Please enter the running environment", options=RunningEnv.__members__.keys())
    else:
...
def create_step_metadata_from_poetry_v2(pyproject_data: dict, clarity_section: dict, env_component_id: str, step_id: str, default_entrypoint: str, default_running_env: str) -> dict:
    ...
    step_to_upload = {"step_name": project_metadata.get("name"),
    ...
    add_entrypoint_and_running_env(step_to_upload, clarity_section, env_component_id, step_id, default_entrypoint, default_running_env)
...
def create_step_metadata_from_poetry_v1(pyproject_data: dict, clarity_section: dict, env_component_id: str, step_id: str, default_entrypoint: str, default_running_env: str) -> dict:
    out.vprint("Creating step metadata from poetry v1")
    poetry_section: dict = pyproject_data.get("tool", {}).get("poetry", {})
    if not poetry_section:
        raise StopCommand("No [tool.poetry] section found in pyproject.toml")
    step_to_upload = {"step_name": poetry_section.get("name"),
    ...
    python_version = poetry_section.get("dependencies", {}).get("python", "").replace('^', '>=').replace('~', '>=')
    validate_python_version(python_version)
...
def handle_new_version(step_to_upload: dict, step_id: str, domain: str, headers: dict, pyproject_data: dict) -> str:
    try:
        version = Version(step_to_upload["step_version"])
        new_version = f"{version.major}.{version.minor}.{version.micro + 1}"
        out.ok(f"Bumped version from {step_to_upload['step_version']} to {new_version}")
        step_to_upload["step_version"] = new_version
        # Create new step with bumped version
        step_id = out.run_sync_function("Creating step with new version", create_step, step_to_upload=step_to_upload, domain=domain, headers=headers)
        if not step_id:
        ...
        raise StopCommand(f"Invalid version format: {step_to_upload['step_version']}. Version must follow semantic versioning")


def get_step_to_upload(pyproject_data: dict, clarity_section: dict, env_component_id: str, step_id: str, default_entrypoint: str, default_running_env: str) -> tuple[dict, dict]:
    # Check if the TOML has v2 format by looking for specific v2 fields
    is_v2 = "project" in pyproject_data
    if is_v2:
        step_to_upload, clarity_section = create_step_metadata_from_poetry_v2(pyproject_data, clarity_section, env_component_id, step_id, default_entrypoint, default_running_env)
    else:
        step_to_upload, clarity_section = create_step_metadata_from_poetry_v1(pyproject_data, clarity_section, env_component_id, step_id, default_entrypoint, default_running_env)
    return step_to_upload, clarity_section


def handle_upload_url_options(step_id: str, domain: str, headers: dict, step_to_upload: dict, pyproject_data: dict) -> tuple[str, str]:
    url_to_upload = out.run_sync_function("Getting url to upload component", get_upload_url, step_id=step_id, domain=domain, headers=headers)
    if url_to_upload.get("type") == "success":
        pass
    elif url_to_upload.get("type") == "override":
        out.warning("Step is in use by expert builder flows, please select Override to update the content or bump version to create a new step")
        answer = input.ask_for_confirmation("Do you want to override the step?")
        if answer:
            url_to_upload = out.run_sync_function("Getting url to upload component", get_upload_url, step_id=step_id, domain=domain, headers=headers, is_override=True)
        else:
            input.ask_for_confirmation("Do you want to bump the step version?", hard_stop=True)
            step_id, pyproject_data = handle_new_version(step_to_upload, step_id, domain, headers, pyproject_data)
            url_to_upload = out.run_sync_function("Getting url to upload component", get_upload_url, step_id=step_id, domain=domain, headers=headers)

    elif url_to_upload.get("type") == "bump":
        out.warning("Step is in use by deployed flows, please bump version to create a new step")
        input.ask_for_confirmation("Do you want to bump the step version?", hard_stop=True)
        step_id, pyproject_data = handle_new_version(step_to_upload, step_id, domain, headers, pyproject_data)
        url_to_upload = out.run_sync_function("Getting url to upload component", get_upload_url, step_id=step_id, domain=domain, headers=headers)
    ...
    while True:
        ...
        if response.status_code != 200:
            ...
            if status == 'RUNNING':
                ...
                out.error(out.bold("Component parsing failed, check the error message below"))
                out.error(error_message)
                raise StopCommand("Component parsing failed")


def parse_and_upload_poetry_package(package_dir: str, workspace_id: str, domain: str, step_id: str, profile_name: str,
                                    auth_token: str, default_entrypoint: str, default_running_env: str, pyc: bool) -> str:
    """
    ...
    Args:
        ...
        step_id: Optional ID of existing step to update
    ...
    """
    headers = {"Authorization": f"Bearer {auth_token}", "workspace": workspace_id}
    env_component_id = f"component_id_{profile_name}"
    # Ensure the package directory exists
    package_path = Path(package_dir)
    if not package_path.exists() or not package_path.is_dir():
        raise StopCommand(f"Package directory {package_dir} does not exist or is not a directory")
    ...
    step_to_upload, clarity_section = get_step_to_upload(pyproject_data, clarity_section, env_component_id, step_id, default_entrypoint, default_running_env)

    # Upload the zip file to the server
    step_id = step_id or clarity_section.get(env_component_id)
    out.vprint(f"current step id: {step_id}")
    if not step_id:
        out.vprint("step id wasn't provided, creating step")
        step_id = out.run_sync_function("Creating step", create_step, step_to_upload=step_to_upload, domain=domain, headers=headers)
        if not step_id:
            step_id, pyproject_data = handle_new_version(step_to_upload, step_id, domain, headers, pyproject_data)
    else:
        out.vprint("Step id provided, updating step")
        answer = input.ask_for_confirmation(f"Are you sure you want to update/override the step: {step_id}?")
        if answer:
            out.run_sync_function("Updating step", update_step, step_id=step_id, step_to_upload=step_to_upload, domain=domain, headers=headers)
        else:
            input.ask_for_confirmation("Would you like to bump the version?", hard_stop=True)
            step_id, pyproject_data = handle_new_version(step_to_upload, step_id, domain, headers, pyproject_data)

    # Update pyproject.toml with the step_id
    clarity_section[env_component_id] = step_id
    ...
    with open(pyproject_path, "w") as f:
        ...
        f.write(toml_str)
    upload_response_data, new_step_id, pyproject_data = handle_upload_url_options(step_id, domain, headers, step_to_upload, pyproject_data)
    ...
    out.run_sync_function("Zipping and uploading component", zip_and_upload_step, step_to_upload=step_to_upload, package_dir=package_dir, url_to_upload=url_to_upload, pyc=pyc)
    out.ok("Uploaded new zip file successfully")
    out.run_sync_function("Server is parsing component", check_component_status, step_id=step_id, domain=domain, headers=headers)
    return step_id
