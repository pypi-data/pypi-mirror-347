...
@click.option('--profile', '-p', type=click.Choice(get_profiles()), help='Profile for login, if not provided use default profile')
@click.option('--override-config-path', '-c', help='Path to config.json, default config stored in ~/.config.json')
@click.option('--existing-id', '-id', help='An existing component id to update (must start with TestStep_)')
...
@click.pass_context
def upload(ctx, profile=None, override_config_path=None, existing_id=None, component_path=None, entrypoint=None, running_env=None, pyc=False, y=False):
    try:
        ctx.obj['yes_to_all'] = y
        commands.upload_component_from_package(ctx, profile, override_config_path, existing_id, component_path, entrypoint, running_env, pyc)
    except Exception as e:
        out.vprint(e)
        raise click.Abort()
