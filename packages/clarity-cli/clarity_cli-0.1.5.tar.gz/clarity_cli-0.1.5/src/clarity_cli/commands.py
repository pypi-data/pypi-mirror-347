...
class CliCommands():
    ...
    def upload_component_from_package(self, ctx, profile=None, override_config_path=None, existing_id=None, component_path=None, default_entrypoint=None, default_running_env=None, pyc=None):
        self.out.setup_context(ctx)
        self.input.setup_context(ctx)
        config, profile_name = self._get_current_profile_and_config(profile, override_config_path)
        web_token = is_logged_in(profile_name)
        ...
        parse_and_upload_poetry_package(package_dir=component_path, domain=config.domain,
                                        ...
                                        default_entrypoint=default_entrypoint, default_running_env=default_running_env, pyc=pyc)
