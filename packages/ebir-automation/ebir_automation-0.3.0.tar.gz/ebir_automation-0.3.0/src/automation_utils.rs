...
pub fn create_control_type_condition(
    ...
    uia.create_property_condition(UIProperty::ControlType, Variant::from(control_type as i32), None)
}
...
pub fn set_element_value(element: &UIElement, value: &str) -> Result<(), uiautomation::errors::Error> {
...
pub fn find_element_by_id_and_type(
    ...
    control_type: ControlType, 
    scope: TreeScope
) -> Result<UIElement, uiautomation::errors::Error> {    // Create condition for automation ID
...
pub fn select_combobox_item_by_index(
    ...
    let item = items.get(index).ok_or_else(|| uiautomation::errors::Error::new(-1, "Failed to get item by index"))?;
    select_element(item)?;
