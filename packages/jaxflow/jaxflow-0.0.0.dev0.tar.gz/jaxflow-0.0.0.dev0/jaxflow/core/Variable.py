...
class Variable:
    ...
    def __jax_array__(self):
        """
        Tells JAX how to convert this object into a JAX array
        when used in jnp operations or JAX transformations.
        """
        return self.value

    def __array__(self, dtype=None):
        """
        When NumPy needs to convert this object to an array,
        it calls __array__.

        Typically, you'd copy or return a host np.array here.
        That may require a device->host transfer if self.value
        is on a GPU/TPU.
        """
        arr = jax.device_get(self.value)  # device->host transfer if needed
