    ...
    for i,line in enumerate(QuoteLines):
        ...
        try:
            if i==0: QuoteLineData.append( ['Quoted', ''.join(line)] )
                ...
                mtrl = line[0]
                try:
                    specs = QuoteLines[i+1][0].split('] [')
                    if not 'sq mtr' in specs[1].lower():
                        try:
                            sqm = f"x {specs[0].split(':').strip()}"
                        except:
                            sqm = "x 1"
                    else:
                        sqm = specs[1].split(' ')[0].strip()
                    # sqm = QuoteLines[i+1][0].split('[')[2].split(']')[0].split(' ')[0]
                    mtrl = line[0]
                    if 'Benchtop' in line[0][:10]:
                        mtrl = line[0].split('Benchtop', 1)[-1].strip() # Get the material name
                except:
                    sqm = "-error-"
                QuoteLineData.append(['Material', mtrl, sqm])
