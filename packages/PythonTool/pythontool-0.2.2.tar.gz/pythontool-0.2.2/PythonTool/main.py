...
class ManimTool:
	...
	class VisDrawArc(Animation):
	    def __init__(self, p1, p2, angle=PI, axis=OUT, **kwargs):
	        ...
	        # 计算半径
	        self.r = np.linalg.norm(p2 - p1)
	        
	        # 创建起点和终点标记
	        self.d1 = Dot(point=p1, color=YELLOW)
	        self.d2 = Dot(point=p2, color=YELLOW)
	        
	        # 创建连接线
	        self.dl = DashedLine(p1, p2)
	        
	        # 创建初始弧线
	        if np.array_equal(axis, OUT):
	            self.arc = ArcBetweenPoints(p1, p2, radius=self.r, color=GREEN)
	        else:
	            self.arc = ArcBetweenPoints(p2, p1, radius=self.r, color=GREEN)
	        
	        # 调用父类构造函数
	        super().__init__(self.arc, **kwargs)
	    ...
	    def interpolate_mobject(self, alpha):
	        ...
	        # 更新点位置
	        self.d2.move_to(rotated_p2)
	        
	        # 更新连接线
	        self.dl.become(DashedLine(self.p1, rotated_p2))
	        
	        # 更新弧线
	        if np.array_equal(self.axis, OUT):
	            self.arc.become(
	                ArcBetweenPoints(
	                    start=self.p1,
	                    end=rotated_p2,
	    ...
	    def begin(self):
	        """动画开始前调用"""
	        super().begin()
	        self.mobjects = [self.d1, self.d2, self.dl, self.arc]
