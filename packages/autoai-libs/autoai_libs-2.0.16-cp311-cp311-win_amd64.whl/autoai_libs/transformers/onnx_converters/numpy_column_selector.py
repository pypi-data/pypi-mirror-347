...
def numpy_column_selector_transformer_converter(scope: Scope, operator: Operator, container: ModelComponentContainer):
    for inpt in operator.inputs:
        output = next((o for o in operator.outputs if inpt.full_name in o.full_name), None)
        if output:
        ...
        for i in op.columns:
            ...
            this_operator.outputs.append(
                scope.declare_local_variable(
                    f"NUM_SELECTED_{inpt.full_name}", type=inpt.type.__class__(shape=inpt.type.shape)
                )
            )
    else:
