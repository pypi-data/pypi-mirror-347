...
################################################################################
import numpy as np
from onnxconverter_common import apply_identity, FloatTensorType
...
def numpy_replace_missing_values_transformer_converter(
    ...
    if missing_values := op.missing_values:
        missing_values_name = scope.get_unique_variable_name(f"missing_values")
        container.add_initializer(
            missing_values_name, onnx_proto.TensorProto.FLOAT, np.array(missing_values).shape, missing_values
        )

        container.add_node(
            "ReplaceFloatValues",
            inputs=[feature_name, missing_values_name],
