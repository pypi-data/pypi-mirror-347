...
class ResourceRepository:
    ...
    def resource_exists(
        ...
        if effective_method == "local":
            try:
                self.get_resource_info(storage_method, **cleared_dict)
                return True
            except ValueError:
                return False
    ...
    def write_dataframe_for_task(
        ...
        **kwargs,
        ...
        self.write_dataframe_for_resource(
            data, resource, overwrite=overwrite, storage_method=storage_method
        )
    ...
    def write_dataframe_for_resource(
        ...
        if self.resource_exists(resource, storage_method) and not overwrite:
            ...
            if effective_method == "local":
                ...
                save_to_api(data, resource, schema=schema, overwrite=overwrite)
                self.add_or_update_resource_list(resource)
            else:
                raise ValueError(
                    "Invalid storage method. Must be either 'local' or 'api'."
                )
    ...
    def convert_units(
        ...
        for _, row in data.iterrows():
            ...
            elif target_currency and target_currency == current_unit:
                new_values.append(current_value)
        ...
        for unit in u_uniq:
            ...
            if base_currency in self.currency_converter.currencies:
                if "year" in data.columns:
                    ...
                    currencies = pd.concat([currencies, currency_year], ignore_index=True)
                else:
    ...
    def group_and_sum(self, df, code_column, group_columns, values_to_sum):
                ...
                for _, sub_group in grouped:
                    try:
                        ...
                        sub_group["base_value"] = sub_group.apply(
                            lambda row: row["value"] * self.ureg(row["unit"])
                            .to(base_unit)
                            .magnitude,
                            axis=1,
                        )
    ...
    def convert_dataframe(
        ...
        for column_name, (classif_name, classif_type) in from_class.items():
            ...
            if not isinstance(concordance, pd.DataFrame):
                logger.warning(f"Correspondence for {classif_name} to Bonsai not found")
                continue
        ...
        for column, concordance in concordances.items():
            ...
            elif from_class[column][1] == "location":
                continue  # 'location' column is handled separately after this loop
        ...
        # Always convert location, using CurrencyConverter (cc)
        for column in data.columns:
            if column in from_class and from_class[column][1] == "location":
                src_class = from_class[column][0] if column in from_class else None
                if src_class and src_class not in self.cc.valid_class:
                    logger.warning(
                        f"'{src_class}' not a valid coco classification, trying anyway..."
                    )
                    src_class = None

                data[column] = self.cc.pandas_convert(
                    data[column],
                    src=src_class,
                    to=classifications["location"],
                    not_found=None,
                    exclude_prefix=["excl.+", ","],
                )
        ...
        if "accounttype" in concordance.columns:
    ...
    def harmonize_with_resource(
        self, dataframe, storage_method: str | None = None, **kwargs
    ):
        ...
        self.write_dataframe_for_resource(
            unique_df, resource, storage_method=storage_method
        )
