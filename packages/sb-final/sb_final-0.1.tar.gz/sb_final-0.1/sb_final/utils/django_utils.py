...
from sb_final.utils.feature_dependencies import get_if_blocks, getCodeBlockNameAndType, remove_multiline_comments
...
class ManageDjangoSettings():
    ...
    def getSettingsVariablesName(self,get_code=False):
        with open(self.settings_path,"r") as file:
            ...
            for chunk in chunks:
                if "import " not in chunk and not chunk.startswith("#") and not chunk.startswith('"'):
                    try:
...
def get_project_settings(project_path):
    ...
    settings_path = f"{project_path}/{path}"
    chunks = []
