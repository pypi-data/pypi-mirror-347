...
from sb_final.utils.utils import findFilePath
from sb_final.utils.django_utils import django_defaults
from sb_final.parsers.python.parser import PythonBlockParser
from sb_final.utils.write_dependency import OneStep, writeCodeToFile
from sb_final.utils.package_utils import get_package_name, is_standard_library
from sb_final.utils.feature_dependencies import extract_words_from_code, get_code_block_names, getBlockDependencies, getFileDependencies
...
def handleRouterURL(view_name,path):
    ...
    with open(path,"r") as file:
        ...
        for chunk in chunks:
            if view_name in chunk and "import" not in chunk:
                feature = chunk
                break
            elif "views" in chunk and "import" in chunk:
