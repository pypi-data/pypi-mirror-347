...
def oneupload(url, file_name, logger):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3',"Referer": "https://oneupload.net/", "Origin": "https://oneupload.net"}
    user_agent = headers['User-Agent']
    headers_dict = {
    'Referer': headers['Referer'],
    'Origin': headers['Origin']
    }
    match = r'file:"(https?://[^"]+\.m3u8[^"]*)"'
    m3u8_url = get_m3u8_url(url=url, headers=headers, match=match, logger=logger)
    if not m3u8_url:
        return False
