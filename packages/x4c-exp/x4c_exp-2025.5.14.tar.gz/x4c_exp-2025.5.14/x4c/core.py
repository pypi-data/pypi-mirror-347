...
class XDataArray:
    ...
    def plot(self, title=None, figsize=None, ax=None, latlon_range=None, add_clabels=False, clevels=None, clabel_kwargs=None,
             ...
             coastline_zorder=99, coastline_width=1, site_markersizes=100, df_sites=None, colname_dict=None, gs='T',
        ...
        da = self.da.squeeze()
        ndim = len(da.dims)
        if (ndim == 2) and ('lat' in da.coords and 'lon' in da.coords) or ('lat' in da.attrs and 'lon' in da.attrs) or ('nlat' in da.dims and 'nlon' in da.dims):
            # map
            if ax is None:
            ...
            if ('grid' in da.attrs) and ('ne' in da.attrs['grid']) and ('lat' not in da.coords and 'lon' not in da.coords) and ('lat' in da.attrs and 'lon' in da.attrs):
               # CAM-SE grid without regridding
                __plt_kws = _plt_kws.copy()
                del(__plt_kws['cbar_kwargs'])
                im = ax.tricontourf(da.lon, da.lat, da,  **__plt_kws)
                if latlon_range is None: ax.set_global()
                if 'add_colorbar' in kws and kws['add_colorbar'] is False:
                    pass
                else:
                    cbar = plt.colorbar(im, ax=ax, **_plt_kws['cbar_kwargs'])
                    cbar.ax.tick_params()
            elif ('comp' in da.attrs) and (da.attrs['comp'] in ['ocn', 'ice']) and ('lat' not in da.coords and 'lon' not in da.coords) and ('TLAT' in da.coords and 'TLONG' in da.coords) or ('ULAT' in da.coords and 'ULONG' in da.coords):
