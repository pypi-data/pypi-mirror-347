...
class Timeseries:
    ...
    def plot(self, spell, t_idx=None, regrid=False, gs='T', ssv='SSH', recalculate_ssv=False, timespan=None, **kws):
        ...
        if len(da.dims) == 2 and ('lat' in da.dims and 'lon' in da.dims) or \
            ('lat' in da.attrs and 'lon' in da.attrs) or ('TLAT' in da.coords and 'TLON' in da.coords) or ('ULAT' in da.coords and 'ULON' in da.coords):
            plot_type = 'map'
        elif len(da.dims) == 2:
            plot_type = 'yz'
