...
from ..utils import All, ensure_bracketed
...
def _collect_column_placeholder(column: SQLCol, ignore_forbidden_chars: bool = False) -> str:
    if isinstance(column, str):
        ...
        if isinstance(column, RecordQuery):
            string, placeholders = column.placeholder_pair()
            if not getattr(column, "alias", False):
...
def _is_all_columns(columns) -> bool:

    if isinstance(columns, (All, type(None))):
        return True
    ...
    if isinstance(columns, (tuple, list)):
        for col in columns:
            if isinstance(col, str) and col == "*":
                return True
            if isinstance(col, (All, type(None))):
                return True
    return False
...
def _format_table_name(table_name: str, validate = True) -> str:
    ...
    if isinstance(table_name, SQLExpression):
        return table_name.placeholder_pair(include_alias=True)
    if validate:
