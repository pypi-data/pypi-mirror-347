...
def standardize_data(df, source_type):
    ...
    # 统一列名映射
    column_mapping = {
        'stock': {'日期': 'date', '开盘': 'open', '收盘': 'close', '最高': 'high', '最低': 'low', '成交量': 'volume'},
        'etf': {'日期': 'date', '开盘': 'open', '收盘': 'close', '最高': 'high', '最低': 'low', '成交量': 'volume'},
...
def get_etf_data(etf_code='510300', start_date=None, end_date=None):
    ...
    try:
        ...
        df = ak.fund_etf_hist_em(symbol=etf_code, start_date=start_date, end_date=end_date)
...
def calculate_moving_averages(data, windows=[5, 20, 60]):
    ...
    for window in windows:
        data[f'ma_{window}'] = data['close'].rolling(window=window, min_periods=window-4).mean()
    return data
...
def classify_market_style(
    ...
    range_threshold: str = "震荡行情"  # 震荡
    reversal_down_threshold: str = "向下反转"  # 反转向下
    trend_down_threshold: str = "下降趋势"  # 趋势向下
    trend_range_down: str = "震荡偏空"
    trend_range_up: str = "震荡偏多"

    # 初始化一个新的DataFrame，复制原DataFrame结构但内容为空
    mkt_style_df = pd.DataFrame(index=df.index, columns=[output_name])
    mkt_style_df[output_name] = np.nan  # 初始化为NaN
    ...
    for i in range(0, len(df)):
        ...
        if (
                ...
                df[ma_20].iloc[i] > df[boll_middle].iloc[i] and
                ma20_slope.iloc[i] > 0 and
                df[rsi_10].iloc[i] > 60 and
                band_width.iloc[i] > band_width.rolling(50, min_periods=1).mean().iloc[i] * 0.8 and  # 布林带宽度较宽
                atr_ratio.iloc[i] > 0.01  # ATR波动率较高
        ):
        ...
        # --- 3. 震荡（条件放宽）---
                ...
                # ATR波动率条件
                atr_ratio.iloc[i] < 0.015  # ATR波动率较低
        ):
        ...
        # 其他情况视为趋势未知
            ...
            if df[ma_5].iloc[i] > df[ma_20].iloc[i]:
                mkt_style_df.at[idx, output_name] = trend_range_up
            else:
                mkt_style_df.at[idx, output_name] = trend_range_down
