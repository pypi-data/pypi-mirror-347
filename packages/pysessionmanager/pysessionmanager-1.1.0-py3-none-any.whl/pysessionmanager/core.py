...
class SessionStoring:
    ...
    def store_sessions_csv(self, sessions: Dict[str, Dict], csv_filename: str = "sessions.csv"):
        with open(csv_filename, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["session_id", "session_name", "start_time", "end_time", "protected", "password"])
    ...
    def store_sessions_sqlite(self, sessions: Dict[str, Dict]):
    ...
    def load_sessions_sqlite(self) -> Dict[str, Dict]:
        sessions = {}
        conn = sqlite3.connect(self.db_name)
        cursor = conn.cursor()
    ...
    def store_sessions_postgresql(self, sessions: Dict[str, Dict], conn_string: str):
...
class SessionManager:
    def __init__(self, load: bool = False, protect_all: bool = False, logging: bool = False):
    ...
    def store_sessions_csv(self, csv_filename: str = "sessions.csv"):
        self.storer.store_sessions_csv(self.sessions, csv_filename)

    def load_sessions_csv(self, csv_filename: str = "sessions.csv"):
        self.sessions = self.storer.load_sessions_csv(csv_filename)

    def store_sessions_sqlite(self):
        self.storer.store_sessions_sqlite(self.sessions)

    def load_sessions_sqlite(self):
        self.sessions = self.storer.load_sessions_sqlite()

    def store_sessions_postgresql(self, conn_string: str):
        self.storer.store_sessions_postgresql(self.sessions, conn_string)

    def load_sessions_postgresql(self, conn_string: str):
        self.sessions = self.storer.load_sessions_postgresql(conn_string)
    ...
    def get_session_by_user_id(self, session_name: str, logging:bool=False) -> Optional[str]:
        """get_session_by_session_name
