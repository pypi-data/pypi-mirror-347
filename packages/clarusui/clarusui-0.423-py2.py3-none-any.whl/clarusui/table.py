...
class Table(GridViz):
    ...
    def set_export_column_renames(self, renames):
        self._check_enhanced()
        for header in self.headers:
            header_value = header.raw_cell_value
            if header_value in renames:
                rename = renames.get(header_value)
                header.add_data_attributes({'data-rename':rename})
    ...
    def _set_rows(self):
        ...
        for row_header in row_headers:
            ...
            rows_by_row_header[row_header] = row_element
            row_idx += 1

        #rows_by_row_header = {row.row_header:row for row in temp_rows}

        for temp_row in temp_rows:
            tr = rows_by_row_header.get(temp_row.parent_row_header)
            if tr:
                tr.add_child_row(temp_row)
                self._drilldown_link = None #disable drilldown

        for temp_row in temp_rows: #another pass after child rows asigned
