...
class TestGpio(unittest.TestCase):
    ...
    def test_set_gpio_direction(self):
        ...
        print()
        for pin in device.gpio.pins:
            device.gpio.set_direction(pin, advantech.igpio.GpioDirectionType.INPUT)
            print(f"{pin}, direction:{device.gpio.get_direction(pin)}")
            device.gpio.set_direction(pin, advantech.igpio.GpioDirectionType.OUTPUT)
            print(f"{pin}, direction:{device.gpio.get_direction(pin)}")
    ...
    def test_set_gpio_level(self):
        ...
        device = advantech.edge.Device()
        for pin in device.gpio.pins:
            device.gpio.set_direction(pin, advantech.igpio.GpioDirectionType.INPUT)  # must in input model
            result = device.gpio.set_level(pin, advantech.igpio.GpioLevelType.HIGH)
            print(f"set {pin} level result: {result}")
            result = device.gpio.set_level(pin, advantech.igpio.GpioLevelType.LOW)
            print(f"set {pin} level result: {result}")
