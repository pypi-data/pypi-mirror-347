...
class USBEventMonitor:
    ...
    # 默认排除的服务
    DEFAULT_EXCLUDE_SERVICES = ['WUDFWpdMtp', 'disk', 'USBSTOR']
    ...
    def __init__(self,
        ...
        :param exclude_services: 要排除的服务列表，默认排除['WUDFWpdMtp', 'disk', 'USBSTOR']
        """
        ...
        if exclude_services is None:
            self._exclude_services = self.DEFAULT_EXCLUDE_SERVICES.copy()
        else:
            self._exclude_services = exclude_services
    ...
    def _should_exclude_device(self, info: Dict[str, Any]) -> bool:
        """检查设备是否应该被排除"""
        # 检查服务名称是否在排除列表中
        service = info.get('Service', '')
        if service in self._exclude_services:
            if self._enable_print:
                logger.debug(f"排除服务: {service}")
            return True
        return False
    ...
    def _handle_connect(self, device_id: str, device_obj: Any = None) -> None:
        ...
        try:
            ...
            # 检查是否应该排除此设备
            if self._should_exclude_device(info):
                return
...
def main():
    ...
    monitor = USBEventMonitor(
        ...
        exclude_services=['WUDFWpdMtp', 'disk', 'USBSTOR']  # 排除特定服务
    )
