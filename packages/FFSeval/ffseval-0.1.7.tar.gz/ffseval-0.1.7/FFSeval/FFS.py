...
class J_2_k_a(Base):
    ...
    def EvalSurface(self):
        '''
        Kriging法による応答曲面を計算
        Calc()のたびに計算するのでは，無駄となるため，一度だけここで実施する
        '''
        df=super().GetData()
        self.dm=dmanage()
        if df['Case']=='PR' or df['Case']=='PJ':
            data=self.dm.Finput('J-2-k-a-1.csv')
            z=[5.0,10.0,20.0]
            X,W=self.dm.Fconv3D(data,z)
            r2_score=self.dm.KrigCalc(X,W)
        if df['Case']=='MR' or df['Case'] =='MJ':
            data=self.dm.Finput('J-2-k-a-2.csv')
            z=[5.0,10.0,20.0]
            X,W=self.dm.Fconv3D(data,z)
            r2_score=self.dm.KrigCalc(X,W)                 
    def Calc(self):
        ...
        if df['Case']=='PJ':
            ...
            H1,sigma=self.dm.Eval(target_point)           
