...
from cognyx_bom_sdk.models import Bom, BomInstance, BomInstanceUpdate, ObjectType
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        ...
        features = FeaturesSettings(
            projects=ProjectSettings(
                bom=BomConfig(default_view="view1", default_type="tree")
            )
        )
        with patch(
...
async def test_get_instance(client):
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        instance_response = BomInstanceResponse(
            ...
            entity_type_id="typeA-id",
            system_attributes=BomInstanceSystemAttributes(
                statuses=[]
            ),
            custom_attributes=[],
            bom_id="bid",
...
async def test_update_instance_attribute(client):
    ...
    with patch.object(client, "update_instance", new_callable=AsyncMock) as mock_update:
        ...
        assert kwargs.get("instance_id") == "iid"

        # Check that the update object has the correct structure
        update_obj = kwargs.get("properties")
