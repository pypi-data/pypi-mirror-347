            ...
            try:
                ...
                subdomains = [
                    Data(
                        x=torch.cat(
                            (
                                subdomain["interpolated_velocity"] / torch.max(torch.abs(subdomain["interpolated_velocity"])),
                                subdomain["interpolated_pressure"].unsqueeze(1) / torch.max(torch.abs(subdomain["interpolated_pressure"])),
                            ),
                            dim=1,
                        ),
                        ...
                        pos=subdomain["pos"],
                        y=torch.cat(
                            (subdomain["velocity"] / torch.max(torch.abs(subdomain["velocity"])),
                             subdomain["pressure"].unsqueeze(1) / torch.max(torch.abs(subdomain["pressure"]))),
                            dim=1,
                        ),
                    )
