...
def new_pr_command(base):
    ...
    prompt = (
        "Based on the following commit messages, generate a suitable pull request title and a detailed description. Respond with a JSON object containing the following keys:"
        "- 'title': A concise title for the pull request.\n"
        "- 'description': A detailed description of the pull request. Ensure that both ‘title’ and ‘description’ are URL-encoded strings, suitable for inclusion in URL query parameters. Do not include any markdown formatting or code block delimiters in your response. Ensure the response is a valid JSON object.\n"
        "Ensure that both ‘title’ and ‘description’ are URL-encoded strings, suitable for inclusion in URL query parameters. Do not include any markdown formatting or code block delimiters in your response. Ensure the response is a valid JSON object."
        "Ensure that any file names are enclosed in backticks (`) to denote code formatting.\n"
        f"Commit messages:\n{commit_messages}"
    )
    ...
    try:
        ...
        if platform == 'github':
            ...
            pr_url = (
                ...
                f"?quick_pull=1&title={urllib.parse.quote(title)}&body={urllib.parse.quote(description)}"
            )
