...
class EventCounters(BaseEstimator, TransformerMixin, TimeValidator):
    ...
    def __post_init__(self):
        if not self.events_df.empty:
            missing = {self.start_col, self.end_col} - set(self.events_df.columns)
            if missing:
                raise ValueError(f"Missing columns in events_df: {missing}")
            ev = self.events_df.sort_values(by=self.start_col).reset_index(drop=True)
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.events_df.empty:
            for col in [
                "event_count", "visited_count", "ignored_count",
                "visit_ratio", "ignore_ratio", "last_event_visited",
                "consecutive_ignores"
            ]:
                out[f"{self.event_name}_{col}"] = self.fillna
            return out

        visited_flags = np.array([
            ((times >= s) & (times <= e)).any()
            for s, e in zip(self.starts_, self.ends_)
        ], dtype=bool)
        ...
        for i in range(n):
            idx = np.searchsorted(self.ends_, times[i], side="right")
            if idx == 0:
                run_ignored = 0
                consec_ignores[i] = 0
                continue
