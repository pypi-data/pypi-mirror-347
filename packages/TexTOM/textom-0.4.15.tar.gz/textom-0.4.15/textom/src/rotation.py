...
def generate_group(generators):
    ...
    while queue:
        current = queue.pop()
        for g in G.copy():
            new = q_mult(np.array(current), np.array(g))
            new = new / np.linalg.norm(new)
            tup = tuple(np.round(new, decimals=7))  # avoid FP errors
            if tup not in G:
                G.add(tup)
                queue.append(tup)
    return np.array(list(G))

def map_to_fz(q, group_quats, q_ref=np.array([1, 0, 0, 0])):
    q = q / np.linalg.norm(q)
    best_q = q
    max_dot = -1
    for g in group_quats:
        q_sym = q_mult(g, q)
