...
class model:
    ...
    def __init__(self, sample_dir, classic=True, single=False, q_det=False, chi_det=False):
    ...
    def _init_diffractlets(self):
        ...
        QX_ew,QY_ew,QZ_ew, Ndet = self.get_reciprocal_space_coordinates()
        Q_ew = np.column_stack([QX_ew,QY_ew,QZ_ew])
    ...
    def _init_odf(self):
        ...
        if d_len > 0:
            ...
            Gs_wrapped = rot.OTPfromQ( 
                np.array([ rot.map_to_fz(qs,Q_group) for qs in Qs ])
             )
            # for sHSHs for each used n and all sample rotation angles
            self.Rs = self.get_Rs_stack(Gs_wrapped)
    ...
    def get_reciprocal_space_coordinates(self):
        ...
        QX,QY,QZ = np.empty_like(Tta_ew), np.empty_like(Tta_ew), np.empty_like(Tta_ew)
        for k in range(QY.shape[0]):
            QX[k], QY[k], QZ[k] = polar_to_3d(self.Qq_det[k], self.Chi_det[k], Tta_ew[k],
        ...
        # plt.legend()
        # plt.show()
        return QX, QY, QZ, Tta_ew.shape[0]

@njit
def polar_to_3d(q:float, chi:float, tta:float, u0=(0,0,1), u90=(0,1,0)):
    # axes
    ...
    point = q * (
        beam_dir * np.cos(tta) + 
        (np.cos(chi) * u0 + np.sin(chi) * u90) * np.sin(tta)
    )
