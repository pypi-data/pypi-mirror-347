...
from .text_set import TextSet  # Changed from essay_set import EssaySet
...
class PredictorExtractor(object):
    ...
    def initialize_dictionaries(self, p_set):
        ...
        if not (hasattr(p_set, '_type')):
            error_message = "needs to be a text set for analysis."
            log.exception(error_message)
            raise util_functions.InputError(p_set, error_message)

        if not (p_set._type == "analysis"):  
            error_message = "needs to be a text set for analysis."
            log.exception(error_message)
            raise util_functions.InputError(p_set, error_message)
        ...
        #Ensures that even with a large amount of input textual features
