from typing import TYPE_CHECKING, Any  # Asegúrate de importar Any
...
class YahooFinanceHeadless:
    ...
    def login(self):
        try:
            ...
            return (
                "A timeout exception has occurred. Most likely it's due "
                "to invalid login credentials. Please try again."
            )

    def _add_cookies_to_jar(self, cookies: list[dict[str, Any]]):
        for cookie in cookies:
            cookie_dict = {
                ...
                "path": cookie["path"],
                "expires": None,  # Add expiration if available
            }
