...
from ybankinplay.base import _YahooFinance
from ybankinplay.utils import _history_dataframe, convert_to_timestamp, flatten_list
...
class Ticker(_YahooFinance):
    ...
    def _financials(
        ...
        try:
            ...
            if prefix:
                ...
                df = df.rename(columns={"sourceDate": "date"})
    ...
    def _history_1m(self, adj_timezone=True, adj_ohlc=False):
        ...
        df = df.sort_values(by=["symbol", "date"])  # Reemplazamos inplace=True
        df = df.fillna(value=0)  # Reemplazamos inplace=True
        return df
    ...
    def option_chain(self):
        ...
        if dataframes:
            ...
            df = df.rename_axis(["symbol", "expiration", "optionType"])
            df = df.fillna(0)
            df = df.sort_index(level=["symbol", "expiration", "optionType"])
            return df
