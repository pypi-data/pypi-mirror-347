...
from pySPT import postprocessing
from pySPT import SliderCombo
from pySPT.utils import find_all_metadata, get_time_str, sort_in_parallel
if __name__ == "__main__":
    from drive_simulation import Simulation
from analyze_simulations import read_dream_fits
...
from resources.distribution_functions import *
...
def calculate_msd_fits(path: str, to_skip = [], save = True, dream_fits_path = None, show_handler = True):
    """
    ...
    paths = find_all_metadata(path)
    post_processing_objects = [postprocessing.PostProcessingJob(metadata_path) for metadata_path in paths]
    post_processing_objects, paths = sort_in_parallel(post_processing_objects, paths) 
    ...
    if save:
        ...
        save_path = save_results(data_results, path)
...
def analyze_msd_fits(data: dict, save_path = ''):
    """
    ...
    for column in data[list(data.keys())[0]]['fits'][0].columns:
        ...
        for path_idx,path in enumerate(data.keys()):
            for fit_idx, fits in enumerate(data[path]['fits']):
                ...
                ax.plot(fits[column], 'o',color=COLORS[path_idx], label=label)
    ...
    if "Expected_Val_1" in data[list(data.keys())[0]]['fits'][0].columns:
        ...
        for path_idx,path in enumerate(data.keys()):
            for fit_idx, fits in enumerate(data[path]['fits']):
                ...
                if fit_idx == 0:
                    label = data[path]['title']
                ax.plot(fits["Expected_Val_1"], 'o',color=COLORS[path_idx], label=label)
                ax.plot(fits["Expected_Val_2"], 's',color=COLORS[path_idx], label=label)
...
def main():
    ...
    data = {
            "E:/Current Microscope Data/CPR-ID-224/26C/eMSDs_Jan_21_2025_11_10_27": {
            "title": "2x/chain, BEFORE", "fits":[]},
            "D:/Northwestern University/RCH-WANG-GROUP - Documents/ORG-RSRCH-WANG-GROUP/Sync to Lab Computers/Chris/CPR-ID-233/eMSDs_Mar_17_2025_11_02_40": {
            "title": "2x/chain, AFTER", "fits":[]},
    }
    analyze_msd_fits(data)
