...
class PostProcessingJob:
    ...
    def get_id(self):
        return utils.search_regex_id(self.opened_path, group=False)
...
def plot_powerlaw(
    ...
    try:
        ...
        logging.info("Error in finding stderr of intercept")
...
def plot_trajectory_lengths(
    ...
    message += f"Mode length: {scipy.stats.mode(counts).mode} (count={scipy.stats.mode(counts).count})"
    ...
    table_var["Mode length"] = scipy.stats.mode(counts).mode
    table_var["Mode count"] = scipy.stats.mode(counts).count
