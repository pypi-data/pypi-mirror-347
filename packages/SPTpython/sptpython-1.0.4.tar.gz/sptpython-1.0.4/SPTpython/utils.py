...
def compatibilize_metadata(metadata):
    ...
    if "SPTpython version" in metadata.keys():
        metadata_version = metadata["SPTpython version"]
    else:
        metadata_version = metadata["pySPT version"]
...
def match_video_file(file, group_idx=None):
    # regex match video file name, returning corresponding group, if desired
    ...
    if not group_idx:
        ...
        return re.search(regex, file).group(group_idx+1)
...
def search_regex_id(s, depth = 0, group = True):
        ...
        if regex_match:
            ...
            for idx, folder in enumerate(parts):
                if re.search(regex, folder):
                    ...
                    break
            to_add = ""
            # Vapor percentage specified (or other information)
            if not group and re.search(r'\d+ ?(?:s|ms) ?(.+)', parts[len(parts)-3+depth]):
                to_add += "-" + re.search(r'\d+ ?(?:s|ms) ?(.+)', parts[len(parts)-3+depth]).group(1)
...
if __name__ == '__main__':
    path = r'D:\Northwestern University\RCH-WANG-GROUP - Documents\ORG-RSRCH-WANG-GROUP\Sync to Lab Computers\Chris\M7.5_X6.5_T1_L0.1\EtOH\400 ms 20%\nf500_d400_e400_p96_NDTiffStack1_May_14_2025_13_27_29'
    print(search_regex_id(path, depth=1))
