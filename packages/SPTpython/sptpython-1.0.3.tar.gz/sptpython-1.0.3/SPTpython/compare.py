...
class Comparison:
    ...
    def plot(
        ...
        if not bypass:
            categories = {title:True for title in function_titles}
            categories["Lag Frame"] = cfg["lag_frame"]
...
def map_compare_titles(titles, delays):
    ...
    for title in titles:
        title_ids.append(utils.search_regex_id(title, depth = 1, group = False))
    ...
    categories = {title:suggestion for (title, suggestion) in zip(titles, suggestions)}
    categories['group by exp'] = False
...
def reorder_lists_custom(l1, l2, numbers = None, bypass = False):
    ...
    if not bypass:
        ...
        categories = {l1_item:i for (l1_item, i) in zip(l1, numbers)}
