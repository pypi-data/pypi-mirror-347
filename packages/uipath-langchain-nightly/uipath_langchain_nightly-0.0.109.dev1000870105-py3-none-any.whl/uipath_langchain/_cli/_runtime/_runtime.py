        ...
        try:
            ...
            async with AsyncSqliteSaver.from_conn_string(
                ...
                if self.context.job_id is None:
                    ...
                    final_chunk = None
                    async for key, value in graph.astream(
                        processed_input,
                        graph_config,
                        stream_mode="updates",
                        subgraphs=True,
                        ...
                        if isinstance(value, dict):
                            for key_content, content in value.items():
                                if step:
                                    logger.info("[%s][%s]", step, key_content)
                                else:
                                    logger.info("[%s]", key_content)
                                if isinstance(content, dict):
                                    messages = content.get("messages", [])
                                    if isinstance(messages, list):
                                        for m in messages:
                                            if isinstance(m, BaseMessage):
                                                logger.info("%s", m.pretty_print())

                                    # Exclude "messages" from content and pretty-print the rest
                                    filtered_content = {
                                        k: v
