...
public:
     ...
     * If a field is not present in the JSON, it is initialized with an empty string.
     *
    ...
    ModelInfo(const json& model_json) {
        ...
        expired_at = model_json.value("expired_at", "");
    }

    /**
     * @brief Converts the model information to a Python dictionary.
     *
     * This method creates and returns a Python dictionary containing the model's
     * name, release date, and expiration date. The dictionary keys are:
     * - "name": The name of the model.
     * - "released_at": The release date of the model.
     * - "expired_at": The expiration date of the model.
     *
     * @return py::dict A Python dictionary with the model's metadata.
     */
    py::dict to_dict() const {
        py::dict model_dict;
        model_dict["name"] = name;
        model_dict["released_at"] = released_at;
        model_dict["expired_at"] = expired_at;
        return model_dict;
    }
};
...
public:
     ...
     */
    WadasModelServer(const std::string url, const std::string version = "v1") {
        server_url = url;
        if (server_url.back() == '/') {
            server_url.pop_back();
        }
     ...
     *
     * @param username The username of the node to register.
     * @param password The password of the node to register.
     * @return std::string The node ID if registration is successful, or an empty string on failure.
     */
    std::string register_node(const std::string& username, const std::string& password) {
        std::string org_code = login(username, password);
        if (org_code.empty()) {
            return "";
        }
    ...
    bool download_model(const std::string& node_id, const std::string& model_name, const std::string& model_path) {
        std::string url = server_url + "/nodes/" + node_id + "/models?model_name=" + model_name;
        int status_code = RestClient::download(url, model_path);
        if (status_code != 200) {
