...
NewStorageFields : TypeAlias = List[Dict[str, Any]]
NewStorageTypes  : TypeAlias = Dict[str, Any]
NewStorageInfo   : TypeAlias = Tuple[NewStorageFields, NewStorageTypes]
...
class CertoraBuildGenerator:
    ...
    def apply_extensions(
        ...
        if not storage_layout:
            build_logger.warning(f"Target contract {target_contract.name} has no storage layout")
            return
    ...
    def extract_slayout(self, original_file: str, ns_storage: Set[NameSpacedStorage]) -> NewStorageInfo:
        ...
        harness_name = self._generate_harness_name(original_file)
        ...
        with tempfile.NamedTemporaryFile(mode="w+t", suffix=".sol", dir=file_dir, delete=True) as tmp_file:
            ...
            var_to_slot = self._write_harness_contract(tmp_file, harness_name, ns_storage)
            tmp_file.flush()

            # Compile the file and extract storage layout
            try:
                ...
                # Use a high index (1000) to avoid conflicts with other compilation artifacts
                sdcs = self.collect_for_file(tmp_file.name, 1000, CompilerLangSol(), Path.cwd(), abs_path, None)
                if not sdcs:
                    raise RuntimeError(f"Failed to compile harness contract for {original_file}")

                # Instead of assuming it's in the first SDC, search through all SDCs
                harness_contract = None
                for sdc in sdcs:
                    found_contract = sdc.find_contract(harness_name)
                    if found_contract:
                        harness_contract = found_contract
                        break
                ...
                for storage_item in layout['storage']:
                    ...
                    if var_name in var_to_slot:
                        ...
                        build_logger.warning(f"Variable {var_name} not found in variable to slot mapping")
    ...
    def _generate_harness_name(self, original_file: str) -> str:
        """
        Generate a unique name for the harness contract based on the contract names in the file.

        Args:
            original_file: Path to the Solidity file

        Returns:
            str: A unique name for the harness contract
        """
        try:
            # Try to extract contract names from the file
            contract_names = []
            ...
            if contract_names:
                # Create a name based on concatenated contract names
                name_base = "_".join(contract_names)
                # Limit length to avoid issues
                if len(name_base) > 30:
                    name_base = name_base[:30]
                return f"{name_base}_Harness"
            else:
                # Fallback to a unique name based on file and timestamp
                file_name = Path(original_file).stem
                timestamp = int(time.time())
                return f"{file_name}_{timestamp}_Harness"
        except Exception as e:
            build_logger.warning(f"Could not generate unique harness name: {str(e)}, using default")
            return "CertoraStorageHarness"

    def _write_harness_contract(self, tmp_file: Any, harness_name: str, ns_storage: Set[NameSpacedStorage]) -> Dict[str, str]:
        """
    ...
    def handle_erc7201_annotations(self) -> None:
        ...
        for target_file in self.context.file_paths:
            for (imported_file, imported_file_ast) in self.asts[target_file].items():
                for node_id, def_node in imported_file_ast.items():
                    ...
                    for n in def_node.get("nodes"):
                        sinfo = self.erc7201_of_node(n)
        ...
        for target in self.get_primary_contracts_from_sdcs():
            ...
            for base in base_contracts:
                try:
                    layout = slayouts[(base[0], base[1])]
                    extensions.add(base[1])
                    harnesses[base[1]] = layout
                except KeyError:
                    pass
            self.apply_extensions(target, extensions, harnesses)
    ...
    def handle_storage_extension_harnesses(self) -> None:
        ...
        for (target, extensions) in storage_extensions.items():
            ...
            self.apply_extensions(target_contract, extensions, extension_to_fields_and_types)
