...
#         # hook enter/leave events on the figure
#         self.mpl_connect("figure_enter_event", self._on_figure_enter)
#         self.mpl_connect("figure_leave_event", self._on_figure_leave)

#         # state for legend animation
#         self._swipe_timer = QTimer(self)
#         self._swipe_timer.timeout.connect(self._swipe_step)
#         self._swipe_steps = 10
#         self._swipe_interval = 10  # ms
#         self._swipe_delta = 0
#         self._swipe_target_offset = 0
...
#             self.legend = self.ax_bottom.legend(
#                 loc="upper right",
#                 fontsize=10,
#                 frameon=True,
#                 labelspacing=0.5,
...
#             frame.set_edgecolor("none")
#             frame.set_boxstyle("round,pad=0.2")
#             self.legend.set_bbox_to_anchor((1.0, 1.0),
#                                         transform=self.ax_bottom.transAxes)
#             self._current_offset = 0
#             self._apply_legend_offset(0)
...
#         # swipe right off-canvas (e.g. +0.3 axes fraction)
#         if hasattr(self, "legend") and not self.navigator.looping:
#             self._start_swipe(0.8)

#     def _on_figure_leave(self, event):
#         # swipe back to original position
#         if hasattr(self, "legend") and not self.navigator.looping:
#             self._start_swipe(0.0)

#     def _start_swipe(self, target_offset):
#         """Begin an animated swipe of the legend’s x-offset in axes coords."""
#         self._swipe_target_offset = target_offset
#         delta = (target_offset - self._current_offset) / self._swipe_steps
#         self._swipe_delta = delta
#         self._swipe_timer.setInterval(self._swipe_interval)
#         self._swipe_timer.start()

#     def _swipe_step(self):
#         # move current offset toward target
#         self._current_offset += self._swipe_delta
#         # clamp at or beyond target
#         if (self._swipe_delta > 0 and self._current_offset >= self._swipe_target_offset) or \
#         (self._swipe_delta < 0 and self._current_offset <= self._swipe_target_offset):
#             self._current_offset = self._swipe_target_offset
#             self._swipe_timer.stop()

#         self._apply_legend_offset(self._current_offset)
#         self.fig.canvas.draw_idle()

#     def _apply_legend_offset(self, offset):
#         """Helper to move the legend by `offset` in axes-fraction units."""
#         # original anchor (1,1) plus offset in X
...
#         # --- 3) blit once for both axes ---
#         QTimer.singleShot(0, lambda: self.fig.canvas.blit(self.fig.bbox))

#         self._last_highlight_override = override

#     def _on_full_draw(self, event):
#         if self.point_highlighted:
#             # draw the highlight onto the full canvas
#             self.highlight_marker_top.set_visible(True)
#             self.top_vline.set_visible(True)
#             self.ax_top.draw_artist(self.highlight_marker_top)
#             self.ax_top.draw_artist(self.top_vline)
#             if not self._last_highlight_override:
#                 self.highlight_marker_bottom.set_visible(True)
#                 self.ax_bottom.draw_artist(self.highlight_marker_bottom)

#         # *then* snapshot the clean background (with highlight baked in)
#         self._background = self.copy_from_bbox(self.fig.bbox)
...
class TrajectoryCanvas(QWidget):
    ...
    def load_trajectories_from_df(self, df, anchors_map=None, roi_map=None, forcerecalc=False):
        ...
        # --- Prompt for channel selection if multi-channel ---
        selected_channel = 1
        if n_chan > 1:
            # build list of channel labels
            chan_labels = [f"Channel {c}" for c in range(1, n_chan+1)]
            choice, ok = QInputDialog.getItem(
                self, "Select channel", "Choose channel for analysis:",
                chan_labels, 0, False
            )
            if not ok:
                return
            selected_channel = int(choice.split()[-1])
        # enforce selection in navigator
        self.navigator._select_channel(selected_channel)

        # ensure custom_load_map exists
        ...
        for t, (traj_num, group) in enumerate(groups):
            ...
            if "Channel" in group.columns:
                ...
                # default to the user‐selected channel
...
class HistogramCanvas(FigureCanvas):
    ...
    def __init__(self, navigator, parent=None):
        ...
        self._update_pending = False
        self._last_histogram_params = (None, None, None, None, None, None, None)
    ...
    def update_histogram(self, image, center, crop_size, sigma=None, intensity=None, background=None, peak=None):
        """
        Instead of updating immediately, store the parameters and schedule an update
        only once per 16 ms (about 60 FPS).
        """

        self._last_histogram_params = (image, center, crop_size, sigma, intensity, background, peak)
    ...
    def _throttled_histogram_update(self):
        ...
        self._update_pending = False
        image, center, crop_size, sigma, intensity, background, peak = self._last_histogram_params
        self._do_update_histogram(image, center, crop_size, sigma, intensity, background, peak)

    def _do_update_histogram(self, image, center, crop_size, sigma=None, intensity=None, background=None, peak=None):
