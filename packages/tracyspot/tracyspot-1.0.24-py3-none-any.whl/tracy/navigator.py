        ...
        if was_valid:
            ...
            if getattr(self, "check_colocalization", False) and self.movie.ndim == 4:
                ...
                traj["colocalization_any"][idx] = (
                    "Yes" if self._compute_colocalization_single_frame(idx) else "No"
                )
                # per-channel
                old_ch = self.analysis_channel
                for ch in traj["colocalization_by_ch"]:
                    self.analysis_channel = ch
                    flag = self._compute_colocalization_single_frame(idx)
                    traj["colocalization_by_ch"][ch][idx] = "Yes" if flag else "No"
                self.analysis_channel = old_ch
