    ...
    * min_displacement (optional - defaults to -max_displacment)
    ...
    def template(self):
        ...
        if self._template is None:
            raise AttributeError("The template wasn't set before being required")
        else:
    ...
    def leapfrog(self):
        ...
        if self._last_point is None:
            raise RuntimeError("Can't use leapfrog until you have measured at least one point.")
        image, image_pos = self._last_point
        self.template = self.generate_template(image)
        if np.any(self.image_shape != image.shape):
            raise MappingError("The image size doesn't match the previous image size")
...
def move_until_motion_detected(tracker, move, displacement, threshold=10, multipliers=2**np.arange(14), detect_cumulative_motion=False):
    ...
    raise MappingError("Moved the stage by {} but saw no motion.".format(multipliers[-1] * displacement))
