...
class TastytradeConnector(BrokerConnector):
	...
	async def _disconnect_internal(self):
		...
		if self._session is not None:
			if self.task_listen_quotes is not None:
				self.task_listen_quotes.cancel()
				self.task_listen_quotes = None
				self.task_listen_greeks.cancel()
				self.task_listen_greeks = None
				self.task_listen_candle.cancel()
				self.task_listen_candle = None

			if self.task_listen_accounts is not None:
				self.task_listen_accounts.cancel()
				self.task_listen_accounts = None

			if self._streamer is not None:
				try:
					await asyncio.shield(self._streamer.close())
					self._streamer = None
	...
	async def requestTickerData(self, symbols: List[str]):
		...
		"""
		self._streamer = await DXLinkStreamer(self._session, disconnect_fn=self._on_streamer_disconnect)
