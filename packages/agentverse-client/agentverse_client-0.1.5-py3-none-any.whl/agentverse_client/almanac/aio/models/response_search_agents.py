...
class ResponseSearchAgents(BaseModel):
    ...
    anyof_schema_1_validator: Optional[WithPaginationListAgent] = None
    # data type: List[Agent]
    anyof_schema_2_validator: Optional[List[Agent]] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[List[Agent], WithPaginationListAgent]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "List[Agent]", "WithPaginationListAgent" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, WithPaginationListAgent):
            error_messages.append(f"Error! Input type `{type(v)}` is not `WithPaginationListAgent`")
        else:
            return v

        # validate data type: List[Agent]
        try:
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[WithPaginationListAgent] = None
        try:
            instance.actual_instance = WithPaginationListAgent.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # deserialize data into List[Agent]
        try:
            # validation
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[Agent], WithPaginationListAgent]]:
