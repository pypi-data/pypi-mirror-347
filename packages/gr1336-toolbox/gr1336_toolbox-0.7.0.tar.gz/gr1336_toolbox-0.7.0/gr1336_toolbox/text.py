...
def normalize_text(text: str) -> str:
    ...
    """
    return simplify_quotes(" ".join(text.lower().split()))
...
def unescape(
    ...
    if isinstance(entry, bytes):
        return entry.decode(encoding="unicode-escape", errors=errors)
    return entry.encode("unicode-escape", errors=errors).decode(
        "unicode-escape", errors=errors
    )
...
def escape(
    ...
    if isinstance(entry, str):
        entry = entry.encode(encoding="utf-8", errors=errors)
