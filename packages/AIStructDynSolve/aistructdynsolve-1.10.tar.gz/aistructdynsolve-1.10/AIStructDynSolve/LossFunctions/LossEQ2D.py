...
class LossEQ2D:
    def __init__(
        ...
        self.duration_time, self.eq_time, self.eq_accX, self.eq_accY, self.dimensionsless_Pt = inputEQ.get_EQparm()
    ...
    def lossODE(self):
        ...
        # 拼接成 (len(x_in), 2)
        acc_interpolation = np.squeeze(np.stack([acc_interpolationX, acc_interpolationY], axis=1))
        acc = torch.from_numpy(acc_interpolation).repeat(1, self.DOF // 2).to(DEFAULT_DTYPE).to(DEVICE)
        #Pt = -self.mass @ acc.T
        # 计算整体的残差
