...
class MaPFNN(NN):
    ...
    """
    Multi-scale Activation Function Parallel Fully-connected neural network that uses independent sub-networks for each
    network output.
    Args:
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers[:-1]):
            ...
            if isinstance(self.activation, list):
                if self.activation[i] == torch.sin:
                    if isinstance(layer, torch.nn.ModuleList):
                        if isinstance(x, list):
                            x = [sum(self.activation[i](f(x_* omega)) for omega in self.omegas) for f, x_ in zip(layer, x)]
                        else:
                            x = [sum(self.activation[i](f(x* omega)) for omega in self.omegas) for f in layer]
                    else:
                        x = sum(self.activation[i](layer(x* omega)) for omega in self.omegas)
                else:
                    if isinstance(layer, torch.nn.ModuleList):
                        if isinstance(x, list):
