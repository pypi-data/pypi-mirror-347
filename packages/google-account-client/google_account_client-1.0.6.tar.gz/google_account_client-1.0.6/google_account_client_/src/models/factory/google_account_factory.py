...
class GoogleAccountFactory(Logger):
    ...
    def create_account(self, name: str, token: str | dict | Credentials | None = None) -> GoogleAccount:
        """
        Creates a new GoogleAccount instance with valid credentials.

        Args:
            name (str): Account identifier.
            token (optional): Path, dict or Credentials instance for the user token.
