...
class GoogleCalendar(GoogleInterface):
    """
    ...
    def calendar_list_events(self, event_count: int = 10, start_date_time: dt.datetime = None, end_date_time: dt.datetime = None) -> list[any]:
        ...
        """
        self.log(f'Getting up to {event_count} events...', 'debug')
        self.log(
            f"Filtering events from {start_date_time.strftime('%Y-%m-%d %H:%M') if start_date_time else 'now'} "
            f"to {end_date_time.strftime('%Y-%m-%d %H:%M') if end_date_time else 'undetermined'}",
            level='debug'
        )
        ...
        for event in events:
            ...
            self.log(start + ' - ' + event.get('summary', 'No title'), 'debug')
        self.log(']', 'debug')
