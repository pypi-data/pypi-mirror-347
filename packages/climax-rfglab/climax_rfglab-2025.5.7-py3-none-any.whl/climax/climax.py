...
class climax(App):
    ...
    def __init__(self, filename: str|list[str]='', channel_strs: Optional[tuple[str]] = ('', ), cmap: str = 'gray', figsize: tuple[int, int] =(4,4)):
        ...
        if filename==[] or filename is None or filename==False or filename=='':
            if 'site-packages' in __file__:
                filename = os.path.join(os.path.dirname(__file__)[:str.rfind(os.path.dirname(__file__), 'site-packages')], climax.__default_filename__)
            filename = os.path.join(os.path.dirname(__file__)[:str.rfind(os.path.dirname(__file__), __package__)], climax.__default_filename__)
            print(f"{filename=}")
