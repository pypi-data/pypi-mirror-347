...
from .instrumentation.utils import parse_body, caller_site
...
class FileExporter:
    ...
    def _export(self) -> None:
        ...
        if not data_dir:
            _log.error("TRAINLOOP_DATA_FOLDER not set - export skipped")
            self.buf.clear()
            return
        ...
        for llm_call in self.buf:
            ...
            req = parse_body(llm_call.get("requestBodyStr", ""))
            res = parse_body(llm_call.get("responseBodyStr", ""))

            _log.info("Request: %s", req)
            _log.info("Response: %s", res)
            ...
            tag = llm_call.get("tag") or "untagged"
            _log.info("Location: %s", loc)
            _log.info("Tag: %s", tag)
            update_registry(data_dir, loc, tag)
            _log.info("Updated registry")

            if llm_call.get("tag") is None:
                _log.info("Untagged call - skipping")
                continue  # skip untagged saves
            ...
            sample = CollectedSample(
                ...
                modelParams=req.get("modelParams", {}),
                startTimeMs=llm_call.get("startTimeMs", 0),
                endTime=llm_call.get("endTimeMs", 0),
