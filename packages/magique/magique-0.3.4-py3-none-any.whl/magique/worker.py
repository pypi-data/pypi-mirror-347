...
class MagiqueWorker(NetworkObject):
    def __init__(
        self,
        service_name: str,
        server_url: str = "wss://magique.spateo.aristoteleo.com/ws",
            ...
            if isinstance(value, InvokeFuture):
                ...
                if future.service_id != self.service_id:
                    logger.info(
                        ...
                        f"Connecting to the server at {self.server_url}"
                    )
    ...
    async def get_another_service(self, service_id: str) -> "ServiceProxy":
        ...
        server = await connect_to_server(self.server_url)
    ...
    async def register_to_server(self, websocket):
        logger.info(f"Connected to the server at {self.server_url}")
        service_info = self.get_service_info()
    ...
    async def register_and_handle_server_requests(
            self, after_register: T.Optional[T.Callable[[object], T.Awaitable]] = None):
        ...
        while True:
            try:
                ...
                async with ws_connect(self.server_url) as websocket:
                    retry_count = 0
    ...
    async def run(self, after_register: T.Optional[T.Callable[[object], T.Awaitable]] = None):
        ...
        if self.link_to_server:
            coroutines.append(self.register_and_handle_server_requests(after_register))
