...
class KsEastmoneyFundamentalApi(BaseFundamentalApi):
    ...
    def ctr(self, method: CTR_METHOD, indicators: list[str], options: str = ''):
        ...
        df = c.ctr(method.value, indicators, options_str)
        if isinstance(df, c.EmQuantData) and df.ErrorCode == 0:
            return RET_ERROR, df
