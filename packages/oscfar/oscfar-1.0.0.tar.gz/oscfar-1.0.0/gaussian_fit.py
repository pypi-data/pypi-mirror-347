...
def sum_of_gaussians(x, *params):
    ...
    *params : float
        ...
        - stddev: Standard deviation (width) of the Gaussian. Stddev should be positive.
...
def sum_of_scattered_gaussians(x, *params):
    ...
    *params : float
        ...
        - mean: Mean (center) of the Gaussian part before convolution (t0).
        - sigma: Standard deviation (width) of the Gaussian part.
                 Must be positive (e.g., enforced by fitter bounds).
        - tau: Scattering timescale (decay constant of the exponential).
...
def find_best_multi_gaussian_fit(
    ...
    dict
        ...
        - 'best_fit': A dict with 'n_components', 'popt', 'pcov', 'bic', 'rss'
                      for the model with the lowest BIC.
        - 'all_fits': A list of dicts, each containing 'n_components', 'popt',
...
def _process_model_fit(args):
    ...
    ------
    ValueError:
        If `initial_flat_params` is invalid, data is empty/mismatched, or
        `model_to_test` is an invalid option.
    """
    x_data = np.asarray(x_data)
    y_data = np.asarray(y_data)
