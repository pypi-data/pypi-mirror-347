...
class flarum_api:
    ...
    def _post_call(self, sUrlPath, dictPayload, sMethodOverride=''):
        ...
        try:
            ...
            if response.status_code in [201, 200]:
                PrintTimeMsg(f"_post_call.ok:len(response.text)={len(response.text)}=")
                oJson = response.json()
                if self.bDebugPrint:
    ...
    def _get_call(self, sUrlPath):
        ...
        try:
            ...
            if response.status_code in [201, 200]:
                PrintTimeMsg(f"_get_call.ok:len(response.text)={len(response.text)}=")
                oJson = response.json()
                if self.bDebugPrint:
    ...
    def get_post_dict_by_discussion_id(self, sDiscussionId):
        ...
        for post in oJson['included']:
            ...
            sId = post['id']
            dictAttr = post['attributes']
            dictPostTitleById[sId] = dictAttr['content']
    ...
    def get_post_topic_by_discussion_id(self, sDiscussionId):
        ...
        dictPostTitleById = self.get_post_dict_by_discussion_id(sDiscussionId)
