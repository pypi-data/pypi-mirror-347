...
class HdDB:
    ...
    @attach_motherduck
    def create_hd_views(self, org: str, db: str, id: str, tbl: str, type: str, config: dict):
        try:
            self.execute("BEGIN TRANSACTION;")

            create_query = f"CREATE TABLE \"{org}__{db}\".hd_views (id VARCHAR, user_table VARCHAR, type VARCHAR, config VARCHAR);"
            print(create_query)
            self.execute(create_query)
    ...
    @attach_motherduck
    def insert_hd_views(self, org: str, db: str, id: str, tbl: str, type: str, config: dict):
        try:
            insert_query = f"INSERT INTO \"{org}__{db}\".hd_views (id, user_table, type, config) VALUES (?, ?, ?, ?);"
            self.execute(insert_query, [id, tbl, type, config])
        except duckdb.Error as e:
