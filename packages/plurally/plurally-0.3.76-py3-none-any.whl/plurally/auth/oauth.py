...
from plurally.datetime_utils import dt_parse, utc_now
...
class Oauth2Authenticator(AbstractOauth2Authenticator):
    ...
    def __init__(
        ...
        self._token_expiry_date = token_expiry_date or (utc_now() - timedelta(days=1))
...
class SingleUseRefreshTokenOauth2Authenticator(Oauth2Authenticator):
    ...
    def __init__(
        ...
        self._client_id: str = self._get_config_value_by_path(("credentials", "client_id"), client_id)
        self._client_secret: str = self._get_config_value_by_path(("credentials", "client_secret"), client_secret)
        self._client_id_name = client_id_name
        self._client_secret_name = client_secret_name
    ...
    def get_token_expiry_date(self) -> datetime:
        ...
        Otherwise, it parses the expiry date string into an datetime object.

        Returns:
            datetime: The parsed or calculated token expiry date.

        Raises:
            TypeError: If the result is not an instance of datetime.
        """
        expiry_date = self._get_config_value_by_path(self._token_expiry_date_config_path)
        result = utc_now() - timedelta(days=1) if expiry_date == "" else dt_parse(expiry_date)
        if isinstance(result, datetime):
            return result
        raise TypeError("Invalid datetime conversion")

    def set_token_expiry_date(self, new_token_expiry_date: datetime) -> None:  # type: ignore[override]
        """
        Sets the token expiry date in the configuration.

        Args:
            new_token_expiry_date (datetime): The new expiry date for the token.
        """
        self._set_config_value_by_path(self._token_expiry_date_config_path, str(new_token_expiry_date))

    def token_has_expired(self) -> bool:
        """Returns True if the token is expired"""
        return utc_now() > self.get_token_expiry_date()
    ...
    def get_new_token_expiry_date(
        ...
        Returns:
            datetime: The calculated expiry date of the access token.
        """
        if token_expiry_date_format:
            return dt_parse(access_token_expires_in)
        else:
            return utc_now() + timedelta(seconds=int(access_token_expires_in))
    ...
    def get_access_token(self) -> str:
        ...
        if self.token_has_expired():
            ...
            new_token_expiry_date: datetime = self.get_new_token_expiry_date(access_token_expires_in, self._token_expiry_date_format)
    ...
    def _get_config_value_by_path(self, config_path: Union[str, Sequence[str]], default: Optional[str] = None) -> str | Any:
        """
