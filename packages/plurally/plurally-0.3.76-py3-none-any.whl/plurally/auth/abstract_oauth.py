...
class DefaultBackoffException(Exception): ...
...
class AbstractOauth2Authenticator(AuthBase):
    ...
    def token_has_expired(self) -> bool:
        ...
        return datetime.now(timezone.utc) > self.get_token_expiry_date()  # type: ignore
    ...
    @tenacity.retry(
        retry=tenacity.retry_if_exception_type(DefaultBackoffException),
        sleep=tenacity.wait_exponential(multiplier=1, min=4, max=10),  # type: ignore
        after=tenacity.after_log(logger, logging.INFO),  # type: ignore
    )
        ...
        try:
            ...
            if e.response is not None:
                if e.response.status_code == 429 or e.response.status_code >= 500:
                    raise DefaultBackoffException(f"Error while refreshing access token: {e}") from e
    ...
    def _parse_token_expiration_date(self, value: Union[str, int]) -> datetime:
        """
    ...
    def _find_and_get_value_from_response(
        ...
        if isinstance(response_data, dict):
            ...
            for _, value in response_data.items():
                result = self._find_and_get_value_from_response(value, key_name, max_depth, current_depth + 1)
        ...
        # get from the nested array object
            ...
            for item in response_data:
                result = self._find_and_get_value_from_response(item, key_name, max_depth, current_depth + 1)
                if result is not None:
                    return result
