...
TEMPORARY_ENVIRONMENT="$INSTALLATION_PATH/temp_env"

setup_virtualenv() {
  echo "Setting up Python virtual environment..."
  rm -rf "$TEMPORARY_ENVIRONMENT"                            # Remove old venv
  virtualenv --system-site-packages "$TEMPORARY_ENVIRONMENT" # Create new venv
  echo "Virtual environment created at $TEMPORARY_ENVIRONMENT."
}
...
echo "Installing $SOURCE in $TEMPORARY_ENVIRONMENT..."
$TEMPORARY_ENVIRONMENT/bin/python -m pip install "$SOURCE" --force-reinstall | tee >(grep -c '^Collecting ' >"$INSTALLATION_PATH/.packages-count")

echo "$SOURCE installed successfully in $TEMPORARY_ENVIRONMENT."

UBO_APP_DIR=$($TEMPORARY_ENVIRONMENT/bin/python -c 'import ubo_app; print(ubo_app.__path__[0])')
...
if [ -z "$WITHOUT_DOCKER" ]; then
  ...
  $UBO_APP_DIR/system/scripts/install_docker.sh
  echo "Docker installed successfully."
fi

rm -rf "$INSTALLATION_PATH/env"
echo "Moving $TEMPORARY_ENVIRONMENT to $INSTALLATION_PATH/env..."
mv "$TEMPORARY_ENVIRONMENT" "$INSTALLATION_PATH/env"
...
sudo XDG_RUNTIME_DIR=/run/user/$(id -u "$USERNAME") -u "$USERNAME" systemctl restart ubo-app
