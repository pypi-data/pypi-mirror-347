...
class DatasetMultiCommand(MultiCommand):
    ...
    def get_command(self, ctx, name):
        if name in self.registry.get_all_managers():
            ...
            @click.option(
                "--force",
                is_flag=True,
                help="Force fetch new data from the API. Useful if the data has been updated on the API.",
                default=False,
                show_default=True,
            )
            @set_log_verbosity()
            ...
            def _download(
                ...
                manager = UnifiedDataManager(self.registry, force=force)
                file_paths = manager.download_by_name(
                    name, ds_name, directory, overwrite, force
                )
            ...
            def download_all(
                ...
                manager = UnifiedDataManager(self.registry, force=force)
                file_paths = manager.download_all(name, directory, overwrite)
    ...
    def format_usage(self, ctx, formatter):
        ...
        formatter.write_usage(self.name, "[DATASET_TYPE] [SUBCOMMAND] [ARGS]...")
...
@click.option(
    ...
    "--refresh",
    is_flag=True,
    help="Fetch all datasets and hydrate the cache.",
    default=False,
    show_default=True,
)
