...
class pyOptSparseWarning:  # noqa: N801
...
class History:
    def __init__(self, fileName, optProb=None, temp=False, flag="r"):
        ...
        if self.flag == "n":
            ...
            if os.path.exists(fileName):
                ...
                raise FileNotFoundError(f"The requested history file {fileName} to open in read-only mode does not exist.")
            self._processDB()
        else:
    ...
    def _processDB(self):
        ...
        if self.metadata["version"] != "2.13.2":
            pyOptSparseWarning(
