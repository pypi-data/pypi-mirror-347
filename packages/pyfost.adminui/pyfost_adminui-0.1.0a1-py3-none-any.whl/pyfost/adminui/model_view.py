from typing import Type, TypeVar, get_args, Literal
...
class ModelView:
    ...
    def __init__(self, admin, model_type: ModelType, getter, pk_name: str = "id"):
        print(
            "------------------------------------------------- NEW MODEL VIEW",
            model_type,
        )
        ...
        self.model_pk_name = pk_name
        self.default_hide_pk = pk_name == "id"

        self._card_prefix = f"{self.admin.prefix}/{self.model_type_name}"
    ...
    def _get_columns(self) -> list[dict]:
        ...
        for field_name, field in self.model_type.model_fields.items():
            ...
            if self.default_hide_pk and field_name == self.model_pk_name:
                c["classes"] += " hidden"
        ...
        for column in columns:
            column_renderer = self.admin.get_column_renderer(self.model_type, column)
    ...
    def set_items(self, items: list[AdminModel]):
        self._items = items
        self._rows = []
        self._by_pk.clear()
        for item in self._items:
            pk = getattr(item, self.model_pk_name)
            r = item.model_dump()
            for k, v in r.items():
                r[k] = item.cell(k, self.admin)
            r["__pk__"] = pk
            self._rows.append(r)
            self._by_pk[pk] = item
    ...
    async def render(self):
        ...
        with ui.element().classes("w-full bg-stone-100 p-5"):
            with ui.row(align_items="center").classes("w-full"):
                ui.label(self.model_type_name).classes("text-h4 my-5")
                ui.space()
