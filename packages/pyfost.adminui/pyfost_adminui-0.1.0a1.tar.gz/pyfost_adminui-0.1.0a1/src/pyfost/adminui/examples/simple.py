...
class Position(AdminModel):
    name: str
    x: float = 0
    y: float = 0


class Tag(AdminModel):
    name: str
    description: str = ""
    color: str | None = None


class MyModel(AdminModel):
    name: str
    description: str
...
admin.add_view(MyModel, get_my_models, pk_name="name")
admin.add_view(Tag, get_tags, pk_name="name")
admin.add_view(Position, get_positions, pk_name="name")
admin.add_view(Project, get_projects, pk_name="name")
...
if __name__ == "__main__":
    if 0:
        ...
        uvicorn.run(
            "pyfost.adminui.examples.simple:app", host="0.0.0.0", port=8001, reload=True
        )
