...
class ModelValueFieldRenderer(FieldRenderer):
    @classmethod
    def handles(cls, admin, model, field, value, editable):
        return isinstance(value, AdminModel)

    @classmethod
    def render_label(cls, admin, model, field_name, value, editable: bool):
        ui.label(f"{field_name}:").classes("text-right")

    @classmethod
    def render_value(cls, admin, model, field_name, value, editable: bool):
        # FIXME: get those from admin and model:
        prefix = "/admin"
        pk = value.id  # value.name
        model_type_name = value.__class__.__name__
        if not editable:
            value.render(field_name, admin)
        else:
            value.render_editor(field_name, admin)
...
class ModelListValueFieldRenderer(FieldRenderer):
    ...
    def handles(cls, admin, model, field, value, editable):
        ...
        for item in value:
            if not isinstance(item, AdminModel):
                # We only handle list where all items are AdminModel
    ...
    def render(cls, admin: Admin, model, field_name, value, editable: bool):
        if not editable:
            ...
            renderer.render_label(admin, model, field_name, value, editable)
            with ui.row():
                for i, item in enumerate(value):
                    model_view = admin.get_model_view_for(item)
                    pk_field_name = model_view.model_pk_name
            ...
            model.render_editor(field_name, admin)
