...
class AsyncAPI(API):
    ...
    async def get_data(
        ...
        data_frames = await tqdm_asyncio.gather(
            *tasks, desc="Fetching data", timeout=timeout, colour="#e4e8ee"
        )
        ...
        if as_dict:
            if as_star_schema:
                ...
                return to_star_schema(
                    data,
                    self._selected_metrics.reset_index(names=["metric"]),
                    as_dict=True,
                )
            return data.reset_index().to_dict("records")
        if as_star_schema:
            ...
            return to_star_schema(
                data, self._selected_metrics.reset_index(names=["metric"])
            )
        return U.parse_timestamp(data) if not data.empty else data
