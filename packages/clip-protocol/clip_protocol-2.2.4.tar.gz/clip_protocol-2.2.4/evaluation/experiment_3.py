...
class Experiment3:
    def __init__(self, df):
        self.df = df
        self.events_names = ["Participant", "AOI Name"]
    ...
    def run_command(self, e, k, m, df):
        ...
        error = compute_error_table(get_real_frequency(df), df_estimated, self.p)
...
def run_experiment_3(df, datasets):
    ...
    print(f"Best k: {experiment.k}, m: {experiment.m}, e: {e_r}")
    ...
    for data in datasets:
        ...
        table, max_error, e = experiment.optimize_e(experiment.k, experiment.m, data_df, e_r)
        end_time = time.time()
    ...
    plot_relative_errors_multiple_tables(tablas, dataset_sizes)
...
if __name__ == "__main__":
    ...
    #data_path = "/Users/martajones/Downloads/datos"
    datasets = []
    dataset_lengths = []

    excel_files = [file for file in os.listdir(data_path) if file.endswith(".xlsx")]
    ...
    sorted_datasets = [d for _, d in sorted(zip(dataset_lengths, datasets), key=lambda x: x[0])]
    middle_index = len(sorted_datasets) // 2
    middle_dataset = sorted_datasets[middle_index]

    run_experiment_3(middle_dataset, datasets)