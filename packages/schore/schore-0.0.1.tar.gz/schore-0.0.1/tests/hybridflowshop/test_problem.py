...
def test_problem_structure(problem):
    """Test the overall structure of the parsed problem."""
    assert problem.num_jobs == 6
    assert problem.num_stages == 3
    assert problem.machines_per_stage == [2, 3, 2]
    assert isinstance(
...
"""
    with pytest.raises(
        ValueError,
        match=r"Stage count mismatch; num_stages=\d+; by machines_per_stage=\d+",
    ):
...
def test_repr(problem):
    ...
    assert repr(problem) == "HybridFlowShopProblem(num_jobs=6, num_stages=3)"
