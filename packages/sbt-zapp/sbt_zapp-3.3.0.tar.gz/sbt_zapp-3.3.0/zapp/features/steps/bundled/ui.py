...
from zapp.features.steps import locator
...
def assert_element_count_comparison(context, count, target, comparator="ровно"):
    actual = len(context.browser.elements(locator(target)).highlight())
    assert_that(
        comparators[comparator](actual, count),
        equal_to(True),
        f'Количество элементов "{target}" должен быть {comparator} "{count}". Фактическое: {actual}',
    )
...
def variable_equals_element(context, name, target):
    ...
    context.browser.element(locator(target)).highlight().should_have_text(
        value_to_compare
    )
...
def variable_not_equals_element(context, name, target):
    ...
    """
    value_to_compare = get_from_variables(name)
    context.browser.element(locator(target)).highlight().should_not_have_text(
        value_to_compare
    )
...
@then(
    'Я убедился что среди "{target}" элемент содержащий текст "{text}" имеет атрибут "{attribute_name}"="{attribute_value}"'
)
...
@then(
    'Я убедился что среди "{target}" элемент с текстом "{text}" имеет атрибут "{attribute_name}" содержащий "{attribute_value}"'
)
...
@then(
    'Я убедился что среди "{target}" элемент содержащий текст "{text}" имеет атрибут "{attribute_name}" содержащий "{attribute_value}"'
)
