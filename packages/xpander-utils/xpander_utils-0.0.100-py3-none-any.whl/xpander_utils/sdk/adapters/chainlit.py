...
class ChainlitAdapter(SDKAdapter):
    ...
    async def process_tool_calls(self,run_id: str, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], is_sequence_agent: bool = False, sequence_agent_graph_items:List[GraphItem] = [], reported_sequence_sub_agent_ids: set = set(), on_node_enter: Optional[Callable] = None, on_node_error: Optional[Callable] = None, check_if_agent_has_dedicated_workers: Optional[Callable] = None, agent_custom_functions: list[GraphItem] = [], local_tools: Dict[str, Callable] = None) -> str | None:
        """
        ...
        for raw_tool_call in tool_calls.values():
            ...
            if is_debug_mode_active:
                ...
                if is_sub_agent_execution:
                    current_step.input = tool_call.payload['bodyParams']['input_task']
                else:
            ...
            if tool_call.type == ToolCallType.LOCAL:
                ...
                if is_max_tool_call_reached:
                    ...
                    if graph_item and graph_item.type.value == AgentGraphItemType.AGENT.value and check_if_agent_has_dedicated_workers:
                        has_dedicated_workers = await check_if_agent_has_dedicated_workers(agent_id=graph_item.item_id,organization_id=self.agent.organization_id)
