...
const char DefaultFont[] = "assets/font/Ubuntu-M.ttf";
...
static PyObject *DrawText(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	//error detection
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "siiiO|s", (char **)kwlist, &text, &x, &y, &size, &color, &font))
	{
		PyErr_SetString(PyExc_TypeError, "Expected arguments: text (str), x (int), y (int), size (int), color (list)");
...
static PyObject *Button(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs,  "siiiiOOOOO|s", (char **) kwlist, &text, &x, &y, &width, &height, &text_color, &normal_color, &hover_color, &clicked_color, &callback, &font))
	{
		PyErr_SetString(PyExc_TypeError, "Expected: text(str), x(int), y(int), w(int), h(int), text_color(list), normal_color(list), hover_color(list), clicked_color(list), callback(func), font(optional)");
...
static PyObject *CenterTextX(PyObject *self, PyObject *args)
{
	...
	if(!PyArg_ParseTuple(args, "si|s", &text, &size, &font))
	{
		PyErr_SetString(PyExc_TypeError, "Expected arguments : text(string), size(int)");
...
//Function LoadIMG(image), to load an image, IMPORTENT : Use this outside of the mainloop.
...
{
    const char *name;  // Changed from string to const char*
    int loop;
...
static PyObject *CheckCollisionRectCircle(PyObject *self, PyObject *args)
{
    ...
    int x2 = 0, y2 = 0;
    float rad = 0.0f;
    ExtractFromList(x2, y2, DummyWidthOrHeightForCircle, DummyWidthOrHeightForCircle, rad, circle);
