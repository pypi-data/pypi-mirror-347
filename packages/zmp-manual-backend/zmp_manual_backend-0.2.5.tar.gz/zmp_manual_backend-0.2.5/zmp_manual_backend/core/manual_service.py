...
class ManualService:
    ...
    async def publish_manual(
        ...
        try:
            ...
            try:
                # Apply timeout to avoid hanging on repository operations
            ...
            try:
                export_success, export_path, mdx_files = await self.export_repository(
                    notion_page_id=notion_page_id,
                    output_dir=os.path.join(repo_path, self.source_dir),
                ...
                logger.error(f"Error during export for job {job_id}: {str(e)}")
                self.active_jobs[job_id].status = JobState.FAILED
                self.active_jobs[job_id].failure_reason = FailureReason.EXPORT_FAILED
                self.active_jobs[job_id].message = f"Export error: {str(e)}"
                return job_id
            ...
            # Set source path for translations
            ...
            try:
                ...
                job_specific_source_path = os.path.join(
                    repo_path,
                    os.path.relpath(export_path_str, os.path.dirname(repo_path)),
                )
                ...
                if translation_success:
                    ...
                    try:
                        ...
                        if push_success:
                            # Update job status for successful completion
            ...
            # Clean up job repository if needed (for failed jobs)
            if (
                job_id in self.active_jobs
                and self.active_jobs[job_id].status == JobState.FAILED
            ):
                try:
                    # Consider cleaning up failed job repositories
                    # This is commented out to preserve failed jobs for debugging
                    # if os.path.exists(job_repo_path):
                    #     shutil.rmtree(job_repo_path)
                    #     logger.info(f"Cleaned up failed job repository: {job_repo_path}")
                    pass
                except Exception as cleanup_e:
                    logger.warning(
                        f"Failed to clean up job repository: {str(cleanup_e)}"
                    )
    ...
    async def _cleanup_old_files(
        ...
        try:
                ...
                for root, dirs, files in os.walk(base_dir, topdown=False):
                    ...
                    if is_directory:
                        ...
                        if (
                            ...
                            target_path = root
                            if not is_safe_to_delete(target_path, repo_path):
                                logger.warning(
            ...
            for base_path in base_paths:
                if not is_safe_to_delete(base_path, repo_path):
    ...
    async def _ensure_repository(self, repo_path) -> bool:
        ...
        try:
            ...
            if is_job_specific:
                ...
                if repo_path.exists():
                    ...
                    try:
                        ...
                        # Add a short delay to ensure filesystem sync
            ...
            if should_clone:
                ...
                # Generate a unique UUID for the temporary directory
                temp_dir_uuid = uuid.uuid4()
                temp_dir = Path(f"/tmp/repo_{temp_dir_uuid}")

                # Check if the temporary directory already exists (very unlikely with UUIDs)
                if temp_dir.exists():
                    logger.warning(f"Temporary directory already exists: {temp_dir}")
                    # Try to use a different UUID
                    temp_dir_uuid = uuid.uuid4()
                    temp_dir = Path(f"/tmp/repo_{temp_dir_uuid}")

                    # If it still exists, handle the edge case
                    if temp_dir.exists():
                        logger.warning(
                            f"Second temporary directory also exists: {temp_dir}"
                        )
                        # Create a timestamp-based directory instead
                        temp_dir = Path(f"/tmp/repo_{int(time.time())}")
                ...
                try:
                    logger.info(
                        f"Cloning repository {self.github_repo_url} with branch {self.github_branch} to temporary directory {temp_dir}"
                    )
                    ...
                    logger.info(
                        f"Successfully cloned repository branch {self.github_branch} to temporary directory"
                    )
                    ...
                    try:
                        ...
                        logger.info(f"Ensured target directory exists: {repo_path}")
                    except Exception as e:
                        ...
                        try:
                            ...
                            gc.collect()
                            await asyncio.sleep(1)
                            await asyncio.to_thread(robust_rmtree, str(temp_dir))
                            logger.info(
                                f"Cleaned up temporary directory after error: {temp_dir}"
                            )
                        except Exception as cleanup_e:
                        ...
                        if hasattr(self, "_current_job_context"):
                            ...
                            if job_id in self.active_jobs:
                                ...
                                self.active_jobs[
                                    ...
                                    f"Failed to create target directory: {str(e)}"
                                )
                            return False

                    # Now move all contents from temp_dir to repo_path with proper error handling
                    try:
                        logger.info(
                            f"Moving repository contents from {temp_dir} to {repo_path}"
                        )
                        # Clean the target directory except .git
                        if repo_path.exists() and any(repo_path.iterdir()):
                            logger.info(f"Cleaning target directory: {repo_path}")
                            for item in repo_path.iterdir():
                                if item.name != ".git":
                                    try:
                                        ...
                                        logger.warning(
                                            f"Failed to remove item {item}: {str(e)}"
                                        )

                        # Use cp -rf to move all contents from temp_dir to repo_path
                        try:
                            await asyncio.to_thread(
                                subprocess.run,
                                ["cp", "-rf", f"{temp_dir}/.", str(repo_path)],
                                check=True,
                            )
                            logger.info(
                                "Successfully copied all repository contents from temp_dir to repo_path"
                            )
                        except subprocess.CalledProcessError as e:
                            logger.error(f"cp -rf failed: {e}")
                            return False
                        except Exception as e:
                            logger.error(f"Unexpected error during cp -rf: {e}")
                            return False
                        ...
                        try:
                            ...
                            await asyncio.to_thread(robust_rmtree, str(temp_dir))
                            logger.info(
                                "Cleaned up temporary directory using robust_rmtree"
                            )
                        except Exception as e:
                            logger.warning(
                        ...
                        # Verify repository is valid after move
                        try:
                            repo = git.Repo(repo_path)
                            if repo.active_branch.name != self.github_branch:
                                logger.warning(
                                    f"Branch mismatch after move: {repo.active_branch.name}, expected: {self.github_branch}"
                                )
                                repo.git.checkout(self.github_branch)
                                logger.info(
                                    f"Checked out correct branch after move: {repo.active_branch.name}"
                                )
                            logger.info("Verified repository is valid after move")
                            del repo
                            gc.collect()
                            await asyncio.sleep(1)
                        except Exception as e:
                            logger.error(
                                f"Repository validation failed after move: {str(e)}"
                            )
                            if hasattr(self, "_current_job_context"):
                                job_id = self._current_job_context["job_id"]
                                if job_id in self.active_jobs:
                                    ...
                                    self.active_jobs[
                                        job_id
                                    ].message = f"Repository validation failed after move: {str(e)}"
                            return False
                    except Exception as e:
                        logger.error(f"Error during repository content move: {str(e)}")
                        # Clean up temporary directory
                        try:
                            del repo
                            gc.collect()
                        ...
                        if hasattr(self, "_current_job_context"):
                            ...
                            if job_id in self.active_jobs:
                                ...
                                self.active_jobs[
                                    ...
                                    f"Failed to move repository contents: {str(e)}"
                                )
                        return False
