...
from starbot.core.bot import StarBot  # isort:skip
...
from starbot.core.utils.chat_formatting import humanize_list, inline, warning
...
#         self.bot: StarBot = bot
#         self.cog: commands.Cog = None

#     @classmethod
#     def _setup(cls, bot: StarBot, cog: commands.Cog) -> None:
#         """
...
class Cog(commands.Cog):
    __authors__: typing.List[str] = ["Scarlet"]
    ...
    async def cog_load(self) -> None:
        ...
        # Prevent StarBot `(timeout)` error.
        asyncio.create_task(self.cog_load_new_task())

    async def cog_load_new_task(self) -> None:
        # Wait until StarBot ready. But `(timeout)` when cog loading when bot starting...
        ...
        # Add SharedCog.
        if self.qualified_name != "Scarlet_Utils":
            try:
                old_cog = await self.bot.remove_cog("Scarlet_Utils")
                Scarlet_Utils = SharedCog(self.bot)
                try:
                    if getattr(old_cog, "sentry", None) is not None:
                        Scarlet_Utils.sentry = old_cog.sentry
                        Scarlet_Utils.sentry.cog = Scarlet_Utils
                    Scarlet_Utils.loops = old_cog.loops
                except AttributeError:
                    pass
                await self.bot.add_cog(
                    Scarlet_Utils, override=True
                )  # `override` shouldn't be required...
            except discord.ClientException:  # Cog already loaded.
                pass
            except Exception as e:
                self.logger.debug("Error when adding the `Scarlet_Utils` cog.", exc_info=e)
            else:
                await Scarlet_Utils.sentry.maybe_send_owners(self)
        # Count this cog (anonymous stats).
        Scarlet_Utils = self.bot.get_cog("Scarlet_Utils")
        counted_cogs = await Scarlet_Utils.config.counted_cogs()
        if self.qualified_name not in counted_cogs:
            try:
                ...
                await Scarlet_Utils.config.counted_cogs.set(counted_cogs)
    ...
    async def cog_unload(self) -> None:
        ...
        for loop in self.loops.copy():
            if self.qualified_name == "Scarlet_Utils" and loop.name == "Sentry Helper":
                continue
        ...
        self.views.clear()
        # Remove SharedCog.
        Scarlet_Utils: SharedCog = self.bot.get_cog("Scarlet_Utils")
        if Scarlet_Utils is not None:
            if Scarlet_Utils.sentry is not None:
                await Scarlet_Utils.sentry.cog_unload(self)
            if not CogsUtils.at_least_one_cog_loaded(self.bot):
                try:
                    discord.utils.get(Scarlet_Utils.loops, name="Sentry Helper").stop_all()
                except ValueError:
                    pass
                await self.bot.remove_cog("Scarlet_Utils")
    ...
    def format_help_for_context(self, ctx: commands.Context) -> str:
        ...
        if self.qualified_name not in ("Scarlet_Utils"):
            text += bold(
    ...
    async def cog_command_error(self, ctx: commands.Context, error: Exception) -> None:
        Scarlet_Utils = ctx.bot.get_cog("Scarlet_Utils")
        ...
        if is_command_error:
            uuid = uuid4().hex
            no_sentry = Scarlet_Utils is None or getattr(Scarlet_Utils, "sentry", None) is None
            if not no_sentry:
                Scarlet_Utils.sentry.last_errors[uuid] = {"ctx": ctx, "error": error}
            ...
            if (
                not no_sentry
                and getattr(Scarlet_Utils.sentry, "display_sentry_manual_command", True)
                and await Scarlet_Utils.senderrorwithsentry.can_run(ctx)
                and not getattr(Scarlet_Utils.senderrorwithsentry, "__is_dev__", False)
            ):
                message += "\n" + inline(
                    "You can send this error to the developer by running the following"
                    f" command:\n{ctx.prefix}Scarlet_Utils senderrorwithsentry {uuid}"
                )
            ...
            if not no_sentry:
                await Scarlet_Utils.sentry.send_command_error(ctx, error)
