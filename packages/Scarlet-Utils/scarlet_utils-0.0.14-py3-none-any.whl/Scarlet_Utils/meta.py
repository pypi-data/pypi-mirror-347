...
import aiohttp
from starbot.core import VersionInfo, commands
from starbot.core import version_info as cur_red_version
...
async def format_info(
    ...
    loops : List[Loop], optional
        List of Loops you want to show, by default []
    ...
    start = f"{qualified_name} by Scarlet.\n<https://github.com/deathamongstlife/StarBots>\n\n"
    ...
    if not red_updated:
        update_msg += "To update StarBot, contact Scarlet\n"
...
async def out_of_date_check(cogname: str, currentver: str) -> None:
    ...
    try:
        ...
        if VersionInfo.from_str(currentver) < vers.cog:
            log.warning(
                f"Your {cogname} cog, from Scarlet, is out of date. You can update your cogs with the "
...
async def _get_latest_vers(cogname: str) -> Vers:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get("https://pypi.org/pypi/StarBot-DiscordBot/json", timeout=3) as r:
            data = await r.json()
