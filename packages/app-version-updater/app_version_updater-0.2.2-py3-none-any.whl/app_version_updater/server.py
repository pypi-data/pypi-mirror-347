...
class UpdaterServer:
    ...
    def app_version(self, os_: Literal["windows", "linux"]) -> str:
        try:
            return self.__find_latest_version(os_).encode()
        except FileNotFoundError:
            raise UpdaterException("404 No client update")

    def app(self, version: str, os_: Literal["windows", "linux"]) -> bytes:
        try:
    ...
    def __find_latest_version(self, os_: Literal["windows", "linux"]) -> str:
    ...
    def __get_file_by_version(self, version: str, os_: Literal["windows", "linux"]) -> Path:
    ...
    def __get_folder_content(self, os_: Literal["windows", "linux"]):
    ...
    def __is_file_valid(self, file_name: str, os_: Literal["windows", "linux"]):
