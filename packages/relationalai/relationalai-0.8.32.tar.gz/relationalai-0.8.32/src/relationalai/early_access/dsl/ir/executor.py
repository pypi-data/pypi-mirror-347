    ...
    def resources(self):
        if not self._resources:
            with debugging.span("create_session"):
                self.config.set("use_graph_index", False)
    ...
    def execute_model(self, model: Model, result_cols: Optional[List[str]] = None) -> DataFrame:
        ...
        if exports:
            for export in exports:
                ...
                self.execute(ir_model, export_ir_model, export.labels, export.target_table)
            if not queries:
                return DataFrame()
        query_ir_model = self.compiler.compile_queries(model.queries())
        return self.execute(ir_model, query_ir_model, result_cols)

    def execute(self, model: ir.Model, task: ir.Model, result_cols: Optional[List[str]] = None, export_to:Optional[str]=None) -> DataFrame:
        ...
        if not export_to:
            ...
            assert result_cols
            raw = self._export(full_code, result_cols, export_to)
            if raw:
