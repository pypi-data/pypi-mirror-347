...
from relationalai.early_access.metamodel.rewrite import Splinter, RewriteListTypes, GarbageCollectNodes
...
class ModelToRel:
    ...
    def _generate_rule(self, rule: ir.Logical) -> list[Union[rel.Def, rel.RawSource]]:
        ...
        if not effects:
            ...
            raise Exception("Cannot create top-level rule with both aggregates and ranks.")
        ...
        if aggregates:
            ...
            for agg in aggregates:
                ...
                for arg in agg.args:
                    if helpers.is_aggregate_input(arg, agg):
                        ...
                        result.append(self.handle_var(arg))
            ...
            for rank in ranks:
                if all(o for o in rank.arg_is_ascending):
                    rel_name = "std::common::sort"
                elif all(not o for o in rank.arg_is_ascending):
                    rel_name= "std::common::reverse_sort"
                else:
                ...
                params = tuple([abstraction, self.handle_var(rank.result), *out_vars, *key_vars])
                exprs.append(rel.atom(rel_name, params))
    ...
    def handle_value_list(self, types, values) -> Tuple[Union[rel.Primitive, rel.MetaValue, rel.Var], ...]:
    ...
    def handle_rank(self, n: ir.Rank) -> rel.Expr:
        return rel.atom(u.rel_operator(n.relation.name), tuple([self.handle_var(n.result)]))
    ...
    def handle_annotation(self, n: ir.Annotation):
        ...
        return rel.Annotation(
            n.relation.name,
            self.handle_value_list(self._relation_types(n.relation), n.args) # type:ignore
        )

    def handle_update(self, n: ir.Update):
        return rel.atom(
            self._relation_name(n.relation),
            self.handle_value_list(self._relation_types(n.relation), n.args)
        )
    ...
    def handle_construct(self, n: ir.Construct):
        args = [self.handle_value(None, v) for v in n.values] + [self.handle(n.id_var)]
    ...
    def _split_tasks(self, tasks: PySequence[ir.Task]) -> tuple[list[ir.Task], list[Union[ir.Update, ir.Output]], list[ir.Aggregate], list[ir.Rank]]:
        effects = []
        aggregates = []
        other_body = []
        ranks = []
    ...
    def _effect_args(self, n: ir.Task):
        if isinstance(n, ir.Output):
            ...
            for _, v in n.aliases:
                args.append(self.handle_value(None, v))
            return tuple(args)
        elif isinstance(n, ir.Update):
            args = self.handle_value_list(self._relation_types(n.relation), n.args)
            return tuple(args)
        else:
