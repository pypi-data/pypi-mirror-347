...
class Relationship(Producer):
    ...
    def __getitem__(self, arg:str|int|Concept) -> Any:
        if isinstance(arg, int):
            ...
            if len(self._fields) <= arg:
                raise ValueError(f"Relationship '{self._name}' has only {len(self._fields)} fields")
            return RelationshipFieldRef(self._parent, self, arg)
        elif isinstance(arg, str):
            if arg not in self._field_names:
                raise ValueError(f"Relationship '{self._name}' has only {self._field_names} fields")
            return RelationshipFieldRef(self._parent, self, self._field_names.index(arg))
        elif isinstance(arg, Concept):
            result:RelationshipFieldRef|None = None
            for idx, ref in enumerate(self._field_refs):
                if result is None and ref._thing._id == arg._id:
                    result = RelationshipFieldRef(self._parent, self, idx)
                else:
...
def rank(*args: Any) -> Aggregate:
    return Aggregate(Relationship.builtins["rank"], *args)
...
class Compiler():
    def __init__(self):
        ...
        self.relations:dict[Relationship|Concept|RelationshipRef|ir.Relation, ir.Relation] = {}
    ...
    def to_relation(self, item:Concept|Relationship|RelationshipRef|ir.Relation) -> ir.Relation:
    ...
    def select(self, fragment:Fragment, items:PySequence[Expression], ctx:CompilerContext):
        ...
        if fields:
            ...
            ctx.add(f.output(fields, annos=annos))
    ...
    def lookup(self, item:Any, ctx:CompilerContext) -> ir.Value|list[ir.Var]:
        if isinstance(item, ConceptExpression):
            ...
            if item._op._name == "rank":
                ...
                projection = flatten([self.lookup(key, agg_ctx) for key in find_keys(unwrapped_args)])

                args = flatten([self.lookup(a, agg_ctx) for a in unwrapped_args])

                internal_vars = set(args + projection)
                ir_node = f.rank(relation, projection, group, args, rank_orders, out)
                ...
                internal_vars = set(raw_args + projection)
                ir_node = f.aggregate(relation, projection, group, args)

            final_ctx = ctx.clone()
            if agg_ctx.items:
                internal = internal_vars - set(flatten(list(ctx.value_map.values())))
            ...
            if not len(into_vars) and item._select:
                into_vars = sub_ctx.into_vars = [f.var(f"v{i}") for i in range(len(item._select))]
