    ...
    def print_node(self, node, indent=0) -> None:
        ...
        #--------------------------------------------------
            ...
            if isinstance(node.expr, Identifier) and node.expr.name in infix:
                ...
                is_agg_or_rank = False
                is_like = False
                if isinstance(node.expr, Identifier):
                    self.print_node(node.expr, indent)
                    is_agg_or_rank = node.expr.name in rel_utils.OPERATORS.values()
                    is_agg_or_rank |= node.expr.name == "std::common::sort"
                    is_agg_or_rank |= node.expr.name == "std::common::reverse_sort"
                    is_like = node.expr.name == "like_match"
