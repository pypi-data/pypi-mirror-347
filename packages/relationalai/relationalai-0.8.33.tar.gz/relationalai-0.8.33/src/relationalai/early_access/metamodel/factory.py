...
def output(values: PySequence[Union[ir.Value, Tuple[str, ir.Value]]], engine: Optional[ir.Engine]=None, annos: PySequence[ir.Annotation]=[]):
    ...
    for x in values:
        if isinstance(x, ir.Var):
            ...
            s.add(x)
    return ir.Output(engine, s.frozen(), FrozenOrderedSet(annos))
...
def rank(relation: ir.Relation, projection: PySequence[ir.Var], group: PySequence[ir.Var], args: PySequence[ir.Var], arg_is_ascending: PySequence[bool], result: ir.Var, engine: Optional[ir.Engine]=None, annos: PySequence[ir.Annotation]=[]):
    return ir.Rank(engine, relation, tuple(projection), tuple(group), tuple(args), tuple(arg_is_ascending), result, FrozenOrderedSet(annos))
