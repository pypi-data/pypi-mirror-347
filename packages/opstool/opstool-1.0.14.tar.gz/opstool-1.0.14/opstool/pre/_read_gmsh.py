...
class Gmsh2OPS:
    ...
    def set_output_file(self,
        ...
        with open(self.out_file, "w+", encoding=encoding) as outf:
            outf.write(
                f"# This file was created by {self.__class__.__name__}\n\n")
            if self.out_type == "py":
    ...
    def read_gmsh_data(self, print_info: bool = True):
        ...
        for dim, etag in self.gmsh_dim_entity_tags:
            bound_dimtags = gmsh.model.getBoundary(dimTags=[(dim, etag)],
                                                   oriented=False)
            ...
            for tag in ele_tags:
                ele_type, node_tags, dim, etag = gmsh.model.mesh.getElement(
                    tag)
        ...
        for iGroup in vGroups:
            ...
            vEntities = gmsh.model.getEntitiesForPhysicalGroup(
                dimGroup, tagGroup)
    ...
    def _print_info(self):
        ...
        for dim, etag in self.gmsh_dim_entity_tags:
            if dim == 0:
                ...
                num_vol += 1
            n += 1
        print(f"Info:: Geometry Information >>>\n"
    ...
    def get_node_tags(
        ...
        if dim_entity_tags is None and physical_group_names is None:
            ...
            entity_tags = [(int(etag[0]), int(etag[1]))
            ...
            for _, ele_nodes in self.gmsh_eles.items():
                if ele_nodes:
                    for tag, ntags in ele_nodes.items():
                        ops.element(ops_ele_type, tag, *ntags[:-1],
                                    *ops_ele_args)
                        ele_tags.append(tag)
            return ele_tags
        elif dim_entity_tags is not None:
    ...
    def get_boundary_dim_tags(
        ...
        _get_boundary_dim_tags(boundary_dimtags, entity_tags,
                               self.gmsh_entities)
...
def _retrieve_entities(lines, entities_idx, physical_tag_name_map):
    ...
    while idx < entities_idx[1]:
        for i in range(num_points):
            ...
            tag = int(contents[0])
            entities[(0,
                      tag)]["Coord"] = [float(data) for data in contents[1:4]]
