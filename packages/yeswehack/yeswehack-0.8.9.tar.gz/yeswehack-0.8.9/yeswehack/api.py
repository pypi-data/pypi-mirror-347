...
class YesWeHack(YwhApiObject):
    ...
    def __init__(self, headers=None, totp_code=None, **kwargs):
        logging.basicConfig()
        ...
        self.session.headers = self._prepare_headers(headers or {})
        if self.pat:
    ...
    def _prepare_headers(self, headers):
        return self._prepare_user_agent(headers)

    def _prepare_user_agent(self, headers):
        existing_user_agent = None
        for header, value in headers.copy().items():
            if header.lower() == "user-agent":
                del headers[header]
                existing_user_agent = value
                break
        user_agent_parts = []
        if existing_user_agent:
            user_agent_parts.append(existing_user_agent)
        user_agent_parts.append(f"yeswehack/{__VERSION__}")
        user_agent_parts.append(f"python/{'.'.join(map(str, sys.version_info[:3]))}")
        user_agent_parts.append(f"platform/{platform.platform()}")
        headers["User-Agent"] = " ".join(user_agent_parts)
        return headers
...
class Log(YwhApiObject):
    ...
    _attrs = [
        ...
        Attr(name="new_ask_for_fix_verification_status", type_=str, default=None),
        Attr(name="old_ask_for_fix_verification_status", type_=str, default=None),
        Attr(name="program", type_=dict, default={}),
        Attr(name="old_program", type_=dict, default={}),
        Attr(name="new_triage_status", type_=str, default=None),
        Attr(name="old_triage_status", type_=str, default=None),
    ]
...
class Report(YwhApiObject):
    ...
    _attrs = [
        ...
        Attr(name="ask_for_fix_verification_status", type_=str),
        Attr(name="attachments", type_=list, default=[]),
        ...
        Attr(name="created_at", type_=str),
        Attr(name="cve", type_=dict, default={}),
        ...
        Attr(name="impact", type_=str, default=""),
    ...
    def put_ask_for_fix_verification_status(self, status, message=None, attachments=None) -> List[Log]:
        data = {
            "status": status,
            "attachments": attachments or [],
        }
        ...
        http_response = self.ywh_api.raw_call(
            method='PUT',
            url=f'{self.ywh_api.api_url}/reports/{self.id}/ask-for-fix-verification-status-update',
            data=json.dumps(data),
            headers=self.ywh_api.apps_headers,
        )
        if not http_response.ok:
            raise APIError(f'Unable to send report ask for fix verification status. Response: {http_response.text}')

        try:
            content = http_response.json()
        except ValueError as e:
            raise APIError(
                f'Unable to parse response when sending report ask for fix verification status. Response: {http_response.text}'
            ) from e

        if not isinstance(content, dict) or 'items' not in content or not isinstance(content.get('items'), list):
            raise APIError(
                f'Unexpected response format when sending report ask for fix verification status. Response: {http_response.text}'
            )
