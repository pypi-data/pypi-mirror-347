...
class TrainAE:
    ...
    def __init__(self, args, path, fix_thres=-1, load_tb=False, log_metrics=False, keep_models=True, log_inputs=True,
        ...
        self.pools = pools
        self.default_params()
        self.fill_missing_params_with_default(args)
    ...
    def default_params(self):
        self.all_params = {
            ...
            'n_meta': 0,
            'embeddings_meta': 0,
            # 'features_to_keep': 'features_proteins.csv',
            'groupkfold': 1,
            'dataset': 'custom',
            'bs': 32,
            'path': './data/',
            ...
            'n_agg': 5,  # Number of trailing values to get stable valid values
    ...
    def fill_missing_params_with_default(self, params):
        ...
        Args:
            params: Dictionary of parameters to be filled.

        Returns:
            params: Updated dictionary with default values for missing parameters.
        """
        # Fill in missing parameters with default values
        params = vars(params)
        for param in list(self.all_params.keys()):
            if param not in params:
                params[param] = self.all_params[param]
            if self.all_params[param] != params[param]:
                self.all_params[param] = params[param]
