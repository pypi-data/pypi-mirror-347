...
def setup_common_resource(app):
    ...
    def wx_mini_code2session(js_code, client_id=None, app_id=None):
        """
        微信mini的code2session
        :param js_code: 三方服务表里的id
        :param client_id:微信扫码的临时授权码
        :param app_id:微信扫码的临时授权码
        :return:
        """
        headers = {key: value for key, value in request.headers if key.lower() != 'host'}
    ...
    @routes.get_route("微信mini的用户信息解密")
    ...
    def get_proper_ding_user_info_by_auth_code(client_id, app_id, auth_code):
        headers = {key: value for key, value in request.headers if key.lower() != 'host'}
        result = capacity_post("/third_service_user/get_proper_ding_user_info_by_auth_code", headers=headers, data={
            "client_id": client_id,
            "app_id": app_id,
            "auth_code": auth_code,
        })
    ...
    def get_proper_ding_user_info_by_code(client_id, app_id, code):
        headers = {key: value for key, value in request.headers if key.lower() != 'host'}
        result = capacity_post("/third_service_user/get_proper_ding_user_info_by_code", headers=headers, data={
            "client_id": client_id,
            "app_id": app_id,
            "code": code,
        })
    ...
    @routes.get_route('微信扫码获取微信用户信息', '/get_wx_user_info_by_code')
    ...
    @routes.get_route('绑定三方信息', '/bind_by_third_user_info')
    def bind_by_third_user_info(user_id: str, service_id: str, open_id: str = None, union_id: str = None,
        ...
        result = capacity_post("/third_service_user/bind_by_third_user_info", headers=headers, data={
            "user_id": user_id,
    ...
    @routes.get_route('解绑三方信息', '/unbind')
