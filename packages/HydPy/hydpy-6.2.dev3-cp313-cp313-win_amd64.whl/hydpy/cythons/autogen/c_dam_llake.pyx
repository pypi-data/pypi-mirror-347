...
cdef class Model:
    ...
    cpdef inline void pic_exchange_v1(self) noexcept nogil:
        cdef numpy.int64_t idx
        self.sequences.fluxes.exchange = 0.0
        for idx in range(self.sequences.inlets.len_e):
            self.sequences.fluxes.exchange = self.sequences.fluxes.exchange + (self.sequences.inlets.e[idx][0])
    ...
    cpdef inline void pic_inflow(self) noexcept nogil:
        ...
        for idx in range(self.sequences.inlets.len_q):
            self.sequences.fluxes.inflow = self.sequences.fluxes.inflow + (self.sequences.inlets.q[idx][0])
    cpdef inline void pic_exchange(self) noexcept nogil:
        cdef numpy.int64_t idx
        self.sequences.fluxes.exchange = 0.0
        for idx in range(self.sequences.inlets.len_e):
            self.sequences.fluxes.exchange = self.sequences.fluxes.exchange + (self.sequences.inlets.e[idx][0])
