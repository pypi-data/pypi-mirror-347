...
class NFA:
    ...
    def __str__(self):
        print("States:")
        for state in self.Q:
            print(f"{set(state) if isinstance(state, frozenset) else state}")
        
        print("\nAlphabet:")
        print(f"  {set(self.E) if isinstance(self.E,frozenset) else self.E }")

        print("\nStart State:")
        print(f"  {set(self.q0) if isinstance(self.q0, frozenset) else self.q0}")
        ...
        for from_state, transitions in self.T.items():
            for symbol, to_state in transitions.items():
                print(f"  δ({set(from_state) if isinstance(from_state, frozenset) else from_state}, '{symbol}') => {set(to_state) if isinstance(to_state, frozenset) else to_state}")
    ...
    def DFA_equivalence(self):
        ...
        return NFA(dfa_states, self.E , dfa_transitions, initial_state, dfa_final_states)
