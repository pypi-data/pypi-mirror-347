...
def round_amount(amount, digits):
    """ Función auxiliar para redondear haciendo que todos los casos de .5 se redondeen hacia arriba """
    multiplier = 10 ** digits
    return math.floor(round(amount * multiplier + 0.5, digits)) / multiplier
...
class ExportationElectronicInvoiceValidator:
    ...
    def _validate_amount(invoice):
        if invoice.get_total_item_amount() != invoice.get_total_amount():
...
class ExportationElectronicInvoiceItem(object):
    ...
    def total_price(self):
        ...
        return round_amount(self.quantity * self.unit_price - self.bonification, 2)
...
class ExportationElectronicInvoice(inv.Invoice):
    ...
    def get_total_amount(self):
        return round_amount(self.total_amount, 2)

    def get_total_item_amount(self):
        total = sum(item.total_price for item in self.array_items)
