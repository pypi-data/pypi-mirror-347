...
logger.info("Looking for .env file in: %s", os.path.join(os.getcwd(), ".env"))

if not FIGMA_API_TOKEN:
    logger.error(
        "FIGMA_API_TOKEN not found in environment variables. Figma tools will not work."
    )
    ...
    for i in range(0, len(node_ids), max_ids_per_request):
        ...
        try:
            ...
            if data.get("err"):
                ...
                for node_id in batch_ids: image_urls_map.setdefault(node_id, None)
            if "images" in data and data["images"]:
                image_urls_map.update(data["images"])
            for node_id in batch_ids: image_urls_map.setdefault(node_id, data.get("images", {}).get(node_id))
        ...
        if page.get("type") == "CANVAS" and isinstance(page.get("children"), list):
            for node in page["children"]:
                if node.get("type") == "FRAME":
                    ...
                    frame_data = {
                        "name": node.get("name", "Unnamed Frame"), "node_id": node.get("id", ""),
                        "type": "FRAME", "page_name": page.get("name", "Unnamed Page"),
                        "page_id": page.get("id"), "width": bbox.get("width"), "height": bbox.get("height")
                    }
                    frames_meta.append(frame_data)
                    if node.get("id"): frame_ids.append(node.get("id"))
    return frames_meta, frame_ids
...
async def _get_figma_screens_info_async(figma_url: str, scale: float, image_format: str) -> Dict[str, Any]:
    ...
    for frame in frames_meta:
        ...
        if frame["image_url"] is None: logger.warning(f"No image URL/error for frame {node_id} ('{frame['name']}') in {file_key}.")
...
async def _get_figma_node_details_json_async(figma_url: str, node_ids_str: str) -> Dict[str, Any]:
    ...
    if not node_id_list: raise ValueError("node_ids parameter is empty or just commas.")
...
async def _get_figma_node_image_url_async(figma_url: str, node_id_str: str, scale: float, image_format: str) -> Dict[str, Any]:
    ...
    if not clean_node_id: raise ValueError("node_id parameter is empty.")
...
def main():
    """Entry point for running the Figma MCP server."""
