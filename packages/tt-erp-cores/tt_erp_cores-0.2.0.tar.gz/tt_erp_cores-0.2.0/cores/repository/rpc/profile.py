...
from cores.component.memcache import list_cacheable
...
class IProfileQueryRepository:
    ...
    def search_data(
        self, search_params: ProfileRPCSearchParams
    ) -> list[Profile]:
        raise NotImplementedError
...
class RPCProfileRepository(IProfileQueryRepository, ClientBase):
    async def _initialize(self):
        self._base_url = Config.PROFILE_BASE_URL
        await self.set_jwt_token_and_headers(
            target_service_id=Config.PROFILE_SERVICE_ID
        )
        return self
    ...
    async def search_data(
        ...
        data = await self.curl_api_with_auth(
            ...
            body=search_params.model_dump(exclude_none=True),
        )
        if not data or data["status_code"] != 200:
            return []
        return [Profile(**item) for item in data["data"]]
...
class ProxyProfileRepository(IProfileQueryRepository):
    ...
    async def search_data(
        ...
        profiles = await self.origin.search_data(search_params)
        return profiles
