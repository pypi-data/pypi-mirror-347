...
class CoreV1DataSourceSpecBronze(BaseModel):
    ...
    pre_transform: Optional[List[List[StrictStr]]] = Field(default=None, description="A set of SQL expressions to apply before writing the data frame ingested by autoloader to the bronze table. Use this to unpack, unwrap or unroll data into individual rows that would otherwise get ingested as a single highly nested row. This is a nested array. Each item in the outer array will result in a different, individual selectExpr execution. This allows to chain multiple passes - for example if the data needs to be unpacked multiple levels. The inner array contains the actual expressions to be included for each selectExpr. ", alias="preTransform")
    __properties: ClassVar[List[str]] = ["clustering", "bronzeTable", "skipBronzeLoading", "preTransform"]
