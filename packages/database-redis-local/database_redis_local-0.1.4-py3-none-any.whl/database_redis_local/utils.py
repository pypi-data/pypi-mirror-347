# TODO Duplicated from database-mysql. Do we need it twice?
import copy
import inspect
import os
...
# TODO Shall we rename LOGGER_CONNECTOR_CODE_OBJECT
...
# TODO Can we change this to class both in database-mysql and database-redis with diffrent implementation
def get_sql_hostname() -> str:
    sql_hostname = our_get_env("REDIS_HOSTNAME")
    return sql_hostname


def get_sql_username() -> str:
    sql_username = our_get_env("REDIS_USERNAME")
    return sql_username


def get_sql_password() -> str:
    sql_password = our_get_env("REDIS_PASSWORD")
    return sql_password


def get_sql_port() -> str:
    # The default port for Redis
    sql_port = our_get_env("REDIS_PORT", default="6379")
    return sql_port
