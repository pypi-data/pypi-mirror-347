...
from custom_base_django.serializers.base import NesteSerializerDefinition, DynamicFieldsModelSerializer
from custom_base_django.utils import FreeField, get_limits, first_upper, Defaults
...
class BaseStructure(BaseModelWitDateNotFiscalDelete):
    ...
    is_authenticated = models.BooleanField(verbose_name="is_authenticated", default=False)
    ...
    @classmethod
    def get_serializer_base_class(cls, struct_name):
        base_class = cls.SerializerBaseStruct()
        obj = cls.objects.filter(struct_name=struct_name).first()
        if not obj:
            return base_class
        base_class.extra_non_required_fields = obj.extra_non_required_fields if obj.extra_non_required_fields else base_class.extra_non_required_fields
        base_class.nested = obj.nested if obj.nested else base_class.nested
        base_class.nested_fields = obj.nested_fields if obj.nested_fields else base_class.nested_fields
        base_class.extra_writable_fields = obj.extra_writable_fields if obj.extra_writable_fields else base_class.extra_writable_fields
        base_class.readonly_fields = obj.readonly_fields if obj.readonly_fields else base_class.readonly_fields
        base_class.exclude_fields = obj.exclude_fields if obj.exclude_fields else base_class.exclude_fields
        base_class.extra_fields = obj.extra_fields if obj.extra_fields else base_class.extra_fields
        base_class.extra_validations = obj.extra_validations if obj.extra_validations else base_class.extra_validations
        base_class.blank_objects = obj.blank_objects if obj.blank_objects else base_class.blank_objects
        base_class.pk_field = obj.pk_field if obj.pk_field else base_class.pk_field
        base_class.model = obj.model if obj.model else base_class.model
        base_class.must_save = obj.must_save if obj.must_save else base_class.must_save
        base_class.is_authenticated = obj.is_authenticated if obj.is_authenticated else base_class.is_authenticated
        base_class.custom_query = obj.custom_query if obj.custom_query else base_class.custom_query
        base_class.fields = obj.fields if obj.fields else base_class.fields
        return base_class
