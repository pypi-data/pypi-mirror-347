...
class PySparkFixtureError(Exception):
    """Base exception for all PySparkFixtures errors."""

    pass


class SchemaNotFoundError(PySparkFixtureError):
    """Exception raised when a fixture's table schema cannot be determined."""

    pass


class DataConversionError(PySparkFixtureError):
    """Raised when data conversion fails during table processing."""

    pass


class TableNotFoundError(PySparkFixtureError):
    """Raised when the requested table id does not exist."""

    pass


class DataParsingError(PySparkFixtureError):
...
class PySparkFixtures:
    ...
    def _clean_table_schema_id(table_schema_id: str) -> str:
        ...
        potential_md_links = re.search(r"\[([^\]]+)\]\([^\)]+\)", clean_table_schema_id)
    ...
    def _extract_table_schema_id(cls, lines: list[str]) -> Optional[str]:
        ...
        for li in lines:
            # If we find a | data has started then there is no table schema id
            if re.match(r"^\|", li):
                break
    ...
    def _is_markdown_table(data: list[list[str]]) -> bool:
        ...
        """
        second_line = data[0]
        return all(re.match(r"^\s*[:-]+\s*$", value) for value in second_line)
    ...
    def _load_fixtures_from_file(
        ...
        Raises:
            SchemaNotFoundError: If table schema ID is found but no schemas_fetcher provided.
        """
        with open(fixtures_file) as f:
            file_content = f.read()
        tables = {}
        schemas = {}
        table_texts = re.split(r"^\s*#*\s*Table:\s*", file_content.strip(), flags=re.MULTILINE)
        ...
        for table_raw in table_texts:
            ...
            if table_schema_id and not schemas_fetcher:
                raise SchemaNotFoundError(
                    f"Table id '{table_name}' has the table schema id: '{table_schema_id}' but not an schema fetcher"
                )
    ...
    def _get_typed_val(col_type: Optional[DataType], col_value: Any) -> Any:
        """Converts a raw column value to its corresponding PySpark typed value.
