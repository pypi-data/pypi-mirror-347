...
def push_env_files(recursive: bool = False, force: bool = False) -> None:
    ...
    for env_file in env_files:
        try:
            ...
            # If the secret exists and force is not set, ask for confirmation
            if secret_exists and not force:
                if not get_user_confirmation(
                    f"Secret already exists for {env_file}. Overwrite?"
                ):
                    console.print(f"⏭️  Skipped {env_file}", style="yellow")
                    continue
