...
def push(
    ...
    force: bool = typer.Option(
        False, "--force", "-f", help="Overwrite existing secrets without confirmation"
    ),
):
    """
    ...
    """
    console.print(
        f"Pushing .env files {'recursively' if recursive else 'in current directory'}{' (force mode)' if force else ''}"
    )
    push_env_files(recursive=recursive, force=force)
    console.print("Done! :heavy_check_mark:", style="green")
