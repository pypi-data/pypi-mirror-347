...
from shops_nocodb_updater.models import ShopCategory, ShopProduct
...
class NocodbClient:
    ...
    async def sync_external_connector(self,
                                      external_data=None,
                                      clean_before_loading : bool=False):

        if not external_data:
            self.logger.info("No external data provided")
            return
        
        products_data,categories_data=external_data["Products"],external_data["Categories"]
        ...
        ShopProduct.__default_lang__= language
        ShopCategory.__default_lang__= language

        categories = [ShopCategory(**item, lang=language) for item in categories_data]
        products = [ShopProduct(**item, lang=language) for item in products_data]
        ...
        num_linked = await self.link_synced_records(
            ...
            target_records_map=synced_records.get(ShopCategory.__tableid__),
            map_records_key="parent_category.id",
