...
from shops_nocodb_updater.utils import download_image, needs_update,get_project_required_meta
from shops_nocodb_updater import ShopCategory, ShopProduct
...
class NocodbClient:
    ...
    async def apply_migrations(
        ...
        if isinstance(model, ShopProduct):
            attrs = model.extra_attributes or []      # if None → []
            extra_fields = [field["name"] for field in attrs]
        else:
            extra_fields = []
        ...
        for field_name, field_type in model.__nocodb_table_schema__(lang=language).items():
            ...
            if existing_column_data:
                if self.record_needs_update(existing_column_data, type_declaration, exlude_fields=["meta", "dtxp", "dtxs","cdf"]):
    ...
    async def delete_all_products(
        ...
        if table_id is None:
            # fallback to some default table, e.g. from self
            raise Exception("No table id provided")
        records = await self.get_table_records(table_id)
        records_id = [item["Id"] for item in records]
        await self.delete_table_record(table_name=table_id, records_id=records_id)
    ...
    async def link_synced_records(
        ...
        model,
        linked_model,
        # **** external_records: The external data containing the relationships, which relathionships???
        ...
        for record in linked_table_records:
            record_id = record["Id"]
            external_id = record.get(target_external_id_field)
        ...
        for ext_id, external_record in external_records_map.items():
            ...
            target_external_id_field_value = external_record.copy()
    ...
    async def synchronize_records(self, records_by_model: dict, upload_attachments: bool = False, set_checkout_mode : bool = False, remove_obsolete_records: bool = False, lang: str ="EN", **mapper_kwargs):
        """
        ...
        if set_checkout_mode:
            
            model=list(records_by_model.values())[0][0]
            model.__metadata__ = await self.get_table_metadata(model.__tableid__)
            self.checkout_field_name=model.__nocodb_field_name__("checkout_mode", lang)
            existing_columns = {col["title"]: col for col in model.__metadata__["columns"]}
            existing_column_names = {col["column_name"]: col for col in model.__metadata__["columns"]}
            default_checkout=existing_columns[self.checkout_field_name]["cdf"].strip("'")
        ...
        for model, records in records_by_model.items():
            ...
            for i in range(0, len(records), batch_size):
                ...
                await self.synchronize_batch_records(batch, upload_attachments=upload_attachments, default_checkout=default_checkout,**mapper_kwargs)
    ...
    async def synchronize_batch_records(self, records: list, upload_attachments: bool = False, default_checkout : Optional[str] = None, **mapper_kwargs):
        """
        ...
        for record_data in data:
            ...
            if default_checkout:
                record_data[self.checkout_field_name] = default_checkout
    ...
    async def sync_external_connector(self,
                                      connector=None,
                                      clean_beforoe_loading : bool=False):
        # categories_data = await load_json_data("./extension/raw_test/examples/real_sync/data/categories.json")
        # products_data = await load_json_data("./extension/raw_test/examples/real_sync/data/bitrix_products.json")

        products_data,categories_data=await connector.get_list_of_products_and_categories()
        # await write_json_data(file_path="./services/bitrix/handlers/meta",categories=categories,products=products)
        language, products_table_id, category_table_id = await get_project_required_meta(self)

        if clean_beforoe_loading:
            await self.delete_all_products(table_id=products_table_id)
            await self.delete_all_products(table_id=category_table_id)

        ShopCategory.__tableid__ = category_table_id
        ShopProduct.__tableid__ = products_table_id
        categories = [ShopCategory(**item) for item in categories_data]
        products = [ShopProduct(**item) for item in products_data]
        await self.apply_migrations(categories[0], migrate_existing_columns=True)
        await self.apply_migrations(ShopProduct(**products_data[0]), migrate_existing_columns=True)
        ...
        num_linked = await self.link_synced_records(
            ...
            external_records=products_data,
            ...
            column_name=ShopProduct.__nocodb_field_name__("category",lang=language)
        )
        # \\\
        ...
        num_linked = await self.link_synced_records(
            model=ShopCategory,
            linked_model=ShopCategory,
            external_records=categories_data,
            source_records_map=synced_records.get(ShopCategory.__tableid__),
            target_records_map=synced_records.get(ShopCategory.__tableid__),
            map_records_key="parent_categories.id",
            column_name=ShopProduct.__nocodb_field_name__("parent_categories",lang=language)
        )
