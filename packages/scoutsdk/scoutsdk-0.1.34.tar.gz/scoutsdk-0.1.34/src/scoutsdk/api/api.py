...
from .types import AssistantResponse, ChatCompletionMessage
...
class ScoutAPI:
    ...
    def put(
        self, url: str, data: RequestType, response_model: Type[ResponseType]
    ) -> Response[ResponseType]:
    ...
    def delete(
        self, url: str, response_model: Type[ResponseType]
    ) -> Response[ResponseType]:
    ...
    def chat_completion(
        ...
        if response_format:
            ...
            try:
                last_message_content = response.get("messages", [])[-1].get(
                    "content", ""
                )
                return response_format.model_validate(json.loads(last_message_content))
    ...
    def create_assistant(
        ...
        if allowed_functions is not None:
            payload["allowed_functions"] = allowed_functions
    ...
    def query_assistant_knowledge(
        ...
        response, status_code = RequestUtils.post(
            url=f"{self._base_url}/api/assistants/{assistant_id}/knowledge",
            headers=self._headers,
            json_payload={
                "query": user_query,
                "strategy": strategy,
