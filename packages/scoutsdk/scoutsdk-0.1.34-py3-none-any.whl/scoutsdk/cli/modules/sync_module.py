...
class SyncModule(BaseModule):
    ...
    def _fix_assistant_paths_with_ref_path(self, assistant: dict, ref_path: Path):
        def join_path(p):
            return str(ref_path / p) if p else p
        ...
        if "package_info" in assistant and assistant["package_info"]:
            ...
            # package_file_name is usually just a filename, but update if needed
        ...
        if "ui_info" in assistant and assistant["ui_info"]:
            ...
            # ui_package_file_name is usually just a filename, but update if needed
                ...
                if existing.get("name") == assistant.name:
                    if self._api_url:
                        ...
                        url_entry.ids.append(
                            AssistantIds(
                                id=assistant.id, assistant_id=existing.get("id")
                            )
                        )
    ...
    def _overwrite_assistant_file(self, assistant_id, file_path, file_uid, scout_api):
    ...
    def _upload_new_assistant_file(self, assistant_id, file_path, scout_api):
    ...
    def _store_deployed_assistant_id(
        ...
        if self._api_url:
            ...
            url_entry.ids.append(
                AssistantIds(id=assistant.id, assistant_id=deployed_id)
            )
