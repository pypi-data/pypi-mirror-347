...
class ScoutFunctionDecorator:
    ...
    def create_pydantic(self, func: Callable):
    ...
    def document_chunker(self, priority: int = 100):
        def decorator(cls):
    ...
    def _call_custom_function_with_registry(
        ...
        parameters_input = model_pydantic.model_validate(parameters_input)

        parameters = {k: v for k, v in parameters_input.__dict__.items()}
    ...
    def list_project_files(
        self, project_directory: str, ignore_patterns: list[str] = []
    ) -> list[str]:
