...
if "WOWOOL_ROOT" in os.environ:
    eot_root = Path(os.environ["WOWOOL_ROOT"]).resolve()
    default_lxware = os.environ["WOWOOL_ROOT"]
else:
    THIS_DIR = Path(__file__).parent.resolve()
    eot_root = THIS_DIR / ".." / "package"

default_lxware = eot_root / "lxware"
...
def default_engine() -> Engine:
    global _engine
    if _engine is None:
        _engine = Engine(language="auto", pytryoshka="true", root_path=str(eot_root))
    return _engine
