...
if "WOWOOL_ROOT" not in os.environ:
    eot_libs = Path(__file__).parent.parent / "package" / "lib"
else:
    eot_libs = Path(os.path.expanduser(os.path.expandvars(str("${WOWOOL_ROOT}/lib"))))

assert eot_libs.exists(), "Path of the library does not exist. {eot_libs}"
sys.path.insert(0, str(eot_libs.resolve()))
...
def parse_wikipedia_data(item, capture, language, concept_base):
    from wowool.native.concept_base import update_concept
    ...
    if language in ibdesc:
        for desc in ibdesc[language]:
            data = None
            if desc[0] == "sd":
                data = concept_base[desc[1]] if desc[1] in concept_base else None
            elif desc[0] == "key_prefix":
                data = ""
                for prefix in desc[1]:
                    concept_type = None
                    for key in [key for key in concept_base.keys() if key.startswith(prefix)]:
                        ...
                        update_concept(
                            ...
                            language,
                            concept_type,
                            {"source": "concept_base_key"},
                        )
                        if verbose:
                            logger.debug(concept_type)
                        known_thing = capture.add_concept(f"{concept_type}")
                        known_thing.add_attribute("source", "concept_base_key")
                        found = True
                        data += suffix + ". "
                    if concept_type:
                        break
                ...
                data = ". ".join(concept_base.keys())

            if data:
                try:
                    if isinstance(data, list):
                        data = " ".join(data)
                    json_str = wowool_plugin.process(language, f"{language}-discovery", data)
                    doc = parse_document(json_str)
                    found = add_descriptions(item, doc, capture, literal, language)
                except Exception as ex:
...
def parse_wikidata_data(item, capture, language, concept_base):
    from wowool.document.analysis.analysis_parser import parse_document

    found = False
    literal = capture.literal()
    for hit in concept_base:
...
def resolve_unkownthingies(ud):
    try:
        from wowool.native.concept_base.session import init_database
        from wowool.native.concept_base.utilities import get_rec

        global _session
        if not _session:
            _session = init_database()

        assert _session, "Could not initialize the concept_base database."

        match = wowool_plugin.match_info()
        capture = match.capture()
        literal = capture.literal()
        language = match.property("language")
        if verbose:
            logger.debug(f"{language}, {literal}")

        items = get_rec(literal, language)
        if items == None or len(items) == 0:
            from wowool.native.concept_base import discover

            items = discover(literal, language)
        if items:
            for item in items:
                if not item.concept:
                    continue
                known_thing = capture.add_concept(item.concept)
                if item.attributes:
                    attributes = json.loads(item.attributes)
                    for key, value in attributes.items():
                        if isinstance(value, list):
                            for item in value:
                                known_thing.add_attribute(key, item)
                        else:
                            known_thing.add_attribute(key, value)
            capture.remove()
    except KeyboardInterrupt:
        exit(-1)
    except Exception as ex:
        traceback.print_exc(file=sys.stderr)
        print("plugin:resolve_unknownthingies:", ex, file=sys.stderr)
