...
class AgentFlowPilot:
    ...
    def load_tool_chain(self, transaction: str,definitions: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            raise ValueError(
                f"任务定义格式不正确:  {self.transaction} 缺少必要的键 {e}"
            )
    ...
    async def _notify(self, status: TaskStatus, task: Optional[Task]):
        ...
        self.logger.info(
            f"通知外部服务任务状态变化: {self.transaction} {status}, task: {task.status.name}"
        )
        notification = TaskNotification(transaction=self.transaction,status=status,
                                        task=copy.deepcopy(task), context=self.context)
        ...
        try:
            ...
            while True:
                ...
                if next_task:
                    ...
                    self.logger.info(
                        f"执行任务: {self.transaction}, {task_name}, 状态: {task.status}"
                    )
                    task.mark_running()
                    self.snapshot = task
                    ...
                    log_msg = (
                        f"已执行任务: {self.transaction}, {task_name}, 状态: {status}"
                    )
                    if status != TaskStatus.COMPLETED:
                        ...
                        self.logger.info(log_msg)
                    if task.definition.is_end_task or status == TaskStatus.FAILED:
                        break
                # 2. 检查是否有因参数缺失而阻塞的任务
                ...
                if blocked_task:
                    ...
                    self.logger.info(
                        f"发现被阻塞的任务: {self.transaction}, {task_name}, 原因: {reason}"
                    )
                    self.snapshot = task
                    continue
            ...
            raise TaskFailedError(
                "TaskManager", f"系统错误: {self.transaction}, {str(e)}"
            ) from e
        ...
        try:
            response = await asyncio.wait_for(
                self.external_service.request_user_input(
                    task_name, reason, task.missing_parameters
                ),
                timeout=30.0,
            )
            self.logger.info(
                f"等待用户输入: {self.transaction}, {response}, {isinstance(response, dict)}"
            )
    ...
    def _mark_task_and_dependencies_failed(self, task_name: str, reason: str) -> None:
        ...
        for name, t in self.tasks.items():
            if task_name in t.definition.depends_on and t.status == TaskStatus.PENDING:
                t.mark_skipped(
                    f"前置任务 {self.transaction}, {task_name} 失败: {reason}"
                )
                asyncio.create_task(
    ...
    async def _execute_task(self, task: Task) -> bool:
        ...
        try:
            ...
            self.logger.exception(
                f"执行任务失败: {self.transaction}, {task.definition.name}"
            )
            return False
