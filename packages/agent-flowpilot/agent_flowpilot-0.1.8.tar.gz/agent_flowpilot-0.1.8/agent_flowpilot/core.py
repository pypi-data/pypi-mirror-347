...
class AgentFlowPilot:
    ...
    async def _notify(self, status: TaskStatus, task: Optional[Task]):
        ...
        self.logger.info(f"通知外部服务任务状态变化: {status}, task: {task.status.name}")
        notification = TaskNotification(status=status, task=copy.deepcopy(task), context=self.context)
    ...
    async def _handle_blocked_task(self, task_name: str, reason: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"等待用户输入: {response}, {isinstance(response, dict)}"
            )
