...
class GuiderData:
    ...
    def last_guide_step_ra(self) -> float:
        return self.last_guide_step.get("RADistanceRaw")

    @property
    def last_guide_step_dec(self) -> float:
        return self.last_guide_step.get("DECDistanceRaw")
...
class MountData:
    ...
    def coordinates_ra(self) -> float:
        return round(self.coordinates.get("RA"), 3) if self.coordinates.get("RA") is not None else None
    ...
    def coordinates_dec(self) -> float:
        return round(self.coordinates.get("Dec"), 3) if self.coordinates.get("Dec") is not None else None
