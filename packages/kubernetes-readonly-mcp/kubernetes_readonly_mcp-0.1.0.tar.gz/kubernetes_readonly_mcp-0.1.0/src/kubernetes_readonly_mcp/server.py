...
def list_pods(namespace: Optional[str] = None, resource=k8s_manager):
    """
...
def list_deployments(namespace: Optional[str] = None, resource=k8s_manager):
    """
    ...
    try:
        ...
        if isinstance(resource, str):
            print(f"Resource is a string: {resource}, initializing KubernetesManager")
            resource = KubernetesManager()
        elif not hasattr(resource, 'get_apps_api'):
            print(f"Resource is not a KubernetesManager instance: {type(resource)}, initializing KubernetesManager")
            resource = KubernetesManager()
...
def get_pod_logs(namespace: str, pod_name: str, container: Optional[str] = None, 
                 tail_lines: Optional[int] = None, previous: bool = False, resource=k8s_manager):
    """
...
# Example of a new tool that uses the same k8s_manager resource
@mcp.tool(description="List all services in a namespace or across all namespaces")
def list_services(namespace: Optional[str] = None, resource=k8s_manager):
    """
...
def list_namespaces(resource=k8s_manager):
    """
...
def get_logs(resource_type: str, namespace: Optional[str] = None, name: Optional[str] = None, 
             ...
             timestamps: bool = False, pretty: bool = False, resource=k8s_manager):
    """
