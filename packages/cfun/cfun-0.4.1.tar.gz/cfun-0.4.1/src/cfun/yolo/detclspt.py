...
class YoloPtModel:
    """YOLO模型类，用于目标检测和分类, 传入的是pt模型(主要是pt模型包含分类的字典)
    ...
    _DETECT = cdata.DX_DET_PT  # 内部检测模型
    _CLASSIFY = cdata.DX_CLS_PT  # 内部分类模型
    ...
    def __init__(
        ...
        # 加载模型
        self.det = YOLO(
            self._resolve_model_path(detect_model, self._DETECT, "Detection")
        )

        self.cls = YOLO(
            self._resolve_model_path(classify_model, self._CLASSIFY, "Classification")
        )
        self.det_imgsz = det_imgsz  # 检测模型的输入大小
        self.cls_imgsz = cls_imgsz

    @staticmethod
    def _resolve_model_path(path, default, name):
    ...
    def predict(self, source: Union[str, Path, Image.Image, np.ndarray]) -> list[dict]:
        ...
        Example:
            ```python
            from cfun.yolo.detclspt import YoloPtModel
            yolo = YoloPtModel()
