...
def box_to_polygon(
    ...
    """将 box 表示（x1, y1, x2, y2）转换为 polygon 表示（四个点）。

    Args:
        box (Union[dict, list]): box 表示，
        key (str): box 的键名，默认为 "box"
        replace (str): polygon 的键名，默认为 "points"， 可以和key相同
        例如 {"box": [x1, y1, x2, y2], "cls": "cat", "conf": 0.9}

    Returns:
        Union[dict, list]: 转换后的 polygon 表示
        例如 {"points": [[x1, y1], [x2, y1], [x2, y2], [x1, y2]], ...}
...
def polygon_to_box(
    ...
    Args:
        ...
        例如 {"points": [[x1, y1], [x2, y1], [x2, y2], [x1, y2]], "cls": "cat", "conf": 0.9}

    Returns:
        Union[dict, list]: 转换后的 box 表示
        例如 {"box": [x1, y1, x2, y2], "cls": "cat", "conf": 0.9}
