...
from .classify import YOLOClassifier
from .detect import YOLODetector


class YoloOnnxModel:
    """YOLO模型类，用于目标检测和分类, 传入的是onnx模型
    ...
    _DETECT = cdata.DX_DET_ONNX
    _CLASSIFY = cdata.DX_CLS_ONNX
    ...
    def __init__(
        ...
        Args:
            detect_model (Optional[str]): 检测模型路径，默认为None，使用默认模型
            classify_model (Optional[str]): 分类模型路径，默认为None，使用默认模型
            det_imgsz (int): 检测模型输入图像大小，默认为640
            cls_imgsz (int): 分类模型输入图像大小，默认为64
        """
        # 加载模型
        self.det = YOLODetector(
            model=self._resolve_path(detect_model, self._DETECT, "Detection"),
            imgsz=det_imgsz,
        )

        self.cls = YOLOClassifier(
            model=self._resolve_path(classify_model, self._CLASSIFY, "Classification"),
            imgsz=cls_imgsz,
        )

    @staticmethod
    def _resolve_path(path, default, name):
        path = default if not path else path
    ...
    def predict(self, source: Union[str, Path, Image.Image, np.ndarray]) -> list[dict]:
        ...
        Example:
            ...
            from cfun.yolo.detclsonnx import YoloOnnxModel
