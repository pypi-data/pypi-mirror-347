...
class Validator:
    ...
    def _process_date_strings(self, start_date, end_date, filter_field_format):
        ...
        while start <= end:
            if current_polymatica_version >= edge_version:
                dates_list.append(self._format_datetime_with_millis(start, date_format_pattern))
            else:
    ...
    def check_create_sphere(self, *args):
        """
        Проверка параметров для функции create_sphere.
        :param args: аргументы для проверки
        :return: результат проверки
        """
        if len(args) == 1:
            return self._check_single_arg_create_sphere(args[0])
        else:
            return self._check_multi_arg_create_sphere(*args)

    @staticmethod
    def _check_single_arg_create_sphere(arg):
        """
        Проверка одиночного аргумента функции create_sphere.
    ...
    def _check_multi_arg_create_sphere(
        self,
        update_params,
        file_type,
        sql_params,
        ...
        :param user_interval: интервал пользователя
        ...
        self._validate_update_params(update_params, file_type, time_zones)
    ...
    def _validate_update_params(self, update_params, file_type, time_zones):
        ...
        if file_type in ("excel", "csv") and update_type != "ручное":
            raise ValueError(
                "For multispheres created from file sources, only manual update is available!"
            )
    ...
    def _validate_schedule(user_schedule, update_type, time_zones):
        """
        Проверка расписания.
        :param user_schedule: расписание пользователя
    ...
    def check_update_cube(
        ...
        """
        Проверка параметров для функции update_cube.
        :param cube_name: имя куба
