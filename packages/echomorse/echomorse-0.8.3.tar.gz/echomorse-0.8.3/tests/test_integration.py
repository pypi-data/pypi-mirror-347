...
class TestIntegration:
    def run_cli_command(
        self, command_args, capture_output=True, check=True, text_input=None
    ):
    ...
    def test_list_voices_cli(self):
        ...
        result = self.run_cli_command(["voices"])
        assert result.returncode == 0
    ...
    def test_list_voices_detailed_cli(self):
        ...
        result = self.run_cli_command(["voices", "--detailed"])
        assert result.returncode == 0
        ...
        # assert "Dog bark" in result.stdout
    ...
    def test_piping_text2code_code2text(self):
        ...
        assert morse_output.strip() == translator.text_to_morse(
            input_text
        )  # Verify intermediate morse
    ...
    def test_piping_text2code_code2audio(self, tmp_path):
        ...
        process2 = subprocess.Popen(
            ["echomorse", "c2a", "-o", str(output_file)],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            text=True,
        )
    ...
    def test_text2audio_output_file_created(self, tmp_path):
        ...
        assert stdout.startswith(
            b"RIFF"
        ), "Output does not start with RIFF (not WAV format)"
        assert (
            stdout[8:12] == b"WAVE"
        ), "Output does not have WAVE at offset 8 (not WAV format)"
