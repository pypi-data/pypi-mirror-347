...
class BacktestBase(object):
    ...
    def get_data(self):
        ...
        if self.datasource == 'excel' :
            raw = pd.read_excel(r'db.xlsx',sheet_name= "adj_price", header=0,skiprows=0,usecols="A:F")
            raw1 = pd.read_excel(r'db.xlsx',sheet_name= "original_price", header=0,skiprows=0,usecols="A:F")
            #省略前面4列後，取第0列為表頭
            raw['date']=pd.to_datetime(raw['date'], format='%Y%m%d')
            raw1['date']=pd.to_datetime(raw1['date'], format='%Y%m%d')
            ...
            raw1 = raw1.loc[pd.to_datetime(self.start,format='%Y%m%d'):pd.to_datetime(self.end,format='%Y%m%d')]
            ...
            print('watch out! ...No data input')

        dvd1 = pd.read_excel(r'dvd_table.xlsx',sheet_name= "dvd",dtype={'ticker': str})
        self.dvd  =dvd1[dvd1['ticker']==self.symbol]
