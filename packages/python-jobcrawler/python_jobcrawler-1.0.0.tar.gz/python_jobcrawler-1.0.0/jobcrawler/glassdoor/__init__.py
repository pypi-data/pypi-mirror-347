...
class Glassdoor(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        location_id, location_type, state_abbr = self._get_location(
        ...
        for page in range(range_start, range_end):
            ...
            try:
                jobs, cursor = self._fetch_jobs_page(
                    scraper_input, location_id, location_type, state_abbr, page, cursor
                )
    ...
    def _fetch_jobs_page(
        ...
        with ThreadPoolExecutor(max_workers=self.jobs_per_page) as executor:
            future_to_job_data = {
                executor.submit(self._process_job, job, state_abbr): job for job in jobs_data
            }
    ...
    def _process_job(self, job_data, state_abbr: str):
        """
        ...
        location_name = job["header"].get("locationName", "") + f", {state_abbr}"
        location_type = job["header"].get("locationType", "")
        age_in_days = job["header"].get("ageInDays")
        is_remote, location = False, None
        date_posted = get_midnight_utc_date(age_in_days)
    ...
    def _get_location(self, location: str, is_remote: bool) -> (int, str, str):
        if not location or is_remote:
            return "11047", "STATE"  # remote options
        # url = f"{self.base_url}/findPopularLocationAjax.htm?maxLocationsToReturn=10&term={location}"
        url = f"{self.base_url}/autocomplete/location?locationTypeFilters=CITY,STATE,COUNTRY&caller=jobs&term={location}"
        ...
        index = next((i for i, item in enumerate(items) if item["locationName"] == location), 0)

        location_type = items[index]["locationType"]
        state_abbr = items[index]["stateAbbreviation"]
        ...
        return int(items[index]["locationId"]), location_type, state_abbr
