...
class LinkedIn(Scraper):
    ...
    def _process_job(
        ...
        # datetime_tag = (
        #     metadata_card.find("time", class_="job-search-card__listdate--new")
        #     if metadata_card
        #     else None
        # )
        # date_posted = None
    ...
    def _get_job_details(self, job_id: str) -> dict:
        ...
        date_content = soup.find(
            "span", class_=lambda x: x and "posted-time-ago__text" in x
        )
        ...
        if date_content is not None:
            date_posted = parse_relative_time_to_iso8601(date_content.get_text(strip=True)) if date_content else ""
