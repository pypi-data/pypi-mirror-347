...
class TinyShakespeare(Dataset):
    ...
    def __getitem__(self, idx):
        start_idx = idx * self.len
        end_idx = start_idx + self.len + 1

        if end_idx > len(self.tokens):
            start_idx = len(self.tokens) - self.len - 1
...
class WikiText(Dataset):
    ...
    def __len__(self):
        return len(self.tokenized) // self.max_len

    def __getitem__(self, idx):
        start_idx = idx * self.len
        end_idx = start_idx + self.len + 1
        if end_idx > len(self.tokenized):
            start_idx = len(self.tokenized) - self.len - 1
            end_idx = len(self.tokenized)
