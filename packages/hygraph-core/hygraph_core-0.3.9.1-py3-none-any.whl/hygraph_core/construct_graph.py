...
def build_timeseries_similarity_graph(
    ...
    if lsh_mode == 'none':
        ...
        if lsh_mode in ('shape', 'both'):
            # Flatten each series and normalize for shape-based LSH
            shape_vectors = []
            for node in ts_nodes:
                vec = node.series.data.values.flatten().astype(float)
                vmin, vmax = vec.min(), vec.max()
                norm = (vec - vmin) / (vmax - vmin) if vmax != vmin else vec * 0
                shape_vectors.append(norm)
            # Ensure all shape_vectors have the same length (pad or truncate)
            max_len = max(v.size for v in shape_vectors)
            shape_vectors = [
                np.pad(v, (0, max_len - v.size), 'constant') if v.size < max_len else v[:max_len]
                for v in shape_vectors
            ]
            shape_dim = max_len  # fixed dimension for hyperplane generation
            # Build shape LSH tables
            shape_planes = [_generate_random_planes(shape_dim, lsh_shape_num_bits)
                            for _ in range(lsh_shape_num_tables)]
            shape_tables = []
            for planes in shape_planes:
                table = {}
                for i, vec in enumerate(shape_vectors):
                    sig = _compute_lsh_signature(vec, planes)
    ...
    if lsh_mode == 'none':
        ...
        # Prepare embeddings
        if lsh_mode in ('shape', 'both'):
            # Flatten and normalize each series for shape-based LSH
            shape_vectors = []
            for node in ts_nodes:
                vec = node.series.data.values.flatten().astype(float)
                vmin, vmax = vec.min(), vec.max()
                norm = (vec - vmin) / (vmax - vmin) if vmax != vmin else vec * 0
                shape_vectors.append(norm)
            shape_dim = shape_vectors[0].shape[0]
            # Build shape LSH tables
            shape_planes = [_generate_random_planes(shape_dim, lsh_shape_num_bits)
                            for _ in range(lsh_shape_num_tables)]
            shape_tables = []
        ...
        if lsh_mode in ('feature', 'both'):
            # Extract statistical feature vectors
            feature_vectors = [node.series.extract_features() for node in ts_nodes]
            feat_dim = feature_vectors[0].shape[0]
            # Build feature LSH tables
            feat_planes = [_generate_random_planes(feat_dim, lsh_feature_num_bits)
        ...
        # Build candidate list by union of LSH buckets
        candidate_matrix = []
