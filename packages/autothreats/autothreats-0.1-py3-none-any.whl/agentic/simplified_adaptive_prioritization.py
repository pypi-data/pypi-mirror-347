...
class SimplifiedAdaptivePrioritization:
    ...
    async def prioritize_vulnerabilities(
        ...
        """
        self.logger.info(
            f"Prioritizing {len(vulnerabilities)} vulnerabilities for job {job_id}"
        )
        ...
        adjusted_vulnerabilities = await self._apply_context_adjustments(
            scored_vulnerabilities, job_id
        )

        # Apply exploitability adjustments
        exploitability_adjusted = self._apply_exploitability_adjustments(
        ...
        # Add priority levels
        prioritized_vulnerabilities = self._add_priority_levels(
    ...
    async def _apply_context_adjustments(
        ...
        for vuln in vulnerabilities:
            ...
            if security_context and vuln.get("in_security_boundary", False):
                ...
                adjusted_score *= 1.2
                adjusted_vuln["adjustment_factors"] = adjusted_vuln.get(
                    "adjustment_factors", []
                ) + ["security_boundary"]
            ...
            if file_path:
                ...
                if any(
                    ...
                    for ext in [".auth.", "security.", "crypto.", "password."]
                ):
                    adjusted_score *= 1.3
                    adjusted_vuln["adjustment_factors"] = adjusted_vuln.get(
                        "adjustment_factors", []
                    ) + ["security_critical_file"]

                # Adjust based on file extension
                if file_path.endswith(".js") or file_path.endswith(".ts"):
                    # JavaScript/TypeScript files (client-side)
                    if any(
                        pattern in vuln.get("description", "").lower()
                        for pattern in ["xss", "csrf", "injection"]
                    ):
                        adjusted_score *= 1.2
                        adjusted_vuln["adjustment_factors"] = adjusted_vuln.get(
                            "adjustment_factors", []
                        ) + ["client_side_vulnerability"]

                elif (
                    file_path.endswith(".py")
                    or file_path.endswith(".java")
                    or file_path.endswith(".go")
                ):
                    # Server-side files
                    if any(
                        pattern in vuln.get("description", "").lower()
                        ...
                        adjusted_vuln["adjustment_factors"] = adjusted_vuln.get(
                            "adjustment_factors", []
                        ) + ["server_side_vulnerability"]

            # Apply pattern-based adjustments
            description = vuln.get("description", "").lower()
            if (
                "password" in description
                or "credential" in description
    ...
    def _apply_exploitability_adjustments(
        self, vulnerabilities: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for vuln in vulnerabilities:
            ...
            for factor, multiplier in exploitability_factors.items():
                if factor in description:
                    ...
                    adjusted_vuln["adjustment_factors"] = adjusted_vuln.get(
                        "adjustment_factors", []
                    ) + [f"exploitable_{factor.replace(' ', '_')}"]
                    break
    ...
    async def generate_prioritization_report(
        self, vulnerabilities: List[Dict[str, Any]], job_id: str
    ) -> Dict[str, Any]:
        """
