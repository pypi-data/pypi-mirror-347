...
class MultiStageAgentOrchestrator:
    ...
    async def _run_pipeline(self, pipeline_id: str, job_id: str, codebase_id: str):
        ...
        try:
            ...
            for stage in self.pipeline_stages:
                stage_id = stage["id"]
                self.logger.info(
                    f"Starting stage {stage_id} for pipeline {pipeline_id}"
                )
                ...
                for dependency in stage["dependencies"]:
                    if (
                        dependency
                        not in self.pipeline_status[pipeline_id]["completed_stages"]
                    ):
                ...
                try:
                    ...
                    self.pipeline_status[pipeline_id]["completed_stages"].append(
                        stage_id
                    )
                    self.logger.info(
                        f"Completed stage {stage_id} for pipeline {pipeline_id}"
                    )
                except Exception as e:
    ...
    async def _execute_stage(
        ...
        for dependency in stage["dependencies"]:
            if dependency in self.stage_results.get(pipeline_id, {}):
                previous_results[dependency] = self.stage_results[pipeline_id][
                    dependency
                ]
        ...
        if stage["parallel"]:
            if stage_id == "vulnerability_pattern_matching":
                return await self._execute_pattern_matching(
                    codebase, job_id, pipeline_id
                )
            elif stage_id == "semantic_vulnerability_analysis":
                return await self._execute_semantic_analysis(
                    codebase, job_id, pipeline_id, previous_results
                )
            elif stage_id == "cross_component_analysis":
                return await self._execute_cross_component_analysis(
                    codebase, job_id, pipeline_id, previous_results
                )
            else:
                self.logger.warning(
                    f"Unknown parallel stage {stage_id}, using default implementation"
                )
                return {
                    "status": "completed",
                    "stage_id": stage_id,
            ...
            return await self._execute_simplified_stage(
        ...
        if stage_id == "context_analysis":
            ...
            try:
                if hasattr(self.workspace, "agents"):
                    agentic_context_agent = self.workspace.agents.get(
                        "agentic_context_agent"
                    )
            ...
            if agentic_context_agent:
                self.logger.info(
                    f"Using agentic context agent for context analysis in job {job_id}"
                )
            ...
            if context_aware_security:
                self.logger.info(
                    f"Using context-aware security for context analysis in job {job_id}"
                )
                security_context = (
            ...
            if "vulnerability_validation" in previous_results:
                vulnerabilities = previous_results["vulnerability_validation"].get(
                    "vulnerabilities", []
                )
            ...
            if adaptive_prioritization and "risk_scoring" in previous_results:
                vulnerabilities = previous_results["risk_scoring"].get(
                    "vulnerabilities", []
                )
                prioritized_vulnerabilities = (
                    await adaptive_prioritization.prioritize_vulnerabilities(
                        vulnerabilities, job_id
                    )
                )
            ...
            if explainable_security and "prioritization" in previous_results:
                vulnerabilities = previous_results["prioritization"].get(
                    "vulnerabilities", []
                )
                ...
                executive_summary = (
                    await explainable_security.generate_executive_summary(
                        explained_vulnerabilities, job_id
                    )
                )
    ...
    async def _execute_cross_component_analysis(
        ...
        code_graph = previous_results.get("code_graph_generation", {}).get(
            "code_graph", {}
        )
    ...
    def _get_pipeline_results(self, pipeline_id: str) -> Dict[str, Any]:
        ...
        for vuln in all_vulnerabilities:
            ...
            if vuln_id not in unique_vulnerabilities:
                ...
                if vuln.get("confidence", 0) > unique_vulnerabilities[vuln_id].get(
                    "confidence", 0
                ):
                    unique_vulnerabilities[vuln_id] = vuln
