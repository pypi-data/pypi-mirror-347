...
class SimplifiedHierarchicalAnalysis:
    ...
    async def analyze_codebase_hierarchically(
        ...
        component_vulnerabilities = await self._analyze_component_vulnerabilities(
            components, codebase, job_id
        )
    ...
    def _determine_component_languages(
        ...
        languages = sorted(
            language_counts.keys(), key=lambda x: language_counts[x], reverse=True
        )
    ...
    def _determine_component_purpose(self, directory: str, files: List[str]) -> str:
        ...
        if any(
            x in directory_lower
            for x in ["auth", "login", "user", "account", "password"]
        ):
            return "authentication"
        elif any(
            x in directory_lower for x in ["admin", "permission", "role", "access"]
        ):
            ...
            x in directory_lower
            for x in ["db", "database", "model", "entity", "repository"]
        ):
            return "data_access"
        elif any(
            x in directory_lower
            for x in ["ui", "view", "component", "page", "template"]
        ):
            ...
            x in directory_lower for x in ["security", "crypto", "encrypt", "decrypt"]
        ):
            return "security"
    ...
    async def _analyze_component_vulnerabilities(
        self, components: List[Dict[str, Any]], codebase: Dict[str, Any], job_id: str
        ...
        for component in components:
            ...
            vulnerabilities = await self._detect_vulnerabilities_in_component(
                component, component_codebase, job_id
            )
    ...
    async def _detect_vulnerabilities_in_component(
        self, component: Dict[str, Any], component_codebase: Dict[str, Any], job_id: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        purpose_patterns = {
            "authentication": [
                ...
                (
                    "sql_injection",
                    r"(execute\(\s*[\"']SELECT.*\+|SELECT.*FROM.*WHERE.*\+)",
                ),
            ],
            "authorization": [
    ...
    def _determine_vulnerability_severity(
        self, pattern_name: str, component_purpose: str
    ) -> str:
        """
        ...
        if component_purpose in critical_purposes and base_severity == "medium":
            ...
            component_purpose not in critical_purposes
            and base_severity == "high"
            and pattern_name != "xss"
        ):
            return "medium"

        return base_severity

    async def merge_component_vulnerabilities(
        self, hierarchical_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        component_vulnerabilities = hierarchical_analysis.get(
            "component_vulnerabilities", {}
        )
