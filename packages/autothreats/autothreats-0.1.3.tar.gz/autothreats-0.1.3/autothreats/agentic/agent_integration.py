...
from ..simplified_base import Agent, AgentModel, SharedWorkspace
from ..utils.agent_decorators import clear_registry, register_agents
...
def register_agentic_agents(
    ...
    if not agent_list:
        logger.info(
            "No agents registered through decorator system, creating mock agents for testing"
        )
...
async def initialize_agentic_agents(agents: List[Agent]) -> None:
    ...
    async def init_agent_with_timeout(agent, timeout=10.0):
        try:
            ...
            logger.warning(
                f"Timeout initializing agentic agent {agent.id} - continuing anyway"
            )
            return False
