    ...
    if task_type in ["risk_scoring", "score_risks"]:
        ...
        try:
            ...
            risk_scores, risk_metrics = await _score_risks(
                agent, vulnerabilities, simulated_scenarios, context, lightweight
            )
...
async def _score_risks(
    ...
    all_scores = [
        score.get("risk_score", 0) for score in vulnerability_scores + scenario_scores
    ]
    if all_scores:
...
async def _score_scenario(
    ...
    if "refined_risk_score" in scenario:
        ...
        likelihood = {"high": 9.0, "medium": 5.0, "low": 2.0}.get(likelihood_text, 5.0)
...
async def _calculate_risk_metrics(
    ...
    for score in all_scores:
        ...
        risk_metrics["risk_distribution"][severity] = (
            risk_metrics["risk_distribution"].get(severity, 0) + 1
        )

    # Calculate risk acceptance
    for score in all_scores:
        acceptance = score.get("acceptance_level", "acceptable")
        risk_metrics["risk_acceptance"][acceptance] = (
            risk_metrics["risk_acceptance"].get(acceptance, 0) + 1
        )

    # Calculate top risks (highest risk scores)
    sorted_scores = sorted(
        all_scores, key=lambda x: x.get("risk_score", 0), reverse=True
    )
