...
def register_simplified_agents(
    workspace: SharedWorkspace, config: Dict[str, Any]
) -> List[Agent]:
    """
    ...
    if not enable_agentic:
        logger.info(
            "Agentic improvements are disabled, skipping simplified agent registration"
        )
        return []
...
async def initialize_simplified_agents(agents: List[Agent]) -> None:
    ...
    async def init_agent_with_timeout(agent, timeout=10.0):
        try:
            ...
            logger.warning(
                f"Timeout initializing simplified agent {agent.id} - continuing anyway"
            )
            return False
        except Exception as e:
    ...
    try:
        results = await asyncio.wait_for(
            asyncio.gather(*init_tasks, return_exceptions=True), timeout=30.0
        )
        success_count = sum(1 for r in results if r is True)
        logger.info(
            f"Successfully initialized {success_count} out of {len(agents)} simplified agents"
        )
    except asyncio.TimeoutError:
        logger.warning(
            f"Overall timeout while initializing simplified agents - continuing with system operation"
        )
    except Exception as e:
...
async def shutdown_simplified_agents(agents: List[Agent]) -> None:
    ...
    async def shutdown_agent_with_timeout(agent, timeout=5.0):
        try:
            ...
            logger.warning(
                f"Timeout shutting down simplified agent {agent.id} - continuing anyway"
            )
            return False
        except Exception as e:
    ...
    try:
        results = await asyncio.wait_for(
            asyncio.gather(*shutdown_tasks, return_exceptions=True), timeout=15.0
        )
        success_count = sum(1 for r in results if r is True)
        logger.info(
            f"Successfully shut down {success_count} out of {len(agents)} simplified agents"
        )
    except asyncio.TimeoutError:
        logger.warning(
            f"Overall timeout while shutting down simplified agents - continuing anyway"
        )
    except Exception as e:
