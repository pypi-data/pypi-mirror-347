...
class AgenticAgent(Agent):
    ...
    async def initialize(self):
        ...
        # Log MCP endpoint information if available
        if hasattr(self.agentic_extension, 'mcp_server') and self.agentic_extension.mcp_server:
            if hasattr(self.agentic_extension.mcp_server, 'endpoint_url'):
                self.logger.info(f"Agent {self.id} MCP endpoint available at {self.agentic_extension.mcp_server.endpoint_url}")
    ...
    async def shutdown(self):
        """Shutdown the agent and its agentic capabilities"""
        self.logger.info(f"Shutting down agent {self.id}")
        
        # Shutdown agentic extension
        if hasattr(self, 'agentic_extension'):
            if hasattr(self.agentic_extension, 'shutdown'):
                try:
                    await self.agentic_extension.shutdown()
                    self.logger.info(f"Agentic extension for agent {self.id} shut down successfully")
                except Exception as e:
                    self.logger.error(f"Error shutting down agentic extension: {e}", exc_info=True)
        
        # Call parent shutdown
        await super().shutdown()
        
        self.logger.info(f"Agent {self.id} shut down successfully")
