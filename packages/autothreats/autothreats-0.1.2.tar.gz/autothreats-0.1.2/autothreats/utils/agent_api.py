...
from ..agentic.base_agent import AgenticAgent
from ..simplified_base import Agent, AgentController, AgentModel, SharedWorkspace
...
def create_simple_agent(
    ...
    if (
        len(params) < 3
        or params[0] != "agent"
        or params[1] != "task_type"
        or params[2] != "task_data"
    ):
        raise TypeError(
            "Agent function must have signature: async def func(agent, task_type, task_data)"
        )

    # Create a simple agent class dynamically
    if is_agentic:
        class SimpleAgent(AgenticAgent):
            def __init__(self, agent_id: str, config: Optional[Dict[str, Any]] = None):
                super().__init__(agent_id, f"agentic_{agent_type}", config)
...
def register_agent(workspace: SharedWorkspace, agent: Agent) -> Agent:
    """
...
def create_from_class(
    ...
    try:
        ...
        if "Can't instantiate abstract class" in str(e):
            raise TypeError(
                f"Can't instantiate abstract class {agent_class.__name__}. Implement all abstract methods."
            )
        raise
...
def wrap_existing_agent(
    ...
    if controller_class is not None:
        if not inspect.isclass(controller_class) or not issubclass(
...
def register_with_orchestrator(
    ...
    try:
        ...
        if spec is None:
            logger.error(
                f"Could not find orchestrator module: {orchestrator_module_path}"
            )
            return
        ...
        if hasattr(orchestrator_module, "_create_agents"):
            ...
            if class_name in source:
                logger.info(
                    f"Agent {class_name} is already registered with the orchestrator"
                )
                return
        ...
        logger.info(
            "Please add the following code to the orchestrator's _create_agents method:"
        )
