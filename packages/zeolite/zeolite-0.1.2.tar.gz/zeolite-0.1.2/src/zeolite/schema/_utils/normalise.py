...
def _rename_columns_from_aliases(
    ...
    for original_col, sanitized_original in original_columns_map:
        target = schema_aliases.get(sanitized_original, None)

        # print(original_col, " -> ", sanitized_original, " -> ", target, " -> ", target not in assigned_targets)
        if target is None:
            continue
        ...
        if target != original_col:
            ...
            if target in assigned_targets or (target in original_columns and target != original_col):
                errors.append(
                    StructureValidationError(
                        schema=schema_name,
                        # column=original_columns_map[sanitized_original],
                        column=original_col,
...
def normalise_column_headers(
    ...
    sanitised = _rename_columns_from_aliases(
        lf, col_defs=col_defs, schema_name=schema_name
    )
