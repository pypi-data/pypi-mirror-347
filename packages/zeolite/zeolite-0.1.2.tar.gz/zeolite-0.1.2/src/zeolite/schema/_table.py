...
class TableSchema:
    ...
    def with_stage(self, stage: str) -> "TableSchema":
        """Set the schema's stage"""
    ...
    def process_data(self, lf: LazyFrame) -> ProcessingResult:
        ...
        if prepped.reject:
            return ProcessingResult(
                data=prepped.data, errors=errors, success=False, failed_stage="prepare"
            )
        ...
        return ProcessingResult(
            ...
            failed_stage="validate" if valid.reject else None,
        )
    ...
    for c in columns:
        ...
        if ref.name in sources:
            ...
            sources[ref.name] = SourceColDef(
                name=ref.name, aliases=col_aliases, if_missing=c.if_missing
            )
