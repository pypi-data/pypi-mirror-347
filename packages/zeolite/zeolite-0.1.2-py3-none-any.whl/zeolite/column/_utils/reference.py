...
from ..._utils.column_id import make_column_id, COLUMN_NAME_DIVIDER, COLUMN_ID_DIVIDER

if TYPE_CHECKING:
    from ...registry import ColumnRegistry
    from ..._types import ColumnNode
    from .._validation import _BaseRowCheck, ColumnCheckType
...
class ColumnRef:
    ...
    def check(
        self, validation: Union["ColumnCheckType", type["_BaseRowCheck"], str]
    ) -> "ColumnRef":
    ...
    def get_name(self) -> str:
        ...
        return self.get_id(include_schema_prefix=False).replace(COLUMN_ID_DIVIDER, COLUMN_NAME_DIVIDER)
    ...
    def from_id(cls, id: str) -> "ColumnRef":
        ...
        # Split parts
        parts = id.split(COLUMN_ID_DIVIDER)
    ...
    def resolve(
        ...
        *,
        ...
        if not node:
            raise ValueError(f"Could not find column with ID {col_id} in registry")
        return node
