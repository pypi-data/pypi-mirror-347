...
def god_eval(encoder_name, sample_size=200, qa_sample_size=100, local_mode=False, multi_hop_hardness_factor=0,
             judged=False, top_k=None, useTool=False, llm=None, reranker_name=None):
    ...
    ragConfig = MyUtils.Init(encoder_name=encoder_name, top_k=top_k, sample_size=sample_size,
                             qa_sample_size=qa_sample_size, local_mode=local_mode,
    ...
    # ragConfig.reset_rag_attributes()
    # ragConfig.run_name = f"{encoder_name}_enhanced_reranked"
    # ragConfig.add_neighbor_chunks_smart = True
    # ragConfig.replace_retrieved_chunks_with_parent_paragraph = True
    # ragConfig.rerank = True
    # ragConfig.reranker = CrossEncoderReranker(model_name=reranker_name)
...
def test_rerankers(reranker_names=None,encoder_name='sentence-transformers/LaBSE', sample_size=200, qa_sample_size=100, local_mode=False, multi_hop_hardness_factor=0,top_k=None):
    import MetaRagTool.Evaluations.TestManager as TestManager
    import MetaRagTool.Utils.MyUtils as MyUtils
    from MetaRagTool.Encoders import CrossEncoderReranker
