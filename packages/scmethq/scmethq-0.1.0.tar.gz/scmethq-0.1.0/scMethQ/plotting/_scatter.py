# Function: plot embedding
# this function is adapted from scanpy.plotting._scatter.embedding  and omicverse
# Description: Plot categorical and continuous metadata into dimensionality reduction. (e.g. umap, pca, etc)
...
def _get_color_source_vector(
    ...
    if (
        ...
        #原来这里只会找到第一个符合条件的基因，现在改成找到所有符合条件的基因，然后返回一个list
        # value_to_plot = adata.var.index[adata.var[gene_symbols] == value_to_plot][
        #     0
        # ]
        if len(value_to_plot) > 1:
            print(f"Note: Multiple regions are matched for {gene_symbols},  mean methylation value will be plotted ")
    # If multiple genes are matched, we need to handle them
    if not isinstance(value_to_plot, str):
        values_list = []
        for gene in value_to_plot:
            if use_raw and gene not in adata.obs.columns:
                values = adata.X.obs_vector(gene)
            else:
                values = adata.obs_vector(gene, layer=layer)
            values_list.append(values)

        # Here we combine the values from multiple genes.
        # Example: taking the mean methylation value across the regions.
        values = np.mean(np.vstack(values_list), axis=0)
    else:
        # only one 
    ...
    if groups and isinstance(values, pd.CategoricalDtype):
...
def _color_vector(
    ...
    对于分类数据，返回的值是从类别调色板或给定的 `palette` 值中选择的颜色列表。
    ...
    if isinstance(values, pd.CategoricalDtype) or values.dtype == 'category':
    ...
    elif not isinstance(values, pd.CategoricalDtype):
        return values, False
