...
class Coverage:
    ...
    def from_bam(cls, bam_fn, gene, load=False):
        ...
        if load:
            ...
            start = min(
                gene.start,
                *[transcript["exons"][0][0] for transcript in gene.transcripts],
            )
            end = max(
                gene.end,
                *[transcript["exons"][-1][1] for transcript in gene.transcripts],
            )
    ...
    def from_alignment(cls, align_fh, gene):
        ...
        start = min(
            gene.start, *[transcript["exons"][0][0] for transcript in gene.transcripts]
        )
        end = max(
            gene.end, *[transcript["exons"][-1][1] for transcript in gene.transcripts]
        )
