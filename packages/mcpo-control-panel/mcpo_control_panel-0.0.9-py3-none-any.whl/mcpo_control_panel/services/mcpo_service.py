...
# (Full version with improved process handling and logging in English)
# ================================================
import asyncio
import logging
import os
...
# --- Tool Aggregation ---
# (get_aggregated_tools_from_mcpo remains unchanged, relies on status and settings)
    ...
    while True:
        ...
        if mcpo_status != "RUNNING":
            ...
            if mcpo_status == "STOPPED" and _health_check_failure_counter > 0:
                 ...
                 await handle_health_check_failure(settings) # Handle the error state
        ...
        try:
            ...
            if 200 <= response.status_code < 300:
                ...
                _health_check_failure_counter += 1
                await handle_health_check_failure(settings) # Handle failure

        except httpx.ConnectError as e:
            logger.error(f"Health Check: Connection error requesting MCPO ({health_check_url}). Error: {e}")
            _health_check_failure_counter += 1
            await handle_health_check_failure(settings)
