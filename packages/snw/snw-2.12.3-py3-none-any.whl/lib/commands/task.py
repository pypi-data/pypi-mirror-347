    ...
    def launch(self):
        ...
        while i < len(self.options.docker_command):
            ...
            if mode is None and (tok == "-m" or tok == "--model"):
                ...
                r = requests.get(os.path.join(self.options.url, "model/describe", model),
                                 params={"short": False}, auth=self.auth)
                if self.options.docker_image is None and r.status_code != 200:
                    raise RuntimeError("cannot infer docker_image for "
                                       "model %s -- %s" % (model, r.text))
                elif r.status_code == 200:
                    parent_config = r.json()
                    if self.options.docker_image is None:
                        self.options.docker_image = parent_config['imageTag']
            ...
            i += 1
        if parent_config:
            if "sub_configs" in parent_config:
                lps, src_lang, tgt_lang = parse_sub_configs(parent_config)
            default_lp = parent_config.get("default_lp")

        #second pass to get config and guess source and target language for multilinugal model
        i = 0
        config = None
        while i < len(self.options.docker_command):
            tok = self.options.docker_command[i]
            if mode is None and (tok == "-c" or tok == "--config"):
                ...
                if "sub_configs" in config:
                    lps, src_lang, tgt_lang = parse_sub_configs(config)
                default_lp = config.get("default_lp")
                i += 1
            i += 1

        if default_lp:
            src_lang, tgt_lang = check_default_lp(default_lp, lps)

        # third pass to apply other options -T, -Tm, -t
        i = 0
        while i < len(self.options.docker_command):
            tok = self.options.docker_command[i]
            if mode is None and (tok == "train" or tok == "trans" or
                ...
                if src_lang == "xx" or tgt_lang == "yy":
                    raise RuntimeError('Cannot guess the lp, if this is a multilingual model please add '
                                       '"default_lp" in the config')
                files = []
                if tok == '-t':
        ...
        if self.options.novalidschema:
            ...
            if not (config is None) or upgraded:
                if model and not model.startswith("LLM"):
        ...
        if self.options.no_test_trans:
            ...
            if src_lang == "xx" or tgt_lang == "yy" and lps:
                src_lang, tgt_lang = lps.pop(0).split("_")
            elif src_lang == "xx" or tgt_lang == "yy":
                raise RuntimeError(f"couldn't deduce source or target language")
...
def parse_sub_configs(config):
    lps = list(config["sub_configs"].keys())
    src_lang = "xx"
    tgt_lang = "yy"
    return lps, src_lang, tgt_lang

def check_default_lp(default_lp, lps):
    if default_lp in lps:
        lps.remove(default_lp)
        return default_lp.split("_")
    else:
        raise RuntimeError(f"default_lp {default_lp} is not supported")