...
class BaseLoader:
    ...
    def project_binned_data(
        ...
        if len(proj_axes) == 1:
            axes_values = axes_values[0]

        proj_data = {}
        for win_id, data in self.binned_data.items():
            proj_data[win_id] = project_data(
        ...
        if len(proj_data) == 1:
            proj_data = list(proj_data.values())[0]

        return (axes_values, proj_data)
    ...
    def export_to_csv(
        ...
        """
        Export loaded data as projection onto given axes to .csv file. Projections are possible
        onto 1 or 2 axes.
