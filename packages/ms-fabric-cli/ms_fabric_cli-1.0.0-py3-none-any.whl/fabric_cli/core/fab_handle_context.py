...
def process_context_path(
    ...
    if isinstance(cur_ctxt, Tenant):
        ...
        raise FabricCLIError(
            f"Traversing into {cur_ctxt.path} is not supported",
            fab_constant.ERROR_INVALID_PATH,
        )
...
def _handle_path_in_ws(
    ...
    if ws_id is None:
        raise FabricCLIError(
            f"Workspace not found '{workspace.name}'",
            fab_constant.ERROR_NOT_FOUND,
        )
    elem_type_str = path_part
    # There are two types of path parts: Name.ElementType and .VirtualWorkspaceType
...
def _handle_path_in_item(
    ...
    if folder is None:
        raise FabricCLIError(
            f"Folder '{path_part}' not found in item '{item.name}'. Valid folders are: {item_folders}",
            fab_constant.ERROR_NOT_SUPPORTED,
        )
...
def _handle_path_in_vws(
    ...
    if cur_ctxt.item_type != item_type:
        raise FabricCLIError(
            f"Item {item_type} not supported in {cur_ctxt.type}",
            fab_constant.ERROR_INVALID_PATH,
        )
