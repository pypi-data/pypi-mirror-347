...
class FabAuth:
    ...
    def _load_env(self):
        ...
        if "FAB_SPN_CLIENT_ID" in os.environ and "FAB_SPN_CLIENT_SECRET" in os.environ:
            ...
            if client_id:
                self._verify_valid_guid_parameter(
    ...
    def _decode_dict_recursively(self, d: dict) -> dict:
        ...
        for key, value in d.items():
            if isinstance(value, bytes):
                ...
                decoded_dict[key] = json.dumps(
                    self._decode_dict_recursively(value))
            else:
        ...
        if "FAB_TOKEN" in os.environ and "FAB_TOKEN_ONELAKE" in os.environ:
            match scope:
                ...
                case con.SCOPE_ONELAKE_DEFAULT:
                    # this call will validate the token we got from the env var
                    self._decode_jwt_token(os.environ["FAB_TOKEN_ONELAKE"], con.ONELAKE_TOKEN_AUDIENCE)
                    return os.environ["FAB_TOKEN_ONELAKE"]
                case con.SCOPE_AZURE_DEFAULT:
                    # this call will validate the token we got from the env var
                    self._decode_jwt_token(os.environ["FAB_TOKEN_AZURE"], con.AZURE_TOKEN_AUDIENCE)
    ...
    def set_access_mode(self, mode, tenant_id=None):
        if mode not in con.AUTH_KEYS[con.IDENTITY_TYPE]:
            raise FabricCLIError(
                f"Invalid access mode: {mode}. Allowed values are: {con.AUTH_KEYS[con.IDENTITY_TYPE]}",
    ...
    def _fetch_public_key_from_aad(self, token):
        ...
        if key is None:
            raise FabricCLIError(
                "Public key not found in JWKS", status_code=con.ERROR_AUTHENTICATION_FAILED
            )
        return key
    ...
    def _decode_jwt_token(self, token, expected_audience=None):
        ...
        try:
            ...
            raise FabricCLIError(
                "Failed to decode JWT token.", status_code=con.ERROR_AUTHENTICATION_FAILED
            )
    ...
    def _get_claims_from_token(self, token, claim_names) -> Optional[dict[str, str]]:
        ...
        claims = {claim_name: payload.get(claim_name) for claim_name in claim_names if payload.get(claim_name) is not None}
    ...
    _Cert = NamedTuple(
        "_Cert", [("pem_bytes", bytes), ("private_key", "Any"),
                  ("fingerprint", bytes)]
    )
