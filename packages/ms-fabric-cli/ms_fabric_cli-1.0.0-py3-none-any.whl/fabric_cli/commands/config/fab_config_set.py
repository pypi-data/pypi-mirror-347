...
def exec_command(args: Namespace) -> None:
    ...
    if _can_key_contain_value(key, value):
        ...
        raise FabricCLIError(
            ErrorMessages.Config.invalid_configuration_value(value, key, fab_constant.CONFIG_KEYS[key]),
            fab_constant.ERROR_INVALID_INPUT,
        )
...
def _set_config(key: str, value: Any, debug: bool = True) -> None:
    ...
    if key in (
        ...
        if value is None:
            raise FabricCLIError("The provided path for valueis None, which is invalid")
    ...
    if response.status_code in (200, 201):
        ...
        for item in data.get("value", []):
            if item.get("displayName", "").lower() == value.lower():
                ...
                return
        raise FabricCLIError(
            ErrorMessages.Config.invalid_capacity(value), fab_constant.ERROR_INVALID_INPUT
        )
