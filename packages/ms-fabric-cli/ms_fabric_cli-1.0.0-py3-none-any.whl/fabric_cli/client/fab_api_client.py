...
class ApiResponse:
    ...
    def append_text(self, text: str, total_pages: int = 0):
        try:
            ...
            raise FabricCLIError(f"Error decoding JSON: {e}")
    ...
    if headers_value is not None:
        if isinstance(args.headers, dict):
            ...
            raise FabricCLIError(
                "Invalid headers format. Expected dictionary.", fab_constant
            )
    ...
    try:
        ...
        for attempt in range(retries_count + 1):
            ...
            match response.status_code:
                ...
                case 404:
                    raise FabricCLIError(
                        "Resource not found - please check the resource name",
        ...
        if response.status_code == 200:
            if check_status:
                ...
                if status == "Succeeded" or status == "Completed":
                    ...
                    fab_logger.log_progress(status)
                    raise FabricCLIError(
                        f"Operation failed: {result_json.get('error')}",
                        fab_constant.ERROR_OPERATION_FAILED,
                    )
                elif status == "Cancelled":
                    fab_logger.log_progress(status)
                    raise FabricCLIError(
                        f"Operation cancelled: {result_json.get('error')}",
