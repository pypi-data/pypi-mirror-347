...
def print_done(text: str) -> None:
    ...
    _safe_print_formatted_text(
        f"<ansigreen>*</ansigreen> {escaped_text}", escaped_text)
...
def print_error(text_or_exception, command: Optional[str] = None) -> None:
    ...
    if isinstance(text_or_exception, FabricCLIError):
        debug = fab_state_config.get_config(
    ...
    if command:
        ...
        _safe_print_formatted_text(
            f"<ansired>x</ansired> {escaped_text}", escaped_text)
...
def print_entries_unix_style(
    ...
    if isinstance(entries, dict):
        ...
        raise FabricCLIError("Invalid entries format provided.")

    if header:
        widths = [
            max(len(field), max(get_visual_length(entry, field)
