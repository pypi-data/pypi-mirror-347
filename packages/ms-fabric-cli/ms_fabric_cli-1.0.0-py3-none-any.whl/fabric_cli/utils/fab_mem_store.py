    ...
    if response.status_code in (200, 201):
        ...
        for ws in data["value"]:
            try:
                ...
                if e.status_code == fab_constant.WARNING_INVALID_WORKSPACE_TYPE:
                    pass
                else:
                    raise e
            else:
...
def get_workspace_id(tenant: Tenant, name) -> str:
    ...
    raise FabricCLIError(f"Workspace not found '{name}'", fab_constant.ERROR_NOT_FOUND)
...
def get_capacity_id(tenant: Tenant, name) -> str:
    ...
    raise FabricCLIError(f"Capacity not found '{name}'", fab_constant.ERROR_NOT_FOUND)
...
def get_connection_id(tenant: Tenant, name) -> str:
    ...
    raise FabricCLIError(f"Connection not found '{name}'", fab_constant.ERROR_NOT_FOUND)
...
def get_gateway_id(tenant: Tenant, name) -> str:
    ...
    raise FabricCLIError(f"Gateway not found '{name}'", fab_constant.ERROR_NOT_FOUND)
...
def get_domain_id(tenant: Tenant, name) -> str:
    ...
    raise FabricCLIError(f"Domain not found '{name}'", fab_constant.ERROR_NOT_FOUND)
    ...
    if response.status_code in (200, 201):
        ...
        for item in data["value"]:
            try:
                ...
                if e.status_code == fab_constant.WARNING_INVALID_ITEM_TYPE:
                    pass
                else:
                    raise e
...
def get_item_id(workspace: Workspace, name) -> str:
    ...
    raise FabricCLIError(f"Item not found '{name}'", fab_constant.ERROR_NOT_FOUND)
...
def get_spark_pool_id(container: VirtualItemContainer, name) -> str:
    ...
    raise FabricCLIError(f"Spark Pool not found '{name}'", fab_constant.ERROR_NOT_FOUND)
...
def get_managed_identity_id(container: VirtualItemContainer, name) -> str:
    ...
    raise FabricCLIError(
        f"Managed Identity not found '{name}'", fab_constant.ERROR_NOT_FOUND
    )
...
def get_managed_private_endpoint_id(container: VirtualItemContainer, name) -> str:
    ...
    raise FabricCLIError(
        f"Managed Private Endpoint not found '{name}'", fab_constant.ERROR_NOT_FOUND
    )
...
def get_external_data_share_id(container: VirtualItemContainer, name) -> str:
    ...
    raise FabricCLIError(
        f"External Data Share not found '{name}'", fab_constant.ERROR_NOT_FOUND
    )
