...
class AggArgs:
    ...
    def flatten_funcs(self):
        # flatten the list of functions
        for list_or_func in self.funcs:
            if isinstance(list_or_func, list):
                for func in list_or_func:
                    yield func
            else:
                yield list_or_func
    ...
    def from_named(cls, kwargs):
        ...
        for relabel, tmp in kwargs.items():
            ...
            # func must be a single function, otherwise raise error
...
def is_supported_agg_func(f):
    return isinstance(f, str) and f in _supported_agg_funcs


def is_supported_agg_funcs(funcs):
    return all(is_supported_agg_func(f) for f in funcs)
