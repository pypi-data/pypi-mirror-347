    ...
    def aggregate(self, func=None, axis=0, *args, **kwargs):
        ...
        if irutils._is_scalar_or_list_or_tuple_of(func, str):
            ...
            if axis == 1:
                ...
                if not is_supported_agg_funcs(targets):
                    reason = (
        ...
        if not reason:
            if not is_supported_agg_funcs(aggArgs.flatten_funcs):
                reason = (
                    "all or few agg functions are not supported"
                    f" {aggArgs.funcs}"
                )
            elif is_bool_non_bool_agg_mixed(aggArgs.flatten_funcs):
                reason = (
