...
class Ensemble:
    ...
    def compare_ensembles_abs(
        ...
        if self.global_clasification:
            self.initialise_global_clustering(counts_A, counts_B, n_clusters=5)
    ...
    def compare_ensembles_abs(self, 
        ...
        if self.global_clasification:
            # Initialise global clustering once (using counts as features)
            self.initialise_global_clustering(counts_A, counts_B, n_clusters=max_clusters)
        ...
        start = time.time()
        for j, mu in tqdm(enumerate(mu_array), desc="Evaluation chemical potentials"):
