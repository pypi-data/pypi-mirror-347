...
class ReplayInfo():
    ...
    def _init_battlefield_vehicle_ego_info(self,one_scenario: dict = None):
        ...
        self.ego_info['x'] = one_scenario['ego_info']['initial_state']['x']
        self.ego_info['y'] = one_scenario['ego_info']['initial_state']['y']
        self.ego_info['yaw_rad'] = one_scenario['ego_info']['initial_state']['yaw_rad']
        self.ego_info['v_mps'] = one_scenario['ego_info']['initial_state']['v_mps']
        self.ego_info['acc_mpss'] = one_scenario['ego_info']['initial_state']['yawrate_radps']
        self.ego_info['yawrate_radps'] = one_scenario['ego_info']['initial_state']['acc_mpss']
...
class ReplayParser():
    ...
    def parse(self,scenario:dict, scene_type) -> ReplayInfo:
        """解析动态场景.动态场景包括: 
        ...
        if scene_type == "B":
            ...
            self._parse_battlefield_scenario(dir_scene_file)
            self._parse_battle_field_hdmaps(scenario) # 解析 地图文件
        else:
            self.replay_info._add_settings(scenario_name=scenario['data']['scene_name'],
                                        scenario_type=scenario['data']['scene_name'].split("_")[1])
            dir_scene_file = scenario['data']['dir_scene_file']
    ...
    def _parse_battlefield_scenario(self, dir_scene_file):
    ...
    def _parse_battle_field_hdmaps(self,scenario:dict) -> None:
        dataroot = scenario['file_info']['dir_maps']
        location = scenario['file_info']['location']
        
        # 获取mask图信息并确定最大方形区域
        self._load_battlefied_mask_and_calculate_square_region(dataroot,location)
    
    def _load_battlefied_mask_and_calculate_square_region(self,dataroot:str,location:str) -> None:
        self.replay_info.hdmaps['image_mask'] = BitMap(dataroot,location,'bitmap_mask', scene_type=self.scene_type)  # 得到整个二进制图的ndarray
...
class ReplayController():
    ...
    def step(self,env,action,old_observation:Observation,traj_future:Dict,observation_last:Observation,traj=200, ha_path=None, client:Optional[Client] = None, slope=0, last_height=None, vis_cir_open=False) -> Observation:
        """
        进行一步更新
        ...
        if self.kinetics_mode == 'complex':
            ...
            new_observation,new_observation_all = self._update_kinetics(env,action,old_observation, traj_future,observation_last,traj,client, ha_path, slope, last_height, vis_cir_open)
            #print("完成一次更新与记录")
        else:
            action = self._action_cheaker(action)
            new_observation = self._update_ego_and_t(action, old_observation)
            new_observation = self._update_other_vehicles(new_observation)
            new_observation = self._update_end_status(new_observation)
            new_observation_all=new_observation
    ...
    def _get_initial_observation(self) -> Observation:
        ...
        return observation,observation_all
    ...
    def _update_kinetics(self,env,action:tuple,old_observation:Observation,traj_future:Dict,observation_last:Observation,traj, client:Client, ha_path, slope=0, last_height=None, vis_cir_open=False) -> Observation:
        """
        使用动力学模型更新状态值
        """
        # 首先分别取出加速度和方向盘转角
        ...
        client.send_message(gear,a,rot,continue_simulation, slope)
        unpacked_data,step_count = client.receive_message()
        for i in range(step_count):
            # 更新状态
            new_observation=self._update_ego(unpacked_data,i,a,old_observation)
            new_observation,new_observation_all=self._update_other_vehicles(new_observation)
            new_observation_all = self._update_end_status(new_observation_all)
            #记录轨迹与可视化
            env.recorder.record(new_observation_all)
            env.visualizer.update(new_observation_all,traj_future,old_observation,traj,ha_path, last_height, vis_cir_open)
        ...
        if new_observation_all.test_setting['end'] == -1:
            #持续运行模型，直至收到下一个控制量
    ...
    def _update_end_status(self,observation:Observation) -> Observation:
        ...
        elif observation.test_setting["scenario_type"] == "mission":
            if utils.is_inside_polygon(observation.vehicle_info['ego']['x'],observation.vehicle_info['ego']['y'],observation.test_setting['goal']):
                status_list += [4]
                print("###log### 主车已到达目标区域\n")
...
class Controller():
    ...
    def init(self,scenario:dict, kinetics_mode:str, scene_type) -> Tuple:
        """初始化运行场景,给定初始时刻的观察值
        ...
        if self.mode == 'replay':
            ...
            self.control_info = self.parser.parse(scenario, scene_type)
            observation,self.observation = self.controller.init(self.control_info)
            self.traj = self.control_info.vehicle_traj
        return observation,self.observation,self.traj


    def step(self,env,action, client:None,traj_future:Dict,observation_last:Observation,traj=200, ha_path=None, slope=0, last_height=None, vis_cir_open=False):
        """
        根据控制量更新状态值，只返回限制范围内观测值
        observation_dis为限制感知范围得观测值
        self.observation包含全部观测信息，用于可视化
        """
        observation_dis,self.observation = self.controller.step(env,action,self.observation,traj_future,observation_last,traj,ha_path, client, slope, last_height, vis_cir_open)
        return observation_dis
