    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                # Set reconnecting flag to track reconnection state
                if retry_count > 0:
                ...
                if "auth" in error_str or "unauthorized" in error_str or "credentials" in error_str or "permission" in error_str:
                    logger.error("Authentication error detected via string matching. Credentials may have expired.")
                    logger.error("Returning to main loop to request fresh credentials")
                    return
