"""
Utility functions for EGO with Co-Kriging support.

This module provides helper functions and utilities for the EGO process
with multi-fidelity modeling capabilities.
"""
...
# Add this at the beginning of your main script or before creating the optimizer
import warnings
# Filter out the specific warning about verbose parameter
...
def evaluate_model_performance(model, likelihood, X_train: torch.Tensor, 
    ...
    # Get rho parameter if it's a Co-Kriging model
    if hasattr(model.covar_module, 'rho'):
        metrics['rho'] = float(model.covar_module.rho.detach().cpu().numpy())
...
def train_ck_model(model, likelihood, X_train: torch.Tensor, 
                  y_train: torch.Tensor, config: TrainingConfig) -> float:
    """Train Co-Kriging model with optimization settings.
    
    Args:
        model: Co-Kriging model to train
        likelihood: Model likelihood
        X_train: Training input data with fidelity indicators
        y_train: Training target data
        config: Training configuration
        
    Returns:
        Current rho value after training
    """
    model.train()
    likelihood.train()
    
    # Create optimizer with different parameter groups for more flexibility
    optimizer = torch.optim.Adam([
        {'params': model.mean_module.parameters(), 'lr': config.learning_rate},
        {'params': model.covar_module.kernel_c.parameters(), 'lr': config.learning_rate},
        {'params': model.covar_module.kernel_d.parameters(), 'lr': config.learning_rate * 0.5},  # Lower learning rate for difference kernel
        {'params': model.covar_module.raw_rho, 'lr': config.learning_rate * 0.1},  # Even lower for rho parameter
    ...
    # Store initial rho value
    initial_rho = float(model.covar_module.rho.detach().cpu().numpy())
    ...
    with gpytorch.settings.cholesky_jitter(config.jitter):
        for i in range(config.training_iter):
            try:
                ...
                if config.verbose and (i == 0 or (i+1) % 10 == 0):
                    current_rho = float(model.covar_module.rho.detach().cpu().numpy())
                    print(f"Iteration {i+1}/{config.training_iter}, Loss: {loss.item():.6f}, Rho: {current_rho:.6f}")
    ...
    # Return the final rho value
    return float(model.covar_module.rho.detach().cpu().numpy())
    
    # Return the final rho value
    return float(model.covar_module.rho.detach().cpu().numpy())

def check_rho_convergence(rho_history: List[float], config: TrainingConfig) -> bool:
    """Check if rho parameter has converged.
    
    Args:
        rho_history: List of rho values from previous iterations
        config: Training configuration
        
    Returns:
        True if rho has converged, False otherwise
    """
    if len(rho_history) < config.rho_patience + 1:
        return False
    
    # Calculate relative changes in recent rho values
    recent_rhos = rho_history[-config.rho_patience-1:]
    relative_changes = [abs(recent_rhos[i] - recent_rhos[i-1]) / recent_rhos[i-1] 
                       for i in range(1, len(recent_rhos))]
    
    # Check if all recent changes are below threshold
    return all(change < config.rho_threshold for change in relative_changes)
...
def save_metrics(metrics_df: pd.DataFrame, iteration: int, acquisition_type: str,
    ...
    Args:
        ...
        rho: Correlation parameter from Co-Kriging model (optional)
    """
...
def save_infill_sample(X_next: np.ndarray, y_next: np.ndarray, 
                      variable_names: list, save_dir: str, fidelity: int = 1):
...
def save_model_data(model, likelihood, scalers: Dict, metadata: Dict, save_dir: str):
    ...
    # Get hyperparameters (different for CoKrigingModel)
    if hasattr(model.covar_module, 'rho'):
        # Co-Kriging model
        hyperparameters = {
            'kernel': model.kernel_type if hasattr(model, 'kernel_type') else 'matern15',
            'rho': float(model.covar_module.rho.detach().numpy()),
            'noise': likelihood.noise.detach().numpy().item(),
            'outputscale_c': float(model.covar_module.kernel_c.outputscale.detach().numpy()),
            'outputscale_d': float(model.covar_module.kernel_d.outputscale.detach().numpy())
        }
        
        # Add lengthscales if present
        if hasattr(model.covar_module.kernel_c.base_kernel, 'lengthscale'):
            hyperparameters['lengthscales_c'] = model.covar_module.kernel_c.base_kernel.lengthscale.detach().numpy().tolist()
            hyperparameters['lengthscales_d'] = model.covar_module.kernel_d.base_kernel.lengthscale.detach().numpy().tolist()
    else:
        # Standard GP model
        hyperparameters = {
    ...
    torch.save(state_dict, os.path.join(save_dir, 'cokriging_model.pth'))
...
def load_model_data(save_dir: str) -> Dict:
    ...
    """
    # Load model state
    state_dict = torch.load(os.path.join(save_dir, 'model.pth'))
...
def prepare_data_with_fidelity(X: np.ndarray, fidelity: int) -> np.ndarray:
    ...
    Returns:
        Input data with fidelity indicator in the last column
    """
    fidelity_col = np.ones((X.shape[0], 1)) * fidelity
    return np.hstack((X, fidelity_col))
