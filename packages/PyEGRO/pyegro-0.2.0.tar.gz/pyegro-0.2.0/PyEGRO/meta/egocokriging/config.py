"""
Configuration management for EGO training with Co-Kriging support.
...
class TrainingConfig:
    ...
    Attributes:
        ...
        kernel: Kernel type for base kernel ('matern25', 'matern15', 'matern05', 'rbf')
        multi_fidelity: Whether to use multi-fidelity modeling
        rho_threshold: Convergence threshold for rho parameter (as percentage)
        rho_patience: Number of iterations without improvement in rho before stopping
    """
    max_iterations: int = 100
    rmse_threshold: float = 0.001
    rmse_patience: int = 10
    relative_improvement: float = 0.01   # 1%
    ...
    rho_threshold: float = 0.05  # 5% convergence threshold for rho parameter
    rho_patience: int = 3
    ...
    def __post_init__(self):
        ...
        # Validate rho threshold
        if not 0 < self.rho_threshold < 1:
            raise ValueError(f"Rho threshold must be between 0 and 1, got {self.rho_threshold}")
            
        # Validate kernel parameter
        valid_kernels = ["matern25", "matern15", "matern05", "rbf"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rho_patience": self.rho_patience
        }