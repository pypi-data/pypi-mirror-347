    ...
    with global_llm():
        def process_pcap(pcap_log):
            ...
            try:
                ...
                if hasattr(reports, "attack_info") and hasattr(reports.attack_info, "is_attack"):
                    if reports.attack_info.is_attack:
                        return reports
                else:
                    loguru.logger.warning(f"Invalid report: {reports}")
            except Exception as e:
                loguru.logger.error(f"Error processing {pcap_log}: {e}")
                return None
            finally:
                loguru.logger.info(f"Report: {reports}")
            return None
