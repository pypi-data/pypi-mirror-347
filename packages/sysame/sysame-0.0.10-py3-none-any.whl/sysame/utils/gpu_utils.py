...
def _get_gpu_specs_windows() -> dict[str, Union[list[dict[str, Any]], int, str]]:
    ...
    try:
        ...
        cmd: str = (
            "Get-WmiObject Win32_VideoController | Select-Object Name, AdapterRAM, CurrentRefreshRate, DriverVersion, VideoModeDescription | ConvertTo-Json"
        )
        ...
        try:
            nvidia_cmd: str = (
                "nvidia-smi --query-gpu=name,memory.total,memory.used,memory.free,temperature.gpu,utilization.gpu,clocks.current.sm,clocks.current.memory --format=csv,noheader"
            )
        ...
        for i, gpu in enumerate(gpu_list):
            ...
            if vendor == "nvidia":
                try:
                    ...
                    if nvidia_result.returncode == 0 and nvidia_result.stdout.strip():
                        lines: list[str] = [
                            l.strip() for l in nvidia_result.stdout.strip().split("\n")
                        ]
                        if i < len(lines):
                            ...
                            if len(parts) >= 7:
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        lines_cap: list[str] = [
                                            l.strip()
                                            for l in cap_result.stdout.strip().split(
                                                "\n"
                                            )
                                        ]
                                        if i < len(lines_cap):
