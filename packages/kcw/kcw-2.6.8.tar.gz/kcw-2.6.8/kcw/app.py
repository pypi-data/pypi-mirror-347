...
class web:
    ...
    def __new__(self,name,appname=None):
        ...
        if self.__name == '__main__' or self.__name=='kcw.kcw' or self.__name=='kcweb.kcweb' or self.__name=='kcwebplus.kcwebplus':
    ...
    def __routes(self,header):
        ...
        if self.__config.fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr=='kcweb' or self.__config.fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr=='kcwebplus':#请求后执行的函数
            try:
                getattr(obj,"rfdsgeagesegdsfdsfdrrsebskcwebafterrequest")() #关闭数据库相关连接
            except:pass
    ...
    def __application(self,env, start_response):
        ...
        if os.path.isfile(p):
            ...
            resheader.append(('Server', "kcw"+config.kcw['version']))
            if type(body) is bytes:
                pass
            else:
                body=bytes(body, encoding='utf-8')
