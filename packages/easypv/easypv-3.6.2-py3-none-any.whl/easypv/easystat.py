...
def PROGRAM_display_df_in_treeview(ori_owercount_easyread, methon, ori, page_size=100):    
    ...
    if psur==0:
        ...
        PROGRAM_create_button(framecanvas0, "智能导入", lambda: SMALL_read_files_and_clean())
        ...
        PROGRAM_create_button(framecanvas0, "暂存合并", lambda: TOOLS_temp_save_df(ori_owercount_easyread.copy()) )
    ...
    if '报告编码' in ori_owercount_easyread.columns:
        PROGRAM_create_button(framecanvas0, "可视面板", lambda:TOOLS_show_analysis_gui(ori_owercount_easyread).run())       
...
def AAA_my_main():
    ...
    try:
        ...
        try:
            biaozhun["器械品种库"]=MYSQL_gettable(memory_db, 'easy_器械规整-品种信息库-测试').reset_index(drop=True)            
        except:
            pass
