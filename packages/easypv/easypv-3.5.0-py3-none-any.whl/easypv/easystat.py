...
def get_chinese_font():
    """
    自动检测系统中存在的中文字体，并优先选择SimHei字体。
    如果没有SimHei，则选择其他可用的中文字体。
    支持Windows和Linux系统。
    返回: 可用中文字体名称或None
    """
    system = platform.system()
    chinese_fonts = []
    preferred_order = ['SimHei', 'Microsoft YaHei', '微软雅黑', '宋体', 'Noto Sans CJK SC', 'WenQuanYi Micro Hei']

    if system == 'Windows':
        # Windows系统 - 更可靠的中文字体检测方法
        ...
        for font in fonts:
            try:
                ...
                name = prop.get_name()
                # 检查是否是常见中文字体
                if any(c in name for c in ['黑体', '宋体', '楷体', '仿宋', '微软雅黑', 'SimHei']):
        ...
        # Linux系统 - 更严格的中文字体检测
        try:
            # 使用fc-list命令获取系统中可用的中文字体
            output = subprocess.check_output(['fc-list', ':lang=zh', '--format=%{family}\\n'], stderr=subprocess.PIPE)
            fonts = output.decode('utf-8').split('\n')
            ...
            for font in fonts:
                if font:
                    # 只保留包含中文标识的字体
                    if any(keyword in font for keyword in ['CJK', 'SC', 'ST', 'Hei', 'Kai', 'Song', 'Fang', '宋体', '黑体']):
                        for f in font.split(','):
                            f = f.strip()
    ...
    if chinese_fonts:
        ...
        print("系统中没有找到中文字体，可能无法正常显示中文。")
        return None

my_font_ch=get_chinese_font()
...
def LLM_qixie(dfs):
    class MedicalDataNormalizer:
        def __init__(self):
            self.root = tk.Tk()
            self.root.title("医疗器械不良事件报告智能规整系统 V1.1")
        ...
        def setup_ui(self):
            ...
            def display_gz():
                if '故障表现(规整)' not in self.df.columns:
                    TOOLS_show_analysis( self.df,'注册证编号/曾用注册证编号', '器械故障表现', '事件发生日期')
                else:
            ...
            def display_gzbw():
                if '故障部位(规整)' not in self.df.columns:
                    ...
                    TOOLS_show_analysis( self.df,'注册证编号/曾用注册证编号', '故障部位(规整)', '事件发生日期')


            def display_sh():
                if '伤害表现(规整)' not in self.df.columns:
                    TOOLS_show_analysis( self.df,'注册证编号/曾用注册证编号', '伤害表现', '事件发生日期')
                else:
                    TOOLS_show_analysis( self.df,'注册证编号/曾用注册证编号', '伤害表现(规整)', '事件发生日期')
            ...
            self.mode_combobox = ttk.Combobox(row2_frame, textvariable=self.mode_var, 
                                            values=["单品类自建术语集","单品类IMDRF术语集","多品类IMDRF术语集"], width=10)
        ...
        def generate_all_terms(self):
            ...
            try:
                ...
                "无明显不良事件",
                "信息不充分"]
                prompt2='且故障表现术语必须是IMDRF以下不良事件一级术语集里头的（一级术语后面用括号简要列出主要是哪些故障）：'+str(imdfr_guzhang)
...
def PROGRAM_display_df_in_treeview(ori_owercount_easyread, methon, ori, page_size=100):    
    ...
    if methon=="psur" and '报告表编码' in ori_owercount_easyread.columns:
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'SOC', '不良反应名称（规整）', '不良反应发生时间'))
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'通用名称', '不良反应名称（规整）', '不良反应发生时间'))
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'批准文号', '生产批号', '不良反应发生时间'))
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'通用名称', 'SOC', '不良反应发生时间'))
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'-监测机构', '医院名称', '报告日期'))
    ...
    if methon=="psur" and  '报告编码' in ori_owercount_easyread.columns  and '器械故障表现' in ori_owercount_easyread.columns   and '-持有人-品名-证号列' in ori_owercount_easyread.columns:
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'-注册证备案证', '器械故障表现', '事件发生日期'))
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'-注册证备案证', '产品批号', '事件发生日期'))
        ...
        PROGRAM_create_button(framecanvas, "图", lambda:TOOLS_show_analysis(ori_owercount_easyread,'-监测机构', '单位名称', '报告日期'))
