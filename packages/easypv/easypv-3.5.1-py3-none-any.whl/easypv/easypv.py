...
def install_linux_packages():
    ...
    # 要安装的pip软件包列表
    pip_packages = ["xlrd==1.2.0", "xlwt", "openpyxl", "sqlalchemy",'python-docx==1.0.0','pycryptodome','packaging','pycryptodome']
    ...
    # 配置pip镜像源
    try:
        subprocess.run(["pip3", "config", "set", "global.index-url", 
    ...
    for package in pip_packages:
        try:
            ...
            subprocess.run(["pip3", "install", "--user", package], check=True)
...
def get_python_cmd():
    """获取适用于当前系统的 Python 命令（python 或 python3）"""
    # 尝试获取 Python 版本
    try:
        # 如果当前运行的 Python 版本 >= 3.7.3，优先使用 'python'
    ...
    # 检查 'python3' 是否可用
    try:
        # 在 Linux/macOS 上，'python3' 通常是 Python 3
    ...
    # 默认回退到 'python'（Windows 通常只有 'python'）
    return "python"

def load_app_old(root, package_name):
    """加载应用程序"""
    global csdir
    ...
    if platform.system() == "Windows":
        ...
        os.system(f'"{py_cmd}" "{package_path}"')
        os.system(f'"{py_cmd}" "{return_pkg}"')
    else:
        # Linux/macOS 直接执行
        os.system(f"{py_cmd} '{package_path}'")
        os.system(f"{py_cmd} '{return_pkg}'")
...
def load_app(root,package_name):
    ...
    os.system(f"{python_executable}  {package_names}")
    os.system(f"{python_executable}  {return_pkg}")
