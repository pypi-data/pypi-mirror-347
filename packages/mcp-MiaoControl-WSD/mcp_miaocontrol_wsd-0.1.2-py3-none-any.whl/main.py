...
# 优化后的登录工具（触发会话初始化）
@mcp.tool()
def register_and_get_devices(key:str = "") -> dict:
    '''
    用户登录，并且返回用户名下设备信息

    AI调用说明：
    - 此接口用于获取用户名下的设备信息，设备信息为json格式，包括序号，0为用户名，1为设备mr，2为设备描述信息，3为设备名称，4为id。
    - 参数key为""时，接口函数使用全局API_KEY(由api_key初始化时赋值，或更新后API_KEY)。
    - 当key不为空时，此接口函数使用此key；认证成功后，更新替换全局API_KEY。
    - 此接口函数通过requests.post发出后，则返回设备信息；或者返回认证失败信息。
    ...
    注1：vr参数，不需要MCP中传入，由接口函数内部构建，最终通过requests.post发出。
    注2：当key为""时，接口函数使用全局API_KEY(由api_key初始化时赋值，或更新后API_KEY)
    ...
    注3：vr参数，不需要MCP中传入，由接口函数内部构建，最终通过requests.post发出。
    注4：当需要使用新账号时(与api_key初始化赋值的API_KEY不同)，传入新key；认证成功后，更新替换全局API_KEY

    参数说明：
    vr参数，不需要MCP中传入，由接口函数内部构建，最终通过requests.post发出。
...
def query_device_data(
    ...
    AI调用说明
    - 该接口用于获取指定mr的设备数据；
    - 设备mr可由register_and_get_devices查询成功后获取；
    - 参数key默认"",此时使用api_key;
    ...
    payload = {
        ...
        "date": tm_stamp,
        "limit": limit,
        "key": tmp_key  # 使用最新key,or,使用API_KEY
    }
    response = requests.post(url, json=payload)
...
def execute_device_action(
    ...
    if rsp_code == "400" or rsp_code == 400:
        return (f"code:{rsp_code},发送成功")
    else:
        return (f"code:{rsp_code},非法key")
