...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel=3, pad=1):
        ...
        self.prob = ProbAttention(in_channels, n_heads = 2)
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, d=2):
        ...
        self.down1 = (Down(d, d*2, kernel=3, pad=1))
        self.down2 = (Down(d*2, d*4, kernel=3, pad=1))
        self.down3 = (Down(d*4, d*8, kernel=3, pad=1))
        self.down4 = (Down(d*8, d*16, kernel=3, pad=1))
...
def create_net(in_ch=1, out_ch=1, dim=64):
    torch.manual_seed(0)
    torch.use_deterministic_algorithms(False)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    
    unet = UNet(in_ch,out_ch,d=dim)
