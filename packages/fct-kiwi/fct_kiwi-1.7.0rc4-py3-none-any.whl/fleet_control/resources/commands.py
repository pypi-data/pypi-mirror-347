...
def delete_variables(
    ...
    service_vars = variables.get("service_vars", {})
    service = next(iter(service_vars.keys()), 0)
...
def update_variables(
    ...
    print(json.dumps(source_vars, indent= 2))
    ...
    def update_variable(var_name, value, existing_vars, is_service_var=False):
        if existing_vars.get(var_name) != value and var_name not in exclude_set:
            if is_service_var and service:
                target.set_service_var(service, var_name, value) # type: ignore
            else:
            ...
            for var_name in list(existing_vars):
                if var_name.startswith("NODE") and var_name not in exclude_set and source_vars.get(var_name) is None:
                    if existing_vars.get(var_name) != "0":
                        if is_service_var and service:
                            target.set_service_var(service, var_name, "0") # type: ignore
                        else:
        ...
        def remove_custom_variable(existing_vars, custom_vars, source_vars, is_service_var=False):
            for var_name in list(existing_vars):
                if (
                    ...
                    if is_service_var and service:
                        target.remove_service_var(service, var_name) # type: ignore
                    else:
...
def change(variables: dict | str, targets: List[str]):
    ...
    Args:
        ...
        targets (List[str]): One or more target bots (format: 4X000) and/or fleets
...
def clone(source: str, targets: tuple, exclude: str | None = None):
    ...
    Args:
        ...
        targets (tuple): One or more target device IDs (format: 4X000) or fleets.
...
def purge(targets: List[str], exclude: str | None = None):
    ...
    Args:
        targets (List[str]): One or more target device IDs (format: 4X000).
        exclude (str | None, optional): Variables to exclude, must be in the format "var1 var2". Defaults to None.
    """
    exclude_vars = set()
    if exclude is not None:
...
def move(fleet: str, targets: tuple, keep_vars: bool, keep_service_vars: bool, clone: bool, semver: str | None = None):
    """ Move target(s) from its fleet to specified FLEET.

    Args:
        fleet (str): Chosen fleet's name (i.e: Test4_x).
        targets (tuple): One or more target device IDs (format: 4X000).
...
def schedule_change(json_file: str, date: str, date_format: str, tz: str, variables: str, targets: tuple) -> None:
    """POST request with variables to change in some targets to a Google Pub/Sub topic at a specified time.

    Args:
        json_file (str): File with the targets and variables to change."
        ...
        variables (str): must be in the format 'variable1=value=service'
        targets (tuple): to apply the changes to separated by spaces
...
def schedule_update(json_file: str, date: str, date_format: str, tz: str, targets: tuple, fleet: str, release:str) -> None:
    """POST request with targets to pin to a specific release.

    Args:
        json_file (str): File with the targets and selected release
        date (str): The date string to convert (e.g., '2024-11-25 15:30:00')
        date_format (str): The format of the date string
        tz (str): The timezone for the schedule date
        targets (tuple): to pin to the release separated by spaces.
        fleet (str): fleet from which the targets and the release belong to
        release (str): semantic version of the target release (i.e: 1.3.12+rev12)
    ...
    def process_input_data(json_file, targets, fleet, release):
        if json_file:
            ...
            printlog(
                "Targets, fleet, and release are required when no JSON file is provided.",
                "ERROR",
            )
            ctx = click.get_current_context()
            ctx.exit(1)
    ...
    # Process input data (from JSON file or command-line arguments)
    message_data = process_input_data(json_file, targets, fleet, release)
    task_id = uuid.uuid1()
    message_data["task_id"] = str(task_id)
    date_object = format_time(date, date_format, tz)
...
def initialize(device: str, fleet: str):
    ...
    if bot_to_delete and click.confirm(
        ...
        move(
            fleet=fleet,
            targets=tuple(bot_to_delete.get("device_name").removeprefix("kiwibot")),
            keep_vars=False,
...
def rename(bot_id: str, new_id: str, version: str | None = None) -> None:
    """Rename TARGET with new ID. Optional new tags for corresponding version read from configuration file.

    Args:
        bot_id (str): Target device ID (format: 4X000).
