...
QUEUE_ID = get_config("QUEUE_ID")
AUTHOR = get_config("AUTHOR", "default")
ORGANIZATION_HANDLE = get_config("ORGANIZATION_HANDLE")
SERVICE_ACCOUNT = get_config("GOOGLE_APPLICATION_CREDENTIALS", "serviceAccountKey.json")
...
def get_custom_set_vars_fleet(fleet: int) -> dict[str, dict[str, str | int]]:
    temp = {"id": fleet} if isinstance(fleet, int) else fleet

    def get_service_install_vars(fleet_id):
        ...
        query_string = (
            "?$filter=service_install/any(si:si/device/any(d:d/belongs_to__application/id eq '{}'))"
        ...
        headers = {
            ...
            "Authorization": f"Bearer {BALENA_API_TOKEN}"
        }
    ...
    raw_env_vars = balena_sdk.models.device.env_var.get_all_by_application(temp["id"])
    raw_service_vars = get_service_install_vars(temp["id"])
    vars_per_bot = {}
    device_ids = set([item["device"]["__id"] for item in raw_env_vars]) # type: ignore
    device_ids.update(set([item["service_install"][0]["device"]["__id"] for item in raw_service_vars])) # type: ignore
    ...
    for i in raw_service_vars:
        bot = i["service_install"][0]["device"]["__id"]  # type: ignore
...
def ask_confirmation(message_data: dict[str, dict], date_object, tz) -> str:
    ...
    if message_data.get("variables"):
        ...
        printlog(f"{message_data['fleet']}: {message_data['release']}", msg_type="OK_PURPLE")
