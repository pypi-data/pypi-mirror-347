...
class LogData:
    ...
    def log_to_gcp(self, severity, log_entry, message):
        if self.gcp_logger:
            resource_labels = {
    ...
    def log(
        ...
        if self.gcp_client:
            ...
            self.log_to_gcp(log_entry.level, log_entry=log_entry_json, message=log_entry.message)
        else:
            self.logger.log(level, msg=log_entry.message, extra={"json_fields":log_entry_json})
