...
class LogData:
    ...
    def __initialize(self, origin: str = "INTERNAL"):
        ...
        self.console_formatter = logging.Formatter(
            ...
            datefmt=DATE_FORMAT)
        self.container_name = os.environ.get("CONTAINER_NAME", "default-container")
        self.pod_name = os.environ.get("POD_NAME", "default-pod")
    ...
    def log_to_gcp(self, severity, message, container=None, pod=None):
        if self.gcp_logger:
            resource = gcp_logging.Resource(
                type="k8s_container",
                labels={
                    "project_id": self.gcp_client.project,
                    "namespace_name": os.environ.get("NAMESPACE", "default"),
                    "pod_name": pod if pod else self.pod_name,
                    "container_name": container if container else self.container_name,
                    "cluster_name": os.environ.get("CLUSTER_NAME", "default-cluster"),
                    "location": os.environ.get("ZONE", "us-central1"),
                }
            )
    ...
    def log(
        ...
        self.log_to_gcp(log_entry.level, message=json.dumps(log_entry))
