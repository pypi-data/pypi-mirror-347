...
def get_logger(
    ...
    This is the recommended way to obtain a logger. It checks for environment
    variables to configure the logger based on the application's configuration.
    
    Args:
        name: Logger name
        level: Log level (default: INFO or DEBUG based on DEBUG environment variable)
        format: Log format (default: TEXT)
        destination: Log destination (default: CONSOLE)
    ...
    """
    # Determine log level based on environment variables if not explicitly provided
    if level is None:
        if os.environ.get("DEBUG") == "1":
            ...
            level = LogLevel.WARNING  # Default to WARNING to suppress INFO logs
