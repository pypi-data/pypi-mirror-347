...
@common_options
def init_command(path, url, force, config_dir, offline, **kwargs):
    """Initialize a knowledge graph for a repository.
    
    This command:
    1. Sets up the repository (local or cloned)
    2. Runs the ingest pipeline to create a knowledge graph
    3. Writes the configuration file
    
    Args:
        path: Path to repository
        url: GitHub repository URL to clone
        force: Whether to force rebuild of an existing graph
