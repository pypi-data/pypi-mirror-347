...
from pyiceberg.io import (
    ...
    TOS_CONNECT_TIMEOUT,
    TOS_SOCKET_TIMEOUT,
    TOS_HIGH_LATENCY_LOG_THRESHOLD,
    TOS_VERSION_AWARE,
    TOS_CREDENTIALS_PROVIDER,
    TOS_DEFAULT_BLOCK_SIZE,
    TOS_DEFAULT_FILL_CACHE,
    TOS_DEFAULT_CACHE_TYPE,
    TOS_MULTIPART_STAGING_DIRS,
    TOS_MULTIPART_SIZE,
    TOS_MULTIPART_THREAD_POOL_SIZE,
    TOS_MULTIPART_THRESHOLD,
    TOS_ENABLE_CRC,
    TOS_ENABLE_VERIFY_SSL,
    TOS_DNS_CACHE_TIMEOUT,
    TOS_PROXY_HOST,
    TOS_PROXY_PORT,
    TOS_PROXY_USERNAME,
    TOS_PROXY_PASSWORD,
    TOS_DISABLE_ENCODING_META,
    TOS_EXPECT_100_CONTINUE_THRESHOLD,
)
...
def _tos(properties: Properties) -> AbstractFileSystem:
    from tosfs import TosFileSystem
    client_kwargs: Dict[str, Any] = {
        "endpoint": get_first_property_value(properties, TOS_ENDPOINT, S3_ENDPOINT),
        "key": get_first_property_value(properties, TOS_ACCESS_KEY_ID, S3_ACCESS_KEY_ID, AWS_ACCESS_KEY_ID),
        "secret": get_first_property_value(properties, TOS_SECRET_ACCESS_KEY, S3_SECRET_ACCESS_KEY, AWS_SECRET_ACCESS_KEY),
        "session_token": get_first_property_value(properties, TOS_SESSION_TOKEN, S3_SESSION_TOKEN, AWS_SESSION_TOKEN),
        "region": get_first_property_value(properties, TOS_REGION, S3_REGION, AWS_REGION),
        "max_retry_num": get_first_property_value(properties, TOS_MAX_RETRY_NUM),
        "max_connections": get_first_property_value(properties, TOS_MAX_CONNECTIONS),
        "connection_timeout": get_first_property_value(properties, TOS_CONNECT_TIMEOUT),
        "socket_timeout": get_first_property_value(properties, TOS_SOCKET_TIMEOUT),
        "high_latency_log_threshold": get_first_property_value(properties, TOS_HIGH_LATENCY_LOG_THRESHOLD),
        "version_aware": get_first_property_value(properties, TOS_VERSION_AWARE),
        "credentials_provider": get_first_property_value(properties, TOS_CREDENTIALS_PROVIDER),
        "default_block_size": get_first_property_value(properties, TOS_DEFAULT_BLOCK_SIZE),
        "default_fill_cache": get_first_property_value(properties, TOS_DEFAULT_FILL_CACHE),
        "default_cache_type": get_first_property_value(properties, TOS_DEFAULT_CACHE_TYPE),
        "multipart_staging_dirs": get_first_property_value(properties, TOS_MULTIPART_STAGING_DIRS),
        "multipart_size": get_first_property_value(properties, TOS_MULTIPART_SIZE),
        "multipart_thread_pool_size": get_first_property_value(properties, TOS_MULTIPART_THREAD_POOL_SIZE),
        "multipart_threshold": get_first_property_value(properties, TOS_MULTIPART_THRESHOLD),
        "enable_crc": get_first_property_value(properties, TOS_ENABLE_CRC),
        "enable_verify_ssl": get_first_property_value(properties, TOS_ENABLE_VERIFY_SSL),
        "dns_cache_timeout": get_first_property_value(properties, TOS_DNS_CACHE_TIMEOUT),
        "proxy_host": get_first_property_value(properties, TOS_PROXY_HOST),
        "proxy_port": get_first_property_value(properties, TOS_PROXY_PORT),
        "proxy_username": get_first_property_value(properties, TOS_PROXY_USERNAME),
        "proxy_password": get_first_property_value(properties, TOS_PROXY_PASSWORD),
        "disable_encoding_meta": get_first_property_value(properties, TOS_DISABLE_ENCODING_META),
        "except100_continue_threshold": get_first_property_value(properties, TOS_EXPECT_100_CONTINUE_THRESHOLD),
    }

    # 移除值为None的键
    client_kwargs = {k: v for k, v in client_kwargs.items() if v is not None}
    return TosFileSystem(**client_kwargs)
